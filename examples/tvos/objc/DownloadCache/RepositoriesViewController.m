...
- (void)viewDidLoad {
  ...
  self.token = [[RLMRealm defaultRealm]
  ...
  NSURLComponents *components = [NSURLComponents
      componentsWithString:@"https://api.github.com/search/repositories"];
  components.queryItems = @[
    [NSURLQueryItem queryItemWithName:@"q" value:@"language:objc"],
            ...
            [realm transactionWithBlock:^{
              for (NSDictionary *item in items) {
                ...
                repository.identifier =
                    [NSString stringWithFormat:@"%@", item[@"id"]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  [[[NSURLSession sharedSession]
        dataTaskWithURL:[NSURL URLWithString:repository.avatarURL]
      completionHandler:^(NSData *_Nullable data,
                          NSURLResponse *_Nullable response,
                          NSError *_Nullable error) {
        if (!error) {
...
- (void)reloadData {
  ...
  if (self.searchField.text.length > 0) {
    self.results = [self.results
        objectsWhere:@"name contains[c] %@", self.searchField.text];
  }
  self.results = [self.results
      sortedResultsUsingKeyPath:@"name"
                      ascending:self.sortOrderControl.selectedSegmentIndex ==
                                0];
