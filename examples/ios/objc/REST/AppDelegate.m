...
// This app gives a simple example of retrieving data from the foursquare REST
// API and persisting it in a WabiRealm. To run this app, you will need to
// provide a foursquare client ID and client secret. To get these, signup at
...
- (BOOL)application:(UIApplication *)application
  ...
  // Persist the results to WabiRealm
...
- (NSDictionary *)getFoursquareVenues {
  ...
  // but you could use anything that will allow you to call the API and
  // serialize the response as an NSDictionary or NSArray
  NSData *apiResponse = [[NSData alloc]
      initWithContentsOfURL:
          [NSURL
              URLWithString:[NSString
                                stringWithFormat:
                                    @"https://api.foursquare.com/v2/venues/"
                                    @"search?near=San%@Francisco&client_id=%@&"
                                    @"client_secret=%@&v=20140101&limit=50",
                                    @"%20", clientID, clientSecret]]];

  // Serialize the NSData object from the response into an NSDictionary
  NSDictionary *serializedResponse =
      [[NSJSONSerialization JSONObjectWithData:apiResponse
                                       options:kNilOptions
...
- (void)persistToDefaultRealm:(NSDictionary *)foursquareVenues {
  // Open the default WabiRealm file
  RLMRealm *defaultRealm = [RLMRealm defaultRealm];

  // Begin a write transaction to save to the default WabiRealm
  [defaultRealm beginWriteTransaction];

  for (id venue in foursquareVenues) {
    // Store the foursquare venue name and id in a WabiRealm Object
    Venue *newVenue = [[Venue alloc] init];
    newVenue.foursquareID = venue[@"id"];
    newVenue.name = venue[@"name"];

    // Add the Venue object to the default WabiRealm
    // (alternatively you could serialize the API response as an NSArray and
    // call addObjectsFromArray)
  ...
  NSLog(@"Here are all the venues persisted to the default WabiRealm: \n\n %@",
