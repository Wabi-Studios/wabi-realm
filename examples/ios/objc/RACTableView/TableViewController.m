...
@implementation Cell
- (instancetype)initWithStyle:(__unused UITableViewCellStyle)style
              reuseIdentifier:(NSString *)reuseIdentifier {
  return self = [super initWithStyle:UITableViewCellStyleSubtitle
...
- (void)attach:(Entry *)entry {
  ...
  if (self.entry == nil) {
    ...
    RAC(self.detailTextLabel, text) =
        [RACObserve(self, entry.date) map:^(NSDate *date) {
...
- (void)viewDidLoad {
  ...
  // Get the singleton GroupParent object from the WabiRealm, creating it
...
- (void)setupUI {
  ...
  RACCommand *addGroup = [[RACCommand alloc] initWithSignalBlock:^(id unused) {
    [self modifyInBackground:^(RLMArray *groups) {
      NSString *name =
          [NSString stringWithFormat:@"Group %d", (int)arc4random()];
        ...
        return @(groups.count > 0);
          ...
          signalBlock:^(id unused) {
            [self modifyInBackground:^(RLMArray *groups) {
              ...
              NSString *name =
                  [NSString stringWithFormat:@"Entry %d", (int)arc4random()];
              [group.entries addObject:[Entry createInDefaultRealmWithValue:@[
                               name, NSDate.date
                             ]]];
            }];
            return [RACSignal empty];
  ...
  self.navigationItem.leftBarButtonItem =
      [[UIBarButtonItem alloc] initWithTitle:@"Add Group"
  ...
  @weakify(self);
  [[self.parent rac_valuesAndChangesForKeyPath:@"groups"
                                       options:0
                                      observer:self]
...
- (NSArray<NSIndexPath *> *)indexSetToIndexPathArray:(NSIndexSet *)indexes
  ...
  [[group rac_valuesAndChangesForKeyPath:@"entries" options:0 observer:self]
      subscribeNext:^(RACTuple *info) { // tuple is value, change dictionary
        ...
        if (indexes) {
          ...
          NSArray *paths = [self indexSetToIndexPathArray:indexes
                                                  section:section];
