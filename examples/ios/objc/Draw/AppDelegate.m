...
- (BOOL)application:(UIApplication *)application
  ...
  if (app.currentUser) {
    RLMRealmConfiguration.defaultConfiguration =
        [app.currentUser configurationWithPartitionValue:@"foo"];
...
- (void)logIn {
  ...
  // Creating a debug credential since this demo is just using the generated
  // access token produced when running the WabiRealm Object Server via the
  // `start-object-server.command`
  RLMCredentials *credential =
      [RLMCredentials credentialsWithEmail:@"demo@realm.io"
                                  password:@"password"];

  // Log the user in (async, the WabiRealm will start syncing once the user is
                 ...
                 if (error) {
                   ...
                   [self.window.rootViewController
                       presentViewController:alertController
                                    animated:YES
                                  completion:nil];
                 } else { // Logged in setup the default WabiRealm
                   RLMRealmConfiguration.defaultConfiguration =
                       [app.currentUser configurationWithPartitionValue:@"foo"];
