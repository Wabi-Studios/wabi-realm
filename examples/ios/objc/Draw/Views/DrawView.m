...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  // Add the draw path to the WabiRealm
...
- (void)addPoint:(CGPoint)point {
  [[RLMRealm defaultRealm] transactionWithBlock:^{
    ...
    DrawPoint *newPoint =
        [DrawPoint createInDefaultRealmWithValue:@[ @(point.x), @(point.y) ]];
...
- (void)motionEnded:(UIEventSubtype)motion withEvent:(UIEvent *)event {
  ...
  UIAlertController *alertController = [UIAlertController
      alertControllerWithTitle:@"Reset Canvas?"
                       message:
                           @"This will clear the WabiRealm database and reset "
                           @"the canvas. Are you sure you wish to proceed?"
