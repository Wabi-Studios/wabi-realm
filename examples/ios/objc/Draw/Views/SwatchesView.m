...
- (void)setupButtons {
  ...
  NSMutableArray *buttons =
      [NSMutableArray arrayWithCapacity:self.colors.count];
  NSInteger tag = 0;
  for (NSString *color in self.colors.allKeys) {
    UIImage *pencilImage = [[UIImage imageNamed:color]
  ...
  CGFloat totalWidth = (self.colors.count * swatchSize.width) +
                       ((self.colors.count - 1) * kSwatchPencilPadding);
  CGFloat x = 0.0f;
  ...
  self.selectedIconView =
      [[UIImageView alloc] initWithImage:[[self class] circleIcon]];
