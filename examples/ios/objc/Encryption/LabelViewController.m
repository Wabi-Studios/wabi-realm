...
- (void)viewDidAppear:(BOOL)animated {
  ...
  // Use an autorelease pool to close the WabiRealm at the end of the block, so
  ...
  @autoreleasepool {
    ...
    RLMRealmConfiguration *configuration =
        [RLMRealmConfiguration defaultConfiguration];
    configuration.encryptionKey = [[NSData alloc] initWithBytes:buffer
  ...
  @autoreleasepool {
    ...
    [self log:@"Saved object: %@", [[[StringObject allObjectsInRealm:realm]
                                       firstObject] stringProp]];
  }
}

// Log a message to the screen since we can't just use NSLog() with no debugger
// attached
...
- (NSData *)getKey {
  ...
  static const uint8_t kKeychainIdentifier[] =
      "io.WabiRealm.EncryptionExampleKey";
  ...
  NSAssert(status == errSecSuccess,
           @"Failed to insert new key in the keychain");
