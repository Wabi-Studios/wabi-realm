...
@property(nonatomic, strong)
    RLMSectionedResults<NSString *, DemoObject *> *sectionedResults;
...
- (void)viewDidLoad {
  ...
  self.sectionedResults = [[DemoObject allObjects]
      sectionedResultsSortedUsingKeyPath:@"contactName"
                               ascending:YES
                                keyBlock:^(DemoObject *object) {
                                  return
                                      [object.contactName substringToIndex:1];
                                }];

  // Set realm notification block
  __weak typeof(self) weakSelf = self;
  self.notification = [self.sectionedResults
      addNotificationBlock:^(RLMSectionedResults<NSString *, DemoObject *> *col,
                             RLMSectionedResultsChange *changes) {
        if (changes) {
          [weakSelf.tableView
              performBatchUpdates:^{
                [weakSelf.tableView
                    deleteRowsAtIndexPaths:changes.deletions
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  cell.textLabel.text = [NSString
      stringWithFormat:@"%@: %@", object.contactName, object.phoneNumber];
...
- (void)tableView:(UITableView *)tableView
  ...
  if (editingStyle == UITableViewCellEditingStyleDelete) {
    ...
    DemoObject *object =
        self.sectionedResults[indexPath.section][indexPath.row];
    ...
    @autoreleasepool {
      ...
      for (NSInteger index = 0; index < 5; index++) {
        ...
        [DemoObject createInRealm:realm
                        withValue:@{
                          @"phoneNumber" : [self randomContactInfo],
                          @"date" : [NSDate date],
...
- (void)add {
  [[RLMRealm defaultRealm] transactionWithBlock:^{
    [DemoObject createInDefaultRealmWithValue:@[
      [self randomContactInfo], [NSDate date], [self randomContactName]
    ]];
  }];
}
...
- (NSString *)randomContactInfo {
  ...
  return [NSString
      stringWithFormat:@"555-55%ld-%ld%ld55", (long)rand1, rand2, rand3];
}
