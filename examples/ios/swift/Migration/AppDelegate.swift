...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  private func addExampleDataToRealm(_ exampleData: (WabiRealm) -> Void) {
    let url = realmUrl(for: schemaVersion, usingTemplate: false)
    let configuration = WabiRealm.Configuration(fileURL: url, schemaVersion: UInt64(schemaVersion))
  ...
  private func performMigration() {
    for oldSchemaVersion in 0 ..< schemaVersion {
      ...
      let realmConfiguration = WabiRealm.Configuration(fileURL: url, schemaVersion: UInt64(schemaVersion), migrationBlock: migrationBlock)
      try! WabiRealm.performMigration(for: realmConfiguration)
      let realm = try! WabiRealm(configuration: realmConfiguration)
      migrationCheck(realm)
    }
  }

  private func realmUrl(for schemaVersion: Int, usingTemplate: Bool) -> URL {
    let defaultURL = WabiRealm.Configuration.defaultConfiguration.fileURL!
