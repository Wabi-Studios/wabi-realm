...
int main(int argc, const char *argv[]) {
  @autoreleasepool {
    // Import JSON
    NSString *jsonFilePath = [[NSBundle mainBundle] pathForResource:@"persons"
                                                             ofType:@"json"];
    ...
    NSArray *personDicts = [NSJSONSerialization JSONObjectWithData:jsonData
                                                           options:0
                                                             error:&error];
    if (error) {
      NSLog(@"There was an error reading the JSON file: %@",
    ...
    for (NSDictionary *personDict in personDicts) {
      ...
      person.birthdate =
          [dateFormatter dateFromString:personDict[@"birthdate"]];
      person.numberOfFriends =
          [(NSNumber *)personDict[@"friendCount"] integerValue];
    ...
    // WabiRealm file saved at default path (~/Documents/default.realm)
  }
  return 0;
}
