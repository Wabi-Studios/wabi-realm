...
///     // be ignored entirely by WabiRealm.
...
///  and mutating it on an object which has been added to a WabiRealm will throw an
///  exception.
///
...
///  result in worse performance when accessing objects managed by a WabiRealm.
...
///  generated and then discarded when reading from a WabiRealm.
///
///  If a class has at least one @Persisted property, all other properties will be
///  ignored by WabiRealm. This means that they will not be persisted and will not
...
public struct Persisted<Value: _Persistable> {
  ...
  internal mutating func initialize(_: ObjectBase, key: PropertyKey) {
  ...
  internal mutating func initializeCollection(_: ObjectBase, key: PropertyKey) -> Value? {
...
extension Persisted: Encodable where Value: Encodable {
  public func encode(to encoder: Encoder) throws {
    ...
    default:
      ...
      throw EncodingError.invalidValue(self, .init(codingPath: encoder.codingPath, debugDescription: "Only unmanaged WabiRealm objects can be encoded using automatic Codable synthesis. You must explicitly define encode(to:) on your model class to support managed WabiRealm objects."))
    }
  }
}
...
/**
 An enum type which can be used with @Persisted and WabiRealm Collections.

 Persisting an enum in WabiRealm requires that it have a raw value and that the raw value by a type which WabiRealm can store.
 ...
 If the WabiRealm contains a value which is not a valid member of the enum (such as
...
// When an existing unmanaged object is added to a WabiRealm, existing non-collection
