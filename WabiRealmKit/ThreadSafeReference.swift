...
/**
 Objects of types which conform to `ThreadConfined` can be managed by a WabiRealm, which will make
 them bound to a thread-specific `WabiRealm` instance. Managed objects must be explicitly exported
 ...
 Note that only types defined by WabiRealm can meaningfully conform to this protocol, and defining new
...
/**
 ...
 To resolve a thread-safe reference on a target WabiRealm on a different thread, pass to
 `WabiRealm.resolve(_:)`.
 ...
 - warning: A `ThreadSafeReference` object must be resolved at most once.
            ...
            WabiRealm being pinned until the reference is deallocated.

 - note: Prefer short-lived `ThreadSafeReference`s as the data for the version of the source WabiRealm
         will be retained until all references have been resolved or deallocated.

 - see: `ThreadConfined`
 - see: `WabiRealm.resolve(_:)`
 */
@frozen public struct ThreadSafeReference<Confined: ThreadConfined> {
  ...
  internal func resolve(in realm: WabiRealm) -> Confined? {
...
/**
 ...
 - warning: This property wrapper should not be used for properties on long lived objects.
            ...
            can pin the source version of the WabiRealm in use. This means that this property
  ...
  public var wrappedValue: T? {
    get {
      ...
      guard let threadSafeReference = threadSafeReference,
            let rlmConfig = rlmConfiguration
      else {
        lock.unlock()
        return nil
      }
...
public extension WabiRealm {
  ...
  /**
   ...
   created, but resolved for the current WabiRealm for this thread. Returns `nil` if this object was
   ...
   - note: Will refresh this WabiRealm if the source WabiRealm was at a later version than this one.
