...
@frozen public enum SyncSubscriptionState: Equatable {
  ...
  case complete
  /// The subscription encountered an error and synchronization is paused for this WabiRealm. You can
...
@frozen public struct SyncSubscriptionSet {
   ...
   */
  public func first<T: Object>(ofType _: T.Type, where query: @escaping (Query<T>) -> Query<Bool>) -> SyncSubscription? {
...
@frozen public struct SyncSubscriptionSetIterator: IteratorProtocol {
  ...
  private func nextIndex(for index: Int?) -> Int? {
    if let index = index, index < rlmSubscriptionSet.count - 1 {
      return index + 1
    }
    return nil
  }
