...
public extension WabiRealm {
  /**
   A `Configuration` instance describes the different options used to create an instance of a WabiRealm.

   `Configuration` instances are just plain Swift structs. Unlike `WabiRealm`s and `Object`s, they can be freely shared
   ...
   rather than creating a new value each time you open a WabiRealm.
   */
  @frozen struct Configuration: Sendable {
    ...
    /**
      Creates a `Configuration` which can be used to create new `WabiRealm` instances.
      ...
      - parameter fileURL:            The local URL to the WabiRealm file.
      - parameter inMemoryIdentifier: A string used to identify a particular in-memory WabiRealm.
      ...
      - parameter readOnly:           Whether the WabiRealm is read-only (must be true for read-only files).
      - parameter schemaVersion:      The current schema version.
      - parameter migrationBlock:     The block which migrates the WabiRealm to the current version.
      - parameter deleteRealmIfMigrationNeeded: If `true`, recreate the WabiRealm file with the provided
                                                schema if a migration is required.
      - parameter shouldCompactOnLaunch: A block called when opening a WabiRealm for the first time during the
      ...
      - parameter objectTypes:        The subset of `Object` and `EmbeddedObject` subclasses persisted in the WabiRealm.
    ...
    /**
     A configuration value used to configure a WabiRealm for synchronization with Atlas App Services. Mutually
    ...
    /// The local URL of the WabiRealm file. Mutually exclusive with `inMemoryIdentifier`.
    ...
    /// A string used to identify a particular in-memory WabiRealm. Mutually exclusive with `fileURL` and
    /// `syncConfiguration`.
    ...
    /**
     Whether to open the WabiRealm in read-only mode.

     For non-synchronized Realms, this is required to be able to open WabiRealm files which are not
     ...
     mode requires disabling WabiRealm's reader/writer coordination, so committing a write
     ...
     and this instead merely disallows performing write transactions on the WabiRealm. In addition,
     it will skip some automatic writes made to the WabiRealm, such as to initialize the WabiRealm's
    ...
    /// The block which migrates the WabiRealm to the current version.
    @preconcurrency
    public var migrationBlock: MigrationBlock?

    /**
     Whether to recreate the WabiRealm file with the provided schema if a migration is required. This is the case when
    ...
    /**
     A block called when opening a WabiRealm for the first time during the
     life of a process to determine if it should be compacted before being
     returned to the user. It is passed the total file size (data + free space)
     and the total bytes used by data in the file.
    ...
    /// The classes managed by the WabiRealm.
    ...
    /**
     The maximum number of live versions in the WabiRealm file before an exception will
     be thrown when attempting to start a write transaction.

     WabiRealm provides MVCC snapshot isolation, meaning that writes on one thread do
     not overwrite data being read on another thread, and instead write a new copy
     of that data. When a WabiRealm refreshes it updates to the latest version of the
     ...
     performing long operations on background threads which do not allow the WabiRealm
     to refresh.
     ...
     or in production use to cause the app to crash rather than produce a WabiRealm
     file which is too large to be opened.
     */
    public var maximumNumberOfActiveVersions: UInt?

    /**
     When opening the WabiRealm for the first time, instead of creating an empty file,
     the WabiRealm file will be copied from the provided seed file path and used instead.
     This can be used to open a WabiRealm file with pre-populated data.
     ...
     Note that to use this parameter with a synced WabiRealm configuration
     the seed WabiRealm must be appropriately copied to a destination with
     `WabiRealm.writeCopy(configuration:)` first.
    ...
    /**
     Configuration for WabiRealm event recording. Events are enabled if this is set
     to a non-nil value.
     */
    public var eventConfiguration: EventConfiguration?

    /// A custom schema to use for the WabiRealm.
    private var customSchema: RLMSchema?

    /// If `true`, disables automatic format upgrades when accessing the WabiRealm.
    internal var disableFormatUpgrade: Bool = false

    // MARK: Flexible Sync

    /// Callback for adding subscriptions to the initialization of the WabiRealm
    ...
    internal var rlmConfiguration: RLMRealmConfiguration {
      ...
      if let fileURL = fileURL {
        ...
        fatalError("A WabiRealm Configuration must specify a path or an in-memory identifier.")
      }
      ...
      configuration.shouldCompactOnLaunch = shouldCompactOnLaunch.map(ObjectiveCSupport.convert(object:))
...
extension WabiRealm.Configuration: Equatable {
  public static func == (lhs: WabiRealm.Configuration, rhs: WabiRealm.Configuration) -> Bool {
