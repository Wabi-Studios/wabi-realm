...
/**
 * The `MongoClient` enables reading and writing on a MongoDB database via the WabiRealm Cloud service.
 *
...
public extension FindOptions {
  ...
  convenience init(_ limit: Int = 0, _ projection: Document? = nil, _ sorting: [Document] = []) {
    self.init()
    self.limit = limit
    self.projection = projection
...
public extension MongoCollection {
  ...
  @preconcurrency
  func find(filter: Document, options: FindOptions = FindOptions(),
  ...
  @preconcurrency
  func findOneDocument(filter: Document, options: FindOptions = FindOptions(),
  ...
  ///   - completion: The resulting bson array of documents or error if one occurs
  @preconcurrency
  func aggregate(pipeline: [Document], _ completion: @escaping MongoFindBlock) {
  ...
  @preconcurrency
  func count(filter: Document, limit: Int? = nil, _ completion: @escaping MongoCountBlock) {
    let bson = ObjectiveCSupport.convert(filter)
  ...
  @preconcurrency
  func deleteOneDocument(filter: Document, _ completion: @escaping MongoCountBlock) {
    let bson = ObjectiveCSupport.convert(filter)
  ...
  @preconcurrency
  func deleteManyDocuments(filter: Document, _ completion: @escaping MongoCountBlock) {
    let bson = ObjectiveCSupport.convert(filter)
  ...
  @preconcurrency
  func updateOneDocument(filter: Document, update: Document, upsert: Bool = false,
  ...
  ///   - completion: The result of the attempt to update a document.
  @preconcurrency
  func updateManyDocuments(filter: Document, update: Document, upsert: Bool = false,
                           _ completion: @escaping MongoUpdateBlock)
  {
    let filterBSON = ObjectiveCSupport.convert(filter)
    let updateBSON = ObjectiveCSupport.convert(update)
  ...
  ///   - completion: The result of the attempt to update a document.
  @preconcurrency
  func findOneAndUpdate(filter: Document, update: Document,
  ...
  @preconcurrency
  func findOneAndReplace(filter: Document, replacement: Document,
                         options: FindOneAndModifyOptions = .init(),
                         _ completion: @escaping MongoFindOneBlock)
  {
    let filterBSON = ObjectiveCSupport.convert(filter)
  ...
  @preconcurrency
  func findOneAndDelete(filter: Document, options: FindOneAndModifyOptions = .init(),
                        _ completion: @escaping MongoFindOneBlock)
  {
    let filterBSON = ObjectiveCSupport.convert(filter)
...
public extension MongoCollection {
  ...
  func insertOne(_ document: Document) async throws -> AnyBSON {
  ...
  func insertMany(_ documents: [Document]) async throws -> [AnyBSON] {
  ...
  func find(filter: Document, options: FindOptions? = nil) async throws -> [Document] {
  ...
  @_unsafeInheritExecutor
  func findOneDocument(filter: Document, options: FindOptions? = nil) async throws -> Document? {
  ...
  func aggregate(pipeline: [Document]) async throws -> [Document] {
  ...
  func count(filter: Document, limit: Int? = nil) async throws -> Int {
  ...
  func deleteOneDocument(filter: Document) async throws -> Int {
  ...
  ///   - filter: Document representing the match criteria
  /// - Returns: `Int` count of deleted documents.
  func deleteManyDocuments(filter: Document) async throws -> Int {
  ...
  private class WatchSubscription<S: Subscriber>: RLMChangeEventDelegate, Subscription where S.Input == AnyBSON, S.Failure == Error {
    ...
    init(publisher: __shared WatchPublisher, subscriber: S) {
      ...
      changeStream = publisher.collection.watch(
        ...
        scheduler: scheduler ?? DispatchQueue.main.schedule
      ) as RLMChangeStream
    }
...
public extension MongoCollection {
  ...
  var changeEvents: AsyncThrowingPublisher<Publishers.WatchPublisher> {
    Publishers.WatchPublisher(collection: self)
  ...
  func changeEvents(onOpen: @Sendable @escaping () -> Void)
  ...
  ///  events.
  func changeEvents(filterIds: [ObjectId], onOpen: (@Sendable () -> Void)? = nil)
    -> AsyncThrowingPublisher<Publishers.WatchPublisher>
  {
  ...
  ///  triggered before this callback is invoked may not produce change
  ///  events.
  func changeEvents(matchFilter: Document, onOpen: (@Sendable () -> Void)? = nil)
    -> AsyncThrowingPublisher<Publishers.WatchPublisher>
  {
