...
/**
 An object representing the WabiRealm App configuration
...
/**
 An object representing a client which performs network calls on
 WabiRealm Cloud user registration & password functions
...
public extension UserProfile {
  ...
  var metadata: Document {
    guard let rlmMetadata = __metadata as RLMBSON?,
          let anyBSON = ObjectiveCSupport.convert(object: rlmMetadata),
          case let .document(metadata) = anyBSON
    else {
      return [:]
    }
...
/**
 An object representing a client which performs network calls on
 WabiRealm Cloud for registering devices to push notifications
...
/// The `App` has the fundamental set of methods for communicating with a WabiRealm
/// application backend.
