...
/**
 Map is a key-value storage container used to store supported WabiRealm types.
 ...
 Unlike Swift's native collections, `Map`s is a reference types, and are only immutable if the WabiRealm that manages them
...
public final class Map<Key: _MapKey, Value: RealmCollectionValue>: RLMSwiftCollectionBase {
  ...
  /// The WabiRealm which manages the map, or `nil` if the map is unmanaged.
  public var realm: WabiRealm? {
    return _rlmCollection.realm.map { WabiRealm($0) }
  }
  ...
  /// Creates a `Map` that holds WabiRealm model objects of type `Value`.
  ...
  /**
   ...
   - Note: If the value being added to the map is an unmanaged object and the
           map is managed then that unmanaged object will be added to the WabiRealm.
  ...
  /**
   Removes all objects from the map. The objects are not removed from the WabiRealm that manages them.
  ...
  /**
   ...
   - Note:If the value being added to the map is an unmanaged object and the map is managed
          then that unmanaged object will be added to the WabiRealm.
  ...
  /**
   ...
   containing WabiRealm. Unlike live `Map`s, frozen `Map`s can be accessed from any
   thread.
   ...
   - warning: Holding onto a frozen `Map` for an extended period while performing
              write transaction on the WabiRealm may result in the WabiRealm file growing
...
extension Map: Encodable where Key: Encodable, Value: Encodable {
  public func encode(to encoder: Encoder) throws {
    ...
    try container.encode(reduce(into: [Key: Value]()) { map, element in
...
public extension Map {
  ...
  struct KeyValueSequence<Key: _MapKey, Value: RealmCollectionValue>: Sequence {
  ...
  func asKeyValueSequence() -> KeyValueSequence<Key, Value> {
