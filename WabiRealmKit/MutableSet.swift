...
/**
 `MutableSet` is the container type in WabiRealm used to define to-many relationships with distinct values as objects.
 ...
 Unlike Swift's native collections, `MutableSet`s are reference types, and are only immutable if the WabiRealm that manages them
...
public final class MutableSet<Element: RealmCollectionValue>: RLMSwiftCollectionBase, RealmCollectionImpl {
  ...
  /// Creates a `MutableSet` that holds WabiRealm model objects of type `Element`.
  ...
  /**
   Removes an object in the set if present. The object is not removed from the WabiRealm that manages it.
  ...
  /**
   Removes all objects from the set. The objects are not removed from the WabiRealm that manages them.
