...
/**
 ...
 that are reported by WabiRealm notifications.
...
/// A type which can be stored in a WabiRealm List, MutableSet, Map, or Results.
///
/// Declaring additional types as conforming to this protocol will not make them
/// actually work. Most of the logic for how to store values in WabiRealm is not
...
///  A type which can appear in a WabiRealm collection inside an Optional.
///
/// :nodoc:
...
public protocol RealmCollection: RealmCollectionBase, Equatable where Iterator == RLMIterator<Element> {
  ...
  /// The WabiRealm which manages the collection, or `nil` for unmanaged collections.
  var realm: WabiRealm? { get }

  /**
   Indicates if the collection can no longer be accessed.

   The collection can no longer be accessed if `invalidate()` is called on the `WabiRealm` that manages the collection.
   */
  ...
  /**
   ...
   will reflect the state of the WabiRealm after the write transaction.
   ...
   - warning: This method cannot be called during a write transaction, or when the containing WabiRealm is read-only.
...
public extension RealmCollection where Element: Encodable {
...
public extension RealmCollection {
               ...
               _ block: @escaping (RealmCollectionChange<Self>) -> Void) -> NotificationToken
  {
    ...
    func observe<A: Actor>(keyPaths: [String]? = nil,
                           ...
                           _ block: @Sendable @escaping (isolated A, RealmCollectionChange<Self>) -> Void) async -> NotificationToken
    {
      await observe(keyPaths: keyPaths, on: actor, block)
    }
  #endif
}
...
public extension RealmCollection where Element: ObjectBase {
  ...
  func observe(keyPaths: [PartialKeyPath<Element>],
               ...
               _ block: @escaping (RealmCollectionChange<Self>) -> Void) -> NotificationToken
  {
    return observe(keyPaths: keyPaths.map(_name(for:)), on: queue, block)
  }
...
public extension RealmCollection {
  /**
   ...
   - returns: An instance of `SectionedResults`.
   */
  func sectioned<Key: _Persistable, O: ObjectBase>(by keyPath: KeyPath<Element, Key>,
  ...
  /**
   ...
   - returns: An instance of `SectionedResults`.
   */
  func sectioned<Key: _Persistable, O: ObjectBase>(by block: @escaping ((Element) -> Key),
                                                   sortDescriptors: [SortDescriptor]) -> SectionedResults<Key, Element> where Element: Projection<O>
  {
    return sectioned(sortDescriptors: sortDescriptors, block)
  }
...
/**
 ...
 non-generic code which can operate on or store multiple types of WabiRealm
...
@frozen public struct AnyRealmCollection<Element: RealmCollectionValue>: RealmCollectionImpl {
  ...
  public init<C: RealmCollection & _ObjcBridgeable>(_ base: C) where C.Element == Element {
    collection = base._rlmObjcValue as! RLMCollection
  }
...
/**
 ...
 should be used when you want to project a `List` of WabiRealm Objects to a list of values.
...
public struct ProjectedCollection<Element>: RandomAccessCollection, CustomStringConvertible, ThreadConfined where Element: RealmCollectionValue {
  ...
  /// The WabiRealm which manages the object.
  ...
  init(_ collection: RLMCollection, keyPath: AnyKeyPath, propertyName: String) {
    backingCollection = AnyRealmCollection(collection: collection)
...
public extension List where Element: ObjectBase & RealmCollectionValue {
  /**
   ...
   ```
    In this code the `Person`'s dogs list will be prijected to the list of dogs names via `projectTo`
    */
  var projectTo: CollectionElementMapper<Element> {
    CollectionElementMapper(collection: collection)
  }
}

public extension MutableSet where Element: ObjectBase & RealmCollectionValue {
  /**
