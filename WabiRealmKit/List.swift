...
/**
 `List` is the container type in WabiRealm used to define to-many relationships.
 ...
 Unlike Swift's native collections, `List`s are reference types, and are only immutable if the WabiRealm that manages them
...
public final class List<Element: RealmCollectionValue>: RLMSwiftCollectionBase, RealmCollectionImpl {
  ...
  /// Creates a `List` that holds WabiRealm model objects of type `Element`.
  ...
  /**
   ...
   If the object is managed by a different WabiRealm than the receiver, a copy is made and added to the WabiRealm managing
   the receiver.
  ...
  /**
   Removes an object at the given index. The object is not removed from the WabiRealm that manages it.
  ...
  /**
   Removes all objects from the list. The objects are not removed from the WabiRealm that manages them.
...
public extension List {
   ...
   */
  func replaceSubrange<C: Collection, R>(_ subrange: R, with newElements: C)
...
extension List: MutableCollection {
  ...
  /**
   ...
   objects are not removed from the WabiRealm that manages them.
  ...
  /**
   ...
   are not removed from the WabiRealm that manages them.
...
extension List: Decodable where Element: Decodable {
  public convenience init(from decoder: Decoder) throws {
    ...
    while !container.isAtEnd {
      try append(container.decode(Element.self))
    }
  }
}
