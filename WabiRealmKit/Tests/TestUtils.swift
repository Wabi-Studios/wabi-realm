...
public extension XCTestCase {
  /// Check whether two test objects are equal (refer to the same row in the same WabiRealm), even if their models
  ...
  /// Check whether two collections containing WabiRealm objects are equal.
  ...
  func assertSucceeds(message _: String? = nil, fileName: StaticString = #file,
  ...
  func assertFails<T>(_ expectedError: WabiRealm.Error.Code, _ message: String? = nil,
                      ...
                      block: () throws -> T)
  {
    do {
      ...
      XCTFail("Expected to catch <\(expectedError)>, but no error was thrown.",
              file: fileName, line: lineNumber)
    } catch let e as WabiRealm.Error where e.code == expectedError {
      if message != nil {
  ...
  func assertFails<T>(_ expectedError: WabiRealm.Error.Code, _ file: URL, _ message: String,
                      fileName: StaticString = #file, lineNumber: UInt = #line,
                      block: () throws -> T)
  {
    do {
  ...
  func assertFails<T>(_ expectedError: Error, _: String? = nil,
                      fileName: StaticString = #file, lineNumber: UInt = #line,
                      block: () throws -> T)
  {
    do {
