...
class MapTests: TestCase {
  ...
  func testUnmanagedMapComparison() {
    ...
    XCTAssertEqual([String: SwiftIntObject?](_immutableCocoaDictionary: map1),
                   [String: SwiftIntObject?](_immutableCocoaDictionary: map2),
                   "instances converted to Swift.Dictionary should be equal")
    XCTAssertEqual([String: SwiftIntObject?](_immutableCocoaDictionary: map1),
                   [String: SwiftIntObject?](_immutableCocoaDictionary: map3),
                   "instances converted to Swift.Dictionary should be equal")
    map3["obj3"] = nil
    map1["obj3"] = nil
    XCTAssertEqual([String: SwiftIntObject?](_immutableCocoaDictionary: map1),
  ...
  func testAllKeysQuery() {
    ...
    if let realm = map.realm {
      ...
      test(on: "object", value: SwiftStringObject?(SwiftStringObject(value: ["hello"])))
    }
  }
  ...
  func testAllValuesQuery() {
    ...
    if let realm = map.realm {
      ...
      test(on: "object",
           values: SwiftStringObject?(SwiftStringObject(value: ["hello"])),
           SwiftStringObject?(SwiftStringObject(value: ["there"])),
           SwiftStringObject?(SwiftStringObject(value: ["bye"])))
    }
  }
  ...
  func testObserveKeyPathRemoved() {
    ...
    let token = mapObj.swiftObjectMap.observe(keyPaths: ["intCol"]) { (changes: RealmMapChange) in
      ...
      case let .initial(map):
        XCTAssertEqual(map.count, 2)
      case let .update(map, deletions, insertions, modifications):
