...
extension SwiftStringObject: AnyValueFactory {
  ...
  static func doubleValue(_: SwiftStringObject) -> Double { fatalError() }
...
class AnyRealmValuePrimitiveTests: TestCase {
  override class var defaultTestSuite: XCTestSuite {
    let suite = XCTestSuite(name: "Any WabiRealm Value Tests")
...
class AnyRealmValueObjectTests: TestCase {
  ...
  func testAssortment() {
    ...
    let tests: ((WabiRealm?) -> Void) = { (realm: WabiRealm?) in
...
func addAnyRealmValueTests<OF: ObjectFactory>(_ suite: XCTestSuite, _: OF.Type) {
...
func addAnyRealmValueMutableSetTests<OF: ObjectFactory>(_ suite: XCTestSuite, _: OF.Type) {
  ...
  func assertValue(_ value: AnyRealmValue, key: String) {
    if case let .object(o) = map[key] {
      XCTAssertTrue(map.contains(where: { key, value in
        key == key && (value.object(SwiftStringObject.self)!.stringCol == o["stringCol"] as! String)
      }))
    } else {
...
func addAnyRealmValueMapTests<OF: ObjectFactory>(_ suite: XCTestSuite, _: OF.Type) {
