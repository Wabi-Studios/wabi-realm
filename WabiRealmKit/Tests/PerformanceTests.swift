...
private func createStringObjects(_ factor: Int) -> WabiRealm {
...
private var smallRealm: WabiRealm!
private var mediumRealm: WabiRealm!
private var largeRealm: WabiRealm!
...
class SwiftPerformanceTests: TestCase {
  ...
  private func copyRealmToTestPath(_ realm: WabiRealm) -> WabiRealm {
    do {
  ...
  func testSyncRealmCreationCached() {
    ...
    var realm: WabiRealm!
    dispatchSyncNewThread {
      realm = try! WabiRealm(configuration: config)
    }
  ...
  func createObjects<T: Object>(_: T.Type, _ create: (T, Int) -> Void) -> WabiRealm {
  ...
  func createSwiftListObjects() -> WabiRealm {
  ...
  func createSwiftMutableSetObjects() -> WabiRealm {
  ...
  func createIntSwiftObjects() -> WabiRealm {
    return createObjects(SwiftObject.self) { object, value in
      object.intCol = value
    }
  }

  func createStringSwiftObjects() -> WabiRealm {
    return createObjects(SwiftObject.self) { object, value in
      object.stringCol = String(value)
    }
  }

  func createOptionalIntObjects() -> WabiRealm {
    return createObjects(SwiftOptionalObject.self) { object, value in
      object.optIntCol.value = value
    }
  }

  func createOptionalStringObjects() -> WabiRealm {
    return createObjects(SwiftOptionalObject.self) { object, value in
...
class SwiftSyncRealmPerformanceTests: TestCase {
  ...
  func testSyncRealmCreationCached() {
    ...
    dispatchSyncNewThread {
      ...
      // the cache lookup is a miss but there's a cached WabiRealm on a
      // different thread
      realm = try! WabiRealm(configuration: config)
    }
...
class SwiftFlexibleSyncRealmPerformanceTests: SwiftSyncRealmPerformanceTests {
  override var config: WabiRealm.Configuration {
