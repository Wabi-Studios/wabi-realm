...
class ObjectSchemaTests: TestCase {
  ...
  var swiftObjectSchema: ObjectSchema {
    return try! WabiRealm().schema["SwiftObject"]!
  }
  ...
  func testPrimaryKeyProperty() {
    ...
    XCTAssertEqual(try! WabiRealm().schema["SwiftPrimaryStringObject"]!.primaryKeyProperty!.name, "stringCol")
  }
  ...
  func testEquals() {
    let objectSchema = swiftObjectSchema
    XCTAssert(try! objectSchema == WabiRealm().schema["SwiftObject"]!)
    XCTAssert(try! objectSchema != WabiRealm().schema["SwiftStringObject"]!)
  }
}
