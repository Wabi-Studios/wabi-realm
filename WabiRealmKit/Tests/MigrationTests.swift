...
class MigrationTests: TestCase {
  private func createDefaultRealm() throws {
    ...
    try autoreleasepool {
      _ = try WabiRealm(configuration: config)
    }
  ...
  private func testMigration(shouldRun: Bool = true, schemaVersion: UInt64 = 1,
                             ...
                             validation: ((WabiRealm, RLMSchema) -> Void)? = nil) throws
  {
    let didRun = Locked(false)
    let config = WabiRealm.Configuration(fileURL: testRealmURL(), schemaVersion: schemaVersion,
    ...
    func withTestFile(_ fn: () throws -> Void) throws {
      ...
      if let validation = validation {
        try autoreleasepool {
          ...
          try validation(WabiRealm(configuration: .init(fileURL: testRealmURL(), schemaVersion: schemaVersion)), schema)
        }
      }
    ...
    try withTestFile {
      try WabiRealm.performMigration(for: config)
    }
    try withTestFile {
      let old = WabiRealm.Configuration.defaultConfiguration
      defer {
        WabiRealm.Configuration.defaultConfiguration = old
      }
    ...
    try withTestFile {
      ...
      WabiRealm.asyncOpen(configuration: config) { _ in
        ex.fulfill()
      }
  ...
  func testSchemaVersionAtURL() {
    assertFails(.invalidDatabase, defaultRealmURL(),
                "WabiRealm at path '\(defaultRealmURL().path)' has not been initialized.")
    {
      // Version should throw before WabiRealm creation
    ...
    } catch WabiRealm.Error.filePermissionDenied {
      // Success!
    } catch WabiRealm.Error.fileAccess {
      // Success!
    } catch {
  ...
  func testMigrationObject() throws {
    ...
    try testMigration { migration, _ in
      ...
      migration.enumerateObjects(ofType: "SwiftObject") { oldObj, newObj in
        ...
        let uuidCol: UUID = .init(uuidString: "137decc8-b300-4954-a233-f89909f4fd89")!
        ...
        // test null in Map
        XCTAssertEqual(map.count, 2)
        XCTAssertEqual(map["nulledObj"], MigrationObject??.some(nil))
  ...
  func testDeleteRealmIfMigrationNeededWithSetCustomSchema() {
    ...
    var config = WabiRealm.Configuration(fileURL: defaultRealmURL(), objectTypes: [SwiftEmployeeObject.self])
  ...
  func testDeleteRealmIfMigrationNeeded() throws {
    try autoreleasepool { _ = try WabiRealm(fileURL: defaultRealmURL()) }
