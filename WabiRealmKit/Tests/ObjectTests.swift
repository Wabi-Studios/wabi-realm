...
class ObjectTests: TestCase {
  ...
  func testRealm() {
    ...
    dispatchSyncNewThread {
      autoreleasepool {
        XCTAssertNotEqual(try! WabiRealm(), persisted.realm!)
      }
    }
  }
  ...
  private func withMigrationObject(block: @escaping @Sendable (MigrationObject, Migration) -> Void) {
    ...
    autoreleasepool {
      let configuration = WabiRealm.Configuration(schemaVersion: 1, migrationBlock: { migration, _ in
  ...
  func testSetValueForKey() {
    ...
    try! WabiRealm().write {
      let persistedObject = try! WabiRealm().create(SwiftObject.self)
  ...
  func checkChange<T: Equatable, U: Equatable>(_ name: String, _ old: T?, _ new: U?, _ change: ObjectChange<ObjectBase>) {
    if case let .change(_, properties) = change {
  ...
  func testModifyMultipleObservedPartialKeyPathLocally() throws {
    ...
    var token = object.observe(keyPaths: [\SwiftObject.intCol, \SwiftObject.stringCol]) { changes in
      if case let .change(_, properties) = changes {
        XCTAssertEqual(properties.count, 1)
      ...
      await withTaskGroup(of: NotificationToken.self) { group in
        while active.value < 10 {
          group.addTask { @CustomGlobalActor in
            ...
            guard let realm = try? await WabiRealm(actor: CustomGlobalActor.shared) else {
              waitingForRealm.withLock { $0 -= 1 }
  ...
  func testThaw() {
    ...
    assertThrows(try! frozenObj.realm!.write {}, reason: "Can't perform transactions on a frozen WabiRealm")
  ...
  func testThawUpdatedOnDifferentThread() {
    ...
    dispatchSyncNewThread {
      let resolved = try! WabiRealm().resolve(tsr)!
      try! WabiRealm().write { resolved.boolCol = false }
    }

    XCTAssert(frozen.thaw()!.boolCol)
    XCTAssert(thawed.boolCol)
    try! WabiRealm().refresh()
