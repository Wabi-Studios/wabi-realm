...
enum Config {
  static let config = WabiRealm.Configuration(inMemoryIdentifier: "collection",
  ...
  AggregateCollection.Element == CTTAggregateObject, AggregateCollection.Index == Int
{
  ...
  func realm() -> WabiRealm {
    return try! WabiRealm(configuration: Config.config)
  }

  func getCollection(_: WabiRealm) -> Collection {
    fatalError("Abstract method. Try running tests using Control-U.")
  }

  func getAggregateableCollectionInWrite(_: WabiRealm) -> AggregateCollection {
    fatalError("Abstract method. Try running tests using Control-U.")
  }
  ...
  func testRealm() {
    XCTAssertEqual(collection.realm!.configuration.fileURL, realm().configuration.fileURL)
  }
  ...
  func testObserve() {
    ...
    private actor TestActor {
      ...
      func observe(_ tsr: ThreadSafeReference<Collection>) async throws -> NotificationToken {
        let realm = try await WabiRealm(configuration: Config.config, actor: self)
  ...
  func expectNoChange(fn: @escaping (WabiRealm) -> Void) {
  ...
  func testThaw() {
    ...
    assertThrows(try! frozenRealm.write {}, reason: "Can't perform transactions on a frozen WabiRealm")
  ...
  func testThawPreviousVersion() {
    ...
    XCTAssertFalse(frozen.isEmpty)
    XCTAssertEqual(live!.count, collection.count)
  }
  ...
  func testThawUpdatedOnDifferentThread() {
    ...
    dispatchSyncNewThread {
      let realm = try! WabiRealm(configuration: self.collection.realm!.configuration)
  ...
  func testFreezeFromWrongThread() {
    dispatchSyncNewThread {
      self.assertThrows(self.collection.freeze(), reason: "WabiRealm accessed from incorrect thread")
    }
  }
  ...
  func testFilterWithInt8Property() {
    ...
    var results = realm().objects(CTTAggregateObject.self).filter("int8Col = %d", Int8(0))
    XCTAssertEqual(results.count, 0)
    results = realm().objects(CTTAggregateObject.self).filter("int8Col = %d", Int8(1))
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).filter("int8Col = %d", Int8(2))
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).filter("int8Col = %d", Int8(3))
    XCTAssertEqual(results.count, 1)

    results = realm().objects(CTTAggregateObject.self).where { $0.int8Col == 0 }
    XCTAssertEqual(results.count, 0)
    results = realm().objects(CTTAggregateObject.self).where { $0.int8Col == 1 }
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).where { $0.int8Col == 2 }
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).where { $0.int8Col == 3 }
    XCTAssertEqual(results.count, 1)
  }

  func testFilterWithInt16Property() {
    _ = makeAggregateableObjects()
    var results = realm().objects(CTTAggregateObject.self).filter("int16Col = %d", Int16(0))
    XCTAssertEqual(results.count, 0)
    results = realm().objects(CTTAggregateObject.self).filter("int16Col = %d", Int16(1))
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).filter("int16Col = %d", Int16(2))
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).filter("int16Col = %d", Int16(3))
    XCTAssertEqual(results.count, 1)

    results = realm().objects(CTTAggregateObject.self).where { $0.int16Col == 0 }
    XCTAssertEqual(results.count, 0)
    results = realm().objects(CTTAggregateObject.self).where { $0.int16Col == 1 }
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).where { $0.int16Col == 2 }
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).where { $0.int16Col == 3 }
    XCTAssertEqual(results.count, 1)
  }

  func testFilterWithInt32Property() {
    _ = makeAggregateableObjects()
    var results = realm().objects(CTTAggregateObject.self).filter("int32Col = %d", Int32(0))
    XCTAssertEqual(results.count, 0)
    results = realm().objects(CTTAggregateObject.self).filter("int32Col = %d", Int32(1))
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).filter("int32Col = %d", Int32(2))
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).filter("int32Col = %d", Int32(3))
    XCTAssertEqual(results.count, 1)

    results = realm().objects(CTTAggregateObject.self).where { $0.int32Col == 0 }
    XCTAssertEqual(results.count, 0)
    results = realm().objects(CTTAggregateObject.self).where { $0.int32Col == 1 }
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).where { $0.int32Col == 2 }
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).where { $0.int32Col == 3 }
    XCTAssertEqual(results.count, 1)
  }

  func testFilterWithInt64Property() {
    _ = makeAggregateableObjects()
    var results = realm().objects(CTTAggregateObject.self).filter("int64Col = %d", Int64(0))
    XCTAssertEqual(results.count, 0)
    results = realm().objects(CTTAggregateObject.self).filter("int64Col = %d", Int64(1))
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).filter("int64Col = %d", Int64(2))
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).filter("int64Col = %d", Int64(3))
    XCTAssertEqual(results.count, 1)

    results = realm().objects(CTTAggregateObject.self).where { $0.int64Col == 0 }
    XCTAssertEqual(results.count, 0)
    results = realm().objects(CTTAggregateObject.self).where { $0.int64Col == 1 }
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).where { $0.int64Col == 2 }
    XCTAssertEqual(results.count, 1)
    results = realm().objects(CTTAggregateObject.self).where { $0.int64Col == 3 }
    XCTAssertEqual(results.count, 1)
  }
}
...
class ResultsFromTableTests: ResultsTests {
  override func getCollection(_ realm: WabiRealm) -> Results<CTTNullableStringObjectWithLink> {
    return realm.objects(CTTNullableStringObjectWithLink.self)
  }

  override func getAggregateableCollectionInWrite(_ realm: WabiRealm) -> Results<CTTAggregateObject> {
...
class ListUnmanagedRealmCollectionTests: ListRealmCollectionTests {
  override func getCollection(_: WabiRealm) -> List<CTTNullableStringObjectWithLink> {
    return CTTStringList(value: [[str1, str2]]).array
  }

  override func getAggregateableCollectionInWrite(_: WabiRealm) -> List<CTTAggregateObject> {
...
class ListNewlyAddedRealmCollectionTests: ListRealmCollectionTests {
  override func getCollection(_ realm: WabiRealm) -> List<CTTNullableStringObjectWithLink> {
    let array = CTTStringList(value: [[str1, str2]])
    realm.add(array)
    return array.array
  }

  override func getAggregateableCollectionInWrite(_ realm: WabiRealm) -> List<CTTAggregateObject> {
...
class MutableSetUnmanagedRealmCollectionTests: MutableSetRealmCollectionTests {
  override func getCollection(_: WabiRealm) -> MutableSet<CTTNullableStringObjectWithLink> {
    return CTTStringSet(value: [[str1, str2]]).set
  }

  override func getAggregateableCollectionInWrite(_: WabiRealm) -> MutableSet<CTTAggregateObject> {
...
class MutableSetNewlyAddedRealmCollectionTests: MutableSetRealmCollectionTests {
  override func getCollection(_ realm: WabiRealm) -> MutableSet<CTTNullableStringObjectWithLink> {
    let set = CTTStringSet(value: [[str1, str2]])
    realm.add(set)
    return set.set
  }

  override func getAggregateableCollectionInWrite(_ realm: WabiRealm) -> MutableSet<CTTAggregateObject> {
...
class LinkingObjectsCollectionTypeTests: RealmCollectionTests<LinkingObjects<CTTNullableStringObjectWithLink>, LinkingObjects<CTTAggregateObject>> {
  override func getCollection(_ realm: WabiRealm) -> LinkingObjects<CTTNullableStringObjectWithLink> {
  ...
  override func getAggregateableCollectionInWrite(_ realm: WabiRealm) -> LinkingObjects<CTTAggregateObject> {
...
class AnyRealmCollectionTests: RealmCollectionTests<AnyRealmCollection<CTTNullableStringObjectWithLink>, AnyRealmCollection<CTTAggregateObject>> {
  override func getCollection(_ realm: WabiRealm) -> AnyRealmCollection<CTTNullableStringObjectWithLink> {
    AnyRealmCollection(realm.create(CTTStringList.self, value: [[str1, str2]]).array)
  }

  override func getAggregateableCollectionInWrite(_ realm: WabiRealm) -> AnyRealmCollection<CTTAggregateObject> {
