...
public extension Publisher {
  func signal(_ semaphore: DispatchSemaphore) -> Publishers.HandleEvents<Self> {
    handleEvents(receiveOutput: { _ in semaphore.signal() })
  }
}
...
class CombinePublisherTestCase: TestCase {
  ...
  override func setUp() {
    super.setUp()
    realm = try! WabiRealm(configuration: WabiRealm.Configuration(inMemoryIdentifier: "CombinePublisherTestCase"))
  ...
  func watchForNotifierAdded() -> XCTestExpectation {
    ...
    subscribeOnQueue.sync {
      let r = try! WabiRealm(configuration: realm.configuration, queue: subscribeOnQueue)
      RLMAddBeforeNotifyBlock(ObjectiveCSupport.convert(object: r)) {
        _ = r // retain the WabiRealm until the block is released
        ex.fulfill()
      }
    }
    return ex
...
class CombineRealmTests: CombinePublisherTestCase {
  ...
  func testWillChangeRemoteWrite() {
    ...
    subscribeOnQueue.async {
      let backgroundRealm = try! WabiRealm(configuration: self.realm.configuration)
...
class CombineObjectPublisherTests: CombinePublisherTestCase {
  ...
  func testChangeSet() {
    ...
    cancellable = changesetPublisher(obj).assertNoFailure().sink { change in
      if case let .change(o, properties) = change {
        XCTAssertEqual(self.obj, o)
  ...
  func testFrozenChangeSetSubscribeOn() {
    ...
    cancellable = changesetPublisher(obj)
      ...
      .sink { arr in
        var prev: SwiftIntObject?
        for change in arr {
          guard case let .change(obj, properties) = change else {
            XCTFail("Expected .change but got \(change)")
            sema.signal()
            return
          }
  ...
  func testMixedMakeThreadSafe() {
    let realm2 = try! WabiRealm(configuration: WabiRealm.Configuration(inMemoryIdentifier: "test2"))
...
private protocol CombineTestCollection {
  static func getCollection(_ realm: WabiRealm) -> Self
  func appendObject()
  func modifyObject()
...
extension Results: CombineTestCollection where Element == ModernAllTypesObject {
  static func getCollection(_ realm: WabiRealm) -> Results<Element> {
...
extension List: CombineTestCollection where Element == ModernAllTypesObject {
  static func getCollection(_ realm: WabiRealm) -> List<Element> {
...
extension MutableSet: CombineTestCollection where Element == ModernAllTypesObject {
  static func getCollection(_ realm: WabiRealm) -> MutableSet<Element> {
...
extension LinkingObjects: CombineTestCollection where Element == ModernAllTypesObject {
  static func getCollection(_ realm: WabiRealm) -> LinkingObjects<Element> {
...
extension AnyRealmCollection: CombineTestCollection where Element == ModernAllTypesObject {
  static func getCollection(_ realm: WabiRealm) -> AnyRealmCollection<Element> {
...
extension Map: CombineTestCollection where Key == String, Value == SwiftObject? {
  static func getCollection(_ realm: WabiRealm) -> Map<Key, Value> {
  ...
  where Collection: CombineTestCollection, Collection: RealmSubscribable, Collection.Element: RealmSectionedObject
{
  ...
  func checkChangeset<SectionedResults: RealmSectionedResult>(
    ...
    case let .initial(collection):
      XCTAssertEqual(collection.isFrozen, frozen)
    case let .update(collection, deletions: del, insertions: ins,
                     modifications: modifications, _, _):
      XCTAssertEqual(collection.isFrozen, frozen)
...
class CombineProjectionPublisherTests: CombinePublisherTestCase {
  ...
  func testChangeSet() {
    ...
    cancellable = changesetPublisher(projection)
      .assertNoFailure()
      .sink { change in
        if case let .change(p, properties) = change {
  ...
  func testFrozenChangeSetSubscribeOn() {
    ...
    cancellable = changesetPublisher(projection)
      ...
      .sink { arr in
        var prev: SimpleProjection?
        for change in arr {
          guard case let .change(p, properties) = change else {
            XCTFail("Expected .change but got \(change)")
            sema.signal()
            return
          }
...
class CombineAsyncRealmTests: CombinePublisherTestCase {
  ...
  func testWillChangeRemoteWrite() {
    ...
    queue.async {
      let realm = try! WabiRealm(configuration: self.realm.configuration, queue: self.queue)
      realm.writeAsync {
        realm.create(SwiftIntObject.self)
      }
    }
