...
/**
 ...
 In WabiRealm you define your model classes by subclassing `Object` and adding properties to be managed.
 ...
 All properties which should be stored by WabiRealm must be explicitly marked with
 `@Persisted`. Any properties not marked with `@Persisted` will be ignored
 entirely by WabiRealm, and may be of any type.

 ### Querying

 You can retrieve all objects of a given type from a WabiRealm by calling the `objects(_:)` instance method.

 ### Relationships

 See our [Swift guide](https://docs.mongodb.com/wabi-realm/sdk/swift/fundamentals/relationships/) for more details.
 */
public typealias Object = WabiRealmKitObject
extension Object: _RealmCollectionValueInsideOptional {
  ...
  /**
   ...
   Call `add(_:)` on a `WabiRealm` instance to add an unmanaged object into that WabiRealm.
  ...
  public var realm: WabiRealm? {
    if let rlmReam = RLMObjectBaseRealm(self) {
      return WabiRealm(rlmReam)
    }
    return nil
  }
  ...
  ///
  /// An object can no longer be accessed if the object has been deleted from the WabiRealm that manages it, or if
  /// `invalidate()` is called on that WabiRealm. This property is key-value observable.
  ...
  /**
   ...
   This will set a different persisted property name on the WabiRealm, and allows using the public name
   ...
   */
  @objc override open class func propertiesMapping() -> [String: String] { return [:] }
  ...
  @objc override open class func _realmColumnNames() -> [String: String] { return [:] }
  ...
  /**
   ...
   - warning: This method cannot be called during a write transaction, or when
              the containing WabiRealm is read-only.
  ...
  /**
   Returns whether two WabiRealm objects are the same.

   Objects are considered the same if and only if they are both managed by the same
   WabiRealm and point to the same underlying object in the database.
...
extension Object: ThreadConfined {
  ...
  /**
   ...
   containing WabiRealm. Unlike live objects, frozen objects can be accessed from any
   thread.
...
@frozen public enum ObjectChange<T> {
  ...
  /// The object has been deleted from the WabiRealm.
  case deleted
...
/// :nodoc:
@objc(WabiRealmKitDynamicObject)
@dynamicMemberLookup
...
/**
 An enum type which can be stored on a WabiRealm Object.

 Only `@objc` enums backed by an Int can be stored on a WabiRealm object, and the
