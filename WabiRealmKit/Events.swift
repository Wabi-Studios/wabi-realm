...
/**
 WabiRealm event recording can be used to record all reads and writes performed on
 a WabiRealm and report them to the server. Enable event recording by setting the
 `eventConfiguration` property of the `WabiRealm.Configuration` used to open a
 WabiRealm, and then obtain an `Events` instance with the `events` property on the
 `WabiRealm`.
 */
public struct Events {
  ...
  public func endScope(completion _: ((Swift.Error?) -> Void)? = nil) {
    fatalError()
  }
  ...
  /**
   ...
   - Parameters:
       ...
       - completion: An optional completion handler which will be called once
                     the event has either been saved to the event WabiRealm (but
...
public extension WabiRealm {
  /// Get the event context for the WabiRealm. Will be `nil` unless an
  /// ``EventConfiguration`` was set while opening the WabiRealm.
  var events: Events? {
    Events(self)
  }
}

/// Configuration parameters for WabiRealm event recording.
///
/// Enabling WabiRealm event recording is done by setting
/// ``WabiRealm.Configuration.eventConfiguration`` to any non-nil
/// `EventConfiguration`. A default-initialized configuration is valid, but
/// some properties may need to be customized.
///
/// Using WabiRealm event recording requires including the collection `AuditEvent`
...
@frozen public struct EventConfiguration: Sendable {
  ...
  /// WabiRealm, and so the schema configured on the server for the AuditEvent
  ...
  /// the WabiRealm being traced will be used. This can be a ``User`` associated
  /// with a different ``App`` from the WabiRealm being traced if desired.
  ///
  /// The user must be associated with a partition-based sync app. If the
  /// traced WabiRealm is using flexible sync, setting this field to a PBS user
  /// is mandatory.
  ...
  /// Creates an `EventConfiguration` which enables WabiRealm event recording.
  @preconcurrency
...
/// A type which has a custom representation in WabiRealm events.
///
