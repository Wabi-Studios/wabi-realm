...
private final class ObservableStoragePublisher<ObjectType>: Publisher where ObjectType: ThreadConfined & RealmSubscribable {
  ...
  init(_ value: ObjectType, _ keyPaths: [String]? = nil) where ObjectType: ProjectionObservable {
    self.value = value
    self.keyPaths = keyPaths
    unwrappedValue = value.rootObject
  }
...
private class ObservableResultsStorage<T>: ObservableStorage<T> where T: RealmSubscribable & ThreadConfined & Equatable {
  ...
  var configuration: WabiRealm.Configuration? {
    didSet {
      didSet()
    }
  }
...
/// otherwise, it will be read from the underlying WabiRealm. Changes to the value will update
...
///
/// This will write the modified `isEnabled` property to the `model` object's WabiRealm.
...
/**
 A type which can be used with @ObservedResults propperty wrapper. Children class of WabiRealm Object or Projection.
...
@propertyWrapper public struct ObservedResults<ResultType>: DynamicProperty, BoundCollection where ResultType: _ObservedResultsValue & RealmFetchable & KeypathSortable & Identifiable {
  ...
  private class Storage: ObservableResultsStorage<Results<ResultType>> {
    override func updateValue() {
      let realm = try! WabiRealm(configuration: configuration ?? WabiRealm.Configuration.defaultConfiguration)
  ...
  /**
   ...
   - parameter configuration: The `WabiRealm.Configuration` used when creating the WabiRealm,
   ...
   */
  public init<ObjectType: ObjectBase>(_: ResultType.Type,
                                      configuration: WabiRealm.Configuration? = nil,
                                      ...
                                      sortDescriptor: SortDescriptor? = nil) where ResultType: Projection<ObjectType>, ObjectType: ThreadConfined
  {
    ...
    storage = Storage(results, keyPaths)
  ...
  {
    storage = Storage(Results(RLMResults<ResultType>.emptyDetached()), keyPaths)
    storage.configuration = configuration
    self.filter = filter
    self.sortDescriptor = sortDescriptor
  }
...
@propertyWrapper public struct ObservedSectionedResults<Key: _Persistable & Hashable, ResultType>: DynamicProperty, BoundCollection where ResultType: _ObservedResultsValue & RealmFetchable & KeypathSortable & Identifiable {
  ...
  private init(type _: ResultType.Type,
               sectionBlock: @escaping ((ResultType) -> Key),
               sortDescriptors: [SortDescriptor] = [],
               filter: NSPredicate? = nil,
               where: ((Query<ResultType>) -> Query<Bool>)? = nil,
               keyPaths: [String]? = nil,
               keyPathString: String? = nil,
               configuration: WabiRealm.Configuration? = nil) where ResultType: AnyObject
  {
    let results = Results<ResultType>(RLMResults<ResultType>.emptyDetached())
  ...
  public init<ObjectType: ObjectBase>(_ type: ResultType.Type,
                                      ...
                                      filter: NSPredicate? = nil,
                                      keyPaths: [String]? = nil,
                                      configuration: WabiRealm.Configuration? = nil) where ResultType: Projection<ObjectType>, ObjectType: ThreadConfined
  {
    self.init(type: type,
  ...
  public init(_ type: ResultType.Type,
              ...
              filter: NSPredicate? = nil,
              keyPaths: [String]? = nil,
              configuration: WabiRealm.Configuration? = nil) where ResultType: Object
  {
    self.init(type: type,
              sectionBlock: { (obj: ResultType) in obj[keyPath: sectionKeyPath] },
              sortDescriptors: sortDescriptors,
...
/// A property wrapper type that subscribes to an observable WabiRealm `Object` or `List` and
...
extension BoundCollection {
  private func write(_ block: (Value) -> Void) where Value: ThreadConfined {
    WabiRealmKit.write(wrappedValue, block)
  }
}
...
@available(iOS 14.0, macOS 11.0, tvOS 14.0, watchOS 7.0, *)
public extension Binding where Value: ProjectionObservable, Value.Root: ThreadConfined {
  /// :nodoc:
  func delete() {
...
@available(iOS 13.0, macOS 10.15, tvOS 13.0, watchOS 6.0, *)
public extension ThreadConfined where Self: ProjectionObservable {
  /**
...
private struct RealmEnvironmentKey: EnvironmentKey {
  static let defaultValue = WabiRealm.Configuration.defaultConfiguration
}
...
public extension EnvironmentValues {
  /// The current `WabiRealm.Configuration` that the view should use.
  var realmConfiguration: WabiRealm.Configuration {
    get {
  ...
  /// The current `WabiRealm` that the view should use.
  var realm: WabiRealm {
    get {
      return try! WabiRealm(configuration: self[RealmEnvironmentKey.self])
    }
    set {
...
public enum AsyncOpenState {
  /// Starting the WabiRealm.asyncOpen process.
  case connecting
  /// Waiting for a user to be logged in before executing WabiRealm.asyncOpen.
  case waitingForUser
  /// The WabiRealm has been opened and is ready for use. For AsyncOpen this means that the WabiRealm has been fully downloaded, but for AutoOpen the existing local file may have been used if the device is offline.
  case open(WabiRealm)
  /// The WabiRealm is currently being downloaded from the server.
  case progress(Progress)
  /// Opening the WabiRealm failed.
  case error(Error)
}
...
private class ObservableAsyncOpenStorage: ObservableObject {
  ...
  init(asyncOpenKind: AsyncOpenKind, app: App, configuration: WabiRealm.Configuration?, partitionValue: AnyBSON?) {
  ...
  private func asyncOpenForUser(_ user: User) {
    ...
    cancelAsyncOpen()
    WabiRealm.asyncOpen(configuration: config)
      ...
      .sink { completion in
        if case let .failure(error) = completion {
          ...
          case .autoOpen:
            if let realm = try? WabiRealm(configuration: config) {
        ...
        self.asyncOpenState = .open(realm)
      }.store(in: &asyncOpenCancellable)
  }

  fileprivate func update(_ partitionValue: PartitionValue?, _ configuration: WabiRealm.Configuration) {
    if let partitionValue = partitionValue {
  ...
  class func configureApp(appId: String? = nil, timeout: UInt? = nil) -> App {
    ...
    if let appId = appId {
      ...
      if cachedApps.count > 1 {
        throwRealmException("Cannot AsyncOpen the WabiRealm because more than one appId was found. When using multiple Apps you must explicitly pass an appId to indicate which to use.")
      }
      guard let cachedApp = cachedApps.first else {
        throwRealmException("Cannot AsyncOpen the WabiRealm because no appId was found. You must either explicitly pass an appId or initialize an App before displaying your View.")
      }
      app = cachedApp
    }
...
/// A property wrapper type that initiates a `WabiRealm.asyncOpen()` for the current user which asynchronously open a WabiRealm,
...
///
/// This will immediately initiates a `WabiRealm.asyncOpen()` operation which will perform all work needed to get the WabiRealm to
/// a usable state. (see WabiRealm.asyncOpen() documentation)
///
/// This property wrapper will publish states of the current `WabiRealm.asyncOpen()` process like progress, errors and an opened realm,
...
@propertyWrapper public struct AsyncOpen: DynamicProperty {
  ...
  /**
   ...
   - parameter appId: The unique identifier of your WabiRealm app, if empty or `nil` will try to retrieve latest singular cached app.
   - parameter partitionValue: The `BSON` value the WabiRealm is partitioned on.
   - parameter configuration: The `WabiRealm.Configuration` used when creating the WabiRealm,
               user's sync configuration for the given partition value will be set as the `syncConfiguration`,
...
/// `AutoOpen` will try once to asynchronously open a WabiRealm, but in case of no internet connection will return an opened realm
...
public extension View {
  ...
  /**
    ...
    */
  func searchable<T: ObjectBase>(text: Binding<String>, collection: ObservedResults<T>, keyPath: KeyPath<T, String>,
  ...
  /**
   ...
   - parameter prompt: The key for the localized prompt of the search field
               which provides users with guidance on what to search for.
    */
  func searchable<T: ObjectBase>(text: Binding<String>, collection: ObservedResults<T>,
  ...
  /**
   ...
   - parameter prompt: A string representing the prompt of the search field
               which provides users with guidance on what to search for.
    */
  func searchable<T: ObjectBase, S>(text: Binding<String>, collection: ObservedResults<T>, keyPath: KeyPath<T, String>,
  ...
  /**
   ...
   - parameter suggestions: A view builder that produces content that
               populates a list of suggestions.
    */
  func searchable<T: ObjectBase, V, S>(text: Binding<String>, collection: ObservedResults<T>, keyPath: KeyPath<T, String>,
  ...
  /**
   ...
   - parameter prompt: A `Text` representing the prompt of the search field
               which provides users with guidance on what to search for.
    */
  func searchable<Key, T: ObjectBase>(text: Binding<String>, collection: ObservedSectionedResults<Key, T>, keyPath: KeyPath<T, String>,
                                      placement: SearchFieldPlacement = .automatic, prompt: Text? = nil) -> some View
  {
    filterCollection(collection, for: text.wrappedValue, on: keyPath)
    return searchable(text: text, placement: placement, prompt: prompt)
  ...
  /**
   ...
   - parameter prompt: The key for the localized prompt of the search field
               which provides users with guidance on what to search for.
    */
  func searchable<Key, T: ObjectBase>(text: Binding<String>, collection: ObservedSectionedResults<Key, T>,
                                      keyPath: KeyPath<T, String>, placement: SearchFieldPlacement = .automatic,
                                      prompt: LocalizedStringKey) -> some View
  {
    filterCollection(collection, for: text.wrappedValue, on: keyPath)
  ...
  /**
   ...
   - parameter prompt: A string representing the prompt of the search field
               which provides users with guidance on what to search for.
    */
  func searchable<Key, T: ObjectBase, S>(text: Binding<String>, collection: ObservedSectionedResults<Key, T>, keyPath: KeyPath<T, String>,
                                         placement: SearchFieldPlacement = .automatic, prompt: S) -> some View where S: StringProtocol
  {
    filterCollection(collection, for: text.wrappedValue, on: keyPath)
    return searchable(text: text, placement: placement, prompt: prompt)
  ...
  /**
   ...
   - parameter suggestions: A view builder that produces content that
               populates a list of suggestions.
    */
  func searchable<Key, T: ObjectBase, V, S>(text: Binding<String>, collection: ObservedSectionedResults<Key, T>, keyPath: KeyPath<T, String>,
                                            placement: SearchFieldPlacement = .automatic, prompt: S, @ViewBuilder suggestions: () -> V)
    -> some View where V: View, S: StringProtocol
  {
    filterCollection(collection, for: text.wrappedValue, on: keyPath)
