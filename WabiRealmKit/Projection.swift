...
public struct Projected<T: ObjectBase, Value>: AnyProjected {
  ...
  /// :nodoc:
  @available(*, unavailable, message: "@Persisted can only be used as a property on a WabiRealm object")
  ...
  public init(_ projectedKeyPath: KeyPath<T, Value>) {
    _projectedKeyPath = projectedKeyPath
  }
}
...
public protocol ProjectionObservable: AnyObject, ThreadConfined {
  /// The Projection's underlying type - a child of WabiRealm `Object` or `EmbeddedObject`.
...
/// ``Projection`` is a light weight model of the original WabiRealm ``Object`` or ``EmbeddedObject``.
...
/// - `Exclusion` - all properties of the original WabiRealm object that were not defined in the projection model will be excluded from projection.
...
/// ### Querying
///
/// You can retrieve all Projections of a given type from a WabiRealm by calling the `objects(_:)` of WabiRealm or `init(projecting:)`
...
public extension ProjectionObservable {
  /**
  ...
  /**
   ...
   - returns: A token which must be held for as long as you want updates to be delivered.
   */
  func observe(keyPaths: [PartialKeyPath<Self>],
               on queue: DispatchQueue? = nil,
               _ block: @escaping (ObjectChange<Self>) -> Void) -> NotificationToken
  {
...
extension Projection: ThreadConfined where Root: ThreadConfined {
  ...
  /**
   ...
   Unlike normal WabiRealm live objects, the frozen copy can be read from any thread, and the values
   read will never update to reflect new writes to the WabiRealm. Frozen collections can be queried
   like any other WabiRealm collection. Frozen objects cannot be mutated, and cannot be observed for
   change notifications.
   Unmanaged WabiRealm objects cannot be frozen.
...
public extension ProjectionObservable {
  /// :nodoc:
  func _observe<S>(_ keyPaths: [String]?, on queue: DispatchQueue?, _ subscriber: S) -> NotificationToken where S: Subscriber, S.Input == Self {
    return observe(keyPaths: keyPaths ?? [], on: queue) { (change: ObjectChange<S.Input>) in
      switch change {
      case let .change(projection, _):
  ...
  /// :nodoc:
  func _observe<S>(_: [String]?, _ subscriber: S) -> NotificationToken where S: Subscriber, S.Input == Void {
  ...
  fileprivate func schema<T: ProjectionObservable>(for obj: T) -> [ProjectionProperty] {
    ...
    for child in Mirror(reflecting: obj).children {
      ...
      guard !originPropertyLabel.isEmpty else {
        throwRealmException("@Projected property '\(label)' must be a part of WabiRealm object")
      }
