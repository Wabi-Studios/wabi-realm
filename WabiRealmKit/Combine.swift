...
/// A protocol which defines a default identity for WabiRealm Objects
///
/// Declaring your Object subclass as conforming to this protocol will supply
/// a default implementation for `Identifiable`'s `id` which works for WabiRealm
/// Objects:
///
...
public extension ObjectKeyIdentifiable where Self: ObjectBase {
  /// A stable identifier for this object. For managed WabiRealm objects, this
...
public extension Publisher {
  /// Freezes all WabiRealm objects and collections emitted by the upstream publisher
  ///
  /// Freezing a WabiRealm object makes it no longer live-update when writes are
  /// made to the WabiRealm and makes it safe to pass freely between threads
  ...
  /// Freezes all WabiRealm object changesets emitted by the upstream publisher.
  ///
  /// Freezing a WabiRealm object changeset makes the included object reference
  /// no longer live-update when writes are made to the WabiRealm and makes it
  ...
  /// Freezes all WabiRealm collection changesets from the upstream publisher.
  ///
  /// Freezing a WabiRealm collection changeset makes the included collection
  /// reference no longer live-update when writes are made to the WabiRealm and
  ...
  /// Freezes all WabiRealm sectioned results changesets from the upstream publisher.
  ///
  /// Freezing a WabiRealm sectioned results changeset makes the included  sectioned results
  /// reference no longer live-update when writes are made to the WabiRealm and
  /// makes it safe to pass freely between threads without using
  ...
  /// Freezes all WabiRealm projection changesets emitted by the upstream publisher.
  ///
  /// Freezing a WabiRealm projection changeset makes the included projection reference
  /// no longer live-update when writes are made to the WabiRealm and makes it
  /// safe to pass freely between threads without using
...
public extension Publisher where Output: ThreadConfined {
  ...
  ///
  /// Each call to `receive(on:)` on a publisher which emits WabiRealm
...
public extension Publisher {
  ...
  /// Enables passing WabiRealm collection changesets to a different dispatch queue.
  ///
  /// Each call to `receive(on:)` on a publisher which emits WabiRealm
  /// thread-confined objects must be proceeded by a call to
  /// `.threadSafeReference()`. The returned publisher handles the required
  ...
  /// Enables passing WabiRealm sectioned results changesets to a different dispatch queue.
  ///
  /// Each call to `receive(on:)` on a publisher which emits WabiRealm
  /// thread-confined objects must be proceeded by a call to
  /// `.threadSafeReference()`. The returned publisher handles the required
...
/// - parameter projection: A projection of WabiRealm Object to observe.
/// - parameter keyPaths: The publisher emits changes on these property keyPaths. If `nil` the publisher emits changes for every property.
...
/// A subscription which wraps a WabiRealm notification.
@available(macOS 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
@frozen public struct ObservationSubscription: Subscription {
  ...
  ///
  /// WabiRealm publishers do not support backpressure and so this function does nothing.
...
/// A subscription which wraps a WabiRealm AsyncOpenTask.
@available(macOS 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
@frozen public struct AsyncOpenSubscription: Subscription {
  private let task: WabiRealm.AsyncOpenTask

  internal init(task: WabiRealm.AsyncOpenTask,
...
/// Combine publishers for WabiRealm types.
///
...
public enum RealmPublishers {
  private static func realm<S: Scheduler>(_ config: RLMRealmConfiguration, _ scheduler: S) -> WabiRealm? {
    try? WabiRealm(RLMRealm(configuration: config, queue: scheduler as? DispatchQueue))
  }

  private static func realm<S: Scheduler>(_ sourceRealm: WabiRealm, _ scheduler: S) -> WabiRealm? {
    return realm(sourceRealm.rlmRealm.configuration, scheduler)
  }

  /// A publisher which emits an asynchronously opened WabiRealm.
  @frozen public struct AsyncOpenPublisher: Publisher {
    /// This publisher can fail if there is an error opening the WabiRealm.
    public typealias Failure = Error
    /// This publisher emits an opened WabiRealm.
    public typealias Output = WabiRealm

    private let configuration: WabiRealm.Configuration
    private let callbackQueue: DispatchQueue
    private let onProgressNotificationCallback: ((SyncSession.Progress) -> Void)?

    internal init(configuration: WabiRealm.Configuration,
    ...
    /// - Returns: A publisher that emits an asynchronously opened WabiRealm.
    ...
    public func receive<S>(subscriber: S) where S: Subscriber, S.Failure == Failure, Output == S.Input {
      subscriber.receive(subscription: AsyncOpenSubscription(task: WabiRealm.AsyncOpenTask(rlmTask: RLMRealm.asyncOpen(with: configuration.rlmConfiguration, callbackQueue: callbackQueue, callback: { rlmRealm, error in
        if let realm = rlmRealm.flatMap(WabiRealm.init) {
          _ = subscriber.receive(realm)
          subscriber.receive(completion: .finished)
        } else {
          subscriber.receive(completion: .failure(error ?? WabiRealm.Error.callFailed))
        }
  ...
  /// A publisher which emits Void each time the WabiRealm is refreshed.
  ///
  /// Despite the name, this actually emits *after* the WabiRealm is refreshed.
  @frozen public struct RealmWillChange: Publisher {
    ...
    private let realm: WabiRealm

    internal init(_ realm: WabiRealm) {
      self.realm = realm
    }

    /// Captures the `NotificationToken` produced by observing a WabiRealm Collection.
    ///
    /// This allows you to do notification skipping when performing a `WabiRealm.write(withoutNotifying:)`. You should use this call if you
    /// require to write to the WabiRealm database and ignore this specific observation chain.
  ...
  @frozen public struct Value<Subscribable: RealmSubscribable>: Publisher where Subscribable: ThreadConfined {
    ...
    ///
    /// For WabiRealm Publishers, this determines which queue the underlying
  ...
  /// A helper publisher used to support `receive(on:)` on WabiRealm publishers.
  @frozen public struct Handover<Upstream: Publisher, S: Scheduler>: Publisher where Upstream.Output: ThreadConfined {
    ...
    internal init(_ upstream: Upstream, _ scheduler: S, _ realm: WabiRealm) {
  ...
  @frozen public struct ObjectChangeset<O: ThreadConfined>: Publisher {
    ...
    /// which properties of that object have changed each time a WabiRealm is
    ...
    /// :nodoc:
    public func receive<S>(subscriber: S) where S: Subscriber, S.Failure == Never, Output == S.Input {
      let token = observe(queue) { change in
        switch change {
  ...
  @frozen public struct DeferredHandoverObjectChangeset<Upstream: Publisher, T: ThreadConfined, S: Scheduler>: Publisher where Upstream.Output == ObjectChange<T> {
    ...
    private enum Handover {
      ...
      // We also hold a reference to a pinned WabiRealm to ensure that the
  ...
  @frozen public struct CollectionChangeset<Collection: RealmCollection>: Publisher {
    ...
    /// :nodoc:
    public func receive<S>(subscriber: S) where S: Subscriber, S.Failure == Never, Output == S.Input {
      let token = collection.observe(keyPaths: keyPaths, on: queue) { change in
  ...
  @frozen public struct DeferredHandoverCollectionChangeset<Upstream: Publisher, T: RealmCollection, S: Scheduler>: Publisher where Upstream.Output == RealmCollectionChange<T> {
    ...
    private enum Handover {
      ...
      // a pinned WabiRealm to ensure that the version which the change
