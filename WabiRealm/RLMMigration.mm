...
- (void)enumerateObjects:(NSString *)className
  ...
  RLMResults *objects = [_realm.schema schemaForClassName:className]
                            ? [_realm allObjects:className]
                            : nil;
  RLMResults *oldObjects = [_oldRealm.schema schemaForClassName:className]
                               ? [_oldRealm allObjects:className]
                               : nil;
...
- (void)execute:(RLMMigrationBlock)block objectClass:(Class)dynamicObjectClass {
  ...
  @autoreleasepool {
    // disable all primary keys for migration and use DynamicObject for all
    // types
...
- (BOOL)deleteDataForClassName:(NSString *)name {
  ...
  TableRef table =
      ObjectStore::table_for_object_type(_realm.group, name.UTF8String);
  if (!table) {
    return false;
  }
