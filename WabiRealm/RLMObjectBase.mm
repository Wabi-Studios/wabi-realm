...
static bool isManagedAccessorClass(Class cls) {
  ...
  return strncmp(className, accessorClassPrefix,
                 sizeof(accessorClassPrefix) - 1) == 0;
}
...
static id validatedObjectForProperty(
  ...
  if (NSArray *array = RLMDynamicCast<NSArray>(value)) {
    if (array.count > properties.count) {
      @throw RLMException(
          ...
          (unsigned long long)properties.count);
    }
    NSUInteger i = 0;
    for (id val in array) {
      RLMProperty *prop = properties[i++];
      [self setValue:validatedObjectForProperty(RLMCoerceToNil(val),
    ...
    for (RLMProperty *prop in properties) {
      id obj = RLMValidatedValueForProperty(value, prop.name,
      ...
      [self setValue:validatedObjectForProperty(RLMCoerceToNil(obj),
                                                objectSchema, prop, schema)
              forKey:prop.name];
    }
  }
...
// Generic Swift properties can't be dynamic, so KVO doesn't work for them by
// default
...
+ (void)initializeLinkedObjectSchemas {
  for (RLMProperty *prop in self.sharedSchema.properties) {
    if (prop.type == RLMPropertyTypeObject &&
      ...
      [[RLMSchema classForString:prop.objectClassName]
          initializeLinkedObjectSchemas];
    }
  }
}
...
- (NSString *)descriptionWithMaxDepth:(NSUInteger)depth {
  ...
  for (RLMProperty *property in _objectSchema.properties) {
    ...
    if ([object respondsToSelector:@selector(descriptionWithMaxDepth:)]) {
      ...
      sub = [NSString
          stringWithFormat:@"<%@ — %lu total bytes>",
                           [dataDescription
                               substringWithRange:NSMakeRange(
                                                      1,
                                                      dataDescription.length -
                                                          2)],
    ...
    [mString appendFormat:@"\t%@ = %@;\n", property.name,
                          [sub stringByReplacingOccurrencesOfString:@"\n"
                                                         withString:@"\n\t"]];
  }
...
- (NSUInteger)hash {
  if (_objectSchema.primaryKeyProperty) {
    ...
    id primaryProperty =
        [self valueForKey:_objectSchema.primaryKeyProperty.name];

    // modify the hash of our primary key value to avoid potential (although
    // unlikely) collisions
...
// This enables to override the propertiesMapping in Swift, it is not to be used
// in Objective-C API.
...
RLMObjectSchema *
RLMObjectBaseObjectSchema(__unsafe_unretained RLMObjectBase *object) {
...
void RLMObjectBaseSetObjectForKeyedSubscript(RLMObjectBase *object,
                                             NSString *key, id obj) {
  if (!object) {
    return;
  }
...
BOOL RLMObjectBaseAreEqual(RLMObjectBase *o1, RLMObjectBase *o2) {
  ...
  if ((o1 && ![o1 isKindOfClass:RLMObjectBase.class]) ||
      (o2 && ![o2 isKindOfClass:RLMObjectBase.class])) {
...
static id resolveObject(RLMObjectBase *obj, RLMRealm *realm) {
  RLMObjectBase *resolved = RLMCreateManagedAccessor(
      obj.class, &realm->_info[obj->_info->rlmObjectSchema.className]);
...
id RLMObjectFreeze(RLMObjectBase *obj) {
  ...
  if (!obj) {
    @throw RLMException(@"Cannot freeze an object in the same write "
...
id RLMValidatedValueForProperty(id object, NSString *key, NSString *className) {
  @try {
    ...
    if ([e.name isEqualToString:NSUndefinedKeyException]) {
      @throw RLMException(@"Invalid value '%@' to initialize object of type "
                          @"'%@': missing key '%@'",
...
struct ObjectChangeCallbackWrapper {
  ...
  void populateProperties(wabi_realm::CollectionChangeSet const &c) {
    ...
    for (RLMProperty *property in object->_info->rlmObjectSchema
             .computedProperties) {
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<RLMPropertyChange: %p> %@ %@ -> %@",
                                    (__bridge void *)self, _name,
                                    _previousValue, _value];
}
@end

enum class TokenState { Initializing, Cancelled, Ready };
...
- (void)observe:(RLMObjectBase *)obj
  ...
  _token = _object.add_notification_callback(
      ObjectChangeCallbackWrapper{block, obj, completion},
      obj->_info->keyPathArrayFromStringArray(keyPaths));
}
...
RLMNotificationToken *RLMObjectBaseAddNotificationBlock(
    RLMObjectBase *obj, NSArray<NSString *> *keyPaths, dispatch_queue_t queue,
    RLMObjectNotificationCallback block) {
  if (!obj->_realm) {
    @throw RLMException(
        @"Only objects which are managed by a WabiRealm support "
  ...
  RLMThreadSafeReference *tsr =
      [RLMThreadSafeReference referenceWithThreadConfined:(id)obj];
  auto token = [[RLMObjectNotificationToken alloc] init];
  token->_realm = obj->_realm;
  RLMRealmConfiguration *config = obj->_realm.configuration;
  dispatch_async(queue, ^{
    @autoreleasepool {
      [token addNotificationBlock:block
...
RLMObjectAddNotificationBlock(RLMObjectBase *obj, RLMObjectChangeBlock block,
                              NSArray<NSString *> *keyPaths,
                              dispatch_queue_t queue) {
  return RLMObjectBaseAddNotificationBlock(
      obj, keyPaths, queue,
      ^(RLMObjectBase *, NSArray<NSString *> *propertyNames, NSArray *oldValues,
        NSArray *newValues, NSError *error) {
        if (error) {
          ...
          auto properties =
              [NSMutableArray arrayWithCapacity:propertyNames.count];
...
uint64_t
RLMObjectBaseGetCombineId(__unsafe_unretained RLMObjectBase *const obj) {
  if (obj.invalidated) {
    RLMVerifyAttached(obj);
  }
  if (obj->_realm) {
...
@implementation WabiRealmKitObject
...
@implementation WabiRealmKitEmbeddedObject
+ (BOOL)accessInstanceVariablesDirectly {
  return NO;
}
@end

@implementation WabiRealmKitAsymmetricObject
+ (BOOL)accessInstanceVariablesDirectly {
  return NO;
}
