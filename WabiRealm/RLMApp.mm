...
public:
  ...
  void send_request_to_server(
    ...
    [m_transport
        sendRequestToServer:rlmRequest
                 completion:^(RLMResponse *response) {
                   ...
                   // Convert the RLMResponse to an app:Response and pass
                   // downstream to the object store
  ...
  id<RLMNetworkTransport> transport() const { return m_transport; }
...
- (instancetype)initWithBaseURL:(nullable NSString *)baseURL
  ...
  if (self = [super init]) {
    ...
    _config.device_info.sdk = "WabiRealm Swift";
    ...
    RLMNSStringToStdString(_config.device_info.platform_version,
                           [processInfo operatingSystemVersionString]
                               ?: @"unknown");
    RLMNSStringToStdString(_config.device_info.sdk_version,
                           REALM_COCOA_VERSION);
    return self;
  }
  return nil;
...
- (instancetype)initWithApp:(std::shared_ptr<app::App>)app
                      token:(app::App::Token &&)token {
  if (self = [super init]) {
    _app = std::move(app);
...
- (instancetype)initWithApp:(std::shared_ptr<realm::app::App>)app {
  if (self = [super init]) {
    ...
    _app = app;
    _syncManager =
        [[RLMSyncManager alloc] initWithSyncManager:_app->sync_manager()];
    return self;
  }
...
- (instancetype)initWithId:(NSString *)appId
  ...
  if (self = [super init]) {
    ...
    _app = RLMTranslateError([&] {
      return app::App::get_shared_app(
          configuration.config,
          [RLMSyncManager configurationWithRootDirectory:rootDirectory
                                                   appId:appId]);
    });
...
+ (instancetype)appWithId:(NSString *)appId
  ...
  RLMApp *app = [[RLMApp alloc] initWithId:appId
...
+ (instancetype)uncachedAppWithId:(NSString *)appId
  ...
  auto app = RLMTranslateError([&] {
    return app::App::get_uncached_app(
        configuration.config,
        [RLMSyncManager configurationWithRootDirectory:rootDirectory
                                                 appId:appId]);
  });
...
- (void)loginWithCredential:(RLMCredentials *)credentials
  ...
  return RLMTranslateError([&] {
    return _app->log_in_with_credentials(credentials.appCredentials,
                                         completion);
  });
}
...
- (RLMUser *)switchToUser:(RLMUser *)syncUser {
  return RLMTranslateError([&] {
    return [[RLMUser alloc] initWithUser:_app->switch_user(syncUser._syncUser)
                                     app:self];
  });
}
...
- (RLMPushClient *)pushClientWithServiceName:(NSString *)serviceName {
  return RLMTranslateError([&] {
    return [[RLMPushClient alloc]
        initWithPushClient:_app->push_notification_client(
...
- (void)setAuthorizationDelegate:(id<RLMASLoginDelegate>)authorizationDelegate
    API_AVAILABLE(ios(13.0), macos(10.15), tvos(13.0), watchos(6.0)) {
  _authorizationDelegate = authorizationDelegate;
}

- (id<RLMASLoginDelegate>)authorizationDelegate API_AVAILABLE(ios(13.0),
                                                              macos(10.15),
                                                              tvos(13.0),
                                                              watchos(6.0)) {
...
- (void)authorizationController:(__unused ASAuthorizationController *)controller
    ...
    API_AVAILABLE(ios(13.0), macos(10.15), tvos(13.0), watchos(6.0)) {
  NSString *jwt =
      [[NSString alloc] initWithData:((ASAuthorizationAppleIDCredential *)
  ...
  [self loginWithCredential:[RLMCredentials credentialsWithAppleToken:jwt]
                 completion:^(RLMUser *user, NSError *error) {
                   if (user) {
                     [self.authorizationDelegate
                         authenticationDidCompleteWithUser:user];
                   } else {
                     [self.authorizationDelegate
                         authenticationDidFailWithError:error];
                   }
                 }];
}
...
- (RLMAppSubscriptionToken *)subscribe:(RLMAppNotificationBlock)block {
  return [[RLMAppSubscriptionToken alloc]
      initWithApp:_app
            token:_app->subscribe([block, self](auto &) { block(self); })];
}
