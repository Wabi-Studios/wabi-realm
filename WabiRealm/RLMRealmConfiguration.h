...
/**
 A block called when opening a WabiRealm for the first time during the life
...
/**
 A block which receives a subscription set instance, that can be used to add an
 initial set of subscriptions which will be executed when the WabiRealm is first
 opened.
 */
RLM_SWIFT_SENDABLE
typedef void (^RLMFlexibleSyncInitialSubscriptionsBlock)(
    RLMSyncSubscriptionSet *_Nonnull subscriptions);

/**
 An `RLMRealmConfiguration` instance describes the different options used to
 create an instance of a WabiRealm.
 ...
 configuration rather than creating a new object each time you open a WabiRealm.
 */
...
/**
 ...
 @return The default WabiRealm configuration.
 */
...
/**
 ...
 @param configuration The new default WabiRealm configuration.
 */
+ (void)setDefaultConfiguration:(RLMRealmConfiguration *)configuration;

#pragma mark - Properties

/// The local URL of the WabiRealm file. Mutually exclusive with
/// `inMemoryIdentifier`; setting one of the two properties will automatically
/// nil out the other.
@property(nonatomic, copy, nullable) NSURL *fileURL;

/// A string used to identify a particular in-memory WabiRealm. Mutually
/// exclusive with `fileURL`, `seedFilePath`and `syncConfiguration`; setting any
/// one of the three properties will automatically nil out the other two.
...
///
/// For non-synchronized Realms, this is required to be able to open WabiRealm
...
/// requires disabling WabiRealm's reader/writer coordination, so committing a
...
/// write transactions on the WabiRealm. In addition, it will skip some
/// automatic writes made to the WabiRealm, such as to initialize the
/// WabiRealm's schema. Setting `readOnly = YES` is not strictly required for
/// Realms which the sync user does not have write access to, but is highly
/// recommended as it will improve error reporting and catch some errors
/// earlier.
///
...
/**
 Whether to recreate the WabiRealm file with the provided schema if a migration
 is required. This is the case when the stored schema differs from the provided
...
/**
 The maximum number of live versions in the WabiRealm file before an exception
 will be thrown when attempting to start a write transaction.

 WabiRealm provides MVCC snapshot isolation, meaning that writes on one thread
 do not overwrite data being read on another thread, and instead write a new
 copy of that data. When a WabiRealm refreshes it updates to the latest version
 of the data and releases the old versions, allowing them to be overwritten by
 ...
 performing long operations on background threads which do not allow the
 WabiRealm to refresh.
...
/**
 When opening the WabiRealm for the first time, instead of creating an empty
 file, the WabiRealm file will be copied from the provided seed file path and
 used instead. This can be used to open a WabiRealm file with pre-populated
 data.

 If a realm file already exists at the configuration's destination path, the
 seed file will not be copied and the already existing realm will be opened
 instead.
 ...
 This option is mutually exclusive with `inMemoryIdentifier`. Setting a
 `seedFilePath` will nil out the `inMemoryIdentifier`.
 */
...
/**
 ...
 This property is mutually exclusive with both `inMemoryIdentifier` and
 `fileURL`; setting any one of the three properties will automatically nil out
 the other two.
