...
NSUInteger RLMUnmanagedFastEnumerate(id collection,
    ...
    if (prop.collection && prop.swiftAccessor) {
      ...
      for (size_t i = 0; i < count; ++i) {
        ...
        base._rlmCollection = [[[cls _backingCollectionType] alloc]
            initWithParent:collection.get(i)
                  property:prop
                parentInfo:info];
...
NSString *RLMDescriptionWithMaxDepth(NSString *name,
  ...
  auto str = [NSMutableString
      stringWithFormat:@"%@<%@> <%p> (\n", name,
                       [collection objectClassName]
                           ?: RLMTypeToString([collection type]),
                       (void *)collection];
...
RLMSortDescriptorsToKeypathArray(NSArray<RLMSortDescriptor *> *properties) {
  ...
  for (RLMSortDescriptor *desc in properties) {
    if ([desc.keyPath rangeOfString:@"@"].location != NSNotFound) {
      @throw RLMException(@"Cannot sort on key path '%@': KVC collection "
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<RLMCollectionChange: %p> insertions: "
...
struct CollectionCallbackWrapper {
  ...
  void operator()(wabi_realm::CollectionChangeSet const &changes) {
    if (ignoreChangesInInitialNotification) {
      ...
      block(collection, nil, nil);
    } else if (!changes.collection_root_was_deleted ||
               !changes.deletions.empty()) {
...
RLMNotificationToken *RLMAddNotificationBlock(id c, id block,
  ...
  if (!queue) {
    ...
    token->_token = [collection
        addNotificationCallback:block
                       keyPaths:info->keyPathArrayFromStringArray(keyPaths)];
    return token;
  }

  RLMThreadSafeReference *tsr =
      [RLMThreadSafeReference referenceWithThreadConfined:collection];
  ...
  dispatch_async(queue, ^{
    ...
    RLMRealm *realm = token->_realm = [RLMRealm realmWithConfiguration:config
                                                                 queue:queue
                                                                 error:nil];
