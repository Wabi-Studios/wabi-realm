...
struct CollectionCallbackWrapper {
  ...
  void operator()(realm::SectionedResultsChangeSet const &changes) {
    ...
    block(collection,
          [[RLMSectionedResultsChange alloc] initWithChanges:changes]);
  }
};

template <typename Function>
__attribute__((always_inline)) auto translateErrors(Function &&f) {
  return translateCollectionError(static_cast<Function &&>(f),
                                  @"SectionedResults");
}
} // anonymous namespace
...
- (NSString *)description {
  return
      [NSString stringWithFormat:
                    @"<RLMSectionedResultsChange: %p> {\n\tinsertions: "
                    @"%@,\n\tdeletions: %@,\n\tmodifications: "
                    @"%@,\n\tsectionsToInsert: %@,\n\tsectionsToRemove: %@\n}",
                    (__bridge void *)self, indexPathToString(self.insertions),
                    indexPathToString(self.deletions),
                    indexPathToString(self.modifications),
                    indexSetToString(self.sectionsToInsert),
                    indexSetToString(self.sectionsToRemove)];
}
...
- (instancetype)initWithSectionedResults:
    (RLMSectionedResults *)sectionedResults {
  if (self = [super init]) {
...
- (NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)state
  ...
  for (NSUInteger index = state->state; index < count && batchCount < len;
       ++index) {
    id<RLMSectionedResult> sectionedResults =
        [_sectionedResult objectAtIndex:index];
...
- (instancetype)initWithResults:(realm::Results &&)results
  ...
  if (self = [super init]) {
    ...
    _sectionedResults = _results.sectioned_results(
        SectionedResultsKeyProjection{_info, _keyBlock});
  }
  return self;
}
...
- (instancetype)initWithResults:(RLMResults *)results
  ...
  if (self = [super init]) {
    ...
    _sectionedResults = results->_results.sectioned_results(
        SectionedResultsKeyProjection{_info, _keyBlock});
  }
  return self;
}
...
- (NSUInteger)count {
  return translateErrors([&] { return _sectionedResults.size(); });
}
...
- (RLMNotificationToken *)addNotificationBlock:
    (void (^)(RLMResults *, RLMSectionedResultsChange *))block {
  ...
  return translateErrors([&] {
    if (realm.isFrozen) {
      return [[RLMSectionedResults alloc]
          initWithSectionedResults:_sectionedResults.freeze(realm->_realm)
                        objectInfo:_info->resolve(realm)
                          keyBlock:_keyBlock];
    } else {
      auto sr = _sectionedResults.freeze(realm->_realm);
      sr.reset_section_callback(
          SectionedResultsKeyProjection{&_info->resolve(realm), _keyBlock});
      return [[RLMSectionedResults alloc]
          initWithSectionedResults:std::move(sr)
                        objectInfo:_info->resolve(realm)
                          keyBlock:_keyBlock];
    }
  });
...
- (NSString *)description {
  ...
  if (_info) {
    objType =
        [NSString stringWithFormat:@"<%@>", _info->rlmObjectSchema.className];
  }
  const NSUInteger maxObjects = 100;
  auto str =
      [NSMutableString stringWithFormat:@"RLMSectionedResults%@ <%p> (\n",
                                        objType, (void *)self];
...
- (NSString *)description {
  const NSUInteger maxObjects = 100;
  auto str =
      [NSMutableString stringWithFormat:@"RLMSection <%p> (\n", (void *)self];
  size_t index = 0, skipped = 0;
  for (id obj in self) {
...
- (id)objectAtIndex:(NSUInteger)index {
  ...
  return translateErrors([&] { return ctx.box(_resultsSection[index]); });
}

- (NSUInteger)count {
  return translateErrors([&] { return _resultsSection.size(); });
}

- (id<RLMValue>)key {
  return translateErrors([&] { return RLMMixedToObjc(_resultsSection.key()); });
}
...
+ (instancetype)objectWithThreadSafeReference:
  ...
  RLMSectionedResults *sr =
      [[RLMSectionedResults alloc] initWithResults:std::move(results)
                                             realm:realm
                                        objectInfo:realm->_info[objType]
                                          keyBlock:metadata.keyBlock];
  return translateErrors([&] {
    return [[RLMSection alloc]
        initWithResultsSection:sr->_sectionedResults[RLMObjcToMixed(
                                   metadata.sectionKey)]
                        parent:sr];
  });
}
...
- (instancetype)resolveInRealm:(RLMRealm *)realm {
  return translateErrors([&] {
    RLMSectionedResults *sr =
        realm.isFrozen ? [_parent freeze] : [_parent thaw];
    return [[RLMSection alloc]
        initWithResultsSection:sr->_sectionedResults[RLMObjcToMixed(self.key)]
                        parent:sr];
  });
}
