...
// being visbile only to the obj-c runtime and not the linker, which means that
...
// performance hit.
[[clang::objc_runtime_visible]] @interface WabiRealmKitObjectId : RLMObjectId
@end

@implementation RLMObjectId
- (instancetype)init {
  if ((self = [super init])) {
    if (auto cls = [WabiRealmKitObjectId class]; cls && cls != self.class) {
    ...
    if (!wabi_realm::ObjectId::is_valid_str(str)) {
      if (error) {
        NSString *msg = [NSString
            stringWithFormat:
                @"Invalid Object ID string '%@': must be 24 hex digits",
                string];
