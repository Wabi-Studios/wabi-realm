...
- (RLMRealmConfiguration *)configurationWithPartitionValue:
  ...
  return [self
      configurationWithPartitionValue:partitionValue
...
- (std::string)pathForPartitionValue:(std::string const &)value {
  ...
  NSString *encodedPartitionValue =
      [@(value.data()) stringByAddingPercentEncodingWithAllowedCharacters:
                           [NSCharacterSet URLPathAllowedCharacterSet]];
  NSString *overEncodedRealmName = [[NSString alloc]
      initWithFormat:@"%@/%@", self.identifier, encodedPartitionValue];
  auto legacyPath = _user->sync_manager()->path_for_realm(
      config, std::string(overEncodedRealmName.UTF8String));
...
- (std::string)pathForFlexibleSync {
  if (!_user) {
    @throw RLMException(@"This is an exceptional state, `RLMUser` cannot be "
                        @"initialised without a reference to `SyncUser`");
  }
...
- (NSArray<RLMSyncSession *> *)allSessions {
  ...
  for (auto session : sessions) {
    [buffer addObject:[[RLMSyncSession alloc]
                          initWithSyncSession:std::move(session)]];
  }
  return [buffer copy];
}
...
- (NSArray<RLMUserIdentity *> *)identities {
  ...
  for (auto &identity : identities) {
    [buffer
        addObject:[[RLMUserIdentity alloc]
                      initUserIdentityWithProviderType:@(identity.provider_type
                                                             .c_str())
...
- (void)refreshCustomDataWithCompletion:(RLMUserCustomDataBlock)completion {
  _user->refresh_custom_data(
      [completion, self](std::optional<app::AppError> error) {
        if (!error) {
...
- (void)linkUserWithCredentials:(RLMCredentials *)credentials
  ...
  _app._realmApp->link_user(
      _user, credentials.appCredentials,
...
- (void)callFunctionNamed:(NSString *)name
  ...
  _app._realmApp->call_function(
      _user, name.UTF8String, args,
...
- (RLMUserSubscriptionToken *)subscribe:(RLMUserNotificationBlock)block {
  return [[RLMUserSubscriptionToken alloc]
      initWithUser:_user
             token:_user->subscribe([block, self](auto &) { block(self); })];
}
@end
...
static NSString *
userProfileMemberToNSString(const std::optional<std::string> &member) {
  if (member == util::none) {
    return nil;
  }
  return @(member->c_str());
...
using UserProfileMember =
    std::optional<std::string> (SyncUserProfile::*)() const;
