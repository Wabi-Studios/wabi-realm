...
// Override all of the methods for creating a XCTestCase object to capture the
// current test name
...
- (void)setUp {
  ...
  if (!self.isParent) {
    // For multi-process tests, the child's concept of a default path needs to
    // match the parent. RLMRealmConfiguration isn't aware of this, but our
    // test's RLMDefaultRealmURL helper does. Use it to reset the default
    // configuration's path so it matches the parent.
...
- (NSString *)locateXCTest {
  ...
  for (NSString *directory in [pathString componentsSeparatedByString:@":"]) {
    NSString *candidatePath =
        [directory stringByAppendingPathComponent:@"xctest"];
...
- (NSTask *)childTaskWithEnvironment:(RLMChildProcessEnvironment *)environment {
  NSString *testName =
      [NSString stringWithFormat:@"%@/%@", self.className, self.testName];
...
- (NSTask *)childTaskWithAppIds:(NSArray *)appIds {
  return [self childTaskWithEnvironment:[[RLMChildProcessEnvironment new]
                                            initWithAppIds:appIds
                                                     email:nil
                                                  password:nil
                                                 identifer:0]];
}
...
- (NSPipe *)filterPipe {
  ...
  pipe.fileHandleForReading.readabilityHandler = ^(NSFileHandle *file) {
    ...
    while ((newline = memchr(start, '\n', end - start))) {
      if (newline < start + 17 ||
          (memcmp(start, "Test Suite", 10) && memcmp(start, "Test Case", 9) &&
           memcmp(start, "     Executed 1 test", 17))) {
        fwrite(start, newline - start + 1, 1, stderr);
      }
      start = newline + 1;
    }

    // Remove everything up to the last newline, leaving any data not
    // newline-terminated in the buffer
    [buffer replaceBytesInRange:NSMakeRange(0, start - (char *)buffer.bytes)
                      withBytes:0
                         length:0];
  };
  return pipe;
...
- (int)runChildAndWaitWithEnvironment:
    (RLMChildProcessEnvironment *)environment {
...
- (int)runChildAndWaitWithAppIds:(NSArray *)appIds {
  return [self runChildAndWaitWithEnvironment:[[RLMChildProcessEnvironment new]
                                                  initWithAppIds:appIds
                                                           email:nil
                                                        password:nil
                                                       identifer:0]];
