...
#import <WabiRealm/RLMSchema_Private.h>
...
static void recordFailure(XCTestCase *self, NSString *message,
                          NSString *fileName, NSUInteger lineNumber) {
  XCTSourceCodeLocation *loc =
      [[XCTSourceCodeLocation alloc] initWithFilePath:fileName
...
void RLMAssertThrowsWithReasonMatchingSwift(
    ...
    NSRegularExpression *regex = [NSRegularExpression
        regularExpressionWithPattern:regexString
                             options:(NSRegularExpressionOptions)0
                               error:nil];
    if ([regex numberOfMatchesInString:reason
                               options:(NSMatchingOptions)0
                                 range:NSMakeRange(0, reason.length)] == 0) {
      NSString *msg = [NSString
          stringWithFormat:@"The given expression threw an exception with "
                           @"reason '%@', but expected to match '%@'",
                           reason, regexString];
  ...
  if (!didThrow) {
    ...
    message = message ? [NSString stringWithFormat:@"%@ (%@)", prefix, message]
                      : prefix;
    recordFailure(self, message, fileName, lineNumber);
  }
}

static void assertThrows(XCTestCase *self, dispatch_block_t block,
                         NSString *message, NSString *fileName,
                         NSUInteger lineNumber,
...
void(RLMAssertThrowsWithName)(XCTestCase *self,
                              ...
                              NSString *fileName, NSUInteger lineNumber) {
  assertThrows(
      self, block, message, fileName, lineNumber, ^NSString *(NSException *e) {
        if ([name isEqualToString:e.name]) {
          return nil;
        }
        return [NSString
            stringWithFormat:@"The given expression threw an exception named "
                             @"'%@', but expected '%@'",
                             e.name, name];
      });
}
...
void(RLMAssertThrowsWithReason)(XCTestCase *self,
                                ...
                                NSString *expected, NSString *message,
                                NSString *fileName, NSUInteger lineNumber) {
  assertThrows(
      self, block, message, fileName, lineNumber, ^NSString *(NSException *e) {
...
void(RLMAssertThrowsWithReasonMatching)(XCTestCase *self,
  ...
  assertThrows(
      self, block, message, fileName, lineNumber, ^NSString *(NSException *e) {
        if ([regex numberOfMatchesInString:e.reason
                                   options:(NSMatchingOptions)0
                                     range:{0, e.reason.length}] > 0) {
          return nil;
        }
        return [NSString
            stringWithFormat:@"The given expression threw an exception with "
...
void(RLMAssertMatches)(XCTestCase *self,
                       __attribute__((noescape)) NSString * (^block)(),
  ...
  if ([regex numberOfMatchesInString:result
                             options:(NSMatchingOptions)0
                               range:NSMakeRange(0, result.length)] == 0) {
    NSString *msg = [NSString
        stringWithFormat:@"The given expression '%@' did not match '%@'%@",
                         result, regexString,
...
void(RLMAssertExceptionReason)(XCTestCase *self, NSException *exception,
  ...
  auto location = [[XCTSourceCodeContext alloc]
      initWithLocation:[[XCTSourceCodeLocation alloc]
                           initWithFilePath:fileName
                                 lineNumber:lineNumber]];
  NSString *desc = [NSString
      stringWithFormat:@"The expression %@ threw an exception with reason "
                       @"'%@', but expected to contain '%@'",
                       expression, exception.reason ?: @"<nil>", expected];
...
static std::string serialize(id obj) {
  auto data = [NSJSONSerialization dataWithJSONObject:obj options:0 error:nil];
...
RLMUser *RLMDummyUser() {
  // Add a fake user to the metadata WabiRealm
  @autoreleasepool {
...
bool RLMCanFork() { return true; }
pid_t RLMFork() { return fork(); }
#else
bool RLMCanFork() { return false; }
pid_t RLMFork() { abort(); }
#endif
