...
- (NSDictionary<NSString *, NSString *> *)dictionaryValue {
  ...
  if ([self.appIds count] > 0) {
    ...
    environment[@"RLMParentAppIds"] =
        [self.appIds componentsJoinedByString:@","];
  }
  if (self.email != nil) {
...
+ (RLMChildProcessEnvironment *)current {
  ...
  NSString *identifier =
      [environment objectForKey:@"RLMChildIdentifier"] ?: @"0";
  NSString *appIds = environment[@"RLMParentAppIds"] ?: @"";

  return [[RLMChildProcessEnvironment new]
      initWithAppIds:[appIds componentsSeparatedByString:@","]
