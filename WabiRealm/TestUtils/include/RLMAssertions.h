...
FOUNDATION_EXTERN
void RLMAssertMatches(XCTestCase *self,
                      __attribute__((noescape)) NSString * (^block)(void),
                      NSString *regexString, NSString *message,
                      NSString *fileName, NSUInteger lineNumber);
...
FOUNDATION_EXTERN
void RLMAssertExceptionReason(XCTestCase *self, NSException *exception,
                              NSString *expected, NSString *expression,
                              NSString *fileName, NSUInteger lineNumber);
...
#define RLMPrimitiveAssertThrows(self, expression, format...)                  \
  ({                                                                           \
    ...
    if (!caughtException) {                                                    \
      _XCTRegisterFailure(                                                     \
          self,                                                                \
          _XCTFailureDescription(_XCTAssertion_Throws, 0, @ #expression),      \
          format);                                                             \
    }                                                                          \
    caughtException;                                                           \
  })
...
#define RLMPrimitiveAssertMatches(self, expression, regexString, format...)    \
  ({                                                                           \
    ...
    if ([regex numberOfMatchesInString:string                                  \
      ...
      _XCTRegisterFailure(                                                     \
          self,                                                                \
          [_XCTFailureDescription(_XCTAssertion_True, 0,                       \
                                  @ #expression @" (EXPR_STRING) matches "     \
                                  @ #regexString)                              \
              stringByReplacingOccurrencesOfString:@"EXPR_STRING"              \
                                        withString:string ?: @"<nil>"],        \
          format);                                                             \
    }                                                                          \
  })
...
#define RLMAssertThrowsWithReason(expression, expected)                        \
  ({                                                                           \
    ...
    RLMAssertExceptionReason(self, exception, expected, @ #expression,         \
                             @"" __FILE__, __LINE__);                          \
    exception;                                                                 \
  })
...
#define RLMAssertThrowsWithCodeMatching(expression, expectedCode, ...)         \
  ({                                                                           \
    NSException *exception = RLMAssertThrows(expression, __VA_ARGS__);         \
    XCTAssertEqual([exception.userInfo[NSUnderlyingErrorKey] code],            \
                   expectedCode, __VA_ARGS__);                                 \
  })

#define RLMValidateError(error, errDomain, errCode, msg)                       \
  do {                                                                         \
...
#define RLMValidateErrorContains(error, errDomain, errCode, msg)               \
  do {                                                                         \
    XCTAssertNotNil(error);                                                    \
    XCTAssertEqual(error.domain, errDomain);                                   \
    XCTAssertEqual(error.code, errCode);                                       \
...
#define RLMValidateRealmError(macroError, errCode, msg, path)                  \
  do {                                                                         \
    NSError *error2 = (NSError *)macroError;                                   \
    RLMValidateError(error2, RLMErrorDomain, errCode,                          \
                     ([NSString stringWithFormat:msg, path]));                 \
    XCTAssertEqualObjects(error2.userInfo[NSFilePathErrorKey], path);          \
  } while (0)

#define RLMValidateRealmErrorContains(macroError, errCode, msg, path)          \
  do {                                                                         \
    NSError *error2 = (NSError *)macroError;                                   \
    RLMValidateErrorContains(error2, RLMErrorDomain, errCode,                  \
                             ([NSString stringWithFormat:msg, path]));         \
    XCTAssertEqualObjects(error2.userInfo[NSFilePathErrorKey], path);          \
  } while (0)

#define RLMAssertRealmException(expr, errCode, msg, path)                      \
  do {                                                                         \
    ...
    RLMValidateRealmError(exception.userInfo[NSUnderlyingErrorKey], errCode,   \
                          msg, path);                                          \
  } while (0)

#define RLMAssertRealmExceptionContains(expr, errCode, msg, path)              \
  do {                                                                         \
    NSException *exception = RLMAssertThrows(expr);                            \
    XCTAssertEqual(exception.name, RLMExceptionName);                          \
    NSString *reason = [NSString stringWithFormat:msg, path];                  \
    ...
    RLMValidateRealmErrorContains(exception.userInfo[NSUnderlyingErrorKey],    \
                                  errCode, msg, path);                         \
  } while (0)
...
#define uncheckedAssertEqual(ex1, ex2)                                         \
  do {                                                                         \
    __typeof__(ex1) value1 = (ex1);                                            \
    __typeof__(ex2) value2 = (ex2);                                            \
    if (value1 != value2) {                                                    \
      NSValue *box1 = [NSValue value:&value1                                   \
                        withObjCType:@encode(__typeof__(ex1))];                \
      NSValue *box2 = [NSValue value:&value2                                   \
                        withObjCType:@encode(__typeof__(ex2))];                \
      _XCTRegisterFailure(                                                     \
          nil, _XCTFailureDescription(_XCTAssertion_Equal, 0, @ #ex1, @ #ex2,  \
                                      _XCTDescriptionForValue(box1),           \
                                      _XCTDescriptionForValue(box2)));         \
    }                                                                          \
  } while (0)

#define uncheckedAssertEqualObjects(ex1, ex2)                                  \
  do {                                                                         \
    id value1 = (ex1);                                                         \
    id value2 = (ex2);                                                         \
    if (value1 != value2 && ![(id)value1 isEqual:value2]) {                    \
      _XCTRegisterFailure(                                                     \
          nil, _XCTFailureDescription(_XCTAssertion_EqualObjects, 0, @ #ex1,   \
                                      @ #ex2, value1, value2));                \
    }                                                                          \
  } while (0)
