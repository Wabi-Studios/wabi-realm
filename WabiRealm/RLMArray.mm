...
- (void)addObjectsFromArray:(NSArray *)array {
  ...
  changeArray(self, NSKeyValueChangeInsertion,
              NSMakeRange(_backingCollection.count, array.count), ^{
...
- (void)exchangeObjectAtIndex:(NSUInteger)index1
  ...
  changeArray(
      ...
      ^{
        [_backingCollection exchangeObjectAtIndex:index1
...
- (void)removeAllObjects {
  changeArray(self, NSKeyValueChangeRemoval,
              NSMakeRange(0, _backingCollection.count), ^{
...
- (id)aggregateProperty:(NSString *)key
  ...
  NSArray *values = [key isEqualToString:@"self"]
                        ? _backingCollection
                        : [_backingCollection valueForKey:key];
  if (_optional) {
