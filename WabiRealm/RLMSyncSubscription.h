...
typedef NS_ENUM(NSUInteger, RLMSyncSubscriptionState) {
  /// The subscription is complete and the server has sent all the data that
  /// matched the subscription
  /// queries at the time the subscription set was updated. The server is now in
  /// a steady-state
  /// synchronization mode where it will stream update as they come.
  RLMSyncSubscriptionStateComplete,
  /// The subscription encountered an error and synchronization is paused for
  /// this WabiRealm. You can
  /// find the error calling error in the subscription set to get a description
  /// of the error. You can
  ...
  /// which means
  /// the server hasn't yet returned all the data that matched the updated
  /// subscription queries.
  RLMSyncSubscriptionStatePending,
  /// The subscription set has been super-ceded by an updated one, this
  /// typically means that
  /// someone is trying to write a subscription on a different instance of the
  /// subscription set.
  /// You should not use a superseded subscription set and instead obtain a new
  /// instance of
...
/**
 `RLMSyncSubscription` is  used to define a Flexible Sync subscription obtained
 from querying a subscription set, which can be used to read or remove/update a
...
/**
 Updates a Flexible Sync's subscription query with an allowed query which will
 be used to bootstrap data from the server when committed.
...
/// :nodoc:
- (void)updateSubscriptionWhere:(NSString *)predicateFormat args:(va_list)args;
...
/**
 `RLMSyncSubscriptionSet` is  a collection of `RLMSyncSubscription`s. This is
 the entry point for adding and removing `RLMSyncSubscription`s.
 */
...
/// Gets the error associated to the subscription set. This will be non-nil in
/// case the current state of the subscription set is
/// `RLMSyncSubscriptionStateError`.
...
/**
 Synchronously performs any transactions (add/remove/update) to the subscription
 set within the block, this will not wait for the server to acknowledge and see
 all the data associated with this collection of subscriptions, and will return
 after committing the subscription transactions.
...
- (void)write:(__attribute__((noescape))void (^)(void))block
    __attribute__((unavailable("Renamed to -update")));
...
- (void)update:(__attribute__((noescape))void (^)(void))block
    onComplete:
        (nullable void (^RLM_SWIFT_SENDABLE)(NSError *_Nullable))onComplete
...
- (void)write:(__attribute__((noescape))void (^)(void))block
    ...
    __attribute__((unavailable("Renamed to -update:onComplete.")));
...
- (nullable RLMSyncSubscription *)
    subscriptionWithClassName:(NSString *)objectClassName
...
/**
 Adds a new subscription to the subscription set which will be sent to the
 server when committed at the end of a write subscription block.
...
/**
 Removes a subscription with the specified query for the object class from the
 subscription set.
...
/**
 ...
 @param objectClassName The class name for the model class to be queried.
 @param predicate  The predicate which will be used to identify the subscription
 to be removed.
 */
- (void)removeSubscriptionWithClassName:(NSString *)objectClassName
                              predicate:(NSPredicate *)predicate;
...
/**
 ...
 @warning This method may only be called during a write subscription block.
 @warning Removing all subscriptions will result in an error if no new
 subscription is added. Server should acknowledge at least one subscription.
 */
...
/**
 Returns the first object in the subscription set list, or `nil` if the
...
/**
 Returns the last object in the subscription set, or `nil` if the subscriptions
 are empty.
