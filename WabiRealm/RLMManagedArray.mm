...
void RLMEnsureArrayObservationInfo(std::unique_ptr<RLMObservationInfo> &info,
  ...
  if (!info && array.class == [RLMManagedArray class]) {
    ...
    info = std::make_unique<RLMObservationInfo>(
        *lv->_ownerInfo, lv->_backingList.get_parent_object_key(), observed);
  }
}
...
- (BOOL)isEqual:(id)object {
  return [object respondsToSelector:@selector(isBackedByList:)] &&
         [object isBackedByList:_backingList];
}
...
- (void)removeObjectsAtIndexes:(NSIndexSet *)indexes {
  changeArray(self, NSKeyValueChangeRemoval, indexes, ^{
    [indexes enumerateIndexesWithOptions:NSEnumerationReverse
                              usingBlock:^(NSUInteger idx, BOOL *) {
...
- (void)addObjectsFromArray:(NSArray *)array {
  changeArray(self, NSKeyValueChangeInsertion,
              NSMakeRange(self.count, array.count), ^{
                RLMAccessorContext context(*_objectInfo);
                for (id obj in array) {
                  RLMArrayValidateMatchingObjectType(self, obj);
    ...
    return translateErrors([&] {
      auto results =
          [RLMResults resultsWithObjectInfo:*_objectInfo
                                    results:_backingList.as_results()];
...
- (id)minOfProperty:(NSString *)property {
  auto column = columnForProperty(property, _backingList, _objectInfo, _type,
...
- (id)sumOfProperty:(NSString *)property {
  ...
  return RLMMixedToObjc(
      translateErrors([&] { return _backingList.sum(column); }));
}
...
- (RLMResults *)objectsWithPredicate:(NSPredicate *)predicate {
  ...
  auto results =
      translateErrors([&] { return _backingList.filter(std::move(query)); });
...
- (NSArray *)objectsAtIndexes:(NSIndexSet *)indexes {
  ...
  while (i != NSNotFound) {
    ...
    // that no out of bounds exceptions are generated. This disallows us to
    // mirror the exception logic in Foundation, but it is better than nothing.
    if (i >= 0 && i < c) {
...
- (instancetype)resolveInRealm:(RLMRealm *)realm {
  ...
  return translateErrors([&] {
    return [[self.class alloc]
        initWithBackingCollection:_backingList.freeze(realm->_realm)
...
- (RLMManagedArrayHandoverMetadata *)objectiveCMetadata {
  RLMManagedArrayHandoverMetadata *metadata =
      [[RLMManagedArrayHandoverMetadata alloc] init];
...
+ (instancetype)
  ...
  return [[RLMManagedArray alloc]
      initWithBackingCollection:std::move(list)
                     parentInfo:parentInfo
