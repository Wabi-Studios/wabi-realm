...
@class RLMFindOptions, RLMFindOneAndModifyOptions, RLMUpdateResult,
    RLMChangeStream, RLMObjectId;

/// Delegate which is used for subscribing to changes on a `[RLMMongoCollection
/// watch]` stream.
...
/// @param error If an error occured when closing the stream, an error will be
/// passed.
...
///
/// Operations against the WabiRealm Cloud server are performed asynchronously.
///
/// - Note:
...
/// Block which returns an object id on a successful insert, or an error should
/// one occur.
RLM_SWIFT_SENDABLE // invoked on a background thread
    typedef void (^RLMMongoInsertBlock)(id<RLMBSON> _Nullable,
                                        NSError *_Nullable);
/// Block which returns an array of object ids on a successful insertMany, or an
/// error should one occur.
RLM_SWIFT_SENDABLE // invoked on a background thread
    typedef void (^RLMMongoInsertManyBlock)(NSArray<id<RLMBSON>> *_Nullable,
                                            NSError *_Nullable);
/// Block which returns an array of Documents on a successful find operation, or
/// an error should one occur.
RLM_SWIFT_SENDABLE // invoked on a background thread
    typedef void (^RLMMongoFindBlock)(
        NSArray<NSDictionary<NSString *, id<RLMBSON>> *> *_Nullable,
        NSError *_Nullable);
/// Block which returns a Document on a successful findOne operation, or an
/// error should one occur.
RLM_SWIFT_SENDABLE // invoked on a background thread
    typedef void (^RLMMongoFindOneBlock)(
        NSDictionary<NSString *, id<RLMBSON>> *_Nullable_result,
        NSError *_Nullable);
/// Block which returns the number of Documents in a collection on a successful
/// count operation, or an error should one occur.
RLM_SWIFT_SENDABLE // invoked on a background thread
    typedef void (^RLMMongoCountBlock)(NSInteger, NSError *_Nullable);
/// Block which returns an RLMUpdateResult on a successful update operation, or
/// an error should one occur.
RLM_SWIFT_SENDABLE // invoked on a background thread
    typedef void (^RLMMongoUpdateBlock)(RLMUpdateResult *_Nullable,
                                        NSError *_Nullable);
/// Block which returns the deleted Document on a successful delete operation,
/// or an error should one occur.
RLM_SWIFT_SENDABLE // invoked on a background thread
...
/// Encodes the provided value to BSON and inserts it. If the value is missing
/// an identifier, one will be generated for it.
/// @param document  A `Document` value to insert.
/// @param completion The result of attempting to perform the insert. An Id will
/// be returned for the inserted object on sucess
...
/// missing identifiers, they will be generated.
/// @param documents  The `Document` values in a bson array to insert.
/// @param completion The result of the insert, returns an array inserted
/// document ids in order
- (void)insertManyDocuments:
            (NSArray<NSDictionary<NSString *, id<RLMBSON>> *> *)documents
...
/// @param completion The resulting bson array of documents or error if one
/// occurs
...
/// Finds the documents in this collection which match the provided filter.
/// @param filterDocument A `Document` as bson that should match the query.
/// @param completion The resulting bson array as a string or error if one
/// occurs
- (void)findWhere:(NSDictionary<NSString *, id<RLMBSON>> *)filterDocument
       completion:(RLMMongoFindBlock)completion NS_REFINED_FOR_SWIFT;
...
- (void)findOneDocumentWhere:
            (NSDictionary<NSString *, id<RLMBSON>> *)filterDocument
                     options:(RLMFindOptions *)options
...
/// @param pipeline A bson array made up of `Documents` containing the pipeline
/// of aggregation operations to perform.
/// @param completion The resulting bson array of documents or error if one
/// occurs
- (void)aggregateWithPipeline:
            (NSArray<NSDictionary<NSString *, id<RLMBSON>> *> *)pipeline
                   completion:(RLMMongoFindBlock)completion
    NS_REFINED_FOR_SWIFT;

/// Counts the number of documents in this collection matching the provided
/// filter.
/// @param filterDocument A `Document` as bson that should match the query.
/// @param limit The max amount of documents to count
/// @param completion Returns the count of the documents that matched the
/// filter.
...
/// @param filterDocument A `Document` as bson that should match the query.
/// @param completion The result of performing the deletion. Returns the count
/// of deleted objects
...
/// @param updateDocument  A bson `Document` representing the update to be
/// applied to a matching document.
/// @param upsert When true, creates a new document if no document matches the
/// query.
/// @param completion The result of the attempt to update a document.
- (void)updateOneDocumentWhere:
            (NSDictionary<NSString *, id<RLMBSON>> *)filterDocument
                updateDocument:
                    (NSDictionary<NSString *, id<RLMBSON>> *)updateDocument
                        upsert:(BOOL)upsert
...
/// @param updateDocument  A bson `Document` representing the update to be
/// applied to a matching document.
/// @param options  `RemoteFindOneAndModifyOptions` to use when executing the
/// command.
/// @param completion The result of the attempt to update a document.
...
/// @param options  `RLMFindOneAndModifyOptions` to use when executing the
/// command.
/// @param completion The result of the attempt to replace a document.
- (void)findOneAndReplaceWhere:
            (NSDictionary<NSString *, id<RLMBSON>> *)filterDocument
           replacementDocument:
               (NSDictionary<NSString *, id<RLMBSON>> *)replacementDocument
                       options:(RLMFindOneAndModifyOptions *)options
                    completion:(RLMMongoFindOneBlock)completion
    NS_REFINED_FOR_SWIFT;
...
/// it was deleted. Unlike `deleteOneDocument`, this action allows you to
/// atomically find and delete a document with the same command. This avoids the
/// risk of other update operations changing the document between separate find
...
/// The resulting stream will be notified of all events on this collection that
/// the active user is authorized to see based on the configured MongoDB rules.
/// @param delegate The delegate that will react to events and errors from the
/// resulting change stream.
/// @param queue Dispatches streaming events to an optional queue, if no queue
/// is provided the main queue is used
...
/// Opens a MongoDB change stream against the collection to watch for changes.
/// The provided BSON document will be used as a match expression filter on the
/// change events coming from the stream.
///
/// See https://docs.mongodb.com/manual/reference/operator/aggregation/match/
/// for documentation around how to define a match filter.
///
/// Defining the match expression to filter ChangeEvents is similar to defining
/// the match expression for triggers:
/// https://docs.mongodb.com/wabi-realm/triggers/database-triggers/
/// @param matchFilter The $match filter to apply to incoming change events
/// @param delegate The delegate that will react to events and errors from the
/// resulting change stream.
/// @param queue Dispatches streaming events to an optional queue, if no queue
/// is provided the main queue is used
- (RLMChangeStream *)watchWithMatchFilter:
                         (NSDictionary<NSString *, id<RLMBSON>> *)matchFilter
                                 delegate:(id<RLMChangeEventDelegate>)delegate
                            delegateQueue:(nullable dispatch_queue_t)queue
    NS_REFINED_FOR_SWIFT;
