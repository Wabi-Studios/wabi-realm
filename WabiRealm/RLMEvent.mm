...
namespace {
util::UniqueFunction<void(std::exception_ptr)>
wrapCompletion(void (^completion)(NSError *)) {
  if (!completion) {
    return nullptr;
  }
  return [=](std::exception_ptr err) {
    @autoreleasepool {
      ...
      try {
        ...
        auto info = @{
          @"ExceptionName" : e.name ?: NSNull.null,
        ...
        completion([NSError errorWithDomain:RLMErrorDomain
                                       code:RLMErrorFail
                                   userInfo:info]);
      } catch (...) {
        NSError *error;
...
convertMetadata(NSDictionary *metadata) {
  std::vector<std::pair<std::string, std::string>> ret;
  ret.reserve(metadata.count);
  [metadata enumerateKeysAndObjectsUsingBlock:[&](NSString *key,
                                                  NSString *value, BOOL *) {
...
void RLMEventCommitScope(RLMEventContext *context, uint64_t scope_id,
                         RLMEventCompletion completion) {
...
void RLMEventRecordEvent(RLMEventContext *context, NSString *activity,
                         NSString *event, NSString *data,
                         RLMEventCompletion completion) {
  auditContext(context)->record_event(
      activity.UTF8String, nsStringToOptionalString(event),
      nsStringToOptionalString(data), wrapCompletion(completion));
}

void RLMEventUpdateMetadata(RLMEventContext *context,
...
public:
  ...
  ~RLMEventSerializer() { scope_complete(); }
