...
RLMClassInfo::computedTableColumn(RLMProperty *property) const {
  ...
  // Get the column key for origin's forward link that links to the property on
  // the target.
  ColKey forwardLinkKey =
      originInfo.tableColumn(property.linkOriginPropertyName);

  // The column key opposite of the origin's forward link is the target's
  // backlink property.
...
RLMClassInfo &RLMClassInfo::linkTargetType(size_t propertyIndex) {
  return realm
      ->_info[rlmObjectSchema.properties[propertyIndex].objectClassName];
}
...
bool RLMClassInfo::isDynamic() const noexcept { return !!dynamicObjectSchema; }

static KeyPath keyPathFromString(RLMRealm *realm, RLMSchema *schema,
  ...
  for (NSString *component in [keyPath componentsSeparatedByString:@"."]) {
    ...
    if (!property) {
      throw RLMException(@"Invalid property name: property '%@' not found in "
                         @"object of type '%@'",
...
RLMClassInfo &RLMSchemaInfo::operator[](NSString *name) {
  ...
  if (it == m_objects.end()) {
    @throw RLMException(
        @"Object type '%@' is not managed by the WabiRealm. "
        @"If using a custom `objectClasses` / `objectTypes` array in your "
        @"configuration, "
...
RLMSchemaInfo::RLMSchemaInfo(RLMRealm *realm) {
  ...
  for (RLMObjectSchema *rlmObjectSchema in rlmSchema.objectSchema) {
    ...
    m_objects.emplace(std::piecewise_construct,
                      ...
                      std::forward_as_tuple(realm, rlmObjectSchema, &*it));
  }
}
...
void RLMSchemaInfo::appendDynamicObjectSchema(
    ...
    __unsafe_unretained RLMRealm *const target_realm) {
  m_objects.emplace(
      std::piecewise_construct, std::forward_as_tuple(objectSchema.className),
      std::forward_as_tuple(target_realm, objectSchema, std::move(schema)));
}
