...
id RLMDictionaryKey(__unsafe_unretained RLMDictionary *const dictionary,
  ...
  if (!key) {
    @throw RLMException(
        @"Invalid nil key for dictionary expecting key of type '%@'.",
  ...
  if (!validated) {
    @throw RLMException(
        @"Invalid key '%@' of type '%@' for expected type '%@'.", key,
        [key class], RLMTypeToString(dictionary.keyType));
  }
  return validated;
}
...
id RLMDictionaryValue(__unsafe_unretained RLMDictionary *const dictionary,
  ...
  if (dictionary->_type != RLMPropertyTypeObject) {
    id validated = RLMValidateValue(value, dictionary->_type,
                                    dictionary->_optional, false, nil);
    if (!validated) {
      @throw RLMException(
          @"Invalid value '%@' of type '%@' for expected type '%@%s'.", value,
          [value class], RLMTypeToString(dictionary->_type),
    ...
    if (!valueObject->_objectSchema) {
      @throw RLMException(
          @"Object cannot be inserted unless the schema is initialized. "
           "This can happen if you try to insert objects into a RLMDictionary "
           "/ Map from a default value or from an overridden unmanaged "
           "initializer (`init()`) or if the key is uninitialized.");
    }
    if (![dictionary->_objectClassName
            isEqualToString:valueObject->_objectSchema.className]) {
      @throw RLMException(
          @"Value of type '%@' does not match RLMDictionary value type '%@'.",
...
static void
changeDictionary(__unsafe_unretained RLMDictionary *const dictionary,
                 dispatch_block_t f) {
...
- (RLMNotificationToken *)addNotificationBlock:
    (void (^)(RLMDictionary *, RLMDictionaryChange *, NSError *))block {
  return RLMAddNotificationBlock(self, block, nil, nil);
}
- (RLMNotificationToken *)addNotificationBlock:(void (^)(RLMDictionary *,
                                                         RLMDictionaryChange *,
                                                         NSError *))block
...
- (RLMNotificationToken *)
    addNotificationBlock:(void (^)(RLMDictionary *, RLMDictionaryChange *,
                                   NSError *))block
...
- (void)setDictionary:(id)dictionary {
  ...
  if (![dictionary
          respondsToSelector:@selector(enumerateKeysAndObjectsUsingBlock:)]) {
    @throw RLMException(@"Cannot set dictionary to object of class '%@'",
...
- (id)valueForKeyPath:(NSString *)keyPath {
  if ([keyPath characterAtIndex:0] != '@') {
    return _backingCollection ? [_backingCollection valueForKeyPath:keyPath]
...
- (void)addEntriesFromDictionary:(id)otherDictionary {
  ...
  if (![otherDictionary
          respondsToSelector:@selector(enumerateKeysAndObjectsUsingBlock:)]) {
    @throw RLMException(@"Cannot add entries from object of class '%@'",
                        [otherDictionary className]);
  }

  changeDictionary(self, ^{
    [otherDictionary
        enumerateKeysAndObjectsUsingBlock:^(id key, id value, BOOL *) {
          _backingCollection[RLMDictionaryKey(self, key)] =
...
- (id)aggregateProperty:(NSString *)key
  ...
  if ([op isEqualToString:@"@min"] || [op isEqualToString:@"@max"]) {
    ...
    return [_backingCollection
        valueForKeyPath:[op stringByAppendingPathExtension:key]];
  }
  ...
  if (!canAggregate(type, allowDate)) {
    ...
    if (_type == RLMPropertyTypeObject) {
      ...
      @throw RLMException(@"%@ is not supported for %@%s dictionary", method,
                          RLMTypeToString(_type), _optional ? "?" : "");
    }
  }

  NSArray *values = [key isEqualToString:@"self"]
                        ? _backingCollection.allValues
                        : [_backingCollection.allValues valueForKey:key];
...
- (BOOL)isEqual:(id)object {
  if (auto dictionary = RLMDynamicCast<RLMDictionary>(object)) {
    return !dictionary.realm &&
           ((_backingCollection.count == 0 &&
             dictionary->_backingCollection.count == 0) ||
...
- (nonnull RLMResults *)objectsWithPredicate:(nonnull NSPredicate *)predicate {
  @throw RLMException(@"This method may only be called on RLMDictionary "
...
NSString *RLMDictionaryDescriptionWithMaxDepth(NSString *name,
  ...
  auto str = [NSMutableString
      stringWithFormat:@"%@<%@, %@> <%p> (\n", name,
                       RLMTypeToString([dictionary keyType]),
                       [dictionary objectClassName]
