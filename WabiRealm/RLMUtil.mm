...
static inline RLMArray *asRLMArray(__unsafe_unretained id const value) {
  return RLMDynamicCast<RLMArray>(value)
             ?: RLMDynamicCast<RLMSwiftCollectionBase>(value)._rlmCollection;
}

static inline RLMSet *asRLMSet(__unsafe_unretained id const value) {
  return RLMDynamicCast<RLMSet>(value)
             ?: RLMDynamicCast<RLMSwiftCollectionBase>(value)._rlmCollection;
}

static inline RLMDictionary *
asRLMDictionary(__unsafe_unretained id const value) {
  return RLMDynamicCast<RLMDictionary>(value)
             ?: RLMDynamicCast<RLMSwiftCollectionBase>(value)._rlmCollection;
}

static inline bool
checkCollectionType(__unsafe_unretained id<RLMCollection> const collection,
  ...
  return collection.type == type && collection.optional == optional &&
         (type != RLMPropertyTypeObject ||
          [collection.objectClassName isEqualToString:objectClassName]);
}
...
void RLMSetSwiftBridgeCallback(id (*callback)(id)) { s_bridgeValue = callback; }
...
bool RLMIsSwiftObjectClass(Class cls) {
  return [cls isSubclassOfClass:WabiRealmKitObject.class] ||
         [cls isSubclassOfClass:WabiRealmKitEmbeddedObject.class];
}
...
static BOOL validateValue(__unsafe_unretained id const value,
  ...
  if (collection) {
    if (auto rlmArray = asRLMArray(value)) {
      ...
      return checkCollectionType(rlmDictionary, type, optional,
                                 objectClassName);
    }
  ...
  case RLMPropertyTypeAny: {
    return !value || [value conformsToProtocol:@protocol(RLMValue)];
  }
  case RLMPropertyTypeLinkingObjects:
    return YES;
  case RLMPropertyTypeObject: {
    // only NSNull, nil, or objects which derive from RLMObject and match the
    // given object class are valid
    RLMObjectBase *objBase = RLMDynamicCast<RLMObjectBase>(value);
    return objBase &&
           [objBase->_objectSchema.className isEqualToString:objectClassName];
  }
...
id RLMValidateValue(__unsafe_unretained id const value, RLMPropertyType type,
                    bool optional, bool collection,
                    __unsafe_unretained NSString *const objectClassName) {
...
void RLMThrowTypeError(__unsafe_unretained id const obj,
  ...
  @throw RLMException(
      @"Invalid value '%@' of type '%@' for '%@%s'%s property '%@.%@'.", obj,
      [obj class], prop.objectClassName ?: RLMTypeToString(prop.type),
      prop.optional ? "?" : "",
      prop.array        ? " array"
      : prop.set        ? " set"
...
void RLMValidateValueForProperty(
    __unsafe_unretained id const obj,
    __unsafe_unretained RLMObjectSchema *const objectSchema,
    __unsafe_unretained RLMProperty *const prop, bool validateObjects) {
  ...
  if (prop.collection) {
    ...
    if (!enumeration) {
      @throw RLMException(
          @"Invalid value (%@) for '%@%s' %@ property '%@.%@': value is not "
          @"enumerable.",
          obj, prop.objectClassName ?: RLMTypeToString(prop.type),
          prop.optional ? "?" : "", prop.array ? @"array" : @"set",
          objectSchema.className, prop.name);
    }
    ...
    if (RLMArray *array = asRLMArray(obj)) {
      if (!checkCollectionType(array, prop.type, prop.optional,
                               prop.objectClassName)) {
        @throw RLMException(
            @"RLMArray<%@%s> does not match expected type '%@%s' for property "
            @"'%@.%@'.",
            array.objectClassName ?: RLMTypeToString(array.type),
            array.optional ? "?" : "",
            prop.objectClassName ?: RLMTypeToString(prop.type),
            prop.optional ? "?" : "", objectSchema.className, prop.name);
      }
      return;
    } else if (RLMSet *set = asRLMSet(obj)) {
      if (!checkCollectionType(set, prop.type, prop.optional,
                               prop.objectClassName)) {
        @throw RLMException(@"RLMSet<%@%s> does not match expected type '%@%s' "
                            @"for property '%@.%@'.",
                            set.objectClassName ?: RLMTypeToString(set.type),
                            set.optional ? "?" : "",
                            prop.objectClassName ?: RLMTypeToString(prop.type),
                            prop.optional ? "?" : "", objectSchema.className,
                            prop.name);
      }
      return;
    } else if (RLMDictionary *dictionary = asRLMDictionary(obj)) {
      if (!checkCollectionType(dictionary, prop.type, prop.optional,
                               prop.objectClassName)) {
        @throw RLMException(
            @"RLMDictionary<%@, %@%s> does not match expected type '%@%s' for "
            @"property '%@.%@'.",
    ...
    if (prop.dictionary) {
      for (id key in enumeration) {
        ...
        if (!RLMValidateValue(value, prop.type, prop.optional, false,
                              prop.objectClassName)) {
...
BOOL RLMIsObjectValidForProperty(
    __unsafe_unretained id const obj,
    __unsafe_unretained RLMProperty *const property) {
  return RLMValidateValue(obj, property.type, property.optional,
                          property.collection, property.objectClassName) != nil;
}

NSDictionary *RLMDefaultValuesForObjectSchema(
    __unsafe_unretained RLMObjectSchema *const objectSchema) {
  ...
  if ([objectSchema.objectClass isSubclassOfClass:RLMObject.class]) {
    defaults = [NSMutableDictionary
        dictionaryWithDictionary:[objectSchema
                                         .objectClass defaultPropertyValues]];
  } else {
...
static NSException *RLMException(NSString *reason,
...
NSException *RLMException(NSString *fmt, ...) {
  ...
  va_start(args, fmt);
  NSException *e =
      RLMException([[NSString alloc] initWithFormat:fmt arguments:args], @{});
  va_end(args);
  return e;
}
...
BOOL RLMIsDebuggerAttached() {
  ...
  if (sysctl(name, sizeof(name) / sizeof(name[0]), &info, &info_size, NULL,
             0) == -1) {
  ...
  return switch_on_type(
      ...
      wabi_realm::util::overload{
          [&](wabi_realm::Obj *) {
            ...
            // have to do a lookup in the table of schemas.
            RLMObjectBase *objBase = v;
            RLMAccessorContext c{
                objBase->_info
                    ? *objBase->_info
...
wabi_realm::UUID RLMObjcToUUID(__unsafe_unretained id const value) {
  ...
  throw RLMException(@"Cannot convert value '%@' of type '%@' to uuid", value,
                     [value class]);
}
...
wabi_realm::Decimal128 RLMObjcToDecimal128(__unsafe_unretained id const value) {
  try {
    ...
    @throw RLMException(
        @"Cannot convert value '%@' of type '%@' to decimal128: %s", value,
        [value class], e.what());
  } @
  throw RLMException(@"Cannot convert value '%@' of type '%@' to decimal128",
                     value, [value class]);
}
...
NSString *RLMDefaultDirectoryForBundleIdentifier(NSString *bundleIdentifier) {
  ...
  // tvOS prohibits writing to the Documents directory, so we use the
  // Library/Caches directory instead.
...
NSDateFormatter *RLMISO8601Formatter() {
  // note: NSISO8601DateFormatter can't be used as it doesn't support
  // milliseconds
