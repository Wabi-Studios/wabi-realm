...
typedef NS_ENUM(NSUInteger, RLMSyncSessionState) {
  ...
  /// WabiRealm due to the user logging out or synchronization being paused.
  RLMSyncSessionStateInactive,
  /// The sync session encountered a fatal error and is permanently invalid; it
  /// should be discarded.
...
/**
 The transfer direction (upload or download) tracked by a given progress
 notification block.

 Progress notification blocks can be registered on sessions if your app wishes
 to be informed how many bytes have been uploaded or downloaded, for example to
 show progress indicator UIs.
 */
...
typedef NS_ENUM(NSUInteger, RLMSyncProgressMode) {
  /**
   ...
   Notifications will always report the latest number of transferred bytes, and
   the most up-to-date number of total transferrable bytes.
   */
  RLMSyncProgressModeReportIndefinitely,
  /**
   The block will, upon registration, store the total number of bytes
   to be transferred. When invoked, it will always report the most up-to-date
   number of transferrable bytes out of that original number of transferrable
   bytes.
...
/**
 The type of a progress notification block intended for reporting a session's
 network activity to the user.

 `transferredBytes` refers to the number of bytes that have been uploaded or
 downloaded. `transferrableBytes` refers to the total number of bytes
 transferred, and pending transfer.
 */
typedef void (^RLMProgressNotificationBlock)(NSUInteger transferredBytes,
                                             NSUInteger transferrableBytes);

RLM_HEADER_AUDIT_BEGIN(nullability, sendability)

/**
 A token object corresponding to a progress notification block on a session
 object.

 To stop notifications manually, call `-invalidate` on it. Notifications should
 be stopped before the token goes out of scope or is destroyed.
 */
...
/**
 ...
 communication between the client (and a local WabiRealm file on disk), and the
 server (and a remote WabiRealm with a given partition value stored on Atlas App
 Services).
...
///
/// This property is KVO-compliant and can be observed to be notified of
/// changes. Be warned that KVO observers for this property may be called on a
/// background thread.
...
/**
 If the session is valid, return a sync configuration that can be used to open
 the WabiRealm associated with this session.
 */
...
/**
 ...
 is called or the WabiRealm file is closed and re-opened.
 */
- (void)suspend;
...
/**
 ...
 @param direction The transfer direction (upload or download) to track in this
 progress notification block.
 ...
 @return A token which must be held for as long as you want notifications to be
 delivered.

 @see `RLMSyncProgressDirection`, `RLMSyncProgress`,
 `RLMProgressNotificationBlock`, `RLMProgressNotificationToken`
 */
...
+ (void)immediatelyHandleError:(RLMSyncErrorActionToken *)token
                   syncManager:(RLMSyncManager *)syncManager;

/**
 Get the sync session for the given WabiRealm if it is a synchronized WabiRealm,
 or `nil` if it is not.
 */
...
/**
 ...
 @see `RLMSyncErrorClientResetError`, `RLMSyncErrorPermissionDeniedError`
 */
RLM_SWIFT_SENDABLE RLM_FINAL @interface RLMSyncErrorActionToken : NSObject
