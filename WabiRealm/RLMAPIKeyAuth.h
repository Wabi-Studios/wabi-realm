...
/// A block type used to return an `RLMUserAPIKey` on success, or an `NSError`
/// on failure
RLM_SWIFT_SENDABLE // invoked on a background thread
    typedef void (^RLMOptionalUserAPIKeyBlock)(RLMUserAPIKey *_Nullable,
                                               NSError *_Nullable);

/// A block type used to return an array of `RLMUserAPIKey` on success, or an
/// `NSError` on failure
RLM_SWIFT_SENDABLE // invoked on a background thread
...
- (void)createAPIKeyWithName:(NSString *)name
    ...
    NS_SWIFT_NAME(createAPIKey(named:completion:));
