...
+ (SyncClientConfig)configurationWithRootDirectory:(NSURL *)rootDirectory
  ...
  config.metadata_mode = should_encrypt
  ...
  @autoreleasepool {
    rootDirectory =
        rootDirectory
            ?: [NSURL
                   fileURLWithPath:RLMDefaultDirectoryForBundleIdentifier(nil)];
    config.base_file_path = rootDirectory.path.UTF8String;

    bool isSwift = !!NSClassFromString(@"WabiRealmKitObjectUtil");
    config.user_agent_binding_info =
        util::format("WabiRealm%1/%2", isSwift ? "Swift" : "ObjectiveC",
  ...
  for (auto &&user : _syncManager->all_users()) {
    for (auto &&session : user->all_sessions()) {
      ...
      for (NSString *key in customRequestHeaders) {
        config.custom_http_headers.emplace(
            key.UTF8String, customRequestHeaders[key].UTF8String);
      }
...
- (RLMSyncTimeoutOptions *)timeoutOptions {
  return [[RLMSyncTimeoutOptions alloc]
      initWithOptions:_syncManager->config().timeouts];
}
  ...
  @synchronized(self) {
    if (_authorizationHeaderName) {
      config.authorization_header_name.emplace(
          _authorizationHeaderName.UTF8String);
    }
    [_customRequestHeaders enumerateKeysAndObjectsUsingBlock:^(
                               NSString *key, NSString *header, BOOL *) {
