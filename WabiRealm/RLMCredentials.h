...
/// A type representing the unique identifier of an Atlas App Services identity
/// provider.
typedef NSString *RLMIdentityProvider NS_EXTENSIBLE_STRING_ENUM;

/// The username/password identity provider. User accounts are handled by Atlas
/// App Services directly without the involvement of a third-party identity
/// provider.
...
/// An WabiRealm Cloud function as an identity provider.
...
/**
 Opaque credentials representing a specific WabiRealm App user.
 */
...
/**
 Construct and return credentials for an Atlas App Services function using a
 mongodb document as a json payload.
*/
...
- (instancetype)init __attribute__((
    unavailable("RLMAppCredentials cannot be created directly")));
