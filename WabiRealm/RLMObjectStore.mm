...
RLMVerifyRealmRead(__unsafe_unretained RLMRealm *const realm) {
  if (!realm) {
    @throw RLMException(@"WabiRealm must not be nil");
  }
  [realm verifyThread];
...
void RLMVerifyInWriteTransaction(__unsafe_unretained RLMRealm *const realm) {
  ...
  if (!realm.inWriteTransaction) {
    @throw RLMException(
        @"Can only add, remove, or create objects in a WabiRealm "
        @"in a write transaction - call beginWriteTransaction "
        @"on an RLMRealm instance first.");
  }
}

void RLMInitializeSwiftAccessor(__unsafe_unretained RLMObjectBase *const object,
                                bool promoteExisting) {
...
static CreatePolicy updatePolicyToCreatePolicy(RLMUpdatePolicy policy) {
  CreatePolicy createPolicy = {
      .create = true, .copy = false, .diff = false, .update = false};
  switch (policy) {
...
RLMObjectBase *RLMCreateObjectInRealmWithValue(RLMRealm *realm,
                                               NSString *className, id value,
                                               RLMUpdatePolicy updatePolicy) {
  RLMVerifyInWriteTransaction(realm);
...
void RLMCreateAsymmetricObjectInRealm(RLMRealm *realm, NSString *className,
                                      id value) {
  RLMVerifyInWriteTransaction(realm);

  CreatePolicy createPolicy = {
      .create = true, .copy = true, .diff = false, .update = false};
...
RLMObjectBase *RLMObjectFromObjLink(RLMRealm *realm,
  ...
  if (auto *tableInfo = realm->_info[objLink.get_table_key()]) {
    ...
    RLMObjectSchema *rlmObjectSchema =
        [RLMObjectSchema objectSchemaForObjectStoreSchema:*schema];
    rlmObjectSchema.accessorClass = cls;
    rlmObjectSchema.isSwiftClass = parentIsSwiftObject;
    realm->_info.appendDynamicObjectSchema(std::move(schema), rlmObjectSchema,
                                           realm);
...
void RLMDeleteObjectFromRealm(__unsafe_unretained RLMObjectBase *const object,
  ...
  if (realm != object->_realm) {
    @throw RLMException(
        @"Can only delete an object from the WabiRealm it belongs to.");
  }
...
RLMResults *RLMGetObjects(__unsafe_unretained RLMRealm *const realm,
                          NSString *objectClassName, NSPredicate *predicate) {
  RLMVerifyRealmRead(realm);
