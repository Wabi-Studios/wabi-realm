...
static NSError *s_canceledError = [NSError
...
- (void)addProgressNotificationOnQueue:(dispatch_queue_t)queue
  ...
  auto wrappedBlock =
      ^(NSUInteger transferred_bytes, NSUInteger transferrable_bytes) {
...
- (void)waitWithCompletion:(void (^)(NSError *))completion {
  ...
  // get_synchronized_realm() synchronously opens the DB and performs
  // file-format upgrades, so we want to dispatch to the background before
  // invoking it.
...
// 2. Use WabiRealm::get_synchronized_realm() to create the WabiRealm file, run
...
// case step 4 will handle doing migrations and compactions etc. in the
// background.
//
// At any point `cancel` can be called from another thread. Cancellation is
// mostly cooperative rather than preemptive: we check at each step if we've
// been cancelled, and if so call the completion with the cancellation error
// rather than proceeding. Downloading the data from the server is the one
// exception to this. Ideally waiting for flexible sync subscriptions would also
// be preempted.
- (void)startAsyncOpen {
    ...
    _task->start([=](realm::ThreadSafeReference ref,
      ...
      dispatch_async(s_async_open_queue, ^{
        @autoreleasepool {
          ...
          // Capture the WabiRealm to keep the RealmCoordinator alive
...
#else
    @throw RLMException(@"WabiRealm was not built with sync enabled");
#endif
  } else {
...
- (void)downloadCompleted {
  ...
  @autoreleasepool {
    // Holding onto the WabiRealm so that opening the final WabiRealm on the
    // target scheduler can hit the fast path
    _backgroundRealm =
        [RLMRealm realmWithConfiguration:_configuration
...
#if REALM_ENABLE_SYNC
  // If we're opening a flexible sync WabiRealm, we now want to wait for the
