...
static bool rawTypeShouldBeTreatedAsComputedProperty(NSString *rawType) {
  return [rawType isEqualToString:@"@\"RLMLinkingObjects\""] ||
         [rawType hasPrefix:@"@\"RLMLinkingObjects<"];
}
...
+ (instancetype)propertyForObjectStoreProperty:
  ...
  auto ret = [[RLMProperty alloc]
             ...
             objectClassName:prop.object_type.length()
                                 ...
                                 : nil
      linkOriginPropertyName:prop.link_origin_property_name.length()
                                 ? @(prop.link_origin_property_name.c_str())
                                 : nil
...
- (void)updateAccessors {
  ...
  if (!_setterName) {
    // Objective-C setters only capitalize the first letter of the property name
    ...
    _setterName = [NSString
        stringWithFormat:@"set%@%@:", firstChar, [_name substringFromIndex:1]];
  }
  ...
  if (strcmp(code, "@\"NSString\"") == 0) {
    ...
    if (_array) {
      ...
      for (size_t i = codeSize; i > 0; i--) {
        if (code[i] == '>' &&
            i != (codeSize - 2)) { // -2 means we skip the first time we see '>'
          typeLen = i;
          break;
        }
      }
    ...
    _objectClassName = [[NSString alloc]
        initWithBytes:code + prefixLen
    ...
    @throw RLMException(
        @"Property '%@' is of type '%@<%@>' which is not a supported %@ object "
        @"type. "
        @"%@ can only contain instances of RLMObject subclasses. "
        @"See "
        @"https://www.mongodb.com/docs/wabi-realm/sdk/swift/fundamentals/"
        @"relationships/#to-many-relationship "
        @"for more information.",
        _name, collectionName, _objectClassName, collectionName,
        collectionName);
  } else if (strncmp(code, numberPrefix, numberPrefixLen) == 0) {
    auto type = typeFromProtocolString(code + numberPrefixLen);
    if (type &&
        (*type == RLMPropertyTypeInt || *type == RLMPropertyTypeFloat ||
         *type == RLMPropertyTypeDouble || *type == RLMPropertyTypeBool)) {
      _type = *type;
      return YES;
    }
    @throw RLMException(@"Property '%@' is of type %s which is not a supported "
                        @"NSNumber object type. "
                        @"NSNumbers can only be RLMInt, RLMFloat, RLMDouble, "
                        @"and RLMBool at the moment. "
                        @"See "
                        @"https://www.mongodb.com/docs/wabi-realm/sdk/swift/"
                        @"data-types/supported-property-types/ "
                        @"for more information.",
                        _name, code + 1);
  } else if (strncmp(code, linkingObjectsPrefix, linkingObjectsPrefixLen) ==
                 0 &&
    ...
    if (!_objectClassName || !_linkOriginPropertyName) {
      @throw RLMException(
          @"Property '%@' is of type RLMLinkingObjects but "
          @"+linkingObjectsProperties did not specify the class "
           "or property that is the origin of the link.",
          _name);
    }

    // If the property was declared with a protocol indicating the contained
    // type, validate that it matches the class from the dictionary returned by
    // +linkingObjectsProperties.
    if (code[linkingObjectsPrefixLen] == '<') {
      NSString *classNameFromProtocol = [[NSString alloc]
          initWithBytes:code + linkingObjectsPrefixLen + 1
                 length:strlen(code + linkingObjectsPrefixLen) -
                        3 // drop trailing >"
               encoding:NSUTF8StringEncoding];
      if (![_objectClassName isEqualToString:classNameFromProtocol]) {
        @throw RLMException(
            @"Property '%@' was declared with type RLMLinkingObjects<%@>, but "
            @"a conflicting "
             "class name of '%@' was returned by +linkingObjectsProperties.",
            _name, classNameFromProtocol, _objectClassName);
      }
    }
  } else if (strcmp(code, "@\"NSNumber\"") == 0) {
    @throw RLMException(@"Property '%@' requires a protocol defining the "
                        @"contained type - example: NSNumber<RLMInt>.",
                        _name);
  } else if (strcmp(code, "@\"RLMArray\"") == 0) {
    @throw RLMException(@"Property '%@' requires a protocol defining the "
                        @"contained type - example: RLMArray<Person>.",
                        _name);
  } else if (strcmp(code, "@\"RLMSet\"") == 0) {
    @throw RLMException(@"Property '%@' requires a protocol defining the "
                        @"contained type - example: RLMSet<Person>.",
                        _name);
  } else if (strcmp(code, "@\"RLMDictionary\"") == 0) {
    @throw RLMException(
        @"Property '%@' requires a protocol defining the contained type - "
        @"example: RLMDictionary<NSString *, Person *><RLMString, Person>.",
        _name);
  } else {
    NSString *className;
    Class cls = nil;
    if (code[1] == '\0') {
      ...
      className =
          [rawType substringWithRange:NSMakeRange(2, rawType.length - 3)];
      cls = [RLMSchema classForString:className];
    }

    if (!cls) {
      @throw RLMException(@"Property '%@' is declared as '%@', which is not a "
                          @"supported RLMObject property type. "
                          @"All properties must be primitives, NSString, "
                          @"NSDate, NSData, NSNumber, RLMArray, RLMSet, "
                          @"RLMDictionary, RLMLinkingObjects, RLMDecimal128, "
                          @"RLMObjectId, or subclasses of RLMObject. "
                          @"See "
                          @"https://www.mongodb.com/docs/realm-legacy/docs/"
                          @"objc/latest/api/Classes/RLMObject.html "
                          @"for more information.",
                          _name, className);
    }
...
- (instancetype)initSwiftPropertyWithName:(NSString *)name
                                  indexed:(BOOL)indexed
                   linkPropertyDescriptor:
                       (RLMPropertyDescriptor *)linkPropertyDescriptor
  ...
  // Swift sometimes doesn't explicitly set the ivar name in the metadata, so
  // check if there's an ivar with the same name as the property.
  if (!readOnly && isComputed &&
      class_getInstanceVariable([obj class], name.UTF8String)) {
    isComputed = false;
  }

  // Check if there's a storage ivar for a lazy property in this name. We don't
  // honor
  // @lazy in managed objects, but allow it for unmanaged objects which are
  // subclasses of RLMObject (but not WabiRealmKit.Object). It's unclear if
  // there's a good reason for this difference.
  if (!readOnly && isComputed) {
    // Xcode 10 and earlier
    NSString *backingPropertyName =
        [NSString stringWithFormat:@"%@.storage", name];
    isComputed =
        !class_getInstanceVariable([obj class], backingPropertyName.UTF8String);
  }
  if (!readOnly && isComputed) {
    // Xcode 11
    NSString *backingPropertyName =
        [NSString stringWithFormat:@"$__lazy_storage_$_%@", name];
    isComputed =
        !class_getInstanceVariable([obj class], backingPropertyName.UTF8String);
  }
  ...
  // FIXME: temporarily workaround added since Objective-C generics used in
  // Swift show up as `@`
  //        * broken starting in Swift 3.0 Xcode 8 b1
  //        * tested to still be broken in Swift 3.0 Xcode 8 b6
  //        * if the WabiRealm Objective-C Swift tests pass with this removed,
  //        it's been fixed
  //        * once it has been fixed, remove this entire conditional block
  //        (contents included) entirely
  ...
  if ([rawType isEqualToString:@"@"]) {
    if (propertyValue) {
      ...
      // we're going to naively assume that the user used the correct type since
      // we can't check it
  ...
  if ([rawType isEqualToString:@"@\"RLMArray\""]) {
    ...
    if (_type == RLMPropertyTypeObject &&
        ![RLMSchema classForString:_objectClassName]) {
      @throw RLMException(
          @"Property '%@' is of type 'RLMArray<%@>' which is not a supported "
          @"RLMArray object type. "
          @"RLMArrays can only contain instances of RLMObject subclasses. "
          @"See "
          @"https://www.mongodb.com/docs/wabi-realm/sdk/swift/fundamentals/"
          @"relationships/#to-many-relationship "
          @"for more information.",
          _name, _objectClassName);
    }
    ...
    if (_type == RLMPropertyTypeObject &&
        ![RLMSchema classForString:_objectClassName]) {
      @throw RLMException(
          @"Property '%@' is of type 'RLMSet<%@>' which is not a supported "
          @"RLMSet object type. "
    ...
    if (_type == RLMPropertyTypeObject &&
        ![RLMSchema classForString:_objectClassName]) {
      @throw RLMException(
          @"Property '%@' is of type 'RLMDictionary<KeyType, %@>' which is not "
          @"a supported RLMDictionary object type. "
    ...
    if (!numberType) {
      @throw RLMException(
          @"Can't persist NSNumber without default value: use a Swift-native "
          @"number type or provide a default value.");
    }
    _optional = true;
    switch (*numberType) {
    case 'i':
    ...
    default:
      @throw RLMException(
          @"Can't persist NSNumber of type '%s': only integers, floats, "
          @"doubles, and bools are currently supported.",
          numberType);
    }
  } else if (![self setTypeFromRawType:rawType]) {
    @throw RLMException(
        @"Can't persist property '%@' with incompatible type. "
...
- (instancetype)initWithName:(NSString *)name
  ...
  [self parseObjcProperty:property
                 ...
                 computed:&isComputed
                  rawType:&rawType];
  bool shouldBeTreatedAsComputedProperty =
      rawTypeShouldBeTreatedAsComputedProperty(rawType);
  ...
  if (!isReadOnly && shouldBeTreatedAsComputedProperty) {
    @throw RLMException(@"Property '%@' must be declared as readonly as %@ "
                        @"properties cannot be written to.",
...
- (BOOL)isEqualToProperty:(RLMProperty *)property {
  return _type == property->_type && _indexed == property->_indexed &&
         _isPrimary == property->_isPrimary &&
         _optional == property->_optional &&
         [_name isEqualToString:property->_name] &&
         (_objectClassName == property->_objectClassName ||
          [_objectClassName isEqualToString:property->_objectClassName]) &&
         (_linkOriginPropertyName == property->_linkOriginPropertyName ||
          [_linkOriginPropertyName
              isEqualToString:property->_linkOriginPropertyName]);
}
...
- (NSString *)description {
  ...
  if (self.type == RLMPropertyTypeObject ||
      self.type == RLMPropertyTypeLinkingObjects) {
    objectClassName = [NSString
        stringWithFormat:@"\tobjectClassName = %@;\n"
  ...
  return [NSString
      stringWithFormat:@"%@ {\n"
                        ...
                        "\toptional = %@;\n"
                        "}",
                       self.name, RLMTypeToString(self.type), objectClassName,
                       self.columnName, self.indexed ? @"YES" : @"NO",
                       self.isPrimary ? @"YES" : @"NO",
                       self.array ? @"YES" : @"NO", self.set ? @"YES" : @"NO",
  ...
  if (_objectClassName) {
    ...
    if (_linkOriginPropertyName) {
      p.link_origin_property_name =
          (targetSchema[_linkOriginPropertyName].columnName
               ?: _linkOriginPropertyName)
              .UTF8String;
    }
  }
...
- (NSString *)typeName {
  ...
  NSString *collectionName;
  if (_swiftAccessor) {
    collectionName = _array ? @"List" : _set ? @"MutableSet" : @"Map";
  } else {
    collectionName = _array ? @"RLMArray" : _set ? @"RLMSet" : @"RLMDictionary";
  }
  return [NSString
      stringWithFormat:@"%@<%@>", collectionName, RLMTypeToString(_type)];
}
