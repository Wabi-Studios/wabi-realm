...
template <typename T>
T getOptional(__unsafe_unretained RLMObjectBase *const obj, uint16_t key,
              bool *gotValue) {
...
void setValue(__unsafe_unretained RLMObjectBase *const obj, ColKey key,
  ...
  } else if (obj->_row.get_linked_object(key).get_key() !=
             val->_row.get_key()) {
...
id RLMCollectionClassForProperty(RLMProperty *prop, bool isManaged) {
  ...
  if (prop.array) {
    ...
    @throw RLMException(@"Invalid collection '%@' for class '%@'.", prop.name,
...
id<RLMCollection> getCollection(__unsafe_unretained RLMObjectBase *const obj,
                                NSUInteger propIndex) {
  RLMVerifyAttached(obj);
...
void assignValue(__unsafe_unretained RLMObjectBase *const obj,
                 __unsafe_unretained RLMProperty *const prop, ColKey key,
...
RLMLinkingObjects *
getLinkingObjects(__unsafe_unretained RLMObjectBase *const obj,
                  __unsafe_unretained RLMProperty *const property) {
  RLMVerifyAttached(obj);
  auto &objectInfo = obj->_realm->_info[property.objectClassName];
  auto &linkOrigin =
      obj->_info->objectSchema->computed_properties[property.index]
...
template <typename Type> id makeBoxedGetter(NSUInteger index) {
  return ^(__unsafe_unretained RLMObjectBase *const obj) {
    return getBoxed<Type>(obj, index);
  };
}
template <typename Type> id makeOptionalGetter(NSUInteger index) {
  return ^(__unsafe_unretained RLMObjectBase *const obj) {
...
template <typename Type> id makeWrapperGetter(NSUInteger index, bool optional) {
  if (optional) {
    return makeOptionalGetter<Type>(index);
  }
  return makeBoxedGetter<Type>(index);
...
willChange(RLMObservationTracker &tracker,
  ...
  tracker.willChange(RLMGetObservationInfo(obj->_observationInfo,
...
void kvoSetValue(__unsafe_unretained RLMObjectBase *const obj, NSUInteger index,
                 ArgType value) {
  RLMVerifyInWriteTransaction(obj);
...
id managedSetter(RLMProperty *prop, const char *type) {
  ...
  case RLMPropertyTypeFloat:
    return boxed ? makeSetter<NSNumber<RLMFloat> *>(prop)
                 : makeSetter<float>(prop);
  case RLMPropertyTypeDouble:
    return boxed ? makeSetter<NSNumber<RLMDouble> *>(prop)
                 : makeSetter<double>(prop);
  case RLMPropertyTypeBool:
    return boxed ? makeSetter<NSNumber<RLMBool> *>(prop)
                 : makeSetter<BOOL, bool>(prop);
  case RLMPropertyTypeString:
  ...
  case RLMPropertyTypeAny:
    return makeSetter<id<RLMValue>>(prop);
  case RLMPropertyTypeLinkingObjects:
    return nil;
  case RLMPropertyTypeObject:
    return makeSetter<RLMObjectBase *>(prop);
  case RLMPropertyTypeObjectId:
    return makeSetter<RLMObjectId *>(prop);
  case RLMPropertyTypeDecimal128:
    return makeSetter<RLMDecimal128 *>(prop);
  case RLMPropertyTypeUUID:
    return makeSetter<NSUUID *>(prop);
  }
}
...
id superGet(RLMObjectBase *obj, NSString *propName) {
  ...
  getter_type superGetter =
      (getter_type)[superClass instanceMethodForSelector:prop.getterSel];
...
void superSet(RLMObjectBase *obj, NSString *propName, id val) {
  typedef void (*setter_type)(RLMObjectBase *, SEL,
                              id<RLMCollection> collection);
  RLMProperty *prop = obj->_objectSchema[propName];
  Class superClass = class_getSuperclass(obj.class);
  setter_type superSetter =
      (setter_type)[superClass instanceMethodForSelector:prop.setterSel];
...
id unmanagedGetter(RLMProperty *prop, const char *) {
  ...
  if (prop.collection) {
    ...
    return ^(RLMObjectBase *obj) {
      id val = superGet(obj, propName);
      if (!val) {
        val = [[cls alloc] initWithObjectType:type
                                     optional:optional
  ...
  return ^(RLMObjectBase *obj, id<NSFastEnumeration> values) {
    ...
    if (prop.type == RLMPropertyTypeObject) {
      collection = [[cls alloc] initWithObjectClassName:prop.objectClassName
                                                keyType:prop.dictionaryKeyType];
    } else {
      collection = [[cls alloc] initWithObjectType:prop.type
...
void addMethod(Class cls, __unsafe_unretained RLMProperty *const prop,
  ...
  if (id block =
          setter(prop, getterType)) { // note: deliberately getterType as it's
                                      // easier to grab the relevant type from
    class_addMethod(cls, sel, imp_implementationWithBlock(block),
                    method_getTypeEncoding(setterMethod));
  }
}

Class createAccessorClass(Class objectClass, RLMObjectSchema *schema,
...
Class RLMManagedAccessorClassForObjectClass(Class objectClass,
                                            ...
                                            const char *name) {
  return createAccessorClass(objectClass, schema, name, managedGetter,
                             managedSetter);
}
...
// implement the class method className on accessors to return the className of
// the base object
...
void RLMReplaceSharedSchemaMethod(Class accessorClass,
                                  RLMObjectSchema *schema) {
  REALM_ASSERT(accessorClass != [WabiRealmKitObject class]);
  Class metaClass = object_getClass(accessorClass);
  IMP imp = imp_implementationWithBlock(^(Class cls) {
    ...
    if (!strncmp(class_getName(cls), accessorClassPrefix,
                 sizeof(accessorClassPrefix) - 1)) {
      return schema;
    }
...
void RLMDynamicValidatedSet(RLMObjectBase *obj, NSString *propName, id val) {
  ...
  if (!prop) {
    @throw RLMException(@"Invalid property name '%@' for class '%@'.", propName,
                        obj->_objectSchema.className);
  }
  if (prop.isPrimary) {
    @throw RLMException(
  ...
  // that can be converted to an embedded object for dynamic link set
  // operations.
  bool is_embedded =
      prop.type == RLMPropertyTypeObject &&
      obj->_info->linkTargetType(prop.index).rlmObjectSchema.isEmbedded;
...
id RLMDynamicGet(__unsafe_unretained RLMObjectBase *const obj,
                 __unsafe_unretained RLMProperty *const prop) {
  if (auto accessor = prop.swiftAccessor;
      accessor && [obj isKindOfClass:obj->_objectSchema.objectClass]) {
...
#define REALM_SWIFT_PROPERTY_ACCESSOR(objc, swift, rlmtype)                    \
  objc RLMGetSwiftProperty##swift(                                             \
      __unsafe_unretained RLMObjectBase *const obj, uint16_t key) {            \
    return get<objc>(obj, key);                                                \
  }                                                                            \
  objc RLMGetSwiftProperty##swift##Optional(                                   \
      __unsafe_unretained RLMObjectBase *const obj, uint16_t key,              \
      bool *gotValue) {                                                        \
    return getOptional<objc>(obj, key, gotValue);                              \
  }                                                                            \
  void RLMSetSwiftProperty##swift(                                             \
      __unsafe_unretained RLMObjectBase *const obj, uint16_t key,              \
      objc value) {                                                            \
    RLMVerifyAttached(obj);                                                    \
...
RLMSet *RLMGetSwiftPropertySet(__unsafe_unretained RLMObjectBase *const obj,
                               uint16_t key) {
  return getCollection(obj, key);
}
RLMDictionary *
RLMGetSwiftPropertyMap(__unsafe_unretained RLMObjectBase *const obj,
                       uint16_t key) {
  return getCollection(obj, key);
}
...
void RLMSetSwiftPropertyObject(
    __unsafe_unretained RLMObjectBase *const obj, uint16_t key,
...
id RLMAccessorContext::propertyValue(
    id obj, size_t propIndex, __unsafe_unretained RLMProperty *const prop) {
  ...
  id value = RLMValidatedValueForProperty(
      obj,
      [obj respondsToSelector:prop.getterSel] ? prop.getterName : prop.name,
...
id RLMAccessorContext::box(wabi_realm::object_store::Dictionary &&d) {
  ...
  REALM_ASSERT(currentProperty);
  return
      [[RLMManagedDictionary alloc] initWithBackingCollection:std::move(d)
                                                   parentInfo:_parentObjectInfo
                                                     property:currentProperty];
}
...
RLMAccessorContext::createObject(id value, wabi_realm::CreatePolicy policy,
  ...
  if ([value isKindOfClass:[NSArray class]] &&
      [value count] > _info.objectSchema->persisted_properties.size()) {
    @throw RLMException(
  ...
  if (objBase) {
    if (objBase.isInvalidated) {
      if (policy.create && !policy.copy) {
        @throw RLMException(@"Adding a deleted or invalidated object to a "
                            @"WabiRealm is not permitted");
      } else {
    ...
    if (policy.copy) {
      if (policy.update || !forceCreate) {
        ...
        // the WabiRealm which is of the correct type
        if (objBase->_realm == _realm &&
            objBase->_row.get_table() == _info.table() &&
            !_info.table()->is_embedded()) {
      ...
      outObj = &objBase->_row;
      // add() on an object already managed by this WabiRealm is a no-op
      if (objBase->_realm == _realm) {
        return {objBase->_row, true};
      }
      if (!policy.create) {
      ...
      if (objBase->_realm) {
        @throw RLMException(
            @"Object is already managed by another WabiRealm. Use "
            @"create instead to copy it into this WabiRealm.");
      }
      if (objBase->_observationInfo &&
          objBase->_observationInfo->hasObservers()) {
        requiresSwiftUIObservers =
            [RLMSwiftUIKVO removeObserversFromObject:objBase];
      ...
      REALM_ASSERT([objBase->_objectSchema.className
          isEqualToString:_info.rlmObjectSchema.className]);
      REALM_ASSERT(
          [objBase isKindOfClass:_info.rlmObjectSchema.unmanagedClass]);
  ...
  if (objBase) {
    for (RLMProperty *prop in _info.rlmObjectSchema.properties) {
      ...
      // accessors retain objects that are no longer accessible via the
      // properties this is mainly an issue when the object graph being added
      // has cycles, as it's not obvious that the user has to set the *ivars* to
      // nil to avoid leaking memory
...
// Most types don't need to distinguish between promote and init so provide a
// default
