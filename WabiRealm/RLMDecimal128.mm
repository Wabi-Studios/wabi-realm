...
// when WabiRealm Swift is being used, we need to produce objects of that type
// rather than our obj-c defined type. objc_runtime_visible marks the type as
// being visible only to the obj-c runtime and not the linker, which means that
// it'll be `nil` at runtime rather than being a linker error if it's not
// defined, and valid if it happens to be defined by some other library (i.e.
// WabiRealm Swift).
//
...
// performance hit.
[[clang::objc_runtime_visible]] @interface WabiRealmKitDecimal128
    : RLMDecimal128
@end
...
- (instancetype)init {
  if (self = [super init]) {
    if (auto cls = [WabiRealmKitDecimal128 class]; cls && cls != self.class) {
...
- (double)doubleValue {
  return
      [NSDecimalNumber decimalNumberWithDecimal:self.decimalValue].doubleValue;
}

- (NSDecimal)decimalValue {
  NSDecimal ret;
  [[[NSScanner alloc] initWithString:@(_value.to_string().c_str())]
      scanDecimal:&ret];
  return ret;
}
