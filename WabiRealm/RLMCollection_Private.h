...
void RLMCollectionSetValueForKey(id<RLMCollectionPrivate> collection,
                                 NSString *key, id _Nullable value);
FOUNDATION_EXTERN NSString *
RLMDescriptionWithMaxDepth(NSString *name, id<RLMCollection> collection,
                           NSUInteger depth);
FOUNDATION_EXTERN void RLMAssignToCollection(id<RLMCollection> collection,
                                             id value);
FOUNDATION_EXTERN void RLMSetSwiftBridgeCallback(id _Nullable (*_Nonnull)(id));

FOUNDATION_EXTERN
RLMNotificationToken *
RLMAddNotificationBlock(id collection, id block,
                        NSArray<NSString *> *_Nullable keyPaths,
                        dispatch_queue_t _Nullable queue);

typedef RLM_CLOSED_ENUM(int32_t, RLMCollectionType){
    RLMCollectionTypeArray = 0, RLMCollectionTypeSet = 1,
