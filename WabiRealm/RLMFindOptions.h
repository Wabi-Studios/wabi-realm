...
/// The maximum number of documents to return. Specifying 0 will return all
/// documents.
...
@property(nonatomic, nullable) id<RLMBSON> sort NS_REFINED_FOR_SWIFT
    __attribute__((deprecated("Use `sorting` instead, which correctly sort "
                              "more than one sort attribute",
                              "sorting")));
...
/// Options to use when executing a `find` command on a `RLMMongoCollection`.
/// @param limit The maximum number of documents to return. Specifying 0 will
...
- (instancetype)initWithLimit:(NSInteger)limit
    ...
    __attribute__((
        deprecated("Please use `initWithLimit:projection:sorting:`")))
