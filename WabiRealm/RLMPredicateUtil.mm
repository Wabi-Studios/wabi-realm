...
struct PredicateExpressionTransformer {
  PredicateExpressionTransformer(ExpressionVisitor visitor)
      : m_visitor(visitor) {}
...
NSExpression *
PredicateExpressionTransformer::visit(NSExpression *expression) const {
  ...
  case NSFunctionExpressionType: {
    ...
    if (expression.operand) {
      return [NSExpression expressionForFunction:visit(expression.operand)
                                    selectorName:expression.function
                                       arguments:arguments];
    } else {
      return [NSExpression expressionForFunction:expression.function
                                       arguments:arguments];
    }
  }
  ...
  case NSUnionSetExpressionType:
    return
        [NSExpression expressionForUnionSet:visit(expression.leftExpression)
                                       with:visit(expression.rightExpression)];
  case NSIntersectSetExpressionType:
    return [NSExpression
        expressionForIntersectSet:visit(expression.leftExpression)
                             with:visit(expression.rightExpression)];
  case NSMinusSetExpressionType:
    return
        [NSExpression expressionForMinusSet:visit(expression.leftExpression)
                                       with:visit(expression.rightExpression)];

  case NSSubqueryExpressionType: {
    NSExpression *collection = expression.collection;
    // NSExpression.collection is declared as id, but appears to always hold an
    // NSExpression for subqueries.
    REALM_ASSERT([collection isKindOfClass:[NSExpression class]]);
    return [NSExpression expressionForSubquery:visit(collection)
                         usingIteratorVariable:expression.variable
                                     predicate:visit(expression.predicate)];
  }
...
#pragma clang diagnostic ignored "-Wpartial-availability"
    return [NSExpression
        expressionForConditional:visit(expression.predicate)
                  trueExpression:visit(expression.trueExpression)
                 falseExpression:visit(expression.falseExpression)];
...
NSPredicate *
PredicateExpressionTransformer::visit(NSPredicate *predicate) const {
  if ([predicate isKindOfClass:[NSCompoundPredicate class]]) {
    ...
    return [[NSCompoundPredicate alloc]
         initWithType:compoundPredicate.compoundPredicateType
...
NSPredicate *transformPredicate(NSPredicate *predicate,
