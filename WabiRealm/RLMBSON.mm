...
- (RLMBSONType)bsonType {
  ...
  if (numberType == *@encode(bool) || numberType == *@encode(char)) {
    return RLMBSONTypeBool;
  } else if (numberType == *@encode(int) || numberType == *@encode(short) ||
...
- (instancetype)initWithBsonDocument:(BsonDocument)bsonDocument {
  if ((self = [self init])) {
    for (auto it = bsonDocument.begin(); it != bsonDocument.end(); ++it) {
      const auto &entry = (*it);
      [self setObject:RLMConvertBsonToRLMBSON(entry.second)
               forKey:@(entry.first.data())];
    }
...
- (RegularExpression)regularExpressionValue {
  ...
  if ((_options & NSRegularExpressionCaseInsensitive) != 0)
    s += 'i';
  if ((_options & NSRegularExpressionUseUnixLineSeparators) != 0)
    s += 'm';
  if ((_options & NSRegularExpressionDotMatchesLineSeparators) != 0)
    s += 's';
  if ((_options & NSRegularExpressionUseUnicodeWordBoundaries) != 0)
    s += 'x';
...
BsonDocument RLMConvertRLMBSONArrayToBsonDocument(NSArray<id<RLMBSON>> *array) {
  ...
  for (NSDictionary<NSString *, id<RLMBSON>> *item in array) {
    [item enumerateKeysAndObjectsUsingBlock:[&](NSString *key, id<RLMBSON> bson,
                                                BOOL *) {
...
NSArray<id<RLMBSON>> *
RLMConvertBsonDocumentToRLMBSONArray(std::optional<BsonDocument> b) {
  if (!b) {
    return @[];
  }
