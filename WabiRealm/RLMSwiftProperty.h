...
#define REALM_FOR_EACH_SWIFT_PRIMITIVE_TYPE(macro)                             \
  macro(bool, Bool, bool) macro(double, Double, double)                        \
      macro(float, Float, float) macro(int64_t, Int64, int)

#define REALM_FOR_EACH_SWIFT_OBJECT_TYPE(macro)                                \
  macro(NSString, String, string) macro(NSDate, Date, date)                    \
      macro(NSData, Data, data) macro(NSUUID, UUID, uuid)                      \
...
#define REALM_SWIFT_PROPERTY_ACCESSOR(objc, swift, rlmtype)                    \
  ...
  objc RLMGetSwiftProperty##swift##Optional(RLMObjectBase *, uint16_t,         \
                                            bool *);                           \
...
void RLMSetSwiftPropertyObject(RLMObjectBase *, uint16_t,
                               RLMObjectBase *_Nullable);
