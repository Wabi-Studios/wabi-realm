...
void RLMCacheRealm(
    __unsafe_unretained RLMRealmConfiguration *const configuration,
    RLMScheduler *scheduler, __unsafe_unretained RLMRealm *const realm) {
...
RLMRealm *RLMGetCachedRealm(
  ...
  if (realm && !realm->_realm->scheduler()->is_on_thread()) {
    ...
    // queue-bound WabiRealm from the wrong queue, or if we have a stale cached
    // WabiRealm which is bound to a thread that no longer exists. In the first
...
RLMRealm *RLMGetFrozenRealmForSourceRealm(
    __unsafe_unretained RLMRealm *const sourceRealm) {
  std::lock_guard lock(s_realmCacheMutex);
  auto &r = *sourceRealm->_realm;
  auto &path = r.config().path;
  NSMapTable *realms = s_realmsPerPath[path];
  if (!realms) {
    s_realmsPerPath[path] = realms = [NSMapTable
        mapTableWithKeyOptions:NSPointerFunctionsIntegerPersonality |
                               NSPointerFunctionsOpaqueMemory
                  valueOptions:NSPointerFunctionsWeakMemory];
  }
  r.read_group();
...
void RLMAddBeforeNotifyBlock(RLMRealm *realm, dispatch_block_t block) {
  static_cast<RLMNotificationHelper *>(realm->_realm->m_binding_context.get())
      ->add_before_notify_block(block);
}
...
RLMAsyncRefreshTask *RLMRealmRefreshAsync(RLMRealm *rlmRealm) {
  ...
  // Refresh is a no-op if the WabiRealm isn't currently in a read transaction
  // or is up-to-date
  ...
  auto &context =
      static_cast<RLMNotificationHelper &>(*realm.m_binding_context);
  context.wait_for_refresh(*latest, ^(bool didRefresh) {
    [refresh complete:didRefresh];
  });
  return refresh;
}
