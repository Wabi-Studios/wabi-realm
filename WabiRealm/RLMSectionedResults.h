...
/**
 A `RLMSectionedResultsChange` object encapsulates information about changes to
 sectioned results that are reported by WabiRealm notifications.

 `RLMSectionedResultsChange` is passed to the notification blocks registered
 with
 `-addNotificationBlock` on `RLMSectionedResults`, and reports what sections and
 rows in the collection changed since the last time the notification block was
 called.

 A complete example of updating a `UITableView` named `tv`:

     [tv beginUpdates];
     [tv deleteRowsAtIndexPaths:changes.deletions
 withRowAnimation:UITableViewRowAnimationAutomatic]; [tv
 insertRowsAtIndexPaths:changes.insertions
 withRowAnimation:UITableViewRowAnimationAutomatic]; [tv
 reloadRowsAtIndexPaths:changes.modifications
 withRowAnimation:UITableViewRowAnimationAutomatic]; [tv
 insertSections:changes.sectionsToInsert
 withRowAnimation:UITableViewRowAnimationAutomatic]; [tv
 deleteSections:changes.sectionsToRemove
 withRowAnimation:UITableViewRowAnimationAutomatic]; [tv endUpdates];

 All of the arrays in an `RLMSectionedResultsChange` are always sorted in
 ascending order.
 */
@interface RLMSectionedResultsChange : NSObject
/// The index paths of objects in the previous version of the collection which
/// have been removed from this one.
@property(nonatomic, readonly) NSArray<NSIndexPath *> *deletions;
/// The index paths in the new version of the collection which were newly
/// inserted.
...
/// The `RLMSectionedResult` protocol defines properties and methods common to
/// both `RLMSectionedResults and RLMSection`
...
/**
 ...
 containing WabiRealm. Unlike live arrays, frozen collections can be accessed
 from any thread.
...
/**
 ...
 The block will be asynchronously called with the initial sectioned results
 collection, and then called again after each write transaction which changes
 either any of the objects in the results, or which objects are in the results.
 ...
 the block is not called at all. See the `RLMSectionedResultsChange`
 documentation for information on how the changes are reported and an example of
 updating a `UITableView`.

 At the time when the block is called, the `RLMSection` / `RLMSectionedResults`
 object will be fully evaluated and up-to-date.
 ...
 the write transaction.
    ...
    RLMSectionedResults<Dog *> *sectionedResults = [results
 sectionedResultsUsingKeyPath:@"age" ascending:YES]; self.token =
 [sectionedResults addNotificationBlock:^(RLMSectionedResults *sectionedResults,
...
- (RLMNotificationToken *)addNotificationBlock:
    (void (^)(id<RLMSectionedResult>, RLMSectionedResultsChange *))block
 ...
 */
- (RLMNotificationToken *)addNotificationBlock:
                              (void (^)(id<RLMSectionedResult>,
                                        RLMSectionedResultsChange *))block
...
/**
 ...
 @note When filtering with key paths a notification will be fired in the
 following scenarios:
    - An object in the collection has been modified at the filtered properties.
    - An object has been modified on the section key path property, and the
 result of that modification has changed it's position in the section, or the
 object may need to move to another section.
    - An object of the same observed type has been inserted or deleted from the
 WabiRealm.
...
- (RLMNotificationToken *)
    addNotificationBlock:(void (^)(id<RLMSectionedResult>,
                                   RLMSectionedResultsChange *))block
...
/**
 ...
 @warning Holding onto a frozen section for an extended period while performing
          write transaction on the WabiRealm may result in the WabiRealm file
 growing to large sizes. See
 `RLMRealmConfiguration.maximumNumberOfActiveVersions` for more information.
 */
- (instancetype)freeze;
...
/**
 ...
 the write transaction.
    ...
    RLMResults<Dog *> *results = [Dog allObjects];
    RLMSectionedResults<Dog *> *sectionedResults = [results
 sectionedResultsUsingKeyPath:@"age" ascending:YES]; RLMSection<Dog *> *section
 = sectionedResults[0] // section with dogs aged '5' already exists.

    self.token = [section addNotificationBlock:^(RLMSection *section,
 RLMSectionedResultsChange *changes) {
         // Only fired once for the example
 ...
 */
- (RLMNotificationToken *)addNotificationBlock:
    (void (^)(RLMSection<RLMKeyType, RLMObjectType> *,
              RLMSectionedResultsChange *))block
    __attribute__((warn_unused_result));
/**
 Registers a block to be called each time the section changes.
 ...
 */
- (RLMNotificationToken *)
    addNotificationBlock:(void (^)(RLMSection<RLMKeyType, RLMObjectType> *,
                                   RLMSectionedResultsChange *))block
                keyPaths:(nullable NSArray<NSString *> *)keyPaths
                   queue:(nullable dispatch_queue_t)queue
    __attribute__((warn_unused_result));
...
/// A lazily evaluated collection that holds elements in sections determined by
/// a section key.
...
- (RLMSection<RLMKeyType, RLMObjectType> *)objectAtIndexedSubscript:
    (NSUInteger)index;
/// Returns the section at a given index.
...
/**
 ...
 The frozen copy is an immutable sectioned results collection which contains the
 same data as this sectioned results collection currently contains, but will not
 update when writes are made to the containing WabiRealm. Unlike live sectioned
 results collections, frozen sectioned results collection can be accessed from
 any thread.

 @warning This method cannot be called during a write transaction, or when the
          containing WabiRealm is read-only.
 @warning Holding onto a frozen sectioned results collection for an extended
 period while performing write transaction on the WabiRealm may result in the
 WabiRealm file growing to large sizes. See
 `RLMRealmConfiguration.maximumNumberOfActiveVersions` for more information.
 */
- (instancetype)freeze;
/**
 ...
 This method resolves a reference to a live copy of the same frozen sectioned
 results collection. If called on a live section, will return itself.
*/
- (instancetype)thaw;
/**
 Indicates if the underlying sectioned results collection is frozen.

 Frozen sectioned results collections are immutable and can be accessed from any
 thread.
 */
@property(nonatomic, readonly, getter=isFrozen) BOOL frozen;
...
/**
 Registers a block to be called each time the sectioned results collection
 changes.
 ...
 At the time when the block is called, the `RLMSectionedResults` object will be
 fully evaluated and up-to-date.
 ...
 */
- (RLMNotificationToken *)addNotificationBlock:
    (void (^)(RLMSectionedResults<RLMKeyType, RLMObjectType> *,
              RLMSectionedResultsChange *))block
    __attribute__((warn_unused_result));
/**
 Registers a block to be called each time the sectioned results collection
