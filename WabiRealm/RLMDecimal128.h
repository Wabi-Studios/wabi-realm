...
///
/// Returns a decimal where `isNaN` is `YES` if the string cannot be parsed as a
/// decimal. `error` is never set and this will never actually return `nil`.
...
@property(class, readonly, copy)
    RLMDecimal128 *minimumDecimalNumber NS_REFINED_FOR_SWIFT;

/// The maximum value for RLMDecimal128.
@property(class, readonly, copy)
    RLMDecimal128 *maximumDecimalNumber NS_REFINED_FOR_SWIFT;
...
/// Comparision operator to check if the right hand side is greater than the
/// current value.
- (BOOL)isGreaterThan:(nullable RLMDecimal128 *)decimalNumber;

/// Comparision operator to check if the right hand side is greater than or
/// equal to the current value.
- (BOOL)isGreaterThanOrEqualTo:(nullable RLMDecimal128 *)decimalNumber;

/// Comparision operator to check if the right hand side is less than the
/// current value.
- (BOOL)isLessThan:(nullable RLMDecimal128 *)decimalNumber;

/// Comparision operator to check if the right hand side is less than or equal
