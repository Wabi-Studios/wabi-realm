...
/**
 ...
 invoked. This task object can be used to observe the state of the download or
 to cancel it. This should be used instead of trying to observe the download via
 the sync session as the sync session itself is created asynchronously, and may
 not exist yet when -[RLMRealm asyncOpenWithConfiguration:completion:] returns.
 */
...
/**
 ...
 async open will never be called. If multiple async opens on the same WabiRealm
 are happening concurrently, all other opens will fail with the error "operation
 cancelled".
 */
- (void)cancel;
@end
