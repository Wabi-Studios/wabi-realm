...
- (void)setObject:(id)newValue atIndexedSubscript:(NSUInteger)index {
  REALM_TERMINATE(
      "Replacing objects at an indexed subscript is not supported on RLMSet");
}
...
- (RLMNotificationToken *)addNotificationBlock:(void (^)(RLMArray *,
                                                         RLMCollectionChange *,
                                                         NSError *))block
...
- (id)aggregateProperty:(NSString *)key
  ...
  if (!canAggregate(type, allowDate)) {
    ...
    if (_type == RLMPropertyTypeObject) {
      ...
      @throw RLMException(@"%@ is not supported for %@%s set", method,
  ...
  NSArray *values = [key isEqualToString:@"self"]
                        ? _backingCollection.array
                        : [_backingCollection.array valueForKey:key];
  if (_optional) {
...
- (id)valueForKeyPath:(NSString *)keyPath {
  if ([keyPath characterAtIndex:0] != '@') {
    return toUnorderedSet(_backingCollection
                              ? [_backingCollection valueForKeyPath:keyPath]
...
- (BOOL)isEqual:(id)object {
  if (auto set = RLMDynamicCast<RLMSet>(object)) {
    return !set.realm && ((_backingCollection.count == 0 &&
                           set->_backingCollection.count == 0) ||
  ...
  if (set->_type != RLMPropertyTypeObject) {
    if (!RLMValidateValue(value, set->_type, set->_optional, false, nil)) {
      @throw RLMException(
          ...
          [value class], RLMTypeToString(set->_type),
          set -> _optional ? "?" : "");
    }
    return;
  }
  ...
  if (!object->_objectSchema) {
    @throw RLMException(
        @"Object cannot be inserted unless the schema is initialized. "
         "This can happen if you try to insert objects into a RLMSet / Set "
         "from a default value or from an overriden unmanaged initializer "
         "(`init()`).");
  }
  if (![set->_objectClassName
...
- (RLMResults *)objectsWithPredicate:(NSPredicate *)predicate {
  @throw RLMException(@"This method may only be called on RLMSet instances "
