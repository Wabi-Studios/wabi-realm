...
extern NSString *const RLMErrorCodeKey
    __attribute((deprecated("use -[NSError code]")));
...
/**
 ...
 associated with the WabiRealm error domain specified in `RLMErrorDomain`.
 */
typedef RLM_ERROR_ENUM(NSInteger, RLMError, RLMErrorDomain){
    /** Denotes a general error that occurred when trying to open a WabiRealm.
     */
    RLMErrorFail = 1,

    /** Denotes a file I/O error that occurred when trying to open a WabiRealm.
     */
    RLMErrorFileAccess = 2,

    /**
     Denotes a file permission error that occurred when trying to open a
     WabiRealm.

     This error can occur if the user does not have permission to open or create
     the specified file in the specified access mode when opening a WabiRealm.
     */
    ...
    /**
     ...
     This error may occur if a WabiRealm file could not be found on disk when
     trying to open a WabiRealm as read-only, or if the directory part of the
    ...
    /**
     ...
     This error may occur if trying to share a WabiRealm file between an i386
     (32-bit) iOS Simulator and the WabiRealm Studio application. In this case,
    ...
    /**
     ...
     space to mmap the WabiRealm file.
     */
    ...
    */
    ...
    /**
     Denotes an error where a WabiRealm file could not be opened because another
     ...
     in-memory WabiRealm in non-in-memory mode.
     */
    RLMErrorIncompatibleSession = 15,

    /**
     Denotes an error that occurs if the file is a valid WabiRealm file, but has
     a file format version which is not supported by this version of WabiRealm.
     This typically means that the file was written by a newer version of
     WabiRealm, but may also mean that it is from a pre-1.0 version of WabiRealm
     (or for synchronized files, pre-10.0).
     */
    RLMErrorUnsupportedFileFormatVersion = 16,

    /**
     Denotes an error that occurs if a synchronized WabiRealm is opened in more
    ...
    /// A file operation failed in a way which does not have a more specific
    /// error code.
    RLMErrorFileOperationFailed = 19,

    /**
     Denotes an error that occurs if the file being opened is not a valid
     WabiRealm file. Some of the possible causes of this are:
     1. The file at the given URL simply isn't a WabiRealm file at all.
     2. The wrong encryption key was given.
     3. The WabiRealm file is encrypted and no encryption key was given.
     4. The WabiRealm file isn't encrypted but an encryption key was given.
     5. The file on disk has become corrupted.
     */
    RLMErrorInvalidDatabase = 20,

    /**
     Denotes an error that occurs if a WabiRealm is opened in the wrong history
     mode. Typically this means that either a local WabiRealm is being opened as
     a synchronized WabiRealm or vice versa.
     */
    RLMErrorIncompatibleHistories = 21,

    /**
     Denotes an error that occurs if objects were written to a flexible sync
     WabiRealm without any active subscriptions for that object type. All
     objects created in flexible sync Realms must match at least one active
...
/**
 An error related to a problem that might be reported by the synchronization
 manager error handler, or a callback on a sync-related API that performs
 asynchronous work.
 */
typedef RLM_ERROR_ENUM(NSInteger, RLMSyncError, RLMSyncErrorDomain){
    /// An error that indicates a problem with the session (a specific WabiRealm
    /// opened for sync).
    ...
    /**
     An error that indicates the WabiRealm needs to be reset.

     A synced WabiRealm may need to be reset because Atlas App Services
     encountered an error and had to be restored from a backup. If the backup
     copy of the remote WabiRealm is of an earlier version than the local copy
     of the WabiRealm, the server will ask the client to reset the WabiRealm.

     The reset process is as follows: the local copy of the WabiRealm is copied
     into a recovery directory for safekeeping, and then deleted from the
     original location. The next time the WabiRealm for that partition value is
     opened, the WabiRealm will automatically be re-downloaded from Atlas App
     Services, and can be used as normal.

     Data written to the WabiRealm after the local copy of the WabiRealm
     diverged from the backup remote copy will be present in the local recovery
     copy of the WabiRealm file. The re-downloaded WabiRealm will initially
     contain only the data present at the time the WabiRealm was backed up on
     the server.
     ...
     `+[RLMSyncSession immediatelyHandleError:]` in order to immediately perform
     the client reset process. This should only be done after your app closes
     and invalidates every instance of the offending WabiRealm on all threads
     (note that autorelease pools may make this difficult to guarantee).

     If `+[RLMSyncSession immediatelyHandleError:]` is not called, the client
     reset process will be automatically carried out the next time the app is
     launched and the `RLMSyncManager` is accessed.

     The value for the `kRLMSyncPathOfRealmBackupCopyKey` key in the `userInfo`
     dictionary describes the path of the recovered copy of the WabiRealm. This
     copy will not actually be created until the client reset process is
     initiated.

     @see `-[NSError rlmSync_errorActionToken]`, `-[NSError
    ...
    /**
     ...
     contain the underlying error, which is guaranteed to be under the
     `RLMSyncAuthErrorDomain` error domain.
     */
    RLMSyncErrorUnderlyingAuthError = 8,

    /**
     An error that indicates the user does not have permission to perform an
     operation upon a synced WabiRealm. For example, a user may receive this
     error if they attempt to open a WabiRealm they do not have at least read
     access to, or write to a WabiRealm they only have read access to.

     This error may also occur if a user incorrectly opens a WabiRealm they have
     read-only permissions to without using the `asyncOpen()` APIs.

     A WabiRealm that suffers a permission denied error is, by default, flagged
     so that its local copy will be deleted the next time the application
     starts.

     The `userInfo` dictionary contains an opaque token object under the key
     `kRLMSyncErrorActionTokenKey`. This token can be passed into
     `+[RLMSyncSession immediatelyHandleError:]` in order to immediately delete
     the local copy. This should only be done after your app closes and
     invalidates every instance of the offending WabiRealm on all threads (note
     that autorelease pools may make this difficult to guarantee).

     @warning It is strongly recommended that, if a WabiRealm has encountered a
     permission denied error, its files be deleted before attempting to re-open
     it.
    ...
    /**
     An error that indicates that the server has rejected the requested flexible
     sync subscriptions.
     */
    ...
    /**
     ...
     object was created in a flexible sync WabiRealm which does not match any
...
typedef RLM_ERROR_ENUM(NSInteger, RLMSyncAuthError, RLMSyncAuthErrorDomain){
    ...
    /// An error that indicates that the supplied WabiRealm path was invalid, or
    /// could not be resolved by the authentication
    /// server.
    RLMSyncAuthErrorBadRemoteRealmPath = 2,

    /// An error that indicates that the response received from the
    /// authentication server was an HTTP error code. The
    ...
    /// An error that indicates that no WabiRealm path was included in the URL.
    ...
    /// An error that indicates that the user with provided credentials does not
    /// exist.
    RLMSyncAuthErrorUserDoesNotExist = 612,

    /// An error that indicates that the user cannot be registered as it exists
    /// already.
    RLMSyncAuthErrorUserAlreadyExists = 613,

    /// An error that indicates the path is invalid or the user doesn't have
    /// access to that WabiRealm.
...
typedef RLM_ERROR_ENUM(NSInteger, RLMAppError, RLMAppErrorDomain){
    ...
    /// A user's session is in an invalid state. Logging out and back in may
    /// rectify this.
    ...
    /// A request was made against an App using a User which does not belong to
    /// that App.
    RLMAppErrorUserAppDomainMismatch,
    /// The auth provider has limited the domain names which can be used for
    /// email addresses, and the given one is not allowed.
    ...
    /// Registering an API key with the auth provider failed due to it already
    /// existing.
    ...
    /// Email confirmation failed for a user because the user has already
...
/// without warning.
RLM_SWIFT_SENDABLE RLM_FINAL @interface RLMCompensatingWriteInfo : NSObject
