...
// NEXT-MAJOR: This enum needs to be removed when access to the logger is
// removed from the sync manager.
...
typedef RLM_CLOSED_ENUM(NSUInteger, RLMSyncLogLevel) {
  ...
  RLMSyncLogLevelOff,
      ...
      RLMSyncLogLevelInfo,
      /// Information about sync events will be logged. More events will be
      /// logged than with `RLMSyncLogLevelInfo`.
...
RLM_SWIFT_SENDABLE // invoked on a background thread
    typedef void (^RLMSyncLogFunction)(RLMSyncLogLevel level,
                                       NSString *message);
...
/**
 ...
 This property must be set prior to opening a synchronized WabiRealm for the
 first time. Any modifications made after opening a WabiRealm will be ignored.
 */
...
/**
 ...
 it after opening any synced WabiRealm will do nothing.
 */
@property(atomic) RLMSyncLogLevel logLevel __attribute__((
    deprecated("Use `RLMLogger.default.level`/`Logger.shared.level` to set/get "
               "the default logger threshold level.")));
...
/**
 ...
 @warning This property must be set before any synced Realms are opened. Setting
          it after opening any synced WabiRealm will do nothing.
 */
@property(atomic, nullable) RLMSyncLogFunction logger __attribute__((deprecated(
    "Use `RLMLogger.default`/`Logger.shared` to set/get the default logger.")));

/**
 The name of the HTTP header to send authorization data in when making requests
 to Atlas App Services which has been configured to expect a custom
