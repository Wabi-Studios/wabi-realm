...
- (void)testPathCannotBeBothInMemoryAndRegularDurability {
  ...
  config.fileURL =
      [NSURL fileURLWithPath:@(inMemoryRealm.configuration.path.c_str())];
  NSError *error; // passing in a reference to assert that this error can't be
                  // caught!
  RLMAssertThrowsWithReason(
      [RLMRealm realmWithConfiguration:config error:&error],
      ([NSString stringWithFormat:@"WabiRealm at path '%@' already opened with "
                                  @"different inMemory settings",
                                  config.fileURL.path]));
}

- (void)testRealmWithPathUsesDefaultConfiguration {
  RLMRealmConfiguration *originalDefaultConfiguration =
      [RLMRealmConfiguration defaultConfiguration];
  RLMRealmConfiguration *newDefaultConfiguration =
      [originalDefaultConfiguration copy];
  newDefaultConfiguration.objectClasses = @[];
  [RLMRealmConfiguration setDefaultConfiguration:newDefaultConfiguration];
  XCTAssertEqual([[[[RLMRealm realmWithURL:RLMTestRealmURL()] configuration]
                     objectClasses] count],
                 0U);
...
- (void)testReadOnlyFile {
  ...
  [NSFileManager.defaultManager setAttributes:@{NSFileImmutable : @YES}
                                 ofItemAtPath:RLMTestRealmURL().path
                                        error:nil];

  // Should not be able to open read-write
  RLMAssertRealmException(
      [self realmWithTestPath], RLMErrorFilePermissionDenied,
      @"Failed to open WabiRealm file at path '%@': Operation not permitted. "
      @"Please use a path where your app has read-write permissions.",
      RLMTestRealmURL().path);

  RLMRealm *realm;
  XCTAssertNoThrow(realm = [self readOnlyRealmWithURL:RLMTestRealmURL()
                                                error:nil]);
...
- (void)testReadOnlyFileInImmutableDirectory {
  ...
  // Delete '*.lock' and '.note' files to simulate opening WabiRealm in an app
  // bundle
  [[NSFileManager defaultManager]
      removeItemAtURL:[RLMTestRealmURL() URLByAppendingPathExtension:@"lock"]
                error:nil];
  [[NSFileManager defaultManager]
      removeItemAtURL:[RLMTestRealmURL() URLByAppendingPathExtension:@"note"]
                error:nil];

  // Make parent directory immutable to simulate opening WabiRealm in an app
  // bundle
  NSURL *parentDirectoryOfTestRealmURL =
      [RLMTestRealmURL() URLByDeletingLastPathComponent];
  [NSFileManager.defaultManager setAttributes:@{NSFileImmutable : @YES}
                                 ofItemAtPath:parentDirectoryOfTestRealmURL.path
                                        error:nil];

  RLMRealm *realm;
  // Read-only WabiRealm should be opened even in immutable directory
  XCTAssertNoThrow(realm = [self readOnlyRealmWithURL:RLMTestRealmURL()
                                                error:nil]);
...
- (void)testReadOnlyRealmMustExist {
  RLMAssertRealmException(
      [self readOnlyRealmWithURL:RLMTestRealmURL() error:nil],
      RLMErrorFileNotFound,
      @"Failed to open WabiRealm file at path '%@': No such file or directory",
      RLMTestRealmURL().path);
}

- (void)testCannotHaveReadOnlyAndReadWriteRealmsAtSamePathAtSameTime {
  NSString *exceptionReason =
      @"WabiRealm at path '.*' already opened with different read permissions";
  @autoreleasepool {
  ...
  @autoreleasepool {
    XCTAssertNoThrow([self readOnlyRealmWithURL:RLMTestRealmURL() error:nil]);
    RLMAssertThrowsWithReasonMatching([self realmWithTestPath],
                                      exceptionReason);
  }
...
- (void)testFilePermissionDenied {
  ...
  NSError *error;
  NSNumber *permissions = [NSFileManager.defaultManager
      attributesOfItemAtPath:RLMTestRealmURL().path
                       error:&error][NSFilePosixPermissions];
  assert(!error);
  [NSFileManager.defaultManager
      setAttributes:@{NSFilePosixPermissions : @(0000)}
       ofItemAtPath:RLMTestRealmURL().path
              error:&error];
  assert(!error);
  ...
  RLMAssertRealmException([self realmWithTestPath],
                          ...
                          @"Failed to open WabiRealm file at path '%@': "
                          @"Permission denied. Please use "
                          @"a path where your app has read-write permissions.",
                          RLMTestRealmURL().path);
  RLMAssertRealmException([self readOnlyRealmWithURL:RLMTestRealmURL()
                                               error:nil],
                          RLMErrorFilePermissionDenied,
                          @"Failed to open WabiRealm file at path '%@': "
                          @"Permission denied. Please use "
                          @"a path where your app has read permissions.",
                          RLMTestRealmURL().path);
...
#define AssertFileUnmodified(oldURL, newURL)                                   \
  do {                                                                         \
...
- (void)testFileFormatUpgradeRequiredDeleteRealmIfNeeded {
  ...
  NSURL *bundledRealmURL = [[NSBundle bundleForClass:[RealmTests class]]
      URLForResource:@"file-format-version-21"
       withExtension:@"realm"];
  [NSFileManager.defaultManager copyItemAtURL:bundledRealmURL
                                        toURL:config.fileURL
                                        error:nil];
...
- (void)testFileFormatUpgradeRequiredButDisabled {
  ...
  RLMAssertThrowsWithCodeMatching([RLMRealm realmWithConfiguration:config
                                                             error:nil],
...
- (void)testFileFormatUpgradeRequiredButReadOnly {
  ...
  NSURL *bundledRealmURL = [[NSBundle bundleForClass:[RealmTests class]]
      URLForResource:@"file-format-version-10"
       withExtension:@"realm"];
  [NSFileManager.defaultManager copyItemAtURL:bundledRealmURL
                                        toURL:config.fileURL
                                        error:nil];
  ...
  RLMAssertRealmException(
      [RLMRealm realmWithConfiguration:config error:nil],
      RLMErrorFileFormatUpgradeRequired,
      @"WabiRealm file at path '%@' cannot be opened in read-only mode because "
      @"it "
      @"has a file format version (10) which requires an upgrade",
      config.fileURL.path);
...
- (void)testUnsupportedFileFormatVersion {
  RLMRealmConfiguration *config = [RLMRealmConfiguration new];
  NSURL *bundledRealmURL = [[NSBundle bundleForClass:[RealmTests class]]
      URLForResource:@"fileformat-pre-null"
       withExtension:@"realm"];
  [NSFileManager.defaultManager copyItemAtURL:bundledRealmURL
                                        toURL:config.fileURL
                                        error:nil];
  ...
  RLMAssertThrowsWithCodeMatching([RLMRealm realmWithConfiguration:config
                                                             error:nil],
                                  RLMErrorUnsupportedFileFormatVersion);
  AssertFileUnmodified(bundledRealmURL, config.fileURL);
}
    ...
    (!TARGET_OS_SIMULATOR || !TARGET_RT_64_BIT)
- (void)testExceedingVirtualAddressSpace {
  ...
  NSString *largeString =
      [[NSString alloc] initWithBytesNoCopy:mem
                                     length:stringLength
  ...
  while (size >= stringLength) {
    ...
    kern_return_t ret = vm_allocate(mach_task_self(), &chunk.address,
                                    chunk.size, VM_FLAGS_ANYWHERE);
  ...
  for (auto chunk : allocatedChunks) {
    kern_return_t ret =
        vm_deallocate(mach_task_self(), chunk.address, chunk.size);
...
- (void)testOpenAsync {
  ...
  // Helpers
  auto assertNoCachedRealm = ^{
    XCTAssertNil(RLMGetAnyCachedRealmForPath(c.pathOnDisk.UTF8String));
  };
  auto fileExists = ^BOOL() {
    return [[NSFileManager defaultManager] fileExistsAtPath:c.pathOnDisk
                                                isDirectory:nil];
  };
  ...
  // Hold exclusive lock on lock file to prevent WabiRealm from being created
...
- (void)testRealmAddAndRemoveObjects {
  ...
  XCTAssertEqual([StringObject objectsInRealm:realm withPredicate:nil].count,
                 3U, @"Expecting 3 objects");
  [realm commitWriteTransaction];
  ...
  XCTAssertEqualObjects([objects.firstObject stringCol], @"a",
                        @"Expecting column to be 'a'");

  [realm beginWriteTransaction];
  [realm deleteObject:objects[2]];
  [realm deleteObject:objects[0]];
  XCTAssertEqual([StringObject objectsInRealm:realm withPredicate:nil].count,
                 1U, @"Expecting 1 object");
  [realm commitWriteTransaction];

  objects = [StringObject allObjectsInRealm:realm];
  XCTAssertEqual(objects.count, 1U, @"Expecting 1 object");
  XCTAssertEqualObjects([objects.firstObject stringCol], @"b",
                        @"Expecting column to be 'b'");
}
...
- (void)testRemoveUnmanagedObject {
  ...
  [self waitForNotification:RLMRealmDidChangeNotification
                      ...
                      block:^{
                        RLMRealm *realm = [self realmWithTestPath];
                        RLMObject *obj = [[StringObject allObjectsInRealm:realm]
                            firstObject];
                        [realm beginWriteTransaction];
...
- (void)testRealmBatchRemoveObjects {
  ...
  [realm beginWriteTransaction];
  [realm deleteObjects:[StringObject objectsInRealm:realm
                                              where:@"stringCol != 'a'"]];
  ...
  [realm beginWriteTransaction];
  ArrayPropertyObject *arrayObj = [ArrayPropertyObject
      createInRealm:realm
          withValue:@[ @"name", @[ @[ @"a" ], @[ @"b" ], @[ @"c" ] ], @[] ]];
  SetPropertyObject *setObj = [SetPropertyObject
      createInRealm:realm
          withValue:@[ @"name", @[ @[ @"d" ], @[ @"e" ], @[ @"f" ] ], @[] ]];
  DictionaryPropertyObject *dictObj = [DictionaryPropertyObject
      createInRealm:realm
          withValue:@{@"stringDictionary" : @{@"a" : @[ @"b" ]}}];
  ...
  // remove NSArray
  NSArray *arrayOfLastObject =
      @[ [[StringObject allObjectsInRealm:realm] lastObject] ];
  [realm beginWriteTransaction];
  ...
  // add objects to collections
  [realm beginWriteTransaction];
  [arrayObj.array addObject:[StringObject createInRealm:realm
                                              withValue:@[ @"a" ]]];
  ...
  dictObj.stringDictionary[@"b"] =
      [[StringObject alloc] initWithValue:@[ @"e" ]];
  [realm commitWriteTransaction];
...
- (void)testAddManagedObjectToOtherRealm {
  ...
  [realm1 transactionWithBlock:^{
    [realm1 addObject:co1];
  }];

  [realm2 beginWriteTransaction];
  XCTAssertThrows([realm2 addObject:co1],
                  @"should reject already-managed object");
  XCTAssertThrows([realm2 addObject:co2],
                  @"should reject linked managed object");
  XCTAssertThrows([realm2 addObject:cao],
                  @"should reject array containing managed object");
  ...
  XCTAssertNoThrow([realm1 addObject:co2],
                   @"should be able to add object which links to object "
                   @"managed by target WabiRealm");
  XCTAssertNoThrow([realm1 addObject:cao],
                   @"should be able to add object with an array containing an "
                   @"object managed by target WabiRealm");
...
- (void)testCopyObjectsInArrayLiteral {
  ...
  [realm2 beginWriteTransaction];
  CircleObject *c2 = [CircleObject createInRealm:realm2
                                       withValue:@[ @"3", @[ @"2", c ] ]];
  [realm2 commitWriteTransaction];
...
- (void)testAddOrUpdate {
  ...
  PrimaryStringObject *obj =
      [[PrimaryStringObject alloc] initWithValue:@[ @"string", @1 ]];
  [realm addOrUpdateObject:obj];
  RLMResults *objects = [PrimaryStringObject allObjects];
  XCTAssertEqual([objects count], 1U, @"Should have 1 object");
  XCTAssertEqual([(PrimaryStringObject *)objects[0] intCol], 1,
                 @"Value should be 1");

  PrimaryStringObject *obj2 =
      [[PrimaryStringObject alloc] initWithValue:@[ @"string2", @2 ]];
  [realm addOrUpdateObject:obj2];
  XCTAssertEqual([objects count], 2U, @"Should have 2 objects");

  // upsert with new secondary property
  PrimaryStringObject *obj3 =
      [[PrimaryStringObject alloc] initWithValue:@[ @"string", @3 ]];
  [realm addOrUpdateObject:obj3];
  XCTAssertEqual([objects count], 2U, @"Should have 2 objects");
  XCTAssertEqual([(PrimaryStringObject *)objects[0] intCol], 3,
                 @"Value should be 3");

  // upsert on non-primary key object should throw
  XCTAssertThrows([realm
      addOrUpdateObject:[[StringObject alloc] initWithValue:@[ @"string" ]]]);
...
- (void)testAddOrUpdateObjectsFromArray {
  ...
  PrimaryStringObject *obj =
      [[PrimaryStringObject alloc] initWithValue:@[ @"string1", @1 ]];
  [realm addObject:obj];
  ...
  PrimaryStringObject *obj3 =
      [[PrimaryStringObject alloc] initWithValue:@[ @"string3", @3 ]];
  ...
  // upsert with array of 2 objects. One is to update the existing value,
  // another is added
  NSArray *array = @[
    [[PrimaryStringObject alloc] initWithValue:@[ @"string2", @4 ]],
...
- (void)testDelete {
  ...
  [realm beginWriteTransaction];
  OwnerObject *obj = [OwnerObject
      createInDefaultRealmWithValue:@[ @"deeter", @[ @"barney", @2 ] ]];
  [realm commitWriteTransaction];
...
- (void)testDeleteObjects {
  ...
  [realm beginWriteTransaction];
  CompanyObject *obj = [CompanyObject createInDefaultRealmWithValue:@[
    @"deeter", @[ @[ @"barney", @2, @YES ] ]
  ]];
  NSArray *objects = @[ obj ];
  [realm commitWriteTransaction];
  ...
  XCTAssertThrows(
      [realm deleteObjects:[CompanyObject allObjectsInRealm:realm]]);
  ...
  [testRealm transactionWithBlock:^{
    [realm transactionWithBlock:^{
      ...
      XCTAssertThrows(
          [testRealm deleteObjects:[CompanyObject allObjectsInRealm:realm]]);
...
- (void)testAddObjectsFromArray {
  ...
  dog.dogName = @"Rex";
  dog.age = 10;
  XCTAssertNoThrow([realm addObjects:@[ dog ]],
                   @"should allow RLMObject in array");
...
- (void)testAutorefreshAfterBackgroundUpdate {
  ...
  [self waitForNotification:RLMRealmDidChangeNotification
                      ...
                      block:^{
                        RLMRealm *realm = [self realmWithTestPath];
                        [realm beginWriteTransaction];
                        [StringObject createInRealm:realm
                                          withValue:@[ @"string" ]];
                        [realm commitWriteTransaction];
                      }];
...
- (void)testBackgroundUpdateWithoutAutorefresh {
  ...
  [self waitForNotification:RLMRealmRefreshRequiredNotification
                      ...
                      block:^{
                        ...
                        XCTAssertEqual(
                            1U, [StringObject allObjectsInRealm:realm].count);
                      }];
...
- (void)testBeginWriteTransactionsNotifiesWithUpdatedObjects {
  ...
  RLMNotificationToken *token =
      [realm addNotificationBlock:^(__unused NSString *note, RLMRealm *realm) {
        XCTAssertEqual(1U, [StringObject allObjectsInRealm:realm].count);
...
- (void)testBeginWriteTransactionsRefreshesRealm {
  ...
  // Set up notification which will be triggered when calling
  ...
  RLMNotificationToken *token =
      [realm addNotificationBlock:^(__unused NSString *note, RLMRealm *realm) {
        ...
        XCTAssertThrows([realm beginWriteTransaction],
                        @"We should already be in a write transaction");
        notificationFired = true;
      }];
...
- (void)testBeginWriteTransactionFromWithinRefreshRequiredNotification {
  ...
  RLMNotificationToken *token =
      [realm addNotificationBlock:^(NSString *note, RLMRealm *realm) {
        ...
        XCTAssertEqual(1U, [StringObject allObjectsInRealm:realm].count);
        [realm cancelWriteTransaction];
        [expectation fulfill]; // note that this will throw if the notification
...
- (void)testBeginWriteTransactionFromWithinRealmChangedNotification {
  ...
  RLMNotificationToken *token =
      [realm addNotificationBlock:^(NSString *note, RLMRealm *realm) {
        ...
        // Refreshes without sending notifications since we're within a
        // notification
        [realm beginWriteTransaction];
...
- (void)testRollbackLink {
  ...
  [realm beginWriteTransaction];
  CircleObject *obj1 = [CircleObject createInRealm:realm
                                         withValue:@[ @"1", NSNull.null ]];
  CircleObject *obj2 = [CircleObject createInRealm:realm
                                         withValue:@[ @"2", NSNull.null ]];
  [realm commitWriteTransaction];
  ...
  // Modify link
  [realm beginWriteTransaction];
  CircleObject *obj4 = [CircleObject createInRealm:realm
                                         withValue:@[ @"4", NSNull.null ]];
  [realm commitWriteTransaction];
...
- (void)testRollbackTransactionWithoutExplicitCommitOrCancel {
  ...
  XCTAssertEqual(0U,
                 [IntObject allObjectsInRealm:[self realmWithTestPath]].count);
}
...
- (void)testInvalidateOnReadOnlyRealm {
  ...
  XCTAssertFalse(
      [[[IntObject allObjectsInRealm:realm] firstObject] isInvalidated]);
}
...
- (void)testThrowingFromDidChangeNotificationFromBeginWriteCancelsTransaction {
  ...
  RLMNotificationToken *token =
      [realm addNotificationBlock:^(NSString *note, RLMRealm *) {
        if (note == RLMRealmDidChangeNotification) {
          throw 0;
        }
      }];
...
- (void)testNotificationsFireEvenWithoutReadTransaction {
  ...
  XCTestExpectation *notificationFired =
      [self expectationWithDescription:@"notification fired"];
  __block RLMNotificationToken *token =
      [realm addNotificationBlock:^(NSString *note, RLMRealm *) {
        if (note == RLMRealmDidChangeNotification) {
...
- (void)testActiveVersionLimit {
  ...
  RLMValidateError(error, RLMErrorDomain, RLMErrorFail,
                   @"Number of active versions (4) in the WabiRealm exceeded "
                   @"the limit of 3");
}
...
- (void)testAsyncTransactionShouldWrite {
  ...
  XCTestExpectation *asyncComplete =
      [self expectationWithDescription:@"async transaction complete"];
  XCTAssertEqual(0U, [StringObject allObjectsInRealm:realm].count);

  [realm
      asyncTransactionWithBlock:^{
...
- (void)testAsyncTransactionShouldWriteOnCommit {
  RLMRealm *realm = RLMRealm.defaultRealm;
  XCTestExpectation *writeComplete =
      [self expectationWithDescription:@"async transaction complete"];
  XCTAssertEqual(0U, [StringObject allObjectsInRealm:realm].count);
  ...
  [self dispatchAsync:^{
    ...
    [realmBg beginAsyncWriteTransaction:^{
      ...
      [realmBg commitAsyncWriteTransaction:^(NSError *error) {
        StringObject *stringObject =
            [StringObject allObjectsInRealm:realmBg].firstObject;
        XCTAssertEqualObjects(stringObject.stringCol, @"string");
...
- (void)testAsyncTransactionShouldCancel {
  RLMRealm *realm = RLMRealm.defaultRealm;
  XCTestExpectation *waitComplete =
      [self expectationWithDescription:@"async wait complete"];

  XCTAssertEqual(0U, [StringObject allObjectsInRealm:realm].count);

  RLMAsyncTransactionId asyncTransactionId =
...
- (void)testAsyncTransactionShouldNotAutoCommitOnCanceledTransaction {
  ...
  [self dispatchAsync:^{
    RLMRealm *realm = [RLMRealm defaultRealmForQueue:self.bgQueue];
    RLMAsyncTransactionId asyncTransactionId = [realm
        asyncTransactionWithBlock:^{
          [realm createObject:StringObject.className
                    withValue:@[ @"string 1" ]];
        }
        onComplete:^(NSError *) {
          [writeComplete fulfill];
...
- (void)testAsyncTransactionShouldAutorefresh {
  ...
  __block RLMNotificationToken *token =
      [realm addNotificationBlock:^(NSString *note, RLMRealm *realm) {
        ...
        if (note ==
            RLMRealmDidChangeNotification) { // Check pointer equality to ensure
                                             // we're using the interned string
                                             // constant
          [notificationFired fulfill];
          [token invalidate];
        }
...
- (void)testAsyncTransactionSyncCommit {
  ...
  [realm beginAsyncWriteTransaction:^{
    [realm createObject:StringObject.className withValue:@[ @"string" ]];
    [realm
        commitAsyncWriteTransaction:^(NSError *) {
          [asyncComplete fulfill];
        }
                      allowGrouping:true];
  }];
...
- (void)testAsyncTransactionSyncAfterAsyncWithoutCommit {
  ...
  XCTAssertEqualObjects(
      [[StringObject allObjectsInRealm:realm].firstObject stringCol],
      @"string 2");
}
...
- (void)testAsyncTransactionChangeNotification {
  RLMRealm *realm = RLMRealm.defaultRealm;
  __block XCTestExpectation *asyncComplete =
      [self expectationWithDescription:@"async transaction complete"];
  ...
  RLMNotificationToken *token = [resultsUnderTest
      addNotificationBlock:^(RLMResults<RLMObject *> *,
                             RLMCollectionChange *_Nullable change, NSError *) {
...
- (void)testAsyncBeginTransactionInAsyncTransaction {
  RLMRealm *realm = [self realmWithTestPath];
  XCTestExpectation *transaction1 =
      [self expectationWithDescription:@"async transaction 1 complete"];
  XCTestExpectation *transaction2 =
      [self expectationWithDescription:@"async transaction 2 complete"];
...
- (void)testAsyncNestedWrites {
  XCTestExpectation *transactionsComplete =
      [self expectationWithDescription:@"async transaction 1 complete"];
...
- (void)testAsyncTransactionCancel {
  ...
  RLMAsyncTransactionId asyncTransactionIdB =
      [realm beginAsyncWriteTransaction:^{
...
- (void)testAsyncTransactionCommit {
  ...
  [realm beginAsyncWriteTransaction:^{
    [StringObject createInRealm:realm
                      withValue:@[ @"with commit should commit" ]];
    [expectation fulfill];
    [realm commitAsyncWriteTransaction];
  }];
  ...
  [realm beginAsyncWriteTransaction:^{
    [StringObject
        createInRealm:realm
            withValue:@[ @"without commit/cancel should not commit" ]];
    [expectation fulfill];
  }];

  [realm beginAsyncWriteTransaction:^{
    [StringObject createInRealm:realm
                      withValue:@[ @"cancel after commit should commit" ]];
    [expectation fulfill];
    RLMAsyncTransactionId asyncTransactionId =
        [realm commitAsyncWriteTransaction:^(NSError *_Nonnull){
            // no op
        }];
    [realm cancelAsyncTransaction:asyncTransactionId];
  }];

  RLMAsyncTransactionId asyncTransactionIdA =
      [realm beginAsyncWriteTransaction:^{
        [StringObject
            createInRealm:realm
                withValue:@[ @"commit after cancel should not commit" ]];
        [expectation fulfill];
  ...
  RLMAsyncTransactionId asyncTransactionIdB =
      [realm beginAsyncWriteTransaction:^{
        [StringObject createInRealm:realm
                          withValue:@[ @"cancel should not commit" ]];
        [expectation fulfill];
        [realm commitAsyncWriteTransaction];
      }];
  [realm cancelAsyncTransaction:asyncTransactionIdB];
...
- (void)testAsyncWriteOnQueueConfinedRealm {
  ...
  dispatch_sync(q, ^{
    qRealm = [RLMRealm defaultRealmForQueue:q];
  });
  ...
  [self waitForExpectationsWithTimeout:2.0 handler:nil];
  XCTAssertEqual(
      1U, [StringObject allObjectsInRealm:[RLMRealm defaultRealm]].count);
}

- (void)testAsyncWriteOnReadonlyRealm {
  @autoreleasepool { // ensure the WabiRealm exists
...
- (void)testAsyncCrossSync {
  ...
  auto expectation = [self expectationWithDescription:@""];
  XCTAssertEqual(
      0U, [StringObject allObjectsInRealm:[RLMRealm defaultRealm]].count);
  ...
  [self waitForExpectationsWithTimeout:2.0 handler:nil];
  XCTAssertEqual(
      2U, [StringObject allObjectsInRealm:[RLMRealm defaultRealm]].count);
}
...
- (void)testHoldRealmAfterSourceThreadIsDestroyed {
  ...
  [realm.configuration fileURL]; // ensure ARC releases the object after the
                                 // thread has finished
}
...
- (void)testBackgroundRealmIsNotified {
  ...
  XCTestExpectation *bgReady =
      [self expectationWithDescription:@"background queue waiting for commit"];
...
- (void)testAddingNotificationOutsideOfRunLoopIsAnError {
  [self dispatchAsyncAndWait:^{
    ...
    CFRunLoopPerformBlock(CFRunLoopGetCurrent(), kCFRunLoopDefaultMode, ^{
      RLMNotificationToken *token;
      XCTAssertNoThrow(
          token = [realm addNotificationBlock:^(NSString *, RLMRealm *){
          }]);
      [token invalidate];
...
- (void)testQueueBoundRealmCaching {
  ...
  RLMRealm *mainQueueRealm1 =
      [RLMRealm defaultRealmForQueue:dispatch_get_main_queue()];
  __block RLMRealm *q1Realm1;
  __block RLMRealm *q2Realm1;
  dispatch_sync(q1, ^{
    q1Realm1 = [RLMRealm defaultRealmForQueue:q1];
  });
  dispatch_sync(q2, ^{
    q2Realm1 = [RLMRealm defaultRealmForQueue:q2];
  });
  ...
  RLMRealm *mainQueueRealm2 =
      [RLMRealm defaultRealmForQueue:dispatch_get_main_queue()];
  __block RLMRealm *q1Realm2;
  __block RLMRealm *q2Realm2;
  dispatch_sync(q1, ^{
    q1Realm2 = [RLMRealm defaultRealmForQueue:q1];
  });
  dispatch_sync(q2, ^{
    q2Realm2 = [RLMRealm defaultRealmForQueue:q2];
  });
...
- (void)testQueueValidation {
  ...
  RLMAssertThrowsWithReason([RLMRealm defaultRealmForQueue:self.bgQueue],
                            @"WabiRealm opened from incorrect dispatch queue.");
  RLMAssertThrowsWithReasonMatching(
      [RLMRealm defaultRealmForQueue:dispatch_get_global_queue(0, 0)],
      @"Invalid queue '.*' \\(.*\\): Realms can only be confined to serial "
      @"queues or the main queue.");
  RLMAssertThrowsWithReason(
      [RLMRealm defaultRealmForQueue:dispatch_queue_create(
                                         "concurrent queue",
                                         DISPATCH_QUEUE_CONCURRENT)],
      @"Invalid queue 'concurrent queue' (OS_dispatch_queue_concurrent): "
      @"Realms can only be confined to serial queues or the main queue.");
...
- (void)testQueueChecking {
  ...
  dispatch_sync(q1, ^{
    q1Realm = [RLMRealm defaultRealmForQueue:q1];
  });
  dispatch_sync(q2, ^{
    q2Realm = [RLMRealm defaultRealmForQueue:q2];
  });
...
- (void)testReusingConfigOnMultipleQueues {
  ...
  dispatch_sync(q1, ^{
    XCTAssertNoThrow([RLMRealm realmWithConfiguration:config
                                                queue:q1
                                                error:nil]);
  });
  dispatch_sync(q2, ^{
...
- (void)testInMemoryRealm {
  @autoreleasepool {
    ...
    [self waitForNotification:RLMRealmDidChangeNotification
                        ...
                        block:^{
                          RLMRealm *inMemoryRealm =
                              [self inMemoryRealmWithIdentifier:@"identifier"];
                          [inMemoryRealm beginWriteTransaction];
                          [StringObject createInRealm:inMemoryRealm
                                            withValue:@[ @"a" ]];
                          [StringObject createInRealm:inMemoryRealm
                                            withValue:@[ @"b" ]];
                          [StringObject createInRealm:inMemoryRealm
                                            withValue:@[ @"c" ]];
                          XCTAssertEqual(
                              3U, [StringObject allObjectsInRealm:inMemoryRealm]
                                      .count);
    ...
    RLMRealm *anotherInMemoryRealm =
        [self inMemoryRealmWithIdentifier:@"identifier2"];
    XCTAssertEqual(0U,
                   [StringObject allObjectsInRealm:anotherInMemoryRealm].count);
  }
...
- (void)testReadOnlyRealmWithMissingTables {
  ...
  @autoreleasepool {
    RLMObjectSchema *objectSchema =
        [RLMObjectSchema schemaForObjectClass:StringObject.class];
  ...
  XCTAssertEqual(results, [results sortedResultsUsingKeyPath:@"intCol"
                                                   ascending:YES]);
...
- (void)testCannotOverwriteWithWriteCopy {
  ...
  XCTAssertFalse([realm writeCopyToURL:RLMTestRealmURL()
                         encryptionKey:nil
                                 error:nil]);
  ...
  writeError = nil;
  XCTAssertFalse([realm writeCopyForConfiguration:configuration
                                            error:&writeError]);
  RLMValidateRealmError(writeError, RLMErrorFileExists, expectedError,
                        RLMTestRealmURL().path);
}
...
- (void)testCannotWriteInNonExistentDirectory {
  ...
  NSString *expectedError =
      @"Failed to open file at path '%@': parent directory does not exist";
  NSError *writeError;
  XCTAssertFalse([realm writeCopyToURL:[NSURL fileURLWithPath:badPath]
                         encryptionKey:nil
                                 error:&writeError]);
  RLMValidateRealmError(writeError, RLMErrorFileNotFound, expectedError,
                        badPath);
...
- (void)testWriteToReadOnlyDirectory {
  ...
  NSNumber *oldPermissions =
      [fm attributesOfItemAtPath:directory error:nil][NSFilePosixPermissions];
  [fm setAttributes:@{NSFilePosixPermissions : @(0100)}
  ...
  RLMValidateRealmError(writeError, RLMErrorFilePermissionDenied,
                        @"Failed to open file at path '%@': Permission denied",
                        RLMTestRealmURL().path);
  ...
  [fm setAttributes:@{NSFilePosixPermissions : oldPermissions}
       ofItemAtPath:directory
              error:nil];
}
...
- (void)testWriteWithNonSpecialCasedError {
  ...
  XCTAssertFalse([realm writeCopyToURL:RLMTestRealmURL()
                         encryptionKey:nil
                                 error:&writeError]);
  RLMValidateRealmError(
      writeError, RLMErrorFileOperationFailed,
      @"Failed to open file at path '%@': Too many open files",
      RLMTestRealmURL().path);
...
- (void)testForbiddenMethodsOnFrozenRealm {
  ...
  RLMAssertThrowsWithReason(
      [realm beginWriteTransaction],
      @"Can't perform transactions on a frozen WabiRealm");
  RLMAssertThrowsWithReason(
      [realm addNotificationBlock:^(RLMNotification, RLMRealm *){
      }],
      @"Frozen Realms do not change and do not have change notifications.");
  RLMAssertThrowsWithReason(
      ([[IntObject allObjectsInRealm:realm]
...
- (void)testFrozenRealmCaching {
  ...
  XCTAssertEqual(
      fr1, fr2); // note: pointer equality as it should return the same instance
...
- (void)testReadAfterInvalidateFrozen {
  ...
  RLMAssertThrowsWithReason(
      [IntObject allObjectsInRealm:realm],
      @"Cannot read from a frozen WabiRealm which has been invalidated.");
}
...
- (void)testThaw {
  ...
  RLMRealm *frozenRealm = [realm freeze];
  XCTAssertTrue(frozenRealm.frozen);
  IntObject *frozenObj = [[IntObject objectsInRealm:frozenRealm
                                              where:@"intCol = 1"] firstObject];
  XCTAssertTrue(frozenObj.frozen);

  RLMRealm *thawedRealm = [realm thaw];
  XCTAssertFalse(thawedRealm.frozen);
  IntObject *thawedObj = [[IntObject objectsInRealm:thawedRealm
                                              where:@"intCol = 1"] firstObject];
  ...
  IntObject *nilObj = [[IntObject objectsInRealm:thawedRealm
                                           where:@"intCol = 1"] firstObject];
  XCTAssertNil(nilObj);
}
...
- (void)testThawDifferentThread {
  ...
  // Thaw on a thread which already has a WabiRealm should use existing
  // reference.
  [self dispatchAsyncAndWait:^{
    RLMRealm *realm = [RLMRealm defaultRealm];
...
- (void)testThawPreviousVersion {
  ...
  XCTAssertNotEqual(frozenRealm.isEmpty, [[RLMRealm defaultRealm] isEmpty],
                    "Contents of frozen WabiRealm should not mutate");

  RLMRealm *thawed = [frozenRealm thaw];
  XCTAssertFalse(thawed.isFrozen);
  XCTAssertEqual(thawed.isEmpty, [[RLMRealm defaultRealm] isEmpty],
                 "Thawed realm should reflect transactions since the original "
...
- (void)testIsEmpty {
  RLMRealm *realm = [RLMRealm defaultRealm];
  XCTAssertTrue(realm.isEmpty, @"WabiRealm should be empty on creation.");

  [realm beginWriteTransaction];
  [StringObject createInRealm:realm withValue:@[ @"a" ]];
  XCTAssertFalse(realm.isEmpty, @"WabiRealm should not be empty within a write "
                                @"transaction after adding an object.");
  [realm cancelWriteTransaction];

  XCTAssertTrue(realm.isEmpty,
                @"WabiRealm should be empty after canceling a write "
                @"transaction that added an object.");

  [realm beginWriteTransaction];
  [StringObject createInRealm:realm withValue:@[ @"a" ]];
  [realm commitWriteTransaction];
  XCTAssertFalse(realm.isEmpty,
                 @"WabiRealm should not be empty after committing a "
                 @"write transaction that added an object.");
}

- (void)testRealmFileAccessNilPath {
  RLMAssertThrowsWithReasonMatching([RLMRealm realmWithURL:self.nonLiteralNil],
                                    @"WabiRealm path must not be empty",
                                    @"nil path");
}

- (void)testRealmFileAccessNoExistingFile {
  NSURL *fileURL =
      [NSURL fileURLWithPath:RLMRealmPathForFile(@"filename.realm")];
...
- (void)testRealmFileAccessInvalidFile {
  ...
  RLMValidateRealmError(
      error, RLMErrorInvalidDatabase,
      @"Failed to open WabiRealm file at path '%@': file is non-empty but too "
      @"small (12 bytes) to be a valid WabiRealm.",
      fileURL.path);
}
...
- (void)testRealmFileAccessFileIsDirectory {
  ...
  XCTAssertNil([RLMRealm realmWithConfiguration:configuration error:&error]);
  RLMValidateRealmError(
      error, RLMErrorFileOperationFailed,
      @"Failed to open WabiRealm file at path '%@': Is a directory",
      testURL.path);
}
...
- (void)testRealmFifoError {
  ...
  // Note that creating a file when a directory with the same name exists
  // produces a different errno, which is good.
  NSURL *fifoURL = [[testURL URLByDeletingPathExtension]
      URLByAppendingPathExtension:@"realm.note"];
  assert(![manager fileExistsAtPath:fifoURL.path]);
  [manager createDirectoryAtPath:fifoURL.path
  ...
  RLMValidateRealmError(error, RLMErrorFileExists,
                        @"Cannot create fifo at path '%@': a non-fifo entry "
...
- (void)testMultipleRealms {
  ...
  XCTAssertEqualObjects([defaultObjects.firstObject stringCol], @"a",
                        @"Expecting column to be 'a'");
  XCTAssertEqualObjects([testObjects.firstObject stringCol], @"b",
                        @"Expecting column to be 'b'");
}

// iOS uses a different locking scheme which breaks how we stop core from
// reinitializing the lock file
...
- (void)testInvalidLockFile {
  ...
  RLMRealm *realm = [RLMRealm
      realmWithConfiguration:RLMRealmConfiguration.defaultConfiguration
                       error:&error];
  XCTAssertNil(realm);
  RLMValidateRealmError(
      error, RLMErrorIncompatibleLockFile,
      @"WabiRealm file '%@' is currently open in another process which cannot "
      @"share access with this process. This could either be due to the "
      @"existing process being a different architecture or due to the existing "
      @"process using an incompatible version of WabiRealm. If the other "
      @"process "
      @"is WabiRealm Studio, you may need to update it (or update WabiRealm if "
      @"your "
      @"Studio version is too new), and if using an iOS simulator, make sure "
      @"that you are using a 64-bit simulator. Underlying problem: "
      @"Architecture mismatch: Mutex size is 255 but should be 1.",
      path);
...
- (void)testCannotMigrateRealmWhenRealmIsOpen {
  ...
  XCTAssertThrows([RLMRealm performMigrationForConfiguration:configuration
                                                       error:nil]);
}

- (void)testNotificationPipeBufferOverfull {
  RLMRealm *realm = [self inMemoryRealmWithIdentifier:@"test"];
  // pipes have a 8 KB buffer on OS X, so verify we don't block after 8192
  // commits
...
- (void)testCanCreate100RealmsWithoutBreakingGCD {
  ...
  for (NSString *realmPath in self.pathsFor100Realms) {
    [realms
        addObject:[RLMRealm realmWithURL:[NSURL fileURLWithPath:realmPath]]];
  }

  XCTestExpectation *expectation =
      [self expectationWithDescription:
                @"Block dispatched to concurrent queue should be executed"];
...
- (void)testThreadIDReuse {
  ...
  // retaining each WabiRealm opened. This verifies that we don't get a
  // WabiRealm from an old thread that no longer exists from the cache.
  NSMutableArray *realms = [NSMutableArray array];
...
- (void)testAuxiliaryFilesAreExcludedFromBackup {
...
#if TARGET_OS_TV
  NSArray *auxiliaryFileExtensions =
      @[ @"management", @"lock" ]; // tvOS does not support named pipes
#else
  ...
  for (NSString *pathExtension in auxiliaryFileExtensions) {
    ...
    NSError *error = nil;
    BOOL success = [[fileURL URLByAppendingPathExtension:pathExtension]
        getResourceValue:&attribute
...
- (void)testDeleteNonexistentRealmFile {
  NSError *error;
  XCTAssertFalse([RLMRealm
      deleteFilesForConfiguration:RLMRealmConfiguration.defaultConfiguration
                            error:&error]);
  XCTAssertNil(error);
}
...
- (void)testDeleteClosedRealmFile {
  ...
  NSFileManager *fm = NSFileManager.defaultManager;
  XCTAssertTrue(
      [fm fileExistsAtPath:[config.fileURL.path
                               stringByAppendingPathExtension:@"lock"]]);
  XCTAssertFalse(
      [fm fileExistsAtPath:[config.fileURL.path
                               stringByAppendingPathExtension:@"management"]]);
#if !TARGET_OS_TV
  XCTAssertFalse(
      [fm fileExistsAtPath:[config.fileURL.path
                               stringByAppendingPathExtension:@"note"]]);
#endif
}

- (void)testDeleteRealmFileWithMissingManagementFiles {
  RLMRealmConfiguration *config = [RLMRealmConfiguration defaultConfiguration];
  [NSFileManager.defaultManager createFileAtPath:config.fileURL.path
                                        contents:nil
                                      attributes:nil];
...
- (void)testDeleteRealmFileWithReadOnlyManagementFiles {
  ...
  NSString *notificationPipe =
      [config.fileURL.path stringByAppendingPathExtension:@"note"];
  [fm createFileAtPath:notificationPipe
              contents:nil
  ...
  [fm setAttributes:@{NSFileImmutable : @NO}
       ofItemAtPath:notificationPipe
              error:nil];
}
...
- (void)testDeleteOpenRealmFile {
  ...
  XCTAssertTrue(
      [NSFileManager.defaultManager fileExistsAtPath:config.fileURL.path]);
}
@end
...
- (void)testSetDefaultLogLevel {
  ...
  RLMLogger *logger = [[RLMLogger alloc]
      initWithLevel:RLMLogLevelAll
        logFunction:^(RLMLogLevel level, NSString *message) {
          NSString *newLogs =
              [logs stringByAppendingFormat:@" %@ %lu %@", [NSDate date], level,
                                            message];
          logs = newLogs;
        }];
...
- (void)testDefaultLogger {
  ...
  logs = @"";
  // Init Custom Logger
  RLMLogger.defaultLogger = [[RLMLogger alloc]
      initWithLevel:RLMLogLevelDebug
        logFunction:^(RLMLogLevel level, NSString *message) {
          NSString *newLogs =
              [logs stringByAppendingFormat:@" %@ %lu %@", [NSDate date], level,
                                            message];
...
- (void)testCustomLoggerLogMessage {
  ...
  [logger logWithLevel:RLMLogLevelInfo
               message:@"%@ IMPORTANT INFO %i", @"TEST:", 0];
