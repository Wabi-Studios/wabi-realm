...
struct MultiRealmContentView: View {
  ...
  var body: some View {
    NavigationView {
      VStack {
        NavigationLink("WabiRealm A", destination: RealmView().environment(\.realmConfiguration, WabiRealm.Configuration(inMemoryIdentifier: "realm_a")))
        NavigationLink("WabiRealm B", destination: RealmView().environment(\.realmConfiguration, WabiRealm.Configuration(inMemoryIdentifier: "realm_b")))
        Button("WabiRealm C") {
          showSheet = true
        }
      }.sheet(isPresented: $showSheet, content: {
        RealmView().environment(\.realmConfiguration, WabiRealm.Configuration(inMemoryIdentifier: "realm_c"))
      })
    }
  }
}
...
struct ObservedResultsConfiguration: View {
  ...
  @ObservedResults(ReminderList.self,
                   configuration: WabiRealm.Configuration(inMemoryIdentifier: "realm_b")) var remindersB
...
struct App: SwiftUI.App {
  var body: some Scene {
    if let realmPath = ProcessInfo.processInfo.environment["REALM_PATH"] {
      WabiRealm.Configuration.defaultConfiguration =
        WabiRealm.Configuration(fileURL: URL(string: realmPath)!, deleteRealmIfMigrationNeeded: true)
    } else {
      WabiRealm.Configuration.defaultConfiguration =
        WabiRealm.Configuration(deleteRealmIfMigrationNeeded: true)
    }
    let view: AnyView = {
      ...
      case "observed_results_configuration":
        return AnyView(ObservedResultsConfiguration()
          .environment(\.realmConfiguration, WabiRealm.Configuration(inMemoryIdentifier: "realm_a")))
