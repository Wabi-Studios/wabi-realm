...
- (void)testCompact {
  RLMRealm *realm = self.realmWithTestPath;
  unsigned long long fileSizeBefore =
      [self fileSize:realm.configuration.fileURL];
  ...
  XCTAssertEqualObjects(
      @"A", [[StringObject allObjectsInRealm:realm].firstObject stringCol]);
  XCTAssertEqualObjects(
      @"B", [[StringObject allObjectsInRealm:realm].lastObject stringCol]);

  unsigned long long fileSizeAfter =
      [self fileSize:realm.configuration.fileURL];
  XCTAssertGreaterThan(fileSizeBefore, fileSizeAfter);
}

- (void)testSuccessfulCompactOnLaunch {
  // Configure the WabiRealm to compact on launch
  ...
  configuration.shouldCompactOnLaunch =
      ^BOOL(NSUInteger totalBytes, NSUInteger usedBytes) {
        ...
        XCTAssertTrue((usedBytes < totalBytes) &&
                      (usedBytes > expectedUsedBytesBeforeMin));
        return true;
      };

  // Confirm expected sizes before and after opening the WabiRealm
  XCTAssertEqual([self fileSize:configuration.fileURL],
                 _expectedTotalBytesBefore);
  RLMRealm *realm = [RLMRealm realmWithConfiguration:configuration error:nil];
  XCTAssertLessThan([self fileSize:configuration.fileURL],
                    _expectedTotalBytesBefore);
...
- (void)testCachedRealmCompactOnLaunch {
  ...
  // Access WabiRealm before opening it with a compaction block
  RLMRealmConfiguration *configuration =
      [RLMRealmConfiguration defaultConfiguration];
  configuration.fileURL = RLMTestRealmURL();
  __unused RLMRealm *firstRealm = [RLMRealm realmWithConfiguration:configuration
                                                             error:nil];

  // Configure the WabiRealm to compact on launch
  RLMRealmConfiguration *configurationWithCompactBlock = [configuration copy];
  __block BOOL compactBlockInvoked = NO;
  configurationWithCompactBlock.shouldCompactOnLaunch =
      ^BOOL(__unused NSUInteger totalBytes, __unused NSUInteger usedBytes) {
  ...
  XCTAssertEqual([self fileSize:configuration.fileURL],
                 _expectedTotalBytesBefore);
  RLMRealm *realm =
      [RLMRealm realmWithConfiguration:configurationWithCompactBlock error:nil];
...
- (void)testCachedRealmOtherThreadCompactOnLaunch {
  // Test that the compaction block never gets called if the WabiRealm is open
  // on a different thread Access WabiRealm before opening it with a compaction
  // block
  RLMRealmConfiguration *configuration =
      [RLMRealmConfiguration defaultConfiguration];
  configuration.fileURL = RLMTestRealmURL();

  dispatch_semaphore_t failedCompactTestCompleteSema =
      dispatch_semaphore_create(0);
  dispatch_semaphore_t bgRealmClosedSema = dispatch_semaphore_create(0);

  XCTestExpectation *realmOpenedExpectation = [self
      expectationWithDescription:@"WabiRealm was opened on background thread"];
  [self dispatchAsync:^{
    @autoreleasepool {
      __unused RLMRealm *firstRealm =
          [RLMRealm realmWithConfiguration:configuration error:nil];
      [realmOpenedExpectation fulfill];
      dispatch_semaphore_wait(failedCompactTestCompleteSema,
  ...
  @autoreleasepool {
    ...
    __unused RLMRealm *realm =
        [RLMRealm realmWithConfiguration:configurationWithCompactBlock
                                   error:nil];
    XCTAssertFalse(compactBlockInvoked);
    XCTAssertEqual([self fileSize:configuration.fileURL],
                   _expectedTotalBytesBefore);
...
- (void)testCompactOnLaunchValidation {
  ...
  BOOL (^compactBlock)(NSUInteger, NSUInteger) =
      ^BOOL(__unused NSUInteger totalBytes, __unused NSUInteger usedBytes) {
        return NO;
      };
...
- (void)testAccessDeniedOnTemporaryFile {
  ...
  configuration.fileURL = RLMTestRealmURL();
  configuration.shouldCompactOnLaunch =
      ^(__unused NSUInteger totalBytes, __unused NSUInteger usedBytes) {
        return YES;
      };
  NSURL *tmpURL = [configuration.fileURL
      URLByAppendingPathExtension:@"tmp_compaction_space"];
