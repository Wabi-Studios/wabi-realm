...
- (void)testCreateManagedObjectManagedChild {
  ...
  XCTAssertTrue(
      [((StringObject *)mo0.anyCol).stringCol isEqualToString:so.stringCol]);
  XCTAssertEqual(mo0.anyCol.rlm_valueType, RLMPropertyTypeObject);
  XCTAssertTrue([((StringObject *)mo0.anyArray.firstObject).stringCol
      isEqualToString:so.stringCol]);

  XCTAssertNotNil(mo1.anyCol);
  XCTAssertTrue(
      [((StringObject *)mo1.anyCol).stringCol isEqualToString:so.stringCol]);
  XCTAssertEqual(mo1.anyCol.rlm_valueType, RLMPropertyTypeObject);
  XCTAssertTrue([((StringObject *)mo1.anyArray.firstObject).stringCol
      isEqualToString:so.stringCol]);
}
...
- (void)testCreateManagedObjectUnmanagedChild {
  ...
  XCTAssertTrue(
      [((StringObject *)mo0.anyCol).stringCol isEqualToString:so.stringCol]);
  XCTAssertEqual(mo0.anyCol.rlm_valueType, RLMPropertyTypeObject);
  XCTAssertTrue([((StringObject *)mo0.anyArray[0]).stringCol
      isEqualToString:so.stringCol]);

  XCTAssertNotNil(mo1.anyCol);
  XCTAssertTrue(
      [((StringObject *)mo1.anyCol).stringCol isEqualToString:so1.stringCol]);
  XCTAssertEqual(mo1.anyCol.rlm_valueType, RLMPropertyTypeObject);
  XCTAssertTrue([((StringObject *)mo1.anyArray[0]).stringCol
      isEqualToString:so.stringCol]);
}
...
- (void)testCreateManagedObject {
  ...
  [r commitWriteTransaction];
  XCTAssertTrue(
      [((StringObject *)mo.anyCol).stringCol isEqualToString:so.stringCol]);
...
- (void)testCreateManagedInt {
  ...
  [r beginWriteTransaction];
  MixedObject *mo =
      [MixedObject createInRealm:r
                       withValue:@[ @123456789, @[ @123456, @67890 ] ]];
  [r commitWriteTransaction];
...
- (void)testCreateManagedFloat {
  ...
  MixedObject *mo =
      [MixedObject createInRealm:r
                       withValue:@[ @1234.5f, @[ @12345.6f, @678.9f ] ]];
  [r commitWriteTransaction];
  XCTAssertTrue([(NSNumber *)mo.anyCol isEqualToNumber:@1234.5f]);
  XCTAssertTrue(
      [mo.anyArray[0] isEqualToNumber:[NSNumber numberWithFloat:12345.6f]]);
  XCTAssertTrue(
      [mo.anyArray[1] isEqualToNumber:[NSNumber numberWithFloat:678.9f]]);
...
- (void)testCreateManagedDouble {
  ...
  MixedObject *mo =
      [MixedObject createInRealm:r
                       withValue:@[ @1234.5, @[ @12345.6, @678.9 ] ]];
  [r commitWriteTransaction];
  XCTAssertTrue([(NSNumber *)mo.anyCol isEqualToNumber:@1234.5]);
  XCTAssertTrue(
      [mo.anyArray[0] isEqualToNumber:[NSNumber numberWithDouble:12345.6]]);
  XCTAssertTrue(
      [mo.anyArray[1] isEqualToNumber:[NSNumber numberWithDouble:678.9]]);
...
- (void)testCreateManagedData {
  ...
  [r beginWriteTransaction];
  MixedObject *mo = [MixedObject createInRealm:r
                                     withValue:@[ d1, @[ d1, d2 ] ]];
  [r commitWriteTransaction];
...
- (void)testCreateManagedDate {
  ...
  XCTAssertEqualWithAccuracy(d1.timeIntervalSince1970,
                             ((NSDate *)mo.anyCol).timeIntervalSince1970, 1);
  XCTAssertEqualWithAccuracy(d1.timeIntervalSince1970,
                             ((NSDate *)mo.anyArray[0]).timeIntervalSince1970,
                             1);
  XCTAssertEqualWithAccuracy(d2.timeIntervalSince1970,
                             ((NSDate *)mo.anyArray[1]).timeIntervalSince1970,
                             1);
...
- (void)testAddManagedDate {
  ...
  // handle lossy margin of error.
  XCTAssertEqualWithAccuracy(d1.timeIntervalSince1970,
                             ((NSDate *)mo.anyCol).timeIntervalSince1970, 1.0);
  XCTAssertEqualWithAccuracy(d1.timeIntervalSince1970,
                             ((NSDate *)mo.anyArray[0]).timeIntervalSince1970,
                             1.0);
  XCTAssertEqualWithAccuracy(d2.timeIntervalSince1970,
...
// Tests that the `RLMSchemaInfo::clone` and `RLMSchemaInfo::operator[]`
// correctly skip class names that are not present in the source schema. If such
// an event were to occur an OOB exception would be thrown.
