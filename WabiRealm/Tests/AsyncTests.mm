...
- (void)verifyNotificationsAreSupported:(__unused bool)isCollection {
  // The normal implementation of this will reject realms with automatic change
...
- (void)testInitialResultsAreDelivered {
  ...
  auto token = [[IntObject objectsWhere:@"intCol > 0"]
      addNotificationBlock:^(RLMResults *results, RLMCollectionChange *change,
                             NSError *e) {
        XCTAssertNil(e);
...
- (void)testResultsPerserveSort {
  ...
  auto token = [[IntObject.allObjects sortedResultsUsingKeyPath:@"intCol"
                                                      ascending:NO]
      addNotificationBlock:^(RLMResults *results, RLMCollectionChange *change,
                             NSError *e) {
...
- (void)testQueryingDeliveredTableResults {
  __block XCTestExpectation *expectation =
      [self expectationWithDescription:@""];
  __block NSUInteger expected = 0;
  auto token = [[IntObject allObjects]
      addNotificationBlock:^(RLMResults *results, RLMCollectionChange *change,
                             NSError *e) {
        XCTAssertEqual([results objectsWhere:@"intCol < 10"].count, expected++);
        [expectation fulfill];
...
- (void)testQueryingDeliveredSortedResults {
  ...
  auto token = [[IntObject.allObjects sortedResultsUsingKeyPath:@"intCol"
      ...
      addNotificationBlock:^(RLMResults *results, RLMCollectionChange *change,
                             NSError *e) {
        XCTAssertEqual(
            [[results objectsWhere:@"intCol < 10"].firstObject intCol],
            expected++);
        [expectation fulfill];
      }];
  [self waitForExpectationsWithTimeout:2.0 handler:nil];
...
- (void)testSortingDeliveredResults {
  ...
  auto token = [[IntObject allObjects]
      addNotificationBlock:^(RLMResults *results, RLMCollectionChange *change,
                             NSError *e) {
        XCTAssertEqual(
            [[results sortedResultsUsingKeyPath:@"intCol"
                                      ascending:NO].firstObject intCol],
            expected++);
        [expectation fulfill];
      }];
  [self waitForExpectationsWithTimeout:2.0 handler:nil];
...
- (void)testQueryingLinkList {
  ...
  __block XCTestExpectation *expectation =
      [self expectationWithDescription:@""];
  __block int expected = 0;
  auto token = [[array.intArray objectsWhere:@"intCol > 0"]
      addNotificationBlock:^(RLMResults<IntObject *> *results,
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    [self dispatchAsyncAndWait:^{
      RLMRealm *realm = [RLMRealm defaultRealm];
      ArrayPropertyObject *array =
          [[ArrayPropertyObject allObjectsInRealm:realm] firstObject];
      ...
      [array.intArray addObject:[IntObject createInRealm:realm
                                               withValue:@[ @(i + 1) ]]];
      [realm commitWriteTransaction];
    }];
    [self waitForExpectationsWithTimeout:2.0 handler:nil];
...
- (RLMNotificationToken *)subscribeAndWaitForInitial:(id<RLMCollection>)query
                                               block:(void (^)(id))block {
  __block XCTestExpectation *exp =
      [self expectationWithDescription:@"wait for initial results"];
  auto token = [query addNotificationBlock:^(
                          id results, RLMCollectionChange *change, NSError *e) {
    XCTAssertNotNil(results);
    XCTAssertNil(e);
    if (exp) {
      [exp fulfill];
...
- (void)testManualRefreshUsesAsyncResultsWhenPossible {
  ...
  auto token = [self subscribeAndWaitForInitial:IntObject.allObjects
                                          block:^(RLMResults *results) {
                                            called = true;
                                          }];
...
- (void)testCommitInOneNotificationDoesNotCancelOtherNotifications {
  ...
  auto token =
      [self subscribeAndWaitForInitial:IntObject.allObjects
                                 block:^(RLMResults *results) {
                                   ++firstBlockCalls;
                                   if (firstBlockCalls == 2) {
                                     ...
                                     [IntObject
                                         createInDefaultRealmWithValue:@[ @1 ]];
                                     [results.realm commitWriteTransaction];
                                   }
                                 }];
  auto token2 = [self subscribeAndWaitForInitial:IntObject.allObjects
                                           block:^(RLMResults *results) {
                                             ++secondBlockCalls;
...
- (void)testAddAndRemoveQueries {
  ...
  @autoreleasepool {
    ...
    [[self subscribeAndWaitForInitial:results
                                block:^(RLMResults *r) {
    ...
    [[self subscribeAndWaitForInitial:[IntObject allObjectsInRealm:realm]
                                block:^(RLMResults *r) {
                                  XCTFail(@"results delivered after removal");
                                }] invalidate];
  }
...
- (void)testMultipleSourceVersionsForAsyncQueries {
  ...
  RLMRealm *realms[10];
  for (int i = 0; i < 10; ++i) {
    RLMRealm *realm = realms[i] = [RLMRealm realmWithConfiguration:config
                                                             error:nil];
    [realm transactionWithBlock:^{
  ...
  // Each WabiRealm should see a different number of objects as they're on
  // different versions
...
- (void)testMultipleSourceVersionsWithNotifiersRemovedBeforeRunning {
  ...
  RLMRealm *realms[10];
  for (int i = 0; i < 10; ++i) {
    RLMRealm *realm = realms[i] =
        [ManualRefreshRealm realmWithConfiguration:config error:nil];
    [realm transactionWithBlock:^{
      [IntObject createInRealm:realm withValue:@[ @(i) ]];
    }];
  }
...
- (void)testMultipleCallbacksForOneQuery {
  ...
  auto token1 = [self subscribeAndWaitForInitial:results
                                           block:^(RLMResults *results) {
                                             ++calls1;
                                           }];
  ...
  auto token2 = [self subscribeAndWaitForInitial:results
                                           block:^(RLMResults *results) {
                                             ++calls2;
                                           }];
  XCTAssertEqual(calls1, 0);
...
- (void)testRemovingBlockFromWithinNotificationBlock {
  ...
  token1 = [self subscribeAndWaitForInitial:results
                                      block:^(RLMResults *results) {
                                        [token1 invalidate];
                                        ++calls;
                                      }];
  token2 = [self subscribeAndWaitForInitial:results
                                      block:^(RLMResults *results) {
                                        [token2 invalidate];
                                        ++calls;
                                      }];
...
- (void)testAddingBlockFromWithinNotificationBlock {
  ...
  // Triggers one call on each block. Nested call is deferred until next
  // refresh.
  [self waitForNotification:RLMRealmDidChangeNotification
                      realm:results.realm
                      block:^{
...
- (void)testAddingNewQueryWithinNotificationBlock {
  ...
  __block RLMNotificationToken *token1, *token2;
  token1 = [self
      subscribeAndWaitForInitial:results1
                           block:^(RLMResults *results) {
                             ++calls;
                             if (calls == 1) {
                               CFRunLoopStop(CFRunLoopGetCurrent());
                               token2 =
                                   [results2 addNotificationBlock:^(
                                                 RLMResults *results,
                                                 RLMCollectionChange *change,
                                                 NSError *error) {
                                     CFRunLoopStop(CFRunLoopGetCurrent());
  ...
  // Triggers one call on outer block, but inner does not get a chance to
  // deliver
  [self dispatchAsync:^{
    [self createObject:0];
  }];
...
- (void)testAddingNewQueryWithinRealmNotificationBlock {
  ...
  __block XCTestExpectation *exp;
  auto realmToken = [RLMRealm.defaultRealm
      addNotificationBlock:^(RLMNotification notification, RLMRealm *realm) {
        CFRunLoopStop(CFRunLoopGetCurrent());
        exp = [self expectationWithDescription:@"query notification"];
        queryToken = [IntObject.allObjects
  ...
  // Make a background commit to trigger a WabiRealm notification
  [self dispatchAsync:^{
    [RLMRealm.defaultRealm transactionWithBlock:^{
    }];
  }];
...
- (void)testTransactionsAfterDeletingArrayLinkView {
  ...
  ArrayPropertyObject *apo =
      [ArrayPropertyObject createInRealm:realm
                               withValue:@[ @"", @[], @[ io ] ]];
  [realm commitWriteTransaction];

  RLMNotificationToken *token1 =
      [self subscribeAndWaitForInitial:apo.intArray
                                 block:^(RLMArray *array) {
                                   XCTAssertTrue(array.invalidated);
                                 }];
  RLMResults *asResults = [apo.intArray objectsWhere:@"intCol = 5"];
  RLMNotificationToken *token2 =
      [self subscribeAndWaitForInitial:asResults
                                 block:^(RLMResults *results) {
  ...
  [self waitForNotification:RLMRealmDidChangeNotification
                      ...
                      block:^{
                        RLMRealm *realm = [RLMRealm defaultRealm];
                        [realm transactionWithBlock:^{
                          [realm deleteObject:[ArrayPropertyObject
...
- (void)testTransactionsAfterDeletingSetLinkView {
  ...
  [realm beginWriteTransaction];
  IntObject *io = [IntObject createInRealm:realm withValue:@[ @5 ]];
  SetPropertyObject *spo =
      [SetPropertyObject createInRealm:realm withValue:@[ @"", @[], @[ io ] ]];
  [realm commitWriteTransaction];

  RLMNotificationToken *token1 =
      [self subscribeAndWaitForInitial:spo.intSet
                                 block:^(RLMSet *set) {
  ...
  [self waitForNotification:RLMRealmDidChangeNotification
                      ...
                      block:^{
                        RLMRealm *realm = [RLMRealm defaultRealm];
                        [realm transactionWithBlock:^{
                          [realm deleteObject:[SetPropertyObject
                                                  allObjectsInRealm:realm]
                                                  .firstObject];
                        }];
                      }];
...
- (void)testInitialResultDiscardsChanges {
  auto token = [IntObject.allObjects
      addNotificationBlock:^(RLMResults *results, RLMCollectionChange *changes,
                             NSError *) {
        XCTAssertEqual(results.count, 1U);
  ...
  [self dispatchAsync:^{
    CFRunLoopPerformBlock(CFRunLoopGetCurrent(), kCFRunLoopDefaultMode, ^{
      auto token = [IntObject.allObjects
          addNotificationBlock:^(RLMResults *results,
                                 RLMCollectionChange *changes, NSError *) {
            if (changes) {
...
- (void)testNotificationDeliveryToQueue {
  ...
  [self dispatchAsync:^{
    ...
    token = [[IntObject allObjectsInRealm:bgRealm]
        addNotificationBlock:^(RLMResults *results, RLMCollectionChange *,
                               NSError *) {
          XCTAssertNotNil(results);
