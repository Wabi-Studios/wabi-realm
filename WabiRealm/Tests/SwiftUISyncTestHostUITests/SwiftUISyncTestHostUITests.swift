...
class SwiftUISyncTestHostUITests: SwiftSyncTestCase {
  ...
  private func openRealm(configuration: WabiRealm.Configuration, for _: User) throws -> WabiRealm {
...
extension SwiftUISyncTestHostUITests {
  ...
  func testObservedResults() throws {
    ...
    let realm = try WabiRealm(configuration: config1)
    try realm.write {
    ...
    let realm2 = try WabiRealm(configuration: config2)
  ...
  func testAsyncOpenWithDeferRealmConfiguration() throws {
    ...
    // Synced WabiRealm and Sync Metadata
...
extension SwiftUISyncTestHostUITests {
  private func populateFlexibleSyncForEmail(_ email: String, n _: Int, _ block: @escaping (WabiRealm) -> Void) throws {
