...
- (void)testDescription {
  ...
  XCTAssertEqualObjects(property.description,
                        @"objectCol {\n"
                        @"\ttype = object;\n"
...
static RLMProperty *makeProperty(NSString *name, RLMPropertyType type,
                                 NSString *objectClassName, BOOL optional) {
...
- (void)testEqualityFromObjectSchema {
  { // Test non-optional property types
    ...
    NSDictionary *expectedProperties = @{
      ...
      @"objectIdCol" :
          makeProperty(@"objectIdCol", RLMPropertyTypeObjectId, nil, NO),
      @"decimalCol" :
          makeProperty(@"decimalCol", RLMPropertyTypeDecimal128, nil, NO),
      @"objectCol" : makeProperty(@"objectCol", RLMPropertyTypeObject,
                                  @"StringObject", YES),
      @"uuidCol" : makeProperty(@"uuidCol", RLMPropertyTypeUUID, nil, NO),
      @"anyCol" : makeProperty(@"anyCol", RLMPropertyTypeAny, nil, NO),
      @"mixedObjectCol" : makeProperty(@"mixedObjectCol", RLMPropertyTypeObject,
                                       @"MixedObject", YES),
    };
    XCTAssertEqual(objectSchema.properties.count,
                   expectedProperties.allKeys.count);
    for (NSString *propertyName in expectedProperties) {
      XCTAssertEqualObjects(objectSchema[propertyName],
  ...
  { // Test optional property types
    ...
    NSDictionary *expectedProperties = @{
      ...
      @"doubleObj" :
          makeProperty(@"doubleObj", RLMPropertyTypeDouble, nil, YES),
      ...
      @"objectId" :
          makeProperty(@"objectId", RLMPropertyTypeObjectId, nil, YES),
      @"decimal" :
          makeProperty(@"decimal", RLMPropertyTypeDecimal128, nil, YES),
  ...
  { // Test indexed property
    RLMObjectSchema *objectSchema =
        [RLMObjectSchema schemaForObjectClass:[IndexedStringObject class]];
    RLMProperty *stringProperty = objectSchema[@"stringCol"];
    RLMProperty *expectedProperty =
        [[RLMProperty alloc] initWithName:@"stringCol"
...
- (void)testTypeToString {
  ...
  XCTAssertEqualObjects(RLMTypeToString(RLMPropertyTypeLinkingObjects),
                        @"linking objects");
