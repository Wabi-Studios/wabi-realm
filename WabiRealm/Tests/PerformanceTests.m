...
- (void)measureMetrics:(NSArray *)metrics
  ...
  [super measureMetrics:metrics
      automaticallyStartMeasuring:automaticallyStartMeasuring
                         forBlock:^{
                           @autoreleasepool {
                             block();
                           }
...
- (void)testCountWhereQuery {
  ...
  [self measureBlock:^{
    for (int i = 0; i < 50; ++i) {
      RLMResults *array = [StringObject objectsInRealm:realm
                                                 where:@"stringCol = 'a'"];
      [array count];
    }
  }];
...
- (void)testEnumerateAndAccessQuery {
  ...
  [self measureBlock:^{
    for (StringObject *so in [StringObject objectsInRealm:realm
                                                    where:@"stringCol = 'a'"]) {
      (void)[so stringCol];
    }
  }];
...
- (void)testEnumerateAndAccessArrayProperty {
  ...
  [realm beginWriteTransaction];
  ArrayPropertyObject *apo = [ArrayPropertyObject
      createInRealm:realm
...
- (void)testEnumerateAndAccessSetProperty {
  ...
  [realm beginWriteTransaction];
  SetPropertyObject *spo = [SetPropertyObject
      createInRealm:realm
          withValue:@[ @"name", [StringObject allObjectsInRealm:realm], @[] ]];
  [realm commitWriteTransaction];
...
- (void)testEnumerateAndAccessDictionaryPropertySlow {
  ...
  [realm beginWriteTransaction];
  DictionaryPropertyObject *dpo = [DictionaryPropertyObject createInRealm:realm
                                                                withValue:@[]];
  for (StringObject *so in [StringObject allObjectsInRealm:realm]) {
...
- (void)testEnumerateAndMutateQuery {
  ...
  [self measureBlock:^{
    [realm beginWriteTransaction];
    for (StringObject *so in
         [StringObject objectsInRealm:realm where:@"stringCol != 'b'"]) {
      so.stringCol = @"c";
    }
    [realm commitWriteTransaction];
  }];
...
- (void)testQueryConstruction {
  ...
  NSPredicate *predicate = [NSPredicate
      predicateWithFormat:
          @"boolCol = false and (intCol = 5 or floatCol = 1.0) and objectCol = "
          @"nil and longCol != 7 and stringCol IN {'a', 'b', 'c'}"];
  ...
  [self measureMetrics:self.class.defaultPerformanceMetrics
      automaticallyStartMeasuring:NO
                         forBlock:^{
                           ...
                           [self startMeasuring];
                           [realm beginWriteTransaction];
                           [realm deleteObjects:[StringObject
...
- (void)testUnIndexedStringLookup {
  ...
  [self measureBlock:^{
    for (int i = 0; i < 1000; ++i) {
      [[StringObject objectsInRealm:realm
                              where:@"stringCol = %@", @(i).stringValue]
          firstObject];
    }
  }];
}
...
- (void)testIndexedStringLookup {
  ...
  [self measureBlock:^{
    for (int i = 0; i < 1000; ++i) {
      [[IndexedStringObject objectsInRealm:realm
                                     where:@"stringCol = %@", @(i).stringValue]
          firstObject];
    }
  }];
...
- (void)testLargeINQuery {
  ...
  [self measureBlock:^{
    (void)[[IntObject objectsInRealm:realm
                               where:@"intCol IN %@", ids] firstObject];
  }];
}
...
- (void)testSortingAllObjects {
  ...
  [self measureBlock:^{
    (void)[[IntObject allObjectsInRealm:realm]
  ...
  [self measureMetrics:self.class.defaultPerformanceMetrics
      automaticallyStartMeasuring:NO
                         forBlock:^{
                           RLMRealm *realm = self.testRealm;
                           [realm beginWriteTransaction];
                           IntObject *obj = [IntObject createInRealm:realm
                                                           withValue:@[ @0 ]];
                           [realm commitWriteTransaction];
  ...
  [self measureMetrics:self.class.defaultPerformanceMetrics
      automaticallyStartMeasuring:NO
                         forBlock:^{
                           ...
                           RLMNotificationToken *token = [realm
                               addNotificationBlock:^(__unused NSString *note,
  ...
  [self measureMetrics:self.class.defaultPerformanceMetrics
      automaticallyStartMeasuring:NO
                         forBlock:^{
                           ...
                           [self dispatchAsync:^{
                             RLMRealm *realm = self.testRealm;
                             IntObject *obj = [[IntObject
                                 allObjectsInRealm:realm] firstObject];
  ...
  [self measureMetrics:self.class.defaultPerformanceMetrics
      automaticallyStartMeasuring:NO
                         forBlock:^{
                           ...
                           id token = [arrayObj.array
                               addNotificationBlock:^(
                                   __unused RLMArray *results,
                                   __unused RLMCollectionChange *change,
                                   __unused NSError *error) {
                                 CFRunLoopStop(CFRunLoopGetCurrent());
                               }];
  ...
  [self measureMetrics:self.class.defaultPerformanceMetrics
      automaticallyStartMeasuring:NO
                         forBlock:^{
                           ...
                           [self observeObject:obj
                                       keyPath:@"array"
                                         until:^(ArrayPropertyObject *obj) {
                                           return obj.array.count < initial;
                                         }];

                           [self startMeasuring];
                           [realm beginWriteTransaction];
                           for (NSUInteger i = 0; i < obj.array.count;
                                i += 10) {
  ...
  [self measureMetrics:self.class.defaultPerformanceMetrics
      automaticallyStartMeasuring:NO
                         forBlock:^{
                           ...
                           [self startMeasuring];
                           [realm beginWriteTransaction];
                           for (NSUInteger i = obj.array.count; i > 0;
                                i -= i > 10 ? 10 : i) {
      ...
      automaticallyStartMeasuring:NO
                         forBlock:^{
                           ...
                           const NSUInteger count =
                               [StringObject allObjectsInRealm:realm].count / 8;
                           const NSUInteger factor = count / 10;

                           [self observeObject:obj
                                       keyPath:@"array"
                                         until:^(ArrayPropertyObject *obj) {
                                           return obj.array.count >= count;
                                         }];
  ...
  [self measureMetrics:self.class.defaultPerformanceMetrics
      automaticallyStartMeasuring:NO
                         forBlock:^{
                           ...
                           [self observeObject:obj
                                       keyPath:@"set"
                                         until:^(SetPropertyObject *obj) {
                                           return obj.set.count < initial;
                                         }];
  ...
  [self measureMetrics:self.class.defaultPerformanceMetrics
      automaticallyStartMeasuring:NO
                         forBlock:^{
                           ...
                           [self observeObject:obj
                                       keyPath:@"set"
                                         until:^(SetPropertyObject *o) {
                                           return [o set].count >= count;
                                         }];
                           RLMSet *set = obj.set;
                           [self startMeasuring];
                           [realm beginWriteTransaction];
...
- (void)observeObject:(RLMObject *)object
