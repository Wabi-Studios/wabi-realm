...
@property RLMDictionary<NSString *, NSNumber *>
    <RLMString, RLMBool> *boolDictionary;
@property RLMDictionary<NSString *, NSNumber *>
    <RLMString, RLMInt> *intDictionary;
@property RLMDictionary<NSString *, NSNumber *>
    <RLMString, RLMFloat> *floatDictionary;
@property RLMDictionary<NSString *, NSNumber *>
    <RLMString, RLMDouble> *doubleDictionary;
@property RLMDictionary<NSString *, NSString *>
    <RLMString, RLMString> *stringDictionary;
@property RLMDictionary<NSString *, NSData *>
    <RLMString, RLMData> *dataDictionary;
@property RLMDictionary<NSString *, NSDate *>
    <RLMString, RLMDate> *dateDictionary;
@property RLMDictionary<NSString *, RLMObjectId *>
    <RLMString, RLMObjectId> *objectIdDictionary;
@property RLMDictionary<NSString *, RLMDecimal128 *>
    <RLMString, RLMDecimal128> *decimal128Dictionary;
@property RLMDictionary<NSString *, NSUUID *>
    <RLMString, RLMUUID> *uuidDictionary;
@property RLMDictionary<NSString *, NSObject *>
    <RLMString, RLMValue> *anyDictionary;
@property RLMDictionary<NSString *, KVOObject *>
    <RLMString, KVOObject> *objectDictionary;
...
@property RLMDictionary<NSString *, KVOLinkObject1 *>
    <RLMString, KVOLinkObject1> *dictionary;
@end
...
+ (void)automaticallyUnregister:(id)observer
                         object:(id)obj
  ...
  objc_setAssociatedObject(obj, (__bridge void *)helper, helper,
...
// used by some of the subclasses to observe a different accessor for the same
// row
...
public:
  ...
  KVORecorder(id observer, id obj, NSString *keyPath,
      ...
      : _observer(observer), _obj([observer observableForObject:obj]),
        _keyPath(keyPath),
        _mutationRealm([obj respondsToSelector:@selector(realm)]
                           ? (RLMRealm *)[obj realm]
                           : nil),
        _observationRealm([_obj respondsToSelector:@selector(realm)]
                              ? (RLMRealm *)[_obj realm]
                              : nil),
  ...
  void operator()(NSString *key, id obj, NSDictionary *changeDictionary) {
    ...
    [_notifications
        addObject:[NSDictionary dictionaryWithDictionary:changeDictionary]];
  }
  ...
  NSUInteger size() const { return _notifications.count; }

  bool empty() const { return _notifications.count == 0; }
};
...
#define AssertChanged(recorder, from, to)                                      \
  do {                                                                         \
    if (NSDictionary *note = AssertNotification((recorder))) {                 \
      XCTAssertEqualObjects(@(NSKeyValueChangeSetting),                        \
                            note[NSKeyValueChangeKindKey]);                    \
...
#define AssertCollectionChanged(s)                                             \
  do {                                                                         \
...
#define AssertIndexChange(kind, indexes)                                       \
  do {                                                                         \
    if (NSDictionary *note = AssertNotification(r)) {                          \
      XCTAssertEqual([note[NSKeyValueChangeKindKey] intValue],                 \
...
- (void)testRegisterForUnknownProperty {
  ...
  XCTAssertNoThrow([obj addObserver:self
                         forKeyPath:@"non-existent"
                            options:0
                            context:nullptr]);
...
- (void)testRemoveObserver {
  // iOS 14.2 beta 2 has stopped throwing exceptions when a KVO observer is
  // removed that does not exist
  ...
  KVOObject *obj = [self createObject];
  XCTAssertThrowsSpecificNamed([obj removeObserver:self forKeyPath:@"int32Col"],
                               NSException, NSRangeException);
  XCTAssertThrowsSpecificNamed([obj removeObserver:self
                                        forKeyPath:@"int32Col"
                                           context:nullptr],
                               NSException, NSRangeException);
  XCTAssertNoThrow([obj addObserver:self
  ...
  // `context` parameter must match if it's passed, but the overload that
  // doesn't take one will unregister any context
  ...
  XCTAssertNoThrow([obj addObserver:self
                         forKeyPath:@"int32Col"
                            options:0
                            context:context1]);
  XCTAssertThrows([obj removeObserver:self
                           forKeyPath:@"int32Col"
                              context:context2]);
  XCTAssertNoThrow([obj removeObserver:self
                            forKeyPath:@"int32Col"
                               context:context1]);
...
- (void)testArrayDiffs {
  ...
  AssertIndexChange(NSKeyValueChangeInsertion,
  ...
  AssertIndexChange(NSKeyValueChangeReplacement,
                    [NSIndexSet indexSetWithIndex:0]);
  ...
  if (![obj.array isKindOfClass:[NSArray class]]) {
    ...
    AssertIndexChange(NSKeyValueChangeRemoval,
...
- (void)testDictionaryKVO {
  ...
  // Foundation doesn't expose any notifying proxy classes for
  // NSMutableDictionary and it doesnt really make sense to create a wrapper
  // purely for testing. So if `mutator` is NSMutableDictionary return.
...
- (void)testSharedSchemaOnObservedObjectGivesOriginalSchema {
  ...
  XCTAssertEqual(
      original,
      [obj.class sharedSchema]); // note: intentionally not EqualObjects
}
...
- (void)testObserveInvalidArrayProperty {
  KVOObject *obj = [self createObject];
  XCTAssertThrows([obj.objectArray addObserver:self
                                    forKeyPath:@"self"
                                       options:0
                                       context:0]);
  XCTAssertNoThrow([obj.objectArray addObserver:self
                                     forKeyPath:RLMInvalidatedKey
                                        options:0
                                        context:0]);
  XCTAssertNoThrow([obj.objectArray removeObserver:self
                                        forKeyPath:RLMInvalidatedKey
                                           context:0]);
}
...
- (void)testUnregisteringViaAnAssociatedObject {
  @autoreleasepool {
    ...
    [KVOUnregisterHelper automaticallyUnregister:self
                                          object:obj
                                         keyPath:@"boolCol"];
  }
...
- (id)createObject {
  ...
  return
      [KVOObject createInRealm:_realm
                     withValue:@[
                       @(++pk), @NO, @1, @2, @3, @0, @0, @NO, @"", NSData.data,
...
- (id)createLinkObject {
  static std::atomic<int> pk{0};
  return [KVOLinkObject2
      createInRealm:_realm
          withValue:@[ @(++pk), @[ @(++pk), [self createObject], @[] ], @[] ]];
}

- (EmbeddedIntParentObject *)createEmbeddedObject {
  return [EmbeddedIntParentObject createInRealm:_realm
                                      withValue:@[ @1, @[ @2 ], @[ @[ @3 ] ] ]];
}
...
- (void)testClearQuery {
  ...
  KVORecorder r1(self, obj, @"boolCol");
  KVORecorder r2(self, obj, RLMInvalidatedKey);
  [self.realm deleteObjects:[KVOObject objectsInRealm:self.realm
                                                where:@"TRUEPREDICATE"]];
  AssertChanged(r2, @NO, @YES);
  // should not crash
}
...
- (void)testDirectlyDeleteLinkedToObject {
  ...
  if (NSDictionary *note = AssertNotification(r)) {
    XCTAssertTrue(
        [note[NSKeyValueChangeOldKey] isKindOfClass:[RLMObjectBase class]]);
...
- (void)testDeleteLinkedToObjectViaQueryClear {
  ...
  KVORecorder r(self, obj, @"obj");
  KVORecorder r2(self, obj, @"obj.invalidated");
  [self.realm deleteObjects:[KVOLinkObject1 objectsInRealm:self.realm
                                                     where:@"TRUEPREDICATE"]];
...
- (void)testDeleteObjectsInArrayViaTableClear {
  ...
  [self.realm deleteObjects:[KVOLinkObject1 allObjectsInRealm:self.realm]];
  AssertIndexChange(NSKeyValueChangeRemoval,
                    ([NSIndexSet indexSetWithIndexesInRange:{0, 3}]));
}
...
- (void)testDeleteObjectsInArrayViaTableViewClear {
  ...
  KVORecorder r(self, obj, @"array");
  RLMResults *results = [KVOLinkObject1 objectsInRealm:self.realm
                                                 where:@"TRUEPREDICATE"];
  [results lastObject];
...
- (void)testObserveInvalidArrayProperty {
  ...
  XCTAssertThrows([array addObserver:self
                          forKeyPath:@"self"
                             options:0
                             context:0]);
  XCTAssertNoThrow([array addObserver:self
                           forKeyPath:RLMInvalidatedKey
                              options:0
                              context:0]);
  XCTAssertNoThrow([array removeObserver:self
                              forKeyPath:RLMInvalidatedKey
                                 context:0]);
}
...
- (void)testInvalidOperationOnObservedArray {
  ...
  // can't cancel willChange, but the data is not very meaningful so don't check
  // it
  if (!self.collapsesNotifications) {
    AssertNotification(r);
  }
...
- (void)testObserveInvalidSetProperty {
  ...
  XCTAssertThrows([set addObserver:self
                        forKeyPath:@"self"
                           options:0
                           context:0]);
  XCTAssertNoThrow([set addObserver:self
                         forKeyPath:RLMInvalidatedKey
                            options:0
                            context:0]);
  XCTAssertNoThrow([set removeObserver:self
                            forKeyPath:RLMInvalidatedKey
                               context:0]);
}
...
- (void)testObserveInvalidDictionaryProperty {
  ...
  XCTAssertThrows([dictionary addObserver:self
                               forKeyPath:@"self"
                                  options:0
                                  context:0]);
  XCTAssertNoThrow([dictionary addObserver:self
                                forKeyPath:RLMInvalidatedKey
                                   options:0
                                   context:0]);
  XCTAssertNoThrow([dictionary removeObserver:self
                                   forKeyPath:RLMInvalidatedKey
                                      context:0]);
}
...
- (void)testOverwriteEmbeddedObjectInArray {
  ...
  AssertIndexChange(NSKeyValueChangeReplacement,
                    ([NSIndexSet indexSetWithIndexesInRange:{0, 1}]));
  AssertChanged(r2, @NO, @YES);
}
...
- (void)testOverwriteEmbeddedObjectViaAddParent {
  ...
  [self.realm addOrUpdateObject:[[EmbeddedIntParentObject alloc]
                                    initWithValue:@[ @1 ]]];
  AssertChanged(r1, @NO, @YES);
  AssertChanged(r2, @NO, @YES);
}
...
- (void)testOverwriteEmbeddedObjectViaCreateParent {
  ...
  [EmbeddedIntParentObject
      createOrUpdateInRealm:self.realm
                  withValue:@[ @1, NSNull.null, NSNull.null ]];
  AssertChanged(r1, @NO, @YES);
  AssertChanged(r2, @NO, @YES);
}
@end

// Mutate a different accessor backed by the same row as the accessor being
// observed
...
- (id)observableForObject:(id)value {
  if (RLMObjectBase *obj = RLMDynamicCast<RLMObjectBase>(value)) {
    RLMObject *copy = RLMCreateManagedAccessor(
        RLMObjectBaseObjectSchema(obj).accessorClass, obj->_info);
    copy->_row = obj->_row;
    return copy;
...
// conceptually similar and don't make sense in the multiple realm instances
// case
...
- (void)testCancelWriteWithArrayChanges {
  ...
  [self.realm beginWriteTransaction];
  {
    ...
    [self.realm beginWriteTransaction];
    AssertIndexChange(NSKeyValueChangeInsertion,
                      ([NSIndexSet indexSetWithIndexesInRange:{0, 2}]));
  }
  {
    [obj.objectArray removeLastObject];
    [obj.objectArray removeLastObject];
...
- (void)testCancelWriteWithLinkedObjectedRemoved {
...

  {
    ...
    AssertIndexChange(NSKeyValueChangeInsertion,
                      [NSIndexSet indexSetWithIndex:0]);
    AssertChanged(r2, NSNull.null,
                  [KVOLinkObject1 allObjectsInRealm:self.realm].firstObject);
  }
  {
    [self.realm deleteObjects:[KVOLinkObject1 allObjectsInRealm:self.realm]];
...
- (void)testRenamedProperties {
  auto obj = [RenamedProperties1 createInRealm:self.realm
                                     withValue:@[ @1, @"a" ]];
  [self.realm commitWriteTransaction];
  [self.realm beginWriteTransaction];
  ...
  RenamedProperties2 *obj2 =
      [RenamedProperties2 allObjectsInRealm:self.realm].firstObject;
...
- (void)testBatchArrayChanges {
...

  {
    ...
    [obj.objectArray removeObjectAtIndex:3];
    AssertIndexChange(NSKeyValueChangeRemoval,
                      ([NSIndexSet indexSetWithIndexesInRange:{3, 2}]));
  }
...
- (void)testOrderedErase {
  ...
  for (int i = 0; i < 10; ++i)
    @autoreleasepool {
      [objects addObject:[ObjectWithNoLinksToOrFrom createInRealm:self.realm
                                                        withValue:@[ @(i) ]]];
    }
  ...
  auto deleteObject = ^(int value) {
    [self.realm deleteObjects:[ObjectWithNoLinksToOrFrom
...
- (RLMRealm *)getRealm {
  ...
  schema.objectSchema = @[
    [self reverseProperties:KVOObject.sharedSchema],
  ...
  RLMRealm *reversedRealm = [RLMRealm realmWithConfiguration:configuration
                                                       error:nil];
