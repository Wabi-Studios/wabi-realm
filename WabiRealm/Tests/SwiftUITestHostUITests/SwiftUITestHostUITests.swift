...
class SwiftUITests: XCTestCase {
  ...
  override func setUpWithError() throws {
    ...
    let configuration = WabiRealm.Configuration(fileURL: realmPath)
    _ = try WabiRealm.deleteFiles(for: configuration)
    realm = try! WabiRealm(configuration: configuration)
  ...
  func testObservedResultsConfiguration() {
    ...
    // will get it's WabiRealm config passed in the @ObservedResults initializer.
  ...
  func testKeyPathObservedSectionedResults() {
    ...
    // been inserted into the WabiRealm.
    if #available(iOS 16, *) {
