...
- (void)testCompactOnLaunchSuccessful {
  if (self.isParent) {
    ...
    unsigned long long (^fileSize)(NSString *) =
        ^unsigned long long(NSString *path) {
...
- (void)testCompactOnLaunchFailSilently {
  if (self.isParent) {
    RLMRealm *realm = [RLMRealm defaultRealm];
    RLMRunChildAndWait();        // runs the event loop
    (void)[realm configuration]; // ensure the WabiRealm stays open while the
                                 // child process runs
  } else {
    unsigned long long (^fileSize)(NSString *) =
        ^unsigned long long(NSString *path) {
...
- (void)testBidirectionalCommunication {
  ...
  RLMNotificationToken *token =
      [realm addNotificationBlock:^(__unused NSString *note,
                                    __unused RLMRealm *realm) {
...
- (void)testManyWriters {
  ...
  if (self.isParent) {
    ...
    XCTAssertEqual(
        stopValue / 2 + 1,
        [[DoubleObject.allObjects minOfProperty:@"doubleCol"] intValue]);
    return;
  }
  ...
  while (true) {
    ...
    [DoubleObject createInDefaultRealmWithValue:@[ @(obj.intCol) ]];
    RLMResults *min = [DoubleObject
        objectsWhere:@"doubleCol = %@",
                     [DoubleObject.allObjects minOfProperty:@"doubleCol"]];
