...
- (void)testLinkingObjectsOnUnmanagedObject {
  PersonObject *don =
      [[PersonObject alloc] initWithValue:@[ @"Don", @60, @[] ]];
  ...
  XCTAssertEqual(
      0u, [don.parents sortedResultsUsingKeyPath:@"age" ascending:YES].count);
  ...
  PersonObject *mark =
      [[PersonObject alloc] initWithValue:@[ @"Mark", @30, @[] ]];
  ...
  RLMAssertThrowsWithReason(
      ([don.parents addNotificationBlock:^(RLMResults *, RLMCollectionChange *,
                                           NSError *){
      }]),
...
- (void)testLinkingObjectsOnFrozenObject {
  ...
  XCTAssertEqualObjects(asArray(frozenHannah.parents), (@[ frozenMark ]));
  XCTAssertEqualObjects(asArray(frozenHannah2.parents),
                        (@[ frozenMark2, frozenDiane ]));
...
- (void)testFilteredLinkingObjects {
  ...
  // Three separate queries so that accessing a property on one doesn't
  // invalidate testing of other properties.
...
- (void)testNotificationSentInitially {
  ...
  RLMNotificationToken *token = [hannah.parents
      addNotificationBlock:^(RLMResults *linkingObjects,
                             RLMCollectionChange *change, NSError *error) {
        XCTAssertEqualObjects([linkingObjects valueForKeyPath:@"self"],
                              (@[ mark ]));
        XCTAssertNil(change);
        XCTAssertNil(error);
        [expectation fulfill];
...
- (void)testNotificationSentOnlyForActualRefresh {
  ...
  __block id expectation = [self expectationWithDescription:@""];
  RLMNotificationToken *token = [hannah.parents
      addNotificationBlock:^(RLMResults *linkingObjects, RLMCollectionChange *,
                             NSError *error) {
        XCTAssertNotNil(linkingObjects);
        XCTAssertNil(error);
  ...
  // waiting for this one also waits for the above one to get a chance to run
  [self
      waitForNotification:RLMRealmRefreshRequiredNotification
                    realm:realm
                    block:^{
                      [self dispatchAsyncAndWait:^{
                        RLMRealm *realm = self.realmWithTestPath;
...
- (void)testRenamedProperties {
  ...
  auto link =
      [LinkToRenamedProperties1 createInRealm:realm
                                    withValue:@[ obj1, obj2, @[ obj1, obj1 ] ]];
  [realm commitWriteTransaction];

  XCTAssertEqualObjects(obj1.linking1.objectClassName,
                        @"LinkToRenamedProperties1");
  XCTAssertEqualObjects(obj1.linking2.objectClassName,
