...
- (void)testBasicLink {
  ...
  XCTAssertEqualObjects([dogs[0] dogName], @"Harvie",
                        @"Harvie is named Harvie");

  OwnerObject *tim = owners[0];
  XCTAssertEqualObjects(tim.dog.dogName, @"Harvie",
                        @"Tim's dog should be Harvie");
}
...
- (void)testMultipleOwnerLink {
  ...
  [realm beginWriteTransaction];
  OwnerObject *fiel = [OwnerObject createInRealm:realm
                                       withValue:@[ @"Fiel", [NSNull null] ]];
  fiel.dog = [DogObject allObjectsInRealm:realm].firstObject;
  [realm commitWriteTransaction];

  XCTAssertEqual([OwnerObject objectsInRealm:realm withPredicate:nil].count,
                 2U);
...
- (void)testLinkRemoval {
  ...
  XCTAssertThrows(
      dog.dogName,
      @"Dog object should be invalid after being deleted from the realm");
...
- (void)testInvalidLinks {
  ...
  [realm beginWriteTransaction];
  XCTAssertThrows([realm addObject:linkObject],
                  @"dateCol not set on linked object");

  StringObject *to = [StringObject createInRealm:realm
                                       withValue:@[ @"testObject" ]];
  NSArray *args = @[ @"Tim", to ];
  XCTAssertThrows([OwnerObject createInRealm:realm withValue:args],
                  @"Inserting wrong object type should throw");
  [realm commitWriteTransaction];
}
...
- (void)testBidirectionalRelationship {
  ...
  CircleObject *obj0 =
      [[CircleObject alloc] initWithValue:@[ @"a", NSNull.null ]];
...
- (void)testAddingCircularReferenceInArrayDoesNotLeakSourceObjects {
  ...
  @autoreleasepool {
    CircularArrayObject *obj0 =
        [[CircularArrayObject alloc] initWithValue:@[ @[] ]];
    CircularArrayObject *obj1 =
        [[CircularArrayObject alloc] initWithValue:@[ @[ obj0 ] ]];
...
- (void)testAddingCircularReferenceInSetDoesNotLeakSourceObjects {
  ...
  @autoreleasepool {
    CircularSetObject *obj0 =
        [[CircularSetObject alloc] initWithValue:@[ @[] ]];
    CircularSetObject *obj1 =
        [[CircularSetObject alloc] initWithValue:@[ @[ obj0 ] ]];
