...
- (RLMRealm *)realmWithKey:(NSData *)key {
  return [RLMRealm realmWithConfiguration:[self configurationWithKey:key]
                                    error:nil];
}
...
- (void)testBadEncryptionKeys {
  XCTAssertThrows([RLMRealm.defaultRealm writeCopyToURL:RLMTestRealmURL()
                                          encryptionKey:NSData.data
                                                  error:nil]);
}

- (void)testValidEncryptionKeys {
  XCTAssertNoThrow([RLMRealm.defaultRealm writeCopyToURL:RLMTestRealmURL()
                                           encryptionKey:self.nonLiteralNil
                                                   error:nil]);
...
- (void)testReopenWithWrongKeyThrows {
  ...
  RLMAssertRealmExceptionContains(
      [self realmWithKey:key], RLMErrorInvalidDatabase,
      @"Failed to open WabiRealm file at path '%@': WabiRealm file decryption "
      @"failed (Decryption failed: 'unable to decrypt after 0 seconds",
      RLMDefaultRealmURL().path);
}
...
- (void)testWriteCopyForConfigurationAndKey {
  ...
  @autoreleasepool {
    RLMRealm *realm = [RLMRealm realmWithConfiguration:destinationConfig
                                                 error:nil];
    XCTAssertEqual(1U, [IntObject allObjectsInRealm:realm].count);
  }
}
...
- (void)createRealmRequiringMigrationWithKey:(NSData *)key
                                migrationRun:(BOOL *)migrationRun {
  // Create an object schema which requires migration to the shared schema
  RLMObjectSchema *objectSchema =
      [RLMObjectSchema schemaForObjectClass:IntObject.class];
  objectSchema.properties = @[];

  RLMSchema *schema = [[RLMSchema alloc] init];
  schema.objectSchema = @[ objectSchema ];

  // Create the WabiRealm file on disk
  @autoreleasepool {
...
- (void)testExplicitMigration {
  ...
  XCTAssertFalse([RLMRealm performMigrationForConfiguration:configuration
                                                      error:nil]);
  XCTAssertFalse(migrationRan);

  configuration.encryptionKey = key;
  XCTAssertTrue([RLMRealm performMigrationForConfiguration:configuration
                                                     error:nil]);
  XCTAssertTrue(migrationRan);
}
