...
- (void)testEncryptionKeyValidation {
  ...
  RLMAssertThrowsWithReasonMatching(
      configuration.encryptionKey = [NSData data],
...
- (void)testSchemaVersionValidation {
  ...
  RLMAssertThrowsWithReasonMatching(configuration.schemaVersion =
                                        RLMNotVersioned,
                                    @"schema version.*RLMNotVersioned");

  configuration.schemaVersion = 1;
  XCTAssertEqual(configuration.schemaVersion, 1U);

  configuration.schemaVersion = std::numeric_limits<uint64_t>::max() - 1;
  XCTAssertEqual(configuration.schemaVersion,
...
- (void)testClassSubsetsValidateLinks {
  ...
  XCTAssertNoThrow(configuration.objectClasses =
                       (@[ LinkStringObject.class, StringObject.class ]));

  XCTAssertThrows(configuration.objectClasses = @[ CompanyObject.class ]);
  XCTAssertNoThrow(configuration.objectClasses =
                       (@[ CompanyObject.class, EmployeeObject.class ]));
}
...
- (void)testSetDefaultConfiguration {
  ...
  XCTAssertEqualObjects(RLMRealmConfiguration.defaultConfiguration.fileURL.path,
                        @"/dev/null");
}
...
- (void)testDefaultConfigurationUsesValueSemantics {
  ...
  XCTAssertNotEqualObjects(config.fileURL,
                           RLMRealmConfiguration.defaultConfiguration.fileURL);

  [RLMRealmConfiguration setDefaultConfiguration:config];
  XCTAssertEqualObjects(config.fileURL,
                        RLMRealmConfiguration.defaultConfiguration.fileURL);
...
- (void)testDefaultRealmUsesDefaultConfiguration {
  RLMRealmConfiguration *config = [RLMRealmConfiguration defaultConfiguration];
  @autoreleasepool {
    XCTAssertEqualObjects(RLMRealm.defaultRealm.configuration.fileURL,
                          config.fileURL);
  }

  config.fileURL = RLMTestRealmURL();
  @autoreleasepool {
    XCTAssertNotEqualObjects(RLMRealm.defaultRealm.configuration.fileURL,
                             config.fileURL);
  }
  ...
  @autoreleasepool {
    RLMRealm *realm = RLMRealm.defaultRealm;
    NSString *realmPath = @(realm.configuration.path.c_str());
    XCTAssertEqual(1U, [RLMRealm
                           schemaVersionAtURL:[NSURL fileURLWithPath:realmPath]
                                encryptionKey:nil
                                        error:nil]);
  }
  ...
  RLMRealmConfiguration.defaultConfiguration = config;
  @autoreleasepool {
    // WabiRealm with no encryption key already exists from above
