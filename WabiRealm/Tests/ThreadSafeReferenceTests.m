...
/// Resolve a thread-safe reference confirming that you can't resolve it a
/// second time.
- (id)assertResolve:(RLMRealm *)realm
          reference:(RLMThreadSafeReference *)reference {
  ...
  RLMAssertThrowsWithReasonMatching(
      [realm resolveThreadSafeReference:reference],
...
- (void)testInvalidThreadSafeReferenceConstruction {
  ...
  ArrayPropertyObject *arrayParent = [[ArrayPropertyObject alloc]
      initWithValue:@[ @"arrayObject", @[ @[ @"a" ] ], @[] ]];
  RLMArray *arrayObject = arrayParent.array;

  RLMAssertThrowsWithReasonMatching(
      [RLMThreadSafeReference referenceWithThreadConfined:stringObject],
      @"Cannot construct reference to unmanaged object");
  RLMAssertThrowsWithReasonMatching(
      [RLMThreadSafeReference referenceWithThreadConfined:arrayObject],
      @"Cannot construct reference to unmanaged object");
...
- (void)testInvalidThreadSafeReferenceUsage {
  ...
  StringObject *stringObject =
      [StringObject createInDefaultRealmWithValue:@{@"stringCol" : @"hello"}];
  RLMThreadSafeReference *ref1 =
      [RLMThreadSafeReference referenceWithThreadConfined:stringObject];
  [realm commitWriteTransaction];

  RLMThreadSafeReference *ref2 =
      [RLMThreadSafeReference referenceWithThreadConfined:stringObject];
  RLMThreadSafeReference *ref3 =
      [RLMThreadSafeReference referenceWithThreadConfined:stringObject];
  [self dispatchAsyncAndWait:^{
    ...
    RLMAssertThrowsWithReasonMatching(
        [realm resolveThreadSafeReference:ref2],
        @"Can only resolve a thread safe reference once");
    // Assert that we can resolve a different reference to the same object.
    XCTAssertEqualObjects([self assertResolve:realm
                                    reference:ref3][@"stringCol"], @"hello");
  }];
}
...
- (void)testPassThreadSafeReferenceToDeletedObject {
  ...
  RLMThreadSafeReference *ref1 =
      [RLMThreadSafeReference referenceWithThreadConfined:intObject];
  RLMThreadSafeReference *ref2 =
      [RLMThreadSafeReference referenceWithThreadConfined:intObject];
...
- (void)testPassThreadSafeReferencesToMultipleObjects {
  ...
  RLMThreadSafeReference *stringObjectRef =
      [RLMThreadSafeReference referenceWithThreadConfined:stringObject];
  RLMThreadSafeReference *intObjectRef =
      [RLMThreadSafeReference referenceWithThreadConfined:intObject];
  XCTAssertEqualObjects(nil, stringObject.stringCol);
  XCTAssertEqual(0, intObject.intCol);
  [self dispatchAsyncAndWait:^{
    RLMRealm *realm = [RLMRealm defaultRealm];
    StringObject *stringObject = [self assertResolve:realm
                                           reference:stringObjectRef];
...
- (void)testPassThreadSafeReferenceToArray {
  ...
  [realm transactionWithBlock:^{
    ...
    DogObject *friday = [DogObject createInDefaultRealmWithValue:@{
      @"dogName" : @"Friday",
      @"age" : @15
    }];
    [object.dogs addObject:friday];
  }];
  RLMThreadSafeReference *dogsArrayRef =
      [RLMThreadSafeReference referenceWithThreadConfined:object.dogs];
  XCTAssertEqual(1ul, object.dogs.count);
  XCTAssertEqualObjects(@"Friday", object.dogs[0].dogName);
  [self dispatchAsyncAndWait:^{
    RLMRealm *realm = [RLMRealm defaultRealm];
    RLMArray<DogObject *> *dogs = [self assertResolve:realm
                                            reference:dogsArrayRef];
    XCTAssertEqual(1ul, dogs.count);
    XCTAssertEqualObjects(@"Friday", dogs[0].dogName);

    [realm transactionWithBlock:^{
      [dogs removeAllObjects];
      DogObject *cookie = [DogObject createInDefaultRealmWithValue:@{
        @"dogName" : @"Cookie",
        @"age" : @8
      }];
      DogObject *breezy = [DogObject createInDefaultRealmWithValue:@{
        @"dogName" : @"Breezy",
        @"age" : @6
      }];
...
- (void)testPassThreadSafeReferenceToResults {
  ...
  RLMResults<StringObject *> *results = [[StringObject
      objectsWhere:@"stringCol != 'C'"] sortedResultsUsingKeyPath:@"stringCol"
                                                        ascending:NO];
  RLMThreadSafeReference *resultsRef =
      [RLMThreadSafeReference referenceWithThreadConfined:results];
  [realm transactionWithBlock:^{
  ...
  [self dispatchAsyncAndWait:^{
    RLMRealm *realm = [RLMRealm defaultRealm];
    RLMResults<StringObject *> *results = [self assertResolve:realm
                                                    reference:resultsRef];
    RLMResults<StringObject *> *allObjects = [StringObject allObjects];
...
- (void)testPassThreadSafeReferenceToLinkingObjects {
  RLMRealm *realm = [RLMRealm defaultRealm];
  DogObject *dogA =
      [[DogObject alloc] initWithValue:@{@"dogName" : @"Cookie", @"age" : @10}];
  DogObject *unaccessedDogB =
      [[DogObject alloc] initWithValue:@{@"dogName" : @"Skipper", @"age" : @7}];
  // Ensures that an `RLMLinkingObjects` without cached results can be handed
  // over

  [realm transactionWithBlock:^{
    [realm addObject:[[OwnerObject alloc]
                         initWithValue:@{@"name" : @"Andrea", @"dog" : dogA}]];
    [realm addObject:[[OwnerObject alloc] initWithValue:@{
             @"name" : @"Mike",
             @"dog" : unaccessedDogB
           }]];
  }];
  ...
  RLMThreadSafeReference *ownersARef =
      [RLMThreadSafeReference referenceWithThreadConfined:dogA.owners];
  RLMThreadSafeReference *ownersBRef = [RLMThreadSafeReference
      referenceWithThreadConfined:unaccessedDogB.owners];
  [self dispatchAsyncAndWait:^{
    RLMRealm *realm = [RLMRealm defaultRealm];
    RLMLinkingObjects<OwnerObject *> *ownersA = [self assertResolve:realm
                                                          reference:ownersARef];
    RLMLinkingObjects<OwnerObject *> *ownersB = [self assertResolve:realm
                                                          reference:ownersBRef];
  ...
  XCTAssertEqualObjects(@"Mike",
                        ((OwnerObject *)unaccessedDogB.owners[0]).name);
  [realm refresh];
  XCTAssertEqual(1ul, dogA.owners.count);
  XCTAssertEqualObjects(@"Mike", ((OwnerObject *)dogA.owners[0]).name);
  XCTAssertEqual(1ul, unaccessedDogB.owners.count);
  XCTAssertEqualObjects(@"Andrea",
                        ((OwnerObject *)unaccessedDogB.owners[0]).name);
}
