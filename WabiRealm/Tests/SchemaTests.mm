...
@interface SchemaTestClassWithSingleDuplicateProperty
    : SchemaTestClassWithSingleDuplicatePropertyBase
@property NSString *string;
@end
...
@interface SchemaTestClassWithMultipleDuplicateProperties
    : SchemaTestClassWithMultipleDuplicatePropertiesBase
@property NSString *string;
@property int integer;
@end
...
+ (NSDictionary *)linkingObjectsProperties {
  return @{
    @"linkingObjects" :
        [RLMPropertyDescriptor descriptorWithClass:SchemaTestsLinkSource.class
                                      propertyName:@"irwlop"]
  };
}
...
+ (NSDictionary *)linkingObjectsProperties {
  return @{
    @"linkingObjects" : [RLMPropertyDescriptor
        descriptorWithClass:
            InvalidLinkingObjectsPropertyMissingSourcePropertyOfLink.class
...
+ (NSDictionary *)linkingObjectsProperties {
  return @{
    @"linkingObjects" : [RLMPropertyDescriptor
        descriptorWithClass:InvalidLinkingObjectsPropertySourcePropertyNotALink
                                .class
...
@interface InvalidLinkingObjectsPropertySourcePropertyLinksElsewhere
    : FakeObject
@property(readonly) RLMLinkingObjects *linkingObjects;
@property IntObject *link;
@end
...
+ (NSDictionary *)linkingObjectsProperties {
  return @{
    @"linkingObjects" : [RLMPropertyDescriptor
        descriptorWithClass:
            InvalidLinkingObjectsPropertySourcePropertyLinksElsewhere.class
               propertyName:@"link"]
  };
}
...
- (void)testSchemaWithObjectClasses {
  RLMSchema *schema = [RLMSchema
      schemaWithObjectClasses:@[ RLMDynamicObject.class, StringObject.class ]];
  XCTAssertEqualObjects((@[ @"RLMDynamicObject", @"StringObject" ]),
                        [[schema.objectSchema valueForKey:@"className"]
                            sortedArrayUsingSelector:@selector(compare:)]);
  XCTAssertNil([RLMSchema.sharedSchema schemaForClassName:@"RLMDynamicObject"]);
}

- (void)testInheritanceInitialization {
  Class testClasses[] = {[SchemaTestClassBase class],
  ...
  auto checkSchema = ^(RLMSchema *schema, NSString *className,
    ...
    for (NSString *propName in properties) {
      XCTAssertEqualObjects(properties[propName],
                            [objectSchema[propName] objectClassName]);
    }
  };
  ...
  do
    @autoreleasepool {
      ...
      for (RLMObjectSchema *objectSchema in objectSchemas) {
        NSString *name =
            [NSString stringWithFormat:@"RLMTestAccessor_%lu_%@", iteration,
                                       objectSchema.className];
        objectSchema.accessorClass = RLMManagedAccessorClassForObjectClass(
            objectSchema.objectClass, objectSchema, name.UTF8String);
        objectSchema.unmanagedClass = RLMUnmanagedAccessorClassForObjectClass(
      ...
      checkSchema(
          schema, @"SchemaTestClassFirstChild",
          @{@"baseCol" : @"IntObject", @"firstChildCol" : @"IntObject"});
      checkSchema(
          schema, @"SchemaTestClassSecondChild",
          @{@"baseCol" : @"IntObject", @"secondChildCol" : @"IntObject"});
      checkSchema(schema, @"SchemaTestClassLink", @{
        @"base" : @"SchemaTestClassBase",
      ...
      [realm createObject:@"SchemaTestClassBase"
                withValue:@{@"baseCol" : @[ @0 ]}];
      [realm createObject:@"SchemaTestClassFirstChild"
                withValue:@{@"baseCol" : @[ @0 ], @"firstChildCol" : @[ @0 ]}];
      [realm createObject:@"SchemaTestClassSecondChild"
                withValue:@{@"baseCol" : @[ @0 ], @"secondChildCol" : @[ @0 ]}];
...
- (void)testObjectSchema {
  ...
  // Due to the automatic initialization of a new realm with all visible classes
  // inheriting from RLMObject, it's difficult to define test cases that verify
  // the absolute correctness of a realm's current type catalogue unless its
  // expected configuration is known at compile time (requires that the set of
  // expected types is always up-to-date). Instead, only a partial verification
  // is performed, which only requires the availability of a well-defined subset
  // of types and ignores any other types that may be included in the realm's
  // type catalogue. If a more fine-grained control with the realm's type
  // inclusion mechanism is introduced later on, these tests should be altered
  // to verify all types.

  NSArray *expectedTypes = @[
    @"AllTypesObject", @"LinkToAllTypesObject", @"StringObject", @"MixedObject",
    @"IntObject"
  ];
  ...
  for (NSString *className in expectedTypes) {
    [objectSchema
        addObject:[RLMObjectSchema
                      schemaForObjectClass:NSClassFromString(className)]];
  }
  ...
  XCTAssertTrue(objectSchemas.count >= expectedTypes.count,
                @"Expecting %lu object schemas in database found %lu",
                (unsigned long)expectedTypes.count,
                (unsigned long)objectSchemas.count);
  ...
  for (NSString *expectedType in expectedTypes) {
    ...
    XCTAssertEqual(
        occurrenceCount, (NSUInteger)1,
        @"Expecting single occurrence of object schema for type %@ found %lu",
        expectedType, (unsigned long)occurrenceCount);
  ...
  XCTAssertTrue(
      identifiedTypesCount >= expectedTypes.count,
      @"Unexpected object schemas in database. Found %lu out of %lu expected",
      (unsigned long)identifiedTypesCount, (unsigned long)expectedTypes.count);

  // Test 4: Test querying object schemas using schemaForClassName: for expected
  // types
  for (NSString *expectedType in expectedTypes) {
    XCTAssertNotNil([schema schemaForClassName:expectedType],
                    @"Expecting to find object schema for type %@ in realm "
                    @"using query, found none",
                    expectedType);
  }

  // Test 5: Test querying object schemas using schemaForClassName: for
  // unexpected types
  XCTAssertNil([schema schemaForClassName:unexpectedType],
               @"Expecting not to find object schema for type %@ in realm "
               @"using query, did find",
               unexpectedType);

  // Test 6: Test querying object schemas using subscription for unexpected
  // types
  for (NSString *expectedType in expectedTypes) {
    XCTAssertNotNil(schema[expectedType],
                    @"Expecting to find object schema for type %@ in realm "
                    @"using subscription, found none",
                    expectedType);
  }

  // Test 7: Test querying object schemas using subscription for unexpected
  // types
  XCTAssertThrows(schema[unexpectedType],
                  @"Expecting asking schema for type %@ in realm using "
                  @"subscription to throw",
                  unexpectedType);
...
- (void)testDescription {
  NSArray *expectedTypes =
      @[ @"AllTypesObject", @"StringObject", @"IntObject" ];
  ...
  XCTAssertEqualObjects(schema.description,
                        @"Schema {\n"
                        @"\tAllTypesObject {\n"
                        @"\t\tboolCol {\n"
                        @"\t\t\ttype = bool;\n"
...
- (void)testClassWithDuplicateProperties {
  RLMAssertThrowsWithReasonMatching(
      [RLMObjectSchema
          schemaForObjectClass:SchemaTestClassWithSingleDuplicateProperty
                                   .class],
      @"'string' .* multiple times .* "
      @"'SchemaTestClassWithSingleDuplicateProperty'");
  RLMAssertThrowsWithReasonMatching(
      [RLMObjectSchema
          schemaForObjectClass:SchemaTestClassWithMultipleDuplicateProperties
                                   .class],
      @"'SchemaTestClassWithMultipleDuplicateProperties' .* declared multiple "
      @"times");
}

- (void)testClassWithInvalidPrimaryKey {
  XCTAssertThrows(
      [RLMObjectSchema schemaForObjectClass:InvalidPrimaryKeyType.class]);
}

- (void)testClassWithMissingPrimaryKey {
  RLMAssertThrowsWithReason(
      [RLMObjectSchema schemaForObjectClass:MissingPrimaryKey.class],
      @"Primary key property 'primaryKey' does not exist on object "
      @"'MissingPrimaryKey'");
}

- (void)testClassWithUnindexableProperty {
  RLMObjectSchema *objectSchema =
      [RLMObjectSchema schemaForObjectClass:UnindexableProperty.class];
  RLMSchema *schema = [[RLMSchema alloc] init];
  schema.objectSchema = @[ objectSchema ];
  RLMAssertThrowsWithReasonMatching(
      [self realmWithTestPathAndSchema:schema],
      @"Property 'UnindexableProperty.unindexable' of type 'double' cannot be "
      @"indexed");
}
...
- (void)testClassWithRequiredPrimitiveArrayProperties {
  RLMObjectSchema *objectSchema =
      [RLMObjectSchema schemaForObjectClass:AllPrimitiveArrays.class];
...
- (void)testClassWithOptionalPrimitiveArrayProperties {
  RLMObjectSchema *objectSchema =
      [RLMObjectSchema schemaForObjectClass:AllOptionalPrimitiveArrays.class];
...
- (void)testClassWithRequiredLinkProperty {
  RLMAssertThrowsWithReasonMatching(
      [RLMObjectSchema schemaForObjectClass:RequiredLinkProperty.class],
      @"cannot be made required.*'object'");
}

- (void)testClassWithInvalidNSNumberProtocolProperty {
  RLMAssertThrowsWithReasonMatching(
      [RLMObjectSchema
          schemaForObjectClass:InvalidNSNumberProtocolObject.class],
      @"Property 'number' is of type \"NSNumber<NSFastEnumeration>\" which is "
      @"not a supported NSNumber object type.");
}

- (void)testClassWithInvalidNSNumberNoProtocolProperty {
  RLMAssertThrowsWithReasonMatching(
      [RLMObjectSchema
          schemaForObjectClass:InvalidNSNumberNoProtocolObject.class],
      @"Property 'number' requires a protocol defining the contained type");
}

- (void)testClassWithReadWriteLinkingObjectsProperty {
  RLMAssertThrowsWithReasonMatching(
      [RLMObjectSchema
          schemaForObjectClass:InvalidReadWriteLinkingObjectsProperty.class],
      @"Property 'linkingObjects' must be declared as readonly .* linking "
      @"objects");
}
...
- (void)testClassWithInvalidLinkingObjectsPropertiesMethod {
  RLMAssertThrowsWithReasonMatching(
      [RLMObjectSchema
          schemaForObjectClass:InvalidLinkingObjectsPropertiesMethod.class],
      @"Property 'linkingObjects' .* but \\+linkingObjectsProperties .* class "
      @"or property");
}

- (void)testClassWithLinkingObjectsPropertyWithProtocol {
  RLMObjectSchema *objectSchema = [RLMObjectSchema
      schemaForObjectClass:ValidLinkingObjectsPropertyWithProtocol.class];
  XCTAssertEqual(objectSchema[@"linkingObjects"].type,
...
- (void)testClassWithInvalidLinkingObjectsPropertyProtocol {
  RLMAssertThrowsWithReasonMatching(
      [RLMObjectSchema
          schemaForObjectClass:InvalidLinkingObjectsPropertyProtocol.class],
      @"Property 'linkingObjects' .* type "
      @"RLMLinkingObjects<NotARealClass>.*conflicting class "
      @"name.*'SchemaTestsLinkSource'");
}
...
- (void)testClassWithInvalidLinkingObjectsPropertyMissingSourcePropertyOfLink {
  ...
  RLMAssertThrowsWithReasonMatching(
      ...
      @"nosuchproperty' declared as origin of linking objects property "
      @"'InvalidLinkingObjectsPropertyMissingSourcePropertyOfLink."
      @"linkingObjects' does not exist");
}
...
- (void)testClassWithInvalidLinkingObjectsPropertySourcePropertyNotALink {
  ...
  config.customSchema = [RLMSchema
      schemaWithObjectClasses:
          @[ InvalidLinkingObjectsPropertySourcePropertyNotALink.class ]];
  RLMAssertThrowsWithReasonMatching(
      [RLMRealm realmWithConfiguration:config error:nil],
      @"Property 'InvalidLinkingObjectsPropertySourcePropertyNotALink.integer' "
      @"declared as origin of linking objects property "
      @"'InvalidLinkingObjectsPropertySourcePropertyNotALink.linkingObjects' "
      @"is not a link");
}

- (void)testClassWithInvalidLinkingObjectsPropertySourcePropertysLinkElsewhere {
  RLMRealmConfiguration *config = [RLMRealmConfiguration defaultConfiguration];
  config.customSchema = [RLMSchema schemaWithObjectClasses:@[
    InvalidLinkingObjectsPropertySourcePropertyLinksElsewhere.class,
    IntObject.class
  ]];
  RLMAssertThrowsWithReasonMatching(
      [RLMRealm realmWithConfiguration:config error:nil],
      @"Property "
      @"'InvalidLinkingObjectsPropertySourcePropertyLinksElsewhere.link' "
      @"declared as origin of linking objects property "
      @"'InvalidLinkingObjectsPropertySourcePropertyLinksElsewhere."
      @"linkingObjects' links to type 'IntObject'");
}

- (void)testMixedIsRejected {
  RLMRealmConfiguration *config = [RLMRealmConfiguration defaultConfiguration];
  RLMAssertThrowsWithReasonMatching(config.objectClasses =
                                        @[ [MixedProperty class] ],
...
- (void)testEmebeddedLinkingToNonEmbedded {
  ...
  config.objectClasses =
      @[ [LinkFromEmbeddedToTopLevel class], [IntObject class] ];
  XCTAssertNoThrow([RLMRealm realmWithConfiguration:config error:nil]);
  config.objectClasses =
      @[ [ArrayFromEmbeddedToTopLevel class], [IntObject class] ];
  XCTAssertNoThrow([RLMRealm realmWithConfiguration:config error:nil]);
}
...
- (void)testEmbeddedWithPrimaryKey {
  ...
  RLMAssertThrowsWithReason(
      [RLMRealm realmWithConfiguration:config error:nil],
      @"Embedded object type 'EmbeddedObjectWithPrimaryKey' cannot have a "
      @"primary key.");
}
...
- (void)testPartialSharedSchemaInit {
  ...
  @autoreleasepool {
    ...
    XCTAssertEqual(realm.schema.objectSchema.count,
                   [NSSet setWithArray:[realm.schema.objectSchema
...
- (void)testPartialSharedSchemaInitInheritance {
  ...
  RLMRealm *realm = [RLMRealm realmWithConfiguration:config error:nil];
  XCTAssertEqual(1U, realm.schema.objectSchema.count);
  XCTAssertEqualObjects(@"NumberObject",
                        [[[[NumberObject alloc] init] objectSchema] className]);
  // Verify that child class doesn't use the parent class's schema
  XCTAssertEqualObjects(
      @"NumberDefaultsObject",
      [[[[NumberDefaultsObject alloc] init] objectSchema] className]);
}
...
- (void)testCreateUnmanagedObjectWithNestedObjectWithUninitializedSchema {
  ...
  XCTAssertTrue(RLMSchema.partialSharedSchema.objectSchema.count == 0);
  XCTAssertNoThrow([[IntegerArrayPropertyObject alloc]
      initWithValue:(@[ @0, @[ @[ @0 ] ] ])]);
  XCTAssertNoThrow(
      [[NonDefaultArrayObject alloc] initWithValue:@[ @[ @[ @0 ] ] ]]);
...
- (void)testKVOSubclassesDoNotAppearInSchema {
  ...
  for (RLMObjectSchema *objectSchema in RLMRealm.defaultRealm.schema
           .objectSchema) {
    XCTAssertEqual([objectSchema.className rangeOfString:@"RLM:"].location,
                   NSNotFound);
  }
...
- (void)testMultipleProcessesTryingToInitializeSchema {
  ...
  if (!self.isParent) {
    RLMSchema *schema =
        [RLMSchema schemaWithObjectClasses:@[ IntObject.class ]];
  ...
  // Spawn a bunch of child processes which will all try to perform the
  // migration
  ...
  RLMNotificationToken *token =
      [syncRealm addNotificationBlock:^(NSString *, RLMRealm *) {
...
- (void)testAddingIndexToExistingColumnInBackgroundProcess {
  ...
  // Insert a value to ensure stuff actually happens when the index is
  // added/removed
  [realm beginWriteTransaction];
  [IntObject createInRealm:realm withValue:@[ @1 ]];
  [realm commitWriteTransaction];
...
- (void)testRemovingIndexFromExistingColumnInBackgroundProcess {
  if (!self.isParent) {
    RLMSchema *schema =
        [RLMSchema schemaWithObjectClasses:@[ IndexedStringObject.class ]];
    RLMObjectSchema *objectSchema = schema.objectSchema[0];
    RLMProperty *prop = objectSchema.properties[0];
    prop.indexed = NO;
...
- (void)testInsertingColumnsInBackgroundProcess {
  ...
  XCTAssertEqual(1U,
                 [IntObject objectsInRealm:realm where:@"intCol = 5"].count);
  ...
  [realm transactionWithBlock:^{
    io = [IntObject createInRealm:realm withValue:@[ @7 ]];
  }];
  XCTAssertEqual(io.intCol, 7);

  [realm transactionWithBlock:^{
    io = [IntObject createInRealm:realm withValue:@{@"intCol" : @8}];
  }];
...
- (void)testExplicitlyIncludedEmbeddedOrphanIsRejectedForSyncRealm {
  ...
  // Test each different order of setting properties because there's a bunch of
  // awkward state involved
  RLMRealmConfiguration *config =
      [user configurationWithPartitionValue:@"dummy"];
  config.objectClasses = @[ OrphanObject.class ];
  RLMAssertThrowsWithReason([RLMRealm realmWithConfiguration:config error:nil],
                            @"Embedded object 'OrphanObject' is unreachable by "
                            @"any link path from top level objects.");
