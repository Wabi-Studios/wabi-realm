...
- (void)testDeleteObjectsInRealm {
  % unman RLMAssertThrowsWithReason(
        [realm deleteObjects:$dictionary],
        @"Cannot delete objects from RLMDictionary");
  % man RLMAssertThrowsWithReason(
        [realm deleteObjects:$dictionary],
        @"Cannot delete objects from RLMManagedDictionary<RLMString, $type>: "
...
- (void)testSetObject {
  ...
  % noany % man %
      r RLMAssertThrowsWithReason($dictionary[$firstKey] = (id)NSNull.null,
                                  @"Invalid value '<null>' of type 'NSNull' "
  ...
  RLMAssertThrowsWithReason(
      [$dictionary setObject:$firstValue forKey:nil],
      ^n @"Invalid nil key for dictionary expecting key of type 'string'.");
  // Fail on set nil for non-optional
  % noany %
      r RLMAssertThrowsWithReason(
          [$dictionary setObject:(id)NSNull.null forKey:$firstKey],
          ^n
          @"Invalid value '<null>' of type 'NSNull' for expected type '$type'");

  % noany RLMAssertThrowsWithReason([$dictionary setObject:(id)$wrong
                                                    forKey:$firstKey],
                                    ^n
...
- (void)testAddObjects {
  % noany RLMAssertThrowsWithReason(
        [$dictionary addEntriesFromDictionary:@{$firstKey : $wrong}],
        ^n @"Invalid value '$wdesc' of type '" $wtype
            "' for expected type '$type'");
  % noany %
      r RLMAssertThrowsWithReason(
          [$dictionary addEntriesFromDictionary:@{$firstKey : (id)NSNull.null}],
          ^n
          @"Invalid value '<null>' of type 'NSNull' for expected type '$type'");
...
- (void)testIndexOfObjectSorted {
  ...
  % man %
      o uncheckedAssertEqual(
          0U, [[$dictionary sortedResultsUsingKeyPath:@"self"
                                            ascending:NO] indexOfObject:$v0]);
  % man %
      o uncheckedAssertEqual(1U, [[$dictionary sortedResultsUsingKeyPath:@"self"
                                                               ascending:NO]
                                     indexOfObject:(id)$v1]);
  % man %
      r uncheckedAssertEqual(
          1U, [[$dictionary sortedResultsUsingKeyPath:@"self"
                                            ascending:NO] indexOfObject:$v0]);
  % man %
      r uncheckedAssertEqual(
          0U, [[$dictionary sortedResultsUsingKeyPath:@"self"
...
- (void)testIndexOfObjectDistinct {
  ...
  % man %
      r uncheckedAssertEqual(
          1U, [[$dictionary distinctResultsUsingKeyPaths:@[ @"self" ]]
                  indexOfObject:$v0]);
  % man %
      r uncheckedAssertEqual(
          0U, [[$dictionary distinctResultsUsingKeyPaths:@[ @"self" ]]
                  indexOfObject:$v1]);
...
- (void)testSort {
  % unman RLMAssertThrowsWithReason(
        [$dictionary sortedResultsUsingKeyPath:@"self" ascending:NO],
        ^n @"This method may only be called on RLMDictionary instances "
  ...
  % unman RLMAssertThrowsWithReason(
        ...
        ^n @"This method may only be called on RLMDictionary instances "
           @"retrieved from an RLMRealm");
  % man RLMAssertThrowsWithReason(
        [$dictionary sortedResultsUsingKeyPath:@"not self" ascending:NO],
        ^n @"can only be sorted on 'self'");

  [$dictionary addEntriesFromDictionary:@{$k0 : $v0, $k1 : $v1}];

  % man uncheckedAssertEqualObjects(
        [[$dictionary sortedResultsUsingDescriptors:@[]] valueForKey:@"self"],
        ^n(@[ $v1, $v0 ]));

  % man %
      r uncheckedAssertEqualObjects(
          [[$dictionary sortedResultsUsingKeyPath:@"self"
...
- (void)testFilter {
  % unman RLMAssertThrowsWithReason(
        [$dictionary objectsWhere:@"TRUEPREDICATE"],
        ^n @"This method may only be called on RLMDictionary instances "
           @"retrieved from an RLMRealm");
  % unman RLMAssertThrowsWithReason(
        [$dictionary objectsWithPredicate:[NSPredicate predicateWithValue:YES]],
        ^n @"This method may only be called on RLMDictionary instances "
           @"retrieved from an RLMRealm");

  % man RLMAssertThrowsWithReason([$dictionary objectsWhere:@"TRUEPREDICATE"],
                                  ^n @"implemented");
  % man RLMAssertThrowsWithReason(
        [$dictionary objectsWithPredicate:[NSPredicate predicateWithValue:YES]],
        ^n @"implemented");
  ...
  % man RLMAssertThrowsWithReason(
        [[$dictionary sortedResultsUsingKeyPath:@"self" ascending:NO] ^ n
...
- (void)testMin {
  % noany % nominmax %
      unman RLMAssertThrowsWithReason(
          [$dictionary minOfProperty:@"self"],
          ^n @"minOfProperty: is not supported for $type dictionary");
  % noany % nominmax %
      man RLMAssertThrowsWithReason(
          [$dictionary minOfProperty:@"self"],
          ^n
          @"Operation 'min' not supported for $type dictionary '$class.$prop'");

  % minmax uncheckedAssertNil([$dictionary minOfProperty:@"self"]);

  [self addObjects];

  % minmax uncheckedAssertEqualObjects([$dictionary minOfProperty:@"self"],
                                       $v0);
}
...
- (void)testMax {
  % noany % nominmax %
      unman RLMAssertThrowsWithReason(
          [$dictionary maxOfProperty:@"self"],
          ^n @"maxOfProperty: is not supported for $type dictionary");
  % noany % nominmax %
      man RLMAssertThrowsWithReason(
          [$dictionary maxOfProperty:@"self"],
          ^n
          @"Operation 'max' not supported for $type dictionary '$class.$prop'");
  ...
  % r %
      minmax uncheckedAssertEqualObjects([$dictionary maxOfProperty:@"self"],
                                         $v1);
  % o %
      minmax uncheckedAssertEqualObjects([$dictionary maxOfProperty:@"self"],
                                         $v0);
...
- (void)testSum {
  % noany % nosum %
      unman RLMAssertThrowsWithReason(
          [$dictionary sumOfProperty:@"self"],
          ^n @"sumOfProperty: is not supported for $type dictionary");
  % noany % nosum %
      man RLMAssertThrowsWithReason(
          [$dictionary sumOfProperty:@"self"],
          ^n
          @"Operation 'sum' not supported for $type dictionary '$class.$prop'");
  ...
  % sum XCTAssertEqualWithAccuracy(
        [$dictionary sumOfProperty:@"self"].doubleValue, sum($values), .001);
}

- (void)testAverage {
  % noany % noavg %
      unman RLMAssertThrowsWithReason(
          [$dictionary averageOfProperty:@"self"],
          ^n @"averageOfProperty: is not supported for $type dictionary");
  % noany % noavg %
      man RLMAssertThrowsWithReason([$dictionary averageOfProperty:@"self"],
                                    ^n @"Operation 'average' not supported for "
                                       @"$type dictionary '$class.$prop'");
  ...
  % avg XCTAssertEqualWithAccuracy(
        [$dictionary averageOfProperty:@"self"].doubleValue, average($values),
        .001);
}
...
- (void)testFastEnumeration {
  ...
  ^{
    ^nl NSDictionary *values = $values;
    ^nl for (id key in $dictionary) {
      ^nl id value = $dictionary[key];
      ^nl uncheckedAssertEqualObjects(values[key], value);
      ^nl
    }
    ^nl uncheckedAssertEqual(values.count, $dictionary.count);
    ^nl
  }();
  ^nl
}
...
- (void)testValueForKeyNumericAggregates {
  ...
  % sum uncheckedAssertEqualObjects([$dictionary valueForKeyPath:@"@sum.self"],
                                    @0);
  % avg uncheckedAssertNil([$dictionary valueForKeyPath:@"@avg.self"]);

  [self addObjects];

  % minmax uncheckedAssertEqualObjects(
        [$dictionary valueForKeyPath:@"@min.self"], $v0);
  % r %
      minmax uncheckedAssertEqualObjects(
          [$dictionary valueForKeyPath:@"@max.self"], $v1);
  % o %
      minmax uncheckedAssertEqualObjects(
          [$dictionary valueForKeyPath:@"@max.self"], $v0);
  % sum XCTAssertEqualWithAccuracy(
        [[$dictionary valueForKeyPath:@"@sum.self"] doubleValue], sum($values),
        .001);
  % avg XCTAssertEqualWithAccuracy(
        [[$dictionary valueForKeyPath:@"@avg.self"] doubleValue],
        average($values), .001);
}

- (void)testSetValueForKey {
  % noany RLMAssertThrowsWithReason([$dictionary setValue:$wrong forKey:$k0],
                                    ^n
                                    @"Invalid value '$wdesc' of type '" $wtype
                                     "' for expected type '$type'");
  % noany %
      r RLMAssertThrowsWithReason(
          [$dictionary setValue:(id)NSNull.null forKey:@"self"],
          ^n
          @"Invalid value '<null>' of type 'NSNull' for expected type '$type'");
...
- (void)testAssignment {
  $dictionary = (id) @{$k1 : $v1};
  ^nl uncheckedAssertEqualObjects($dictionary[$k1], $v1);
...
- (void)testQueryBasicOperators {
  ...
  % string % noany % man %
      nominmax RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"ANY $prop > %@", $v0]),
          ^n @"Operator '>' not supported for string queries on Dictionary.");
  % any % string % man %
      nominmax RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"ANY $prop > %@", $v0]),
          ^n @"Operator '>' not supported for string queries on Dictionary.");
...
- (void)testQueryBasicOperatorsOverLink {
  ...
  % o % man %
      minmax RLMAssertCount(LinkTo$class, 0, @"ANY link.$prop < %@", $v1);
  % man % minmax RLMAssertCount(LinkTo$class, 1, @"ANY link.$prop <= %@", $v0);
