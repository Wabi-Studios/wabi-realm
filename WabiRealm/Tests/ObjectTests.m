...
- (void)testKeyedSubscripting {
  EmployeeObject *objs = [[EmployeeObject alloc]
      initWithValue:@{@"name" : @"Test0", @"age" : @23, @"hired" : @NO}];
  ...
  EmployeeObject *obj0 = [EmployeeObject
      createInRealm:realm
          withValue:@{@"name" : @"Test1", @"age" : @24, @"hired" : @NO}];
  EmployeeObject *obj1 = [EmployeeObject
      createInRealm:realm
          withValue:@{@"name" : @"Test2", @"age" : @25, @"hired" : @YES}];
...
- (void)testCannotUpdatePrimaryKey {
  ...
  RLMAssertThrowsWithReason(intObj[@"intCol"] = @1,
                            @"Primary key can't be changed");
  RLMAssertThrowsWithReason([intObj setValue:@1 forKey:@"intCol"],
                            @"Primary key can't be changed");

  [realm addObject:stringObj];

  RLMAssertThrowsWithReason(stringObj.stringCol = @"a",
                            @"Primary key can't be changed");
  RLMAssertThrowsWithReason(stringObj[@"stringCol"] = @"a",
                            @"Primary key can't be changed");
  RLMAssertThrowsWithReason([stringObj setValue:@"a" forKey:@"stringCol"],
                            @"Primary key can't be changed");
...
- (void)testDataTypes {
  ...
  [AllTypesObject
      createInRealm:realm
          withValue:@[
            @YES, @506, @7.7f, @8.8, @"banach", bin2, timeNow, @YES, @(-20),
            [RLMDecimal128 decimalWithNumber:@2], objectId2,
            [[NSUUID alloc]
                initWithUUIDString:@"137DECC8-B300-4954-A233-F89909F4FD89"],
            NSNull.null
          ]];
  ...
  XCTAssertTrue([row1.uuidCol.UUIDString
                    isEqualToString:@"137DECC8-B300-4954-A233-F89909F4FD89"],
                @"row1.uuidCol");
  XCTAssertTrue([row2.uuidCol.UUIDString
                    isEqualToString:@"137DECC8-B300-4954-A233-F89909F4FD89"],
                @"row2.uuidCol");
...
- (void)testObjectSubclass {
  ...
  XCTAssertEqualObjects(@"StringSubclassObject",
                        [StringSubclassObject className]);

  RLMRealm *realm = [RLMRealm defaultRealm];
  [realm beginWriteTransaction];
  [StringObject createInDefaultRealmWithValue:@[ @"string" ]];
  StringSubclassObject *obj = [StringSubclassObject
      createInDefaultRealmWithValue:@[ @"string", @"string2" ]];
  ...
  [realm beginWriteTransaction];
  StringLinkObject *linkObject =
      [StringLinkObject createInDefaultRealmWithValue:@[ NSNull.null, @[] ]];
  RLMAssertThrowsWithReasonMatching(
      linkObject.stringObjectCol = obj,
      @"Can't .*StringSubclassObject.*StringObject");
  RLMAssertThrowsWithReasonMatching(
      [linkObject.stringObjectArrayCol addObject:obj],
...
- (void)testDateDistantFuture {
  RLMRealm *realm = [RLMRealm defaultRealm];
  [realm beginWriteTransaction];
  DateObject *dateObject = [DateObject createInRealm:realm
                                           withValue:@[ NSDate.distantFuture ]];
  [realm commitWriteTransaction];
...
- (void)testDate50kYears {
  NSCalendarUnit units =
      (NSCalendarUnit)(NSCalendarUnitMonth | NSCalendarUnitYear |
                       NSCalendarUnitDay);
  ...
  [realm beginWriteTransaction];
  DateObject *dateObject = [DateObject createInRealm:realm
                                           withValue:@[ components.date ]];
  [realm commitWriteTransaction];

  XCTAssertEqualObjects(components.date, dateObject.dateCol);
}

static void testDatesInRange(NSTimeInterval from, NSTimeInterval to,
                             void (^check)(NSDate *, NSDate *)) {
...
- (void)testExactRepresentationOfDatesAroundReferenceDate {
  ...
  dateObject.dateCol =
      [NSDate dateWithTimeIntervalSinceReferenceDate:nextafter(1e-9, -DBL_MAX)];
  XCTAssertEqualObjects(dateObject.dateCol, zero);

  // Very slightly over 1ns (since 1e-9 can't be exactly represented by a
  // double)
  ...
  dateObject.dateCol =
      [NSDate dateWithTimeIntervalSinceReferenceDate:nextafter(0, -DBL_MAX)];
  XCTAssertEqualObjects(dateObject.dateCol, zero);
  dateObject.dateCol =
      [NSDate dateWithTimeIntervalSinceReferenceDate:nextafter(-1e-9, DBL_MAX)];
  XCTAssertEqualObjects(dateObject.dateCol, zero);
...
- (void)testDatesOutsideOfTimestampRange {
  NSDate *date = [NSDate date];
  NSDate *maxDate =
      [NSDate dateWithTimeIntervalSince1970:(double)(1ULL << 63) + .999999999];
  NSDate *minDate =
      [NSDate dateWithTimeIntervalSince1970:-(double)(1ULL << 63) - .999999999];
  NSDate *justOverMaxDate = [NSDate
      dateWithTimeIntervalSince1970:nextafter(maxDate.timeIntervalSince1970,
                                              DBL_MAX)];
  NSDate *justUnderMaxDate = [NSDate
      dateWithTimeIntervalSince1970:nextafter(maxDate.timeIntervalSince1970,
                                              -DBL_MAX)];
  NSDate *justOverMinDate = [NSDate
      dateWithTimeIntervalSince1970:nextafter(minDate.timeIntervalSince1970,
                                              DBL_MAX)];
  NSDate *justUnderMinDate = [NSDate
      dateWithTimeIntervalSince1970:nextafter(minDate.timeIntervalSince1970,
                                              -DBL_MAX)];
  ...
  dateObject.dateCol =
      [NSDate dateWithTimeIntervalSinceReferenceDate:0.0 / 0.0];
  XCTAssertEqualObjects(dateObject.dateCol,
  ...
  XCTAssertEqualObjects(dateObject.dateCol, maxDate);
  dateObject.dateCol =
      [NSDate dateWithTimeIntervalSinceReferenceDate:1.0 / 0.0];
  XCTAssertEqualObjects(dateObject.dateCol, maxDate);
...
- (void)testDataSizeLimits {
  ...
  obj.data1 = obj.data2 = [NSData dataWithBytesNoCopy:buffer
                                               length:maxSize
                                         freeWhenDone:YES];
  ...
  XCTAssertTrue(strcmp((const char *)obj.data1.bytes + obj.data1.length -
                           sizeof("hello") - 1,
                       "hello") == 0);
  XCTAssertTrue(strcmp((const char *)obj.data2.bytes + obj.data2.length -
                           sizeof("hello") - 1,
                       "hello") == 0);

  // A blob over 16 MB should throw (and not crash)
  [realm beginWriteTransaction];
  RLMAssertThrowsWithReason(obj.data1 =
                                [NSData dataWithBytesNoCopy:malloc(maxSize + 1)
                                                     length:maxSize + 1
                                               freeWhenDone:YES],
                            @"Binary too big");
  [realm commitWriteTransaction];
...
- (void)testStringSizeLimits {
  ...
  // A blob over 16 MB should throw (and not crash)
  [realm beginWriteTransaction];
  XCTAssertThrows(obj.stringCol = [[NSString alloc]
                      initWithBytesNoCopy:calloc(maxSize + 1, 1)
                                   length:maxSize + 1
                                 encoding:NSUTF8StringEncoding
                             freeWhenDone:YES]);
  [realm commitWriteTransaction];
...
- (void)testAddingObjectNotInSchemaThrows {
  ...
  [realm beginWriteTransaction];
  RLMAssertThrowsWithReasonMatching(
      [realm addObject:[[IntObject alloc] initWithValue:@[ @1 ]]],
      @"Object type 'IntObject' is not managed by the WabiRealm.*custom "
      @"`objectClasses`");
  RLMAssertThrowsWithReasonMatching(
      ...
      @"`objectClasses`");
  XCTAssertNoThrow(
      [realm addObject:[[StringObject alloc] initWithValue:@[ @"A" ]]]);
  XCTAssertNoThrow([StringObject createInRealm:realm withValue:@[ @"A" ]]);
  [realm cancelWriteTransaction];
}

static void addProperty(Class cls, const char *name, const char *type,
                        size_t size, size_t align, id getter) {
  objc_property_attribute_t objectColAttrs[] = {
      {"T", type},
      {"V", name},
  };
  class_addIvar(cls, name, size, align, type);
  class_addProperty(cls, name, objectColAttrs,
                    sizeof(objectColAttrs) / sizeof(objc_property_attribute_t));

  char encoding[4] = " @:";
  encoding[0] = *type;
  class_addMethod(cls, sel_registerName(name),
                  imp_implementationWithBlock(getter), encoding);
}

- (void)testObjectSubclassAddedAtRuntime {
  Class objectClass =
      objc_allocateClassPair(RLMObject.class, "RuntimeGeneratedObject", 0);
  addProperty(objectClass, "objectCol", "@\"RuntimeGeneratedObject\"",
              sizeof(id), alignof(id), ^(__unused id obj) {
                return nil;
              });
  addProperty(objectClass, "intCol", "i", sizeof(int), alignof(int),
              ^int(__unused id obj) {
                return 0;
              });
  ...
  RLMRealm *realm = [RLMRealm realmWithConfiguration:configuration error:nil];
  [realm beginWriteTransaction];
  id object = [objectClass createInRealm:realm
                               withValue:@{
                                 @"objectCol" : [[objectClass alloc] init],
                                 @"intCol" : @17
                               }];
    ...
    if (prop.optional) {
      ...
      for (NSUInteger j = 0; j < keys.count; ++j) {
        ...
        if ([key isEqualToString:key2]) {
          XCTAssertEqualObjects(object[key2],
                                prop.optional ? nil : defaultValues[key2]);
        } else {
...
- (void)testDynamicDefaultPropertyValues {
  void (^assertDifferentPropertyValues)(DynamicDefaultObject *,
                                        DynamicDefaultObject *) =
      ^(DynamicDefaultObject *obj1, DynamicDefaultObject *obj2) {
        ...
        XCTAssertNotEqualWithAccuracy(
            ...
            obj2.dateCol.timeIntervalSinceReferenceDate, 0.01f);
        XCTAssertNotEqualObjects(obj1.stringCol, obj2.stringCol);
        XCTAssertNotEqualObjects(obj1.binaryCol, obj2.binaryCol);
      };
  assertDifferentPropertyValues([[DynamicDefaultObject alloc] init],
                                [[DynamicDefaultObject alloc] init]);
  RLMRealmConfiguration *configuration =
      [RLMRealmConfiguration defaultConfiguration];
  configuration.objectClasses = @[ [DynamicDefaultObject class] ];
  RLMRealm *realm = [RLMRealm realmWithConfiguration:configuration error:nil];
  [realm beginWriteTransaction];
  assertDifferentPropertyValues([DynamicDefaultObject createInRealm:realm
                                                          withValue:@{}],
                                [DynamicDefaultObject createInRealm:realm
                                                          withValue:@{}]);
  [realm cancelWriteTransaction];
}
...
- (void)testCanUseIgnoredProperty {
  ...
  obj.url = url;
  [realm addObject:obj];
  XCTAssertEqual(obj.url, url,
                 @"ignored properties should still be assignable and gettable "
  ...
  XCTAssertEqual(obj.url, url,
                 @"ignored properties should still be assignable and gettable "
                 @"outside a write block");

  IgnoredURLObject *obj2 =
      [[IgnoredURLObject objectsWithPredicate:nil] firstObject];
  XCTAssertNotNil(obj2, @"object with ignored property should still be stored "
                        @"and accessible through the realm");

  XCTAssertEqualObjects(obj2.name, obj.name,
                        @"managed property should be the same");
  XCTAssertNil(obj2.url,
               @"ignored property should be nil when getting from realm");
}
...
- (void)testCreateInRealmValidationForDictionary {
  ...
  NSDictionary *dictValidAllTypes = [AllTypesObject values:0
                                              stringObject:nil
                                               mixedObject:nil];
  [realm beginWriteTransaction];
  ...
  for (NSString *keyToInvalidate in dictValidAllTypes) {
    ...
    RLMAssertThrowsWithReasonMatching(
        [AllTypesObject createInRealm:realm withValue:invalidInput],
        @"Invalid value '.*'");
  }
...
- (void)testCreateInRealmValidationForArray {
  ...
  NSData *bin1 = [[NSData alloc] initWithBytes:bin length:sizeof bin / 2];
  NSDate *timeNow = [NSDate dateWithTimeIntervalSince1970:1000000];
  NSArray *const arrayValidAllTypes = @[
    @NO, @54, @0.7f, @0.8, @"foo", bin1, timeNow, @NO, @(99),
    [RLMDecimal128 decimalWithNumber:@2], [RLMObjectId objectId],
  ...
  // Test NSArray
  XCTAssertNoThrow(
      ([AllTypesObject createInRealm:realm withValue:arrayValidAllTypes]),
      @"Creating object with valid value types should not throw exception");
...
- (void)testCreateInRealmReusesExistingObjects {
  ...
  OwnerObject *owner =
      [OwnerObject createInDefaultRealmWithValue:@[ @"name", dog ]];
  XCTAssertTrue([owner.dog isEqualToObject:dog]);
  XCTAssertEqual(1U, DogObject.allObjects.count);

  DogArrayObject *dogArray =
      [DogArrayObject createInDefaultRealmWithValue:@[ @[ dog ] ]];
...
- (void)testCreateInRealmReusesExistingNestedObjectsByPrimaryKey {
  ...
  [realm beginWriteTransaction];
  PrimaryEmployeeObject *eo =
      [PrimaryEmployeeObject createInRealm:realm
                                 withValue:@[ @"Samuel", @19, @NO ]];
  PrimaryCompanyObject *co = [PrimaryCompanyObject
      createInRealm:realm
          withValue:@[
            @"WabiRealm", @[ eo ], @[ eo ], @{@"key" : eo}, eo, @[ eo ]
          ]];
  [realm commitWriteTransaction];

  [realm beginWriteTransaction];
  [PrimaryCompanyObject
  ...
  [realm beginWriteTransaction];
  [PrimaryCompanyObject createOrUpdateInRealm:realm
                                    withValue:@{
                                      @"name" : @"WabiRealm",
                                      @"wrappedIntern" : @[ eo ]
                                    }];
...
- (void)testCreateInRealmCopiesFromOtherRealm {
  ...
  DogObject *dog = [DogObject createInDefaultRealmWithValue:@[ @"Fido", @5 ]];
  OwnerObject *owner = [OwnerObject createInRealm:realm2
                                        withValue:@[ @"name", dog ]];
  XCTAssertFalse([owner.dog isEqualToObject:dog]);
  XCTAssertEqual(1U, DogObject.allObjects.count);
  XCTAssertEqual(1U, [DogObject allObjectsInRealm:realm2].count);

  DogArrayObject *dogArray = [DogArrayObject createInRealm:realm2
                                                 withValue:@[ @[ dog ] ]];
...
- (void)testCreateInRealmWithOtherObjects {
  RLMRealm *realm = [RLMRealm defaultRealm];
  [realm beginWriteTransaction];
  DateObjectNoThrow *object = [DateObjectNoThrow
      createInDefaultRealmWithValue:@[ NSDate.date, NSDate.date ]];
...
- (void)testObjectDescription {
  ...
  void (^descriptionAsserts)(NSString *) = ^(NSString *description) {
    XCTAssertTrue([description rangeOfString:@"name"].location != NSNotFound,
                  @"column names should be displayed when calling "
                  @"\"description\" on RLMObject subclasses");
    XCTAssertTrue([description rangeOfString:@"Peter"].location != NSNotFound,
                  @"column values should be displayed when calling "
                  @"\"description\" on RLMObject subclasses");

    XCTAssertTrue([description rangeOfString:@"age"].location != NSNotFound,
                  @"column names should be displayed when calling "
                  @"\"description\" on RLMObject subclasses");
    XCTAssertTrue([description rangeOfString:[@30 description]].location !=
                      NSNotFound,
                  @"column values should be displayed when calling "
                  @"\"description\" on RLMObject subclasses");
    ...
    XCTAssertTrue([description rangeOfString:@"hired"].location != NSNotFound,
                  @"column names should be displayed when calling "
                  @"\"description\" on RLMObject subclasses");
    XCTAssertTrue([description rangeOfString:[@YES description]].location !=
                      NSNotFound,
                  @"column values should be displayed when calling "
                  @"\"description\" on RLMObject subclasses");
  };
  ...
  NSString *objDescription =
      [[[EmployeeObject objectsWithPredicate:nil] firstObject] description];
  descriptionAsserts(objDescription);

  soInit = [[EmployeeObject alloc] init];
  soInit.age = 20;
  XCTAssert([soInit.description rangeOfString:@"(null)"].location !=
            NSNotFound);
}
...
- (void)testDataObjectDescription {
  ...
  [realm beginWriteTransaction];
  char longData[200];
  [DataObject createInRealm:realm
                  withValue:@[
                    [NSData dataWithBytes:&longData length:200],
                    [NSData dataWithBytes:&longData length:2]
                  ]];
  [realm commitWriteTransaction];

  DataObject *obj = [DataObject allObjectsInRealm:realm].firstObject;
  XCTAssertTrue([obj.description rangeOfString:@"200 total bytes"].location !=
                NSNotFound);
  XCTAssertTrue([obj.description rangeOfString:@"2 total bytes"].location !=
                NSNotFound);
}
...
- (void)testDeletedObjectDescription {
  ...
  [realm beginWriteTransaction];
  EmployeeObject *obj = [EmployeeObject createInRealm:realm
                                            withValue:@[ @"Peter", @30, @YES ]];
...
- (void)testEquality {
  ...
  XCTAssertFalse([obj isEqual:[NSObject new]],
                 @"Comparing an RLMObject to a non-RLMObject should be false.");
  XCTAssertFalse([obj isEqualToObject:(RLMObject *)[NSObject new]],
                 @"Comparing an RLMObject to a non-RLMObject should be false.");
  XCTAssertTrue([obj isEqual:obj], @"Same instance.");
  XCTAssertTrue([obj isEqualToObject:obj], @"Same instance.");
  XCTAssertFalse([obj isEqualToObject:otherObj],
                 @"Comparison outside of realm.");
  ...
  XCTAssertFalse([obj isEqualToObject:otherObj],
                 @"One in realm, the other is not.");
  XCTAssertTrue([obj isEqualToObject:[IntObject allObjects][0]],
  ...
  XCTAssertFalse([obj isEqualToObject:[IntObject allObjects][1]],
                 @"Same table, different index.");
  XCTAssertFalse([obj isEqualToObject:[BoolObject allObjects][0]],
                 @"Different tables.");
}
...
- (void)testCrossThreadAccess {
  ...
  [self dispatchAsyncAndWait:^{
    RLMAssertThrowsWithReason(obj.intCol, @"incorrect thread");
  }];
}
...
- (void)testIsDeleted {
  ...
  // Delete as result of query since then obj2's realm could point to a
  // different instance
  ...
  RLMAssertThrowsWithReason([ArrayPropertyObject createInRealm:realm
                                                     withValue:propObject],
                            @"deleted or invalidated");

  [realm commitWriteTransaction];

  XCTAssertEqual(obj1.invalidated, YES);
  XCTAssertNil(obj1.realm, @"WabiRealm should be nil after deletion");
}
...
- (void)testPrimaryKey {
  ...
  [PrimaryNullableIntObject
      createInDefaultRealmWithValue:(@{@"optIntCol" : @2, @"value" : @0})];
  [PrimaryNullableIntObject createInDefaultRealmWithValue:@[ NSNull.null ]];
  RLMAssertThrowsWithReason(
      [PrimaryNullableIntObject createInDefaultRealmWithValue:(@[ @1, @0 ])],
      @"existing primary key value");
  RLMAssertThrowsWithReason(
      [PrimaryNullableIntObject
          createInDefaultRealmWithValue:(@[ NSNull.null, @0 ])],
      @"existing primary key value");
...
- (void)testCreateOrUpdate {
  ...
  [PrimaryNullableStringObject
      createOrUpdateInRealm:realm
                  withValue:@{@"stringCol" : @"string2", @"intCol" : @2}];
  ...
  [PrimaryNullableStringObject
      createOrUpdateInDefaultRealmWithValue:@[ @"string", @3 ]];
...
- (void)testCreateOrUpdateNestedObjects {
  ...
  [PrimaryNestedObject createOrUpdateInDefaultRealmWithValue:@[
    @0, @[ @"string", @1 ], @[ @[ @"string", @1 ] ], @[ @"string" ],
    @[ @[ @1 ] ], @[ @[ @1 ] ], @""
  ]];
  XCTAssertEqual([[PrimaryNestedObject allObjects] count], 1U,
                 @"Should have 1 object");
  XCTAssertEqual([[PrimaryStringObject allObjects] count], 1U,
                 @"Should have 1 object");
  XCTAssertEqual([[PrimaryIntObject allObjects] count], 1U,
                 @"Should have 1 object");
  ...
  XCTAssertEqual([[PrimaryStringObject allObjects] count], 1U,
                 @"Should have 1 object");
  XCTAssertEqual([PrimaryStringObject.allObjects.lastObject intCol], 2,
                 @"intCol should be 2");
  XCTAssertEqualObjects([PrimaryNestedObject.allObjects.lastObject stringCol],
                        @"", @"stringCol should not have been updated");
  XCTAssertEqual(
      1U, [PrimaryNestedObject.allObjects.lastObject primaryIntArray].count,
      @"intArray should not have been overwritten");
  XCTAssertEqual(
      1U, [PrimaryNestedObject.allObjects.lastObject primaryIntSet].count,
      @"intSet should not have been overwritten");
  XCTAssertEqual([[StringObject allObjects] count], 2U,
  ...
  XCTAssertEqual(2, obj.primaryStringObject.intCol,
                 @"primaryStringObject should not have changed");
  XCTAssertEqualObjects(obj.stringCol, @"updated",
                        @"stringCol should have been updated");
  XCTAssertEqual(0U, obj.primaryIntArray.count,
                 @"intArray should not have been emptied");
  XCTAssertEqual(0U, obj.primaryIntSet.count,
                 @"intSet should not have been emptied");
  XCTAssertNil(obj.stringObject, @"stringObject should be nil");

  // inserting new object should update nested
  obj = [PrimaryNestedObject createOrUpdateInDefaultRealmWithValue:@[
    @1, @[ @"string", @3 ], @[ @[ @"string", @3 ] ], @[ @"string" ], @[], @[],
    @""
  ]];
  XCTAssertEqual([[PrimaryNestedObject allObjects] count], 2U,
                 @"Should have 2 objects");
  XCTAssertEqual([[PrimaryStringObject allObjects] count], 1U,
                 @"Should have 1 object");
  XCTAssertEqual([(PrimaryStringObject *)[[PrimaryStringObject allObjects]
                     lastObject] intCol],
                 3, @"intCol should be 3");

  // test addOrUpdateObject
  obj.primaryStringObject =
      [PrimaryStringObject createInDefaultRealmWithValue:@[ @"string2", @1 ]];
  PrimaryNestedObject *obj1 = [[PrimaryNestedObject alloc] initWithValue:@[
    @1, @[ @"string2", @4 ], @[ @[ @"string2", @4 ] ], @[ @"string" ],
    @[ @[ @1 ], @[ @2 ] ], @[ @[ @1 ], @[ @2 ] ], @""
  ]];
  [realm addOrUpdateObject:obj1];
  XCTAssertEqual([[PrimaryNestedObject allObjects] count], 2U,
                 @"Should have 2 objects");
  XCTAssertEqual([[PrimaryStringObject allObjects] count], 2U,
                 @"Should have 2 objects");
  XCTAssertEqual([[PrimaryIntObject allObjects] count], 2U,
                 @"Should have 2 objects");
  XCTAssertEqual([(PrimaryStringObject *)[[PrimaryStringObject allObjects]
                     lastObject] intCol],
                 4, @"intCol should be 4");
...
- (void)testCreateOrUpdateWithReorderedColumns {
  @autoreleasepool {
    // Create a WabiRealm file with the properties in reverse order
  ...
  XCTAssertEqual(
      [PrimaryStringObject objectInRealm:realm forPrimaryKey:@"a"].intCol, 5);

  // Values in array are used in property declaration order, not table column
  // order
  [PrimaryStringObject createOrUpdateInRealm:realm withValue:@[ @"a", @6 ]];
  XCTAssertEqual(
      [PrimaryStringObject objectInRealm:realm forPrimaryKey:@"a"].intCol, 6);

  [PrimaryStringObject
      createOrUpdateInRealm:realm
                  withValue:@{@"stringCol" : @"a", @"intCol" : @7}];
  XCTAssertEqual(
      [PrimaryStringObject objectInRealm:realm forPrimaryKey:@"a"].intCol, 7);
  [realm commitWriteTransaction];
}
...
- (void)testObjectInSet {
  ...
  // set object with primary and non primary keys as they both override isEqual
  // and hash
  ...
  // primary key objects should match even with duplicate instances of the same
  // object
  XCTAssertTrue([dict containsObject:obj]);
  XCTAssertTrue(
      [dict containsObject:[[PrimaryStringObject allObjects] firstObject]]);

  // non-primary key objects should only match when comparing identical
  // instances
...
- (void)testObjectForKey {
  [RLMRealm.defaultRealm beginWriteTransaction];
  PrimaryStringObject *strObj =
      [PrimaryStringObject createInDefaultRealmWithValue:@[ @"key", @0 ]];
  PrimaryNullableStringObject *nullStrObj = [PrimaryNullableStringObject
      createInDefaultRealmWithValue:@[ NSNull.null, @0 ]];
  PrimaryIntObject *intObj =
      [PrimaryIntObject createInDefaultRealmWithValue:@[ @0 ]];
  PrimaryNullableIntObject *nonNullIntObj =
      [PrimaryNullableIntObject createInDefaultRealmWithValue:@[ @0 ]];
  PrimaryNullableIntObject *nullIntObj =
      [PrimaryNullableIntObject createInDefaultRealmWithValue:@[ NSNull.null ]];
  [RLMRealm.defaultRealm commitWriteTransaction];
  ...
  // wrong PK type
  RLMAssertThrowsWithReasonMatching(
      [PrimaryStringObject objectForPrimaryKey:@0],
      @"Invalid value '0' of type '.*Number.*' for 'string' property "
      @"'PrimaryStringObject.stringCol'.");
  RLMAssertThrowsWithReasonMatching(
      [PrimaryStringObject objectForPrimaryKey:@[]],
      @"of type '.*Array.*' for 'string' property "
      @"'PrimaryStringObject.stringCol'.");
  RLMAssertThrowsWithReasonMatching(
      [PrimaryIntObject objectForPrimaryKey:@""],
      @"Invalid value '' of type '.*String.*' for 'int' property "
      @"'PrimaryIntObject.intCol'.");
  RLMAssertThrowsWithReason([PrimaryIntObject objectForPrimaryKey:NSNull.null],
                            @"Invalid value '<null>' of type 'NSNull' for "
                            @"'int' property 'PrimaryIntObject.intCol'.");
  RLMAssertThrowsWithReason([PrimaryIntObject objectForPrimaryKey:nil],
                            @"Invalid value '(null)' of type '(null)' for "
                            @"'int' property 'PrimaryIntObject.intCol'.");
  RLMAssertThrowsWithReason(
      [PrimaryStringObject objectForPrimaryKey:NSNull.null],
      @"Invalid value '<null>' of type 'NSNull' for 'string' property "
      @"'PrimaryStringObject.stringCol'.");
  RLMAssertThrowsWithReason(
      [PrimaryStringObject objectForPrimaryKey:nil],
      @"Invalid value '(null)' of type '(null)' for 'string' property "
      @"'PrimaryStringObject.stringCol'.");
  ...
  XCTAssertEqualObjects(strObj,
                        [PrimaryStringObject objectForPrimaryKey:@"key"]);
  XCTAssertEqualObjects(nullStrObj, [PrimaryNullableStringObject
                                        objectForPrimaryKey:NSNull.null]);
  XCTAssertEqualObjects(nullStrObj,
                        [PrimaryNullableStringObject objectForPrimaryKey:nil]);
  XCTAssertEqualObjects(intObj, [PrimaryIntObject objectForPrimaryKey:@0]);
  XCTAssertEqualObjects(nonNullIntObj,
                        [PrimaryNullableIntObject objectForPrimaryKey:@0]);
  XCTAssertEqualObjects(
      nullIntObj, [PrimaryNullableIntObject objectForPrimaryKey:NSNull.null]);
  XCTAssertEqualObjects(nullIntObj,
                        [PrimaryNullableIntObject objectForPrimaryKey:nil]);

  // nil realm throws
  RLMAssertThrowsWithReason([PrimaryIntObject objectInRealm:self.nonLiteralNil
                                              forPrimaryKey:@0],
...
- (void)testClassExtension {
  ...
  BaseClassStringObject *objectFromRealm =
      [BaseClassStringObject allObjects][0];
...
- (void)testFreezeInsideWriteTransaction {
  ...
  RLMAssertThrowsWithReason([obj freeze],
                            @"Cannot freeze an object in the same write "
                            @"transaction as it was created in.");
  [realm commitWriteTransaction];
...
- (void)testThawPreviousVersion {
  ...
  obj.intCol = 1;
  [realm commitWriteTransaction];
  XCTAssertNotEqual(obj.intCol, frozen.intCol,
                    @"Frozen object shouldn't mutate");

  IntObject *thawed = [frozen thaw];
  XCTAssertFalse(thawed.frozen);
  XCTAssertEqual(thawed.intCol, obj.intCol,
                 @"Thawed object should reflect transactions since the "
                 @"original reference was frozen.");
}

- (void)testThawUpdatedOnDifferentThread {
  IntObject *obj = managedObject();
  RLMThreadSafeReference *tsr =
      [RLMThreadSafeReference referenceWithThreadConfined:obj];
  ...
  IntObject *thawed = [frozen thaw];
  XCTAssertEqual(thawed.intCol, 0,
                 @"Thaw shouldn't reflect background transactions until main "
                 @"thread realm is refreshed");
