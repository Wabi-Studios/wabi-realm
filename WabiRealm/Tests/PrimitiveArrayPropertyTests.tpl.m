...
- (void)testDeleteObjectsInRealm {
  RLMAssertThrowsWithReason([realm deleteObjects:$allArrays],
...
- (void)testAddObject {
  % noany RLMAssertThrowsWithReason(
        [$array addObject:$wrong], ^n @"Invalid value '$wdesc' of type '" $wtype
                                       "' for expected type '$type'");
  % noany %
      r RLMAssertThrowsWithReason(
          [$array addObject:NSNull.null],
          ^n
          @"Invalid value '<null>' of type 'NSNull' for expected type '$type'");
...
- (void)testAddObjects {
  % noany RLMAssertThrowsWithReason([$array addObjects:@[ $wrong ]],
                                    ^n
                                    @"Invalid value '$wdesc' of type '" $wtype
                                     "' for expected type '$type'");
  % noany %
      r RLMAssertThrowsWithReason(
          [$array addObjects:@[ NSNull.null ]],
          ^n
          @"Invalid value '<null>' of type 'NSNull' for expected type '$type'");
...
- (void)testInsertObject {
  % noany RLMAssertThrowsWithReason([$array insertObject:$wrong atIndex:0],
                                    ^n
                                    @"Invalid value '$wdesc' of type '" $wtype
                                     "' for expected type '$type'");
  % noany %
      r RLMAssertThrowsWithReason(
          ...
          ^n
          @"Invalid value '<null>' of type 'NSNull' for expected type '$type'");
  RLMAssertThrowsWithReason([$array insertObject:$v0 atIndex:1],
                            ^n
...
- (void)testRemoveObject {
  RLMAssertThrowsWithReason([$allArrays removeObjectAtIndex:0],
                            ^n
                            @"Index 0 is out of bounds (must be less than 0).");
  ...
  % r RLMAssertThrowsWithReason(
        ...
        ^n @"Index 2 is out of bounds (must be less than 2).");
  % o RLMAssertThrowsWithReason(
        [$array removeObjectAtIndex:3],
        ^n @"Index 3 is out of bounds (must be less than 3).");
...
- (void)testReplace {
  RLMAssertThrowsWithReason([$array replaceObjectAtIndex:0 withObject:$v0],
                            ^n
                            @"Index 0 is out of bounds (must be less than 0).");

  [$array addObject:$v0];
  ^nl [$array replaceObjectAtIndex:0 withObject:$v1];
  ^nl uncheckedAssertEqualObjects($array[0], $v1);
  ^nl

      % o [$array replaceObjectAtIndex:0 withObject:NSNull.null];
  ^nl uncheckedAssertEqualObjects($array[0], NSNull.null);

  % noany RLMAssertThrowsWithReason([$array replaceObjectAtIndex:0
                                                      withObject:$wrong],
                                    ^n
                                    @"Invalid value '$wdesc' of type '" $wtype
                                     "' for expected type '$type'");
...
- (void)testMove {
  RLMAssertThrowsWithReason([$allArrays moveObjectAtIndex:0 toIndex:1],
                            ^n
                            @"Index 0 is out of bounds (must be less than 0).");
  RLMAssertThrowsWithReason([$allArrays moveObjectAtIndex:1 toIndex:0],
                            ^n
...
- (void)testExchange {
  RLMAssertThrowsWithReason(
      [$allArrays exchangeObjectAtIndex:0 withObjectAtIndex:1],
      ^n @"Index 0 is out of bounds (must be less than 0).");
  RLMAssertThrowsWithReason(
      [$allArrays exchangeObjectAtIndex:1 withObjectAtIndex:0],
      ^n @"Index 1 is out of bounds (must be less than 0).");
...
- (void)testIndexOfObject {
  ...
  % noany RLMAssertThrowsWithReason([$array indexOfObject:$wrong],
                                    ^n
                                    @"Invalid value '$wdesc' of type '" $wtype
                                     "' for expected type '$type'");
  ...
  % noany %
      r RLMAssertThrowsWithReason(
          [$array indexOfObject:NSNull.null],
          ^n
          @"Invalid value '<null>' of type 'NSNull' for expected type '$type'");
...
- (void)testIndexOfObjectSorted {
  ...
  % man %
      r uncheckedAssertEqual(
          0U, [[$array sortedResultsUsingKeyPath:@"self"
                                       ascending:NO] indexOfObject:$v1]);
  % man %
      r uncheckedAssertEqual(
          2U, [[$array sortedResultsUsingKeyPath:@"self"
                                       ascending:NO] indexOfObject:$v0]);

  % man %
      o uncheckedAssertEqual(
          0U, [[$array sortedResultsUsingKeyPath:@"self"
                                       ascending:NO] indexOfObject:$v1]);
  % man %
      o uncheckedAssertEqual(
          2U, [[$array sortedResultsUsingKeyPath:@"self"
                                       ascending:NO] indexOfObject:$v0]);
  % man %
      o uncheckedAssertEqual(4U, [[$array sortedResultsUsingKeyPath:@"self"
                                                          ascending:NO]
...
- (void)testIndexOfObjectDistinct {
  ...
  % man %
      r uncheckedAssertEqual(0U,
                             [[$array distinctResultsUsingKeyPaths:@[ @"self" ]]
                                 indexOfObject:$v0]);
  % man %
      r uncheckedAssertEqual(1U,
                             [[$array distinctResultsUsingKeyPaths:@[ @"self" ]]
                                 indexOfObject:$v1]);

  % man %
      o uncheckedAssertEqual(0U,
                             [[$array distinctResultsUsingKeyPaths:@[ @"self" ]]
                                 indexOfObject:$v0]);
  % man %
      o uncheckedAssertEqual(2U,
                             [[$array distinctResultsUsingKeyPaths:@[ @"self" ]]
                                 indexOfObject:$v1]);
  % man %
      o uncheckedAssertEqual(1U,
                             [[$array distinctResultsUsingKeyPaths:@[ @"self" ]]
                                 indexOfObject:NSNull.null]);
}

- (void)testIndexOfObjectWhere {
  % man RLMAssertThrowsWithReason([$array indexOfObjectWhere:@"TRUEPREDICATE"],
                                  @"implemented");
  % man RLMAssertThrowsWithReason([[$array sortedResultsUsingKeyPath:@"self"
                                                           ascending:NO] ^
                                      n indexOfObjectWhere:@"TRUEPREDICATE"],
                                  @"implemented");

  % unman uncheckedAssertEqual(NSNotFound,
                               [$array indexOfObjectWhere:@"TRUEPREDICATE"]);

  [self addObjects];

  % unman uncheckedAssertEqual(0U,
                               [$array indexOfObjectWhere:@"TRUEPREDICATE"]);
  % unman uncheckedAssertEqual(NSNotFound,
                               [$array indexOfObjectWhere:@"FALSEPREDICATE"]);
}

- (void)testIndexOfObjectWithPredicate {
  % man RLMAssertThrowsWithReason(
        [$array
            indexOfObjectWithPredicate:[NSPredicate predicateWithValue:YES]],
        @"implemented");
  % man RLMAssertThrowsWithReason(
        [[$array sortedResultsUsingKeyPath:@"self" ascending:NO] ^ n
            indexOfObjectWithPredicate:[NSPredicate predicateWithValue:YES]],
        @"implemented");

  % unman uncheckedAssertEqual(
        NSNotFound,
        [$array
            indexOfObjectWithPredicate:[NSPredicate predicateWithValue:YES]]);
  ...
  % unman uncheckedAssertEqual(
        0U, [$array indexOfObjectWithPredicate:[NSPredicate
...
- (void)testSort {
  % unman RLMAssertThrowsWithReason(
        [$array sortedResultsUsingKeyPath:@"self" ascending:NO],
  ...
  % unman RLMAssertThrowsWithReason(
        ...
        ^n @"This method may only be called on RLMArray instances retrieved "
           @"from an RLMRealm");
  % man RLMAssertThrowsWithReason([$array sortedResultsUsingKeyPath:@"not self"
                                                          ascending:NO],
                                  ^n @"can only be sorted on 'self'");

  % man % r [$array addObjects:@[ $v0, $v1, $v0 ]];
  % man % o [$array addObjects:@[ $v0, $v1, NSNull.null, $v1, $v0 ]];

  % man %
      r uncheckedAssertEqualObjects(
          [[$array sortedResultsUsingDescriptors:@[]] valueForKey:@"self"],
          ^n(@[ $v0, $v1, $v0 ]));
  % man %
      o uncheckedAssertEqualObjects(
          [[$array sortedResultsUsingDescriptors:@[]] valueForKey:@"self"],
          ^n(@[ $v0, $v1, NSNull.null, $v1, $v0 ]));

  % man %
      r uncheckedAssertEqualObjects(
          [[$array sortedResultsUsingKeyPath:@"self"
...
- (void)testFilter {
  % unman RLMAssertThrowsWithReason(
        ...
        ^n @"This method may only be called on RLMArray instances retrieved "
           @"from an RLMRealm");
  % unman RLMAssertThrowsWithReason(
        [$array objectsWithPredicate:[NSPredicate predicateWithValue:YES]],
        ^n @"This method may only be called on RLMArray instances retrieved "
           @"from an RLMRealm");

  % man RLMAssertThrowsWithReason([$array objectsWhere:@"TRUEPREDICATE"],
                                  ^n @"implemented");
  % man RLMAssertThrowsWithReason(
        [$array objectsWithPredicate:[NSPredicate predicateWithValue:YES]],
        ^n @"implemented");
...
- (void)testNotifications {
  % unman RLMAssertThrowsWithReason(
        ...
        ^n @"Change notifications are only supported on managed collections.");
}

- (void)testMin {
  % noany % nominmax %
      unman RLMAssertThrowsWithReason(
          [$array minOfProperty:@"self"],
          ^n @"minOfProperty: is not supported for $type array");
  % noany % nominmax %
      man RLMAssertThrowsWithReason(
          [$array minOfProperty:@"self"],
          ^n @"Operation 'min' not supported for $type list '$class.$prop'");
...
- (void)testMax {
  % noany % nominmax %
      unman RLMAssertThrowsWithReason(
          [$array maxOfProperty:@"self"],
          ^n @"maxOfProperty: is not supported for $type array");
  % noany % nominmax %
      man RLMAssertThrowsWithReason(
          [$array maxOfProperty:@"self"],
          ^n @"Operation 'max' not supported for $type list '$class.$prop'");
...
- (void)testSum {
  % noany % nosum %
      unman RLMAssertThrowsWithReason(
          [$array sumOfProperty:@"self"],
          ^n @"sumOfProperty: is not supported for $type array");
  % noany % nosum %
      man RLMAssertThrowsWithReason(
          [$array sumOfProperty:@"self"],
          ^n @"Operation 'sum' not supported for $type list '$class.$prop'");

  % sum uncheckedAssertEqualObjects([$array sumOfProperty:@"self"], @0);

  [self addObjects];

  % sum XCTAssertEqualWithAccuracy([$array sumOfProperty:@"self"].doubleValue,
                                   sum($values), .001);
}
...
- (void)testAverage {
  % noany % noavg %
      unman RLMAssertThrowsWithReason(
          [$array averageOfProperty:@"self"],
          ^n @"averageOfProperty: is not supported for $type array");
  % noany % noavg %
      man RLMAssertThrowsWithReason(
          [$array averageOfProperty:@"self"],
          ^n
          @"Operation 'average' not supported for $type list '$class.$prop'");

  % avg uncheckedAssertNil([$array averageOfProperty:@"self"]);

  [self addObjects];

  % avg XCTAssertEqualWithAccuracy(
        [$array averageOfProperty:@"self"].doubleValue, average($values), .001);
}
...
- (void)testFastEnumeration {
  ...
  ^{
    ...
    ^nl for (id value in $array) {
      ^nl uncheckedAssertEqualObjects(values[i++ % values.count], value);
      ^nl
    }
    ^nl uncheckedAssertEqual(i, $array.count);
    ^nl
  }();
  ^nl
}
...
- (void)testValueForKeyNumericAggregates {
  ...
  % noany %
      sum uncheckedAssertEqualObjects([$array valueForKeyPath:@"@sum.self"],
                                      @0);
  % noany % avg uncheckedAssertNil([$array valueForKeyPath:@"@avg.self"]);

  [self addObjects];

  % minmax uncheckedAssertEqualObjects([$array valueForKeyPath:@"@min.self"],
                                       $v0);
  % minmax uncheckedAssertEqualObjects([$array valueForKeyPath:@"@max.self"],
                                       $v1);
  % noany %
      sum XCTAssertEqualWithAccuracy(
          [[$array valueForKeyPath:@"@sum.self"] doubleValue], sum($values),
          .001);
  % noany %
      avg XCTAssertEqualWithAccuracy(
          [[$array valueForKeyPath:@"@avg.self"] doubleValue], average($values),
          .001);
}
...
- (void)testValueForKeyLength {
  ...
  % string uncheckedAssertEqualObjects([$array valueForKey:@"length"],
                                       ([$values valueForKey:@"length"]));
}
...
- (void)testUnionOfObjects {
  uncheckedAssertEqualObjects(
      [$allArrays valueForKeyPath:@"@unionOfObjects.self"], @[]);
  uncheckedAssertEqualObjects(
      [$allArrays valueForKeyPath:@"@distinctUnionOfObjects.self"], @[]);
  ...
  uncheckedAssertEqualObjects([$array valueForKeyPath:@"@unionOfObjects.self"],
                              ^n($values2));
  uncheckedAssertEqualObjects(sortedDistinctUnion($array, @"Objects", @"self"),
                              ^n($values));
}
...
- (void)testUnionOfArrays {
  ...
  % man %
      r uncheckedAssertEqualObjects(
          [allRequired valueForKeyPath:@"@unionOfArrays.$prop"], @[]);
  % man %
      o uncheckedAssertEqualObjects(
          [allOptional valueForKeyPath:@"@unionOfArrays.$prop"], @[]);
  % man %
      r uncheckedAssertEqualObjects(
          [allRequired valueForKeyPath:@"@distinctUnionOfArrays.$prop"], @[]);
  % man %
      o uncheckedAssertEqualObjects(
          [allOptional valueForKeyPath:@"@distinctUnionOfArrays.$prop"], @[]);

  % man %
      any XCTAssertEqualObjects(
          [allRequired valueForKeyPath:@"@unionOfArrays.$prop"], @[]);
  % man %
      any XCTAssertEqualObjects(
          [allRequired valueForKeyPath:@"@distinctUnionOfArrays.$prop"], @[]);
  ...
  % man %
      r uncheckedAssertEqualObjects(
          [allRequired valueForKeyPath:@"@unionOfArrays.$prop"], ^n($values2));
  % man %
      o uncheckedAssertEqualObjects(
          [allOptional valueForKeyPath:@"@unionOfArrays.$prop"], ^n($values2));
  % man %
      r uncheckedAssertEqualObjects(
          sortedDistinctUnion(allRequired, @"Arrays", @"$prop"), ^n($values));
  % man %
      o uncheckedAssertEqualObjects(
          sortedDistinctUnion(allOptional, @"Arrays", @"$prop"), ^n($values));

  % man %
      any uncheckedAssertEqualObjects(
          [allRequired valueForKeyPath:@"@unionOfArrays.$prop"], ^n($values2));
  % man %
      any uncheckedAssertEqualObjects(
          sortedDistinctUnion(allRequired, @"Arrays", @"$prop"), ^n($values));
...
- (void)testSetValueForKey {
  RLMAssertThrowsWithReason(
      [$allArrays setValue:@0 forKey:@"not self"],
      ^n @"this class is not key value coding-compliant for the key not self.");
  % noany RLMAssertThrowsWithReason([$array setValue:$wrong forKey:@"self"],
                                    ^n
                                    @"Invalid value '$wdesc' of type '" $wtype
                                     "' for expected type '$type'");
  % noany %
      r RLMAssertThrowsWithReason(
          [$array setValue:NSNull.null forKey:@"self"],
          ^n
          @"Invalid value '<null>' of type 'NSNull' for expected type '$type'");
...
- (void)testAssignment {
  ...
  $array = (id)$values;
  ^nl uncheckedAssertEqualObjects([$array valueForKey:@"self"], ($values));
  ^nl
...
- (void)testDynamicAssignment {
  $obj[@"$prop"] = (id) @[ $v1 ];
  ^nl uncheckedAssertEqualObjects($obj[@"$prop"][0], $v1);

  // Should replace and not append
  $obj[@"$prop"] = (id)$values;
  ^nl uncheckedAssertEqualObjects([$obj[@"$prop"] valueForKey:@"self"],
                                  ($values));
  ^nl
...
- (void)testQueryBasicOperators {
  ...
  % man % nominmax %
      noany RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"ANY $prop > %@", $v0]),
          ^n @"Operator '>' not supported for type '$basetype'");
}
...
- (void)testQueryBetween {
  ...
  % noany % man %
      nominmax RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm
                            where:@"ANY $prop BETWEEN %@", @[ $v0, $v1 ]]),
          ^n @"Operator 'BETWEEN' not supported for type '$basetype'");
...
- (void)testQuerySum {
  ...
  % noany % nodate % nosum %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@sum = %@", $v0]),
          ^n @"Invalid keypath '$prop.@sum': @sum can only be applied to a "
             @"collection of numeric values.");
  % noany % date %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@sum = %@", $v0]),
          ^n @"Cannot sum or average date properties");

  % noany % sum %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@sum = %@", $wrong]),
          ^n @"@sum on a property of type $basetype cannot be compared with "
             @"'$wdesc'");
  % noany % sum %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@sum.prop = %@", $wrong]),
          ^n @"Invalid keypath '$prop.@sum.prop': @sum on a collection of "
             @"values must appear at the end of a keypath.");
  % noany % sum %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@sum = %@", NSNull.null]),
          ^n @"@sum on a property of type $basetype cannot be compared with "
             @"'<null>'");
...
- (void)testQueryAverage {
  ...
  % noany % nodate % noavg %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@avg = %@", $v0]),
          ^n @"Invalid keypath '$prop.@avg': @avg can only be applied to a "
             @"collection of numeric values.");
  % noany % date %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@avg = %@", $v0]),
  ...
  % noany % avg %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@avg = %@", $wrong]),
          ^n @"@avg on a property of type $basetype cannot be compared with "
             @"'$wdesc'");
  % noany % avg %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@avg.prop = %@", $wrong]),
          ^n @"Invalid keypath '$prop.@avg.prop': @avg on a collection of "
             @"values must appear at the end of a keypath.");
  ...
  % noany % avg %
      man RLMAssertCount($class, 1U, @"$prop.@avg == %@", NSNull.null);
...
- (void)testQueryMin {
  ...
  % noany % nominmax %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@min = %@", $v0]),
          ^n @"Invalid keypath '$prop.@min': @min can only be applied to a "
             @"collection of numeric values.");
  % noany % minmax %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@min = %@", $wrong]),
          ^n @"@min on a property of type $basetype cannot be compared with "
             @"'$wdesc'");
  % minmax %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@min.prop = %@", $wrong]),
          ^n @"Invalid keypath '$prop.@min.prop': @min on a collection of "
             @"values must appear at the end of a keypath.");
...
- (void)testQueryMax {
  ...
  % noany % nominmax %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@max = %@", $v0]),
          ^n @"Invalid keypath '$prop.@max': @max can only be applied to a "
             @"collection of numeric values.");
  % noany % minmax %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@max = %@", $wrong]),
          ^n @"@max on a property of type $basetype cannot be compared with "
             @"'$wdesc'");
  % minmax %
      man RLMAssertThrowsWithReason(
          ([$class objectsInRealm:realm where:@"$prop.@max.prop = %@", $wrong]),
          ^n @"Invalid keypath '$prop.@max.prop': @max on a collection of "
             @"values must appear at the end of a keypath.");
...
- (void)testQueryBasicOperatorsOverLink {
  ...
  % man % nominmax %
      noany RLMAssertThrowsWithReason(
          ([LinkTo$class objectsInRealm:realm
                                  where:@"ANY link.$prop > %@", $v0]),
          ^n @"Operator '>' not supported for type '$basetype'");
}
