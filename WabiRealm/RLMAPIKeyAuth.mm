...
wrapCompletion(RLMOptionalUserAPIKeyBlock completion) {
  return
      [completion](App::UserAPIKey userAPIKey, std::optional<AppError> error) {
        if (error) {
          return completion(nil, makeError(*error));
        }
        return completion([[RLMUserAPIKey alloc] initWithUserAPIKey:userAPIKey],
                          nil);
      };
}
...
- (void)fetchAPIKeysWithCompletion:(RLMUserAPIKeysBlock)completion {
  self.client.fetch_api_keys(
      self.currentUser, ^(const std::vector<App::UserAPIKey> &userAPIKeys,
        ...
        for (auto &userAPIKey : userAPIKeys) {
          [apiKeys
              addObject:[[RLMUserAPIKey alloc] initWithUserAPIKey:userAPIKey]];
        }
