...
class SwiftUIServerTests: SwiftSyncTestCase {
  ...
  private func configuration<T: BSON>(user: User, partition: T) -> WabiRealm.Configuration {
  ...
  func testAsyncOpenThrowExceptionWithoutCachedApp() throws {
    ...
    assertThrows(AsyncOpen(partitionValue: #function),
                 reason: "Cannot AsyncOpen the WabiRealm because no appId was found. You must either explicitly pass an appId or initialize an App before displaying your View.")
  }
  ...
  func testAsyncOpenThrowExceptionWithMoreThanOneCachedApp() throws {
    ...
    _ = App(id: "fake 2")
    assertThrows(AsyncOpen(partitionValue: #function),
                 reason: "Cannot AsyncOpen the WabiRealm because more than one appId was found. When using multiple Apps you must explicitly pass an appId to indicate which to use.")
  }
  ...
  // In case of no internet connection AutoOpen should return an opened WabiRealm, offline-first approach
