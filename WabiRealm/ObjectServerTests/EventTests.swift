...
class SwiftEventTests: SwiftSyncTestCase {
  ...
  func config(partition: String = UUID().uuidString) -> WabiRealm.Configuration {
  ...
  func openRealm(_ configuration: WabiRealm.Configuration? = nil) throws -> WabiRealm {
    let realm = try openRealm(configuration: configuration ?? config())
    ...
    // main WabiRealm after opening it.
  ...
  func testBasicWithAsyncOpen() throws {
    let realm = WabiRealm.asyncOpen(configuration: config()).await(self)
  ...
  func testLinkTracking() throws {
    ...
    func assertEvent(_ activity: String, personCount: Int, _ value: NSDictionary, line: UInt = #line) {
      XCTAssertEqual(result.filter { $0.activity == activity &&
                       $0.parsedData!["type"] as! String == "SwiftPerson"
      }.count,
      personCount, line: line)
      let event = result.filter { $0.activity == activity &&
        $0.parsedData!["type"] as! String == "LinkToSwiftPerson"
      }.first
