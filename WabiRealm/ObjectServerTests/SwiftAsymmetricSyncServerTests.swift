...
////////////////////////////////////////////////////////////////////////////
  ...
  class SwiftObjectAsymmetric: AsymmetricObject {
    ...
    @Persisted var decimal: Decimal128 = .init(1)
    @Persisted var uuid: UUID = .init(uuidString: "85d4fbee-6ec6-47df-bfa1-615931903d7e")!
    @Persisted var objectId: ObjectId = .init("6058f12682b2fbb1f334ef1d")
  ...
  class SwiftAsymmetricSyncTests: SwiftSyncTestCase {
    ...
    func testOpenLocalRealmWithAsymmetricObjectError() throws {
      let configuration = WabiRealm.Configuration(objectTypes: [SwiftObjectAsymmetric.self])
      XCTAssertThrowsError(try WabiRealm(configuration: configuration)) { error in
        XCTAssertEqual(error.localizedDescription, "Schema validation failed due to the following errors:\n- Asymmetric table \'SwiftObjectAsymmetric\' not allowed in a local WabiRealm")
      }
    }
    ...
    extension SwiftAsymmetricSyncTests {
      func config() async throws -> WabiRealm.Configuration {
      ...
      func realm() async throws -> WabiRealm {
      ...
      func testCreateHugeAsymmetricObject() async throws {
        ...
        try realm.write {
          for _ in 0 ..< 2 {
            realm.create(HugeObjectAsymmetric.self, value: ["data": Data(repeating: 16, count: 1_000_000)])
          }
        }
        waitForUploads(for: realm)
