...
- (void)roundTripForPartitionValue:(id<RLMBSON>)value testName:(SEL)callerName {
  ...
  RLMCredentials *credentials = [self basicCredentialsWithName:name
                                                      register:YES
                                                           app:app];
  ...
  RLMCredentials *writeCredentials =
      [self basicCredentialsWithName:[name stringByAppendingString:@"Writer"]
                            register:YES
                                 app:app];
  ...
  XCTAssertEqual(
      [Person objectsInRealm:realm where:@"firstName = 'John'"].count, 1UL);
...
- (void)testRoundTripForObjectIdPartitionValue {
  [self
      roundTripForPartitionValue:[[RLMObjectId alloc]
...
- (void)testRoundTripForUUIDPartitionValue {
  [self roundTripForPartitionValue:
            [[NSUUID alloc]
...
- (void)testRoundTripForStringPartitionValue {
  [self roundTripForPartitionValue:@"1234567890ab1234567890ab" testName:_cmd];
}

- (void)testRoundTripForIntPartitionValue {
  [self roundTripForPartitionValue:@1234567890 testName:_cmd];
}
