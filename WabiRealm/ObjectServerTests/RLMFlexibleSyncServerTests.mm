...
- (void)testFlexibleSyncRealmFilePath {
  ...
  NSString *expected = [NSString
      stringWithFormat:@"mongodb-wabi-realm/%@/%@/flx_sync_default.realm",
                       self.flexibleSyncAppId, user.identifier];
...
- (void)testCheckVersionAfterAddSubscription {
  ...
  [subs update:^{
    [subs addSubscriptionWithClassName:Person.className where:@"age > 15"];
  }];
...
- (void)testAddAndFindSubscriptionByQuery {
  ...
  RLMSyncSubscription *foundSubscription =
      [subs subscriptionWithClassName:Person.className where:@"age > 15"];
...
- (void)testAddAndFindSubscriptionWithCompoundQuery {
  ...
  RLMSyncSubscription *foundSubscription =
      [subs subscriptionWithClassName:Person.className
                                where:@"firstName == %@ and lastName == %@",
                                      @"John", @"Doe"];
  XCTAssertNotNil(foundSubscription);
  XCTAssertNil(foundSubscription.name);
  XCTAssert(foundSubscription.queryString,
...
- (void)testAddAndFindSubscriptionWithPredicate {
  ...
  [subs update:^{
    [subs
        addSubscriptionWithClassName:Person.className
                           predicate:[NSPredicate
                                         predicateWithFormat:@"age == %d", 20]];
  }];

  XCTAssertEqual(subs.version, 1UL);
  XCTAssertEqual(subs.count, 1UL);

  RLMSyncSubscription *foundSubscription = [subs
      subscriptionWithClassName:Person.className
                      predicate:[NSPredicate
                                    predicateWithFormat:@"age == %d", 20]];
  XCTAssertNotNil(foundSubscription);
  XCTAssertNil(foundSubscription.name);
...
- (void)testAddSubscriptionWithoutWriteThrow {
  ...
  RLMAssertThrowsWithReason([subs addSubscriptionWithClassName:Person.className
                                                         where:@"age > 15"],
                            @"Can only add, remove, or update subscriptions "
                            @"within a write subscription block.");
}
...
- (void)testAddAndFindSubscriptionByName {
  ...
  RLMSyncSubscription *foundSubscription =
      [subs subscriptionWithName:@"person_older_15"];
  XCTAssertNotNil(foundSubscription);
...
- (void)testAddDuplicateNamedSubscriptionWillThrow {
  ...
  [subs update:^{
    ...
    RLMAssertThrowsWithReason(
        [subs addSubscriptionWithClassName:Person.className
                          subscriptionName:@"person_age"
                                     where:@"age > 20"],
        @"A subscription named 'person_age' already exists. If you meant to "
        @"update the existing subscription please use the `update` method.");
  }];
  ...
  RLMSyncSubscription *foundSubscription =
      [subs subscriptionWithName:@"person_age"];
  XCTAssertNotNil(foundSubscription);
...
- (void)testAddDuplicateSubscriptionWithDifferentName {
  ...
  RLMSyncSubscription *foundSubscription =
      [subs subscriptionWithName:@"person_age_1"];
  XCTAssertNotNil(foundSubscription);

  RLMSyncSubscription *foundSubscription2 =
      [subs subscriptionWithName:@"person_age_2"];
  XCTAssertNotNil(foundSubscription2);

  XCTAssertNotEqualObjects(foundSubscription.name, foundSubscription2.name);
  XCTAssertEqualObjects(foundSubscription.queryString,
                        foundSubscription2.queryString);
  XCTAssertEqualObjects(foundSubscription.objectClassName,
                        foundSubscription2.objectClassName);
}
...
- (void)testRemoveSubscriptionWithoutWriteThrow {
  ...
  XCTAssertEqual(subs.version, 1UL);
  XCTAssertEqual(subs.count, 1UL);
  RLMAssertThrowsWithReason([subs removeSubscriptionWithName:@"person_age_1"],
                            @"Can only add, remove, or update subscriptions "
                            @"within a write subscription block.");
}
...
- (void)testRemoveSubscriptionByQuery {
  ...
  [subs update:^{
    [subs removeSubscriptionWithClassName:Person.className
                                    where:@"firstName == %@", @"John"];
    [subs removeSubscriptionWithClassName:Person.className
                                predicate:[NSPredicate
                                              predicateWithFormat:
  ...
  RLMSyncSubscription *foundSubscription2 =
      [subs subscriptionWithName:@"person_firstname"];
  XCTAssertNil(foundSubscription2);

  RLMSyncSubscription *foundSubscription3 =
      [subs subscriptionWithName:@"person_lastname"];
...
- (void)testRemoveAllSubscription {
  ...
  RLMSyncSubscription *foundSubscription =
      [subs subscriptionWithName:@"person_age_3"];
  XCTAssertNil(foundSubscription);
}
...
- (void)testRemoveAllSubscriptionForType {
  ...
  RLMSyncSubscription *foundSubscription =
      [subs subscriptionWithName:@"dog_name"];
  XCTAssertNotNil(foundSubscription);
...
- (void)testUpdateSubscriptionQueryWithoutWriteThrow {
  ...
  RLMSyncSubscription *foundSubscription =
      [subs subscriptionWithName:@"subscription_1"];
  XCTAssertNotNil(foundSubscription);

  RLMAssertThrowsWithReason(
      [foundSubscription
          updateSubscriptionWithPredicate:
              [NSPredicate predicateWithFormat:@"name == 'Tomas'"]],
      @"Can only add, remove, or update subscriptions within a write "
      @"subscription block.");
}
...
- (void)testSubscriptionSetIterate {
  ...
  [subs update:^{
    for (int i = 0; i < numberOfSubs; ++i) {
      [subs
          addSubscriptionWithClassName:Person.className
                      subscriptionName:[NSString
                                           stringWithFormat:@"person_age_%d", i]
                                 where:[NSString
                                           stringWithFormat:@"age > %d", i]];
    }
  }];
  ...
  __weak id objects[(unsigned long)pow(numberOfSubs, 2.0) +
                    (unsigned long)numberOfSubs];
  NSInteger count = 0;
...
- (void)testSubscriptionSetFirstAndLast {
  ...
  XCTAssertEqualObjects(
      ...
      ([NSString stringWithFormat:@"person_age_%d", numberOfSubs]));
  XCTAssertEqualObjects(
      lastSubscription.queryString,
      ([NSString stringWithFormat:@"age > %d", numberOfSubs]));
}
...
- (void)testSubscriptionSetSubscript {
  ...
  XCTAssertEqualObjects(
      objectAtIndexSubscription.name,
      ([NSString stringWithFormat:@"person_age_%d", index + 1]));
  XCTAssertEqualObjects(objectAtIndexSubscription.queryString,
                        ([NSString stringWithFormat:@"age > %d", index + 1]));
}
@end
...
- (void)createPeople:(RLMRealm *)realm partition:(SEL)cmd {
  ...
  for (int i = 1; i <= numberOfSubs; ++i) {
    Person *person = [[Person alloc]
        initWithPrimaryKey:[RLMObjectId objectId]
                       age:i
...
- (void)testFlexibleSyncAddQuery {
  ...
  [self
      writeQueryAndCompleteForRealm:realm
                              block:^(RLMSyncSubscriptionSet *subs) {
                                [subs
                                    addSubscriptionWithClassName:Person
                                                                     .className
                                                subscriptionName:@"person_age"
                                                           where:
                                                               @"age > 15 and "
                                                ...
                                                subscriptionName:@"person_age"
                                                           where:
                                                               ...
                                                               NSStringFromSelector(
                                                                   _cmd)];
                                [subs
                                    addSubscriptionWithClassName:Dog.className
                                                subscriptionName:
                                                    @"dog_breed_labradoodle"
                                                           where:
                                                               @"breed == "
                                                               @"'Labradoodle' "
                                                               @"and partition "
...
- (void)testFlexibleSyncRemoveAllQueriesForType {
  ...
  [self writeQueryAndCompleteForRealm:realm
                                block:^(RLMSyncSubscriptionSet *subs) {
                                  [subs removeAllSubscriptionsWithClassName:
                                            Person.className];
                                }];
  CHECK_COUNT(0, Person, realm);
  CHECK_COUNT(1, Dog, realm);
...
- (void)testFlexibleSyncUpdateQuery {
  ...
  [self writeQueryAndCompleteForRealm:realm
                                block:^(RLMSyncSubscriptionSet *subs) {
                                  RLMSyncSubscription *foundSub =
                                      [subs subscriptionWithName:@"person_age"];
                                  [foundSub updateSubscriptionWhere:
                                                @"age > 20 and partition == %@",
...
- (void)testFlexibleSyncAddObjectOutsideQuery {
  ...
  self.flexibleSyncApp.syncManager.errorHandler =
      ^(NSError *error, RLMSyncSession *) {
        RLMValidateError(error, RLMSyncErrorDomain, RLMSyncErrorWriteRejected,
                         @"Client attempted a write that is outside of "
                         @"permissions or query filters; it has been reverted");
        NSArray<RLMCompensatingWriteInfo *> *info =
            error.userInfo[RLMCompensatingWriteInfoKey];
        ...
        XCTAssertEqualObjects(
            ...
            ([NSString stringWithFormat:
                           @"write to \"%@\" in table \"Person\" not allowed; "
                           @"object is outside of the current query view",
                           invalidObjectPK]));
        [ex fulfill];
      };
  [realm transactionWithBlock:^{
    [realm addObject:[[Person alloc] initWithPrimaryKey:invalidObjectPK
                                                    age:10
                                              firstName:@"Nic"
                                               lastName:@"Cages"]];
  }];
...
- (void)testFlexibleSyncInitialSubscription {
  ...
  RLMRealmConfiguration *config = [user
  ...
  [RLMRealm asyncOpenWithConfiguration:config
                         callbackQueue:dispatch_get_main_queue()
                              callback:^(RLMRealm *realm, NSError *error) {
                                XCTAssertNil(error);
                                XCTAssertEqual(realm.subscriptions.count, 1UL);
                                // Adding this sleep, because there seems to be
                                // a timing issue after this commit in baas
                                // https://github.com/10gen/baas/commit/64e75b3f1fe8a6f8704d1597de60f9dda401ccce,
                                // data take a little longer to be downloaded to
                                // the realm even though the sync client changed
                                // the subscription state to completed.
                                sleep(1);
                                CHECK_COUNT(11, Person, realm);
                                [ex fulfill];
                              }];
...
- (void)testFlexibleSyncInitialSubscriptionRerunOnOpen {
  ...
  XCTestExpectation *ex2 =
      [self expectationWithDescription:@"download-realm-2"];
  [RLMRealm asyncOpenWithConfiguration:config
                         callbackQueue:dispatch_get_main_queue()
  ...
  [self dispatchAsyncAndWait:^{
    ...
    // set as the WabiRealm was already open
    XCTAssertEqual(openCount, 2);
  }];
}
...
- (void)testFlexibleSyncInitialOnConnectionTimeout {
  ...
  [RLMRealm
      asyncOpenWithConfiguration:config
                   callbackQueue:dispatch_get_main_queue()
                        callback:^(RLMRealm *realm, NSError *error) {
...
#if 0 // FIXME: this is no longer an error and needs to be updated to something
      // which is
