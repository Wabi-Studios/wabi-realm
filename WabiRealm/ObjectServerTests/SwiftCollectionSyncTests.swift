...
class CollectionSyncTestCase: SwiftSyncTestCase {
  var readRealm: WabiRealm!
  var writeRealm: WabiRealm!

  override func setUp() {
    super.setUp()

    let readUser = logInUser(for: basicCredentials(name: name + " read user", register: true))
    let writeUser = logInUser(for: basicCredentials(name: name + " write user", register: true))
  ...
  func write(_ fn: (WabiRealm) -> Void) throws {
  ...
  private func roundTrip<T>(keyPath: KeyPath<SwiftCollectionSyncObject, List<T>>,
                            values: [T], partitionValue _: String = #function) throws
  {
  ...
  func testLists() throws {
    ...
    try roundTrip(keyPath: \.dateList, values: [Date(timeIntervalSince1970: 10_000_000),
                                                Date(timeIntervalSince1970: 20_000_000),
                                                Date(timeIntervalSince1970: 30_000_000)])
  ...
  private func roundTrip<T>(set: MutableSetKeyValues<T>,
                            ...
                            partitionValue _: String = #function) throws
  {
    checkCount(expected: 0, readRealm, SwiftCollectionSyncObject.self)
  ...
  func testSets() throws {
    ...
    try roundTrip(set: (\.dateSet, [Date(timeIntervalSince1970: 10_000_000),
                                    Date(timeIntervalSince1970: 20_000_000),
                                    Date(timeIntervalSince1970: 30_000_000)]),
                  otherSet: (\.otherDateSet, [Date(timeIntervalSince1970: 30_000_000),
                                              Date(timeIntervalSince1970: 40_000_000),
                                              Date(timeIntervalSince1970: 50_000_000)]))
  ...
  func testMaps() throws {
    ...
    try roundTrip(keyPath: \.dataMap, values: [Data(repeating: 1, count: 64),
                                               ...
                                               Data(repeating: 4, count: 64),
                                               Data(repeating: 5, count: 64)])
    try roundTrip(keyPath: \.dateMap, values: [Date(timeIntervalSince1970: 10_000_000),
                                               Date(timeIntervalSince1970: 20_000_000),
                                               Date(timeIntervalSince1970: 30_000_000),
                                               Date(timeIntervalSince1970: 40_000_000),
                                               Date(timeIntervalSince1970: 50_000_000)])
