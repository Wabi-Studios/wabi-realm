...
- (RLMRealmConfiguration *)configurationWithTestSelector:(SEL)sel {
  auto config =
      [self configurationWithPartitionValue:NSStringFromSelector(sel)];
...
- (void)testCustomTokenAuthentication {
  RLMUser *user =
      [self logInUserForCredentials:[self jwtCredentialWithAppId:self.appId]];
  XCTAssertTrue(
      [user.profile.metadata[@"anotherName"] isEqualToString:@"Bar Foo"]);
  XCTAssertTrue([user.profile.metadata[@"name"] isEqualToString:@"Foo Bar"]);
  XCTAssertTrue(
      [user.profile.metadata[@"occupation"] isEqualToString:@"firefighter"]);
}

- (void)testCallFunction {
  XCTestExpectation *expectation =
      [self expectationWithDescription:
                @"should get sum of arguments from remote function"];
...
- (void)testLogoutCurrentUser {
  ...
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"should log out current user"];
...
- (void)testLogoutSpecificUser {
  RLMUser *firstUser = [self
      logInUserForCredentials:[self
                                  basicCredentialsWithName:NSStringFromSelector(
                                                               _cmd)
                                                  register:YES]];
  RLMUser *secondUser = [self
      logInUserForCredentials:[self basicCredentialsWithName:@"test1@10gen.com"
                                                    register:YES]];

  XCTAssertEqualObjects(self.app.currentUser.identifier, secondUser.identifier);
  // `[app currentUser]` will now be `secondUser`, so let's logout firstUser and
...
- (void)testSwitchUser {
  ...
  XCTAssertEqualObjects([app switchToUser:syncUserA].identifier,
                        syncUserA.identifier);
}
  ...
  RLMUser *firstUser = [self
      logInUserForCredentials:[self
                                  basicCredentialsWithName:NSStringFromSelector(
                                                  ...
                                                  register:YES]];
  RLMUser *secondUser = [self
      logInUserForCredentials:[self basicCredentialsWithName:@"test2@10gen.com"
                                                    register:YES]];

  XCTAssert(
      [self.app.currentUser.identifier isEqualToString:secondUser.identifier]);

  XCTestExpectation *removeUserExpectation =
      [self expectationWithDescription:@"should remove user"];

  [secondUser removeWithCompletion:^(NSError *error) {
    XCTAssert(!error);
    XCTAssert(self.app.allUsers.count == 1);
    XCTAssert(
        [self.app.currentUser.identifier isEqualToString:firstUser.identifier]);
  ...
  [self
      logInUserForCredentials:[self
                                  basicCredentialsWithName:NSStringFromSelector(
                                                  ...
                                                  register:YES]];
  RLMUser *secondUser = [self
      logInUserForCredentials:[self basicCredentialsWithName:@"test3@10gen.com"
                                                    register:YES]];

  XCTAssert(
      [self.app.currentUser.identifier isEqualToString:secondUser.identifier]);

  XCTestExpectation *deleteUserExpectation =
      [self expectationWithDescription:@"should delete user"];
...
- (void)testDeviceRegistration {
  ...
  auto expectation =
      [self expectationWithDescription:@"should register device"];
  [client registerDeviceWithToken:@"token"
  ...
  [client deregisterDeviceForUser:self.app.currentUser
                       completion:^(NSError *error) {
                         XCTAssertNil(error);
                         [expectation fulfill];
                       }];
...
- (void)fixme_testMultipleRegisterDevice {
  ...
  XCTestExpectation *registerExpectation =
      [self expectationWithDescription:@"should register device"];
  XCTestExpectation *secondRegisterExpectation =
      [self expectationWithDescription:
                @"should not throw error when attempting to register again"];
...
static NSString *randomEmail() {
  return [NSString stringWithFormat:@"%@@%@.com", generateRandomString(10),
                                    generateRandomString(10)];
}

- (void)testRegisterEmailAndPassword {
  XCTestExpectation *expectation = [self
      expectationWithDescription:@"should register with email and password"];

  NSString *randomPassword = generateRandomString(10);
  [self.app.emailPasswordAuth registerUserWithEmail:randomEmail()
...
- (void)testConfirmUser {
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"should try confirm user and fail"];

  [self.app.emailPasswordAuth
      confirmUser:randomEmail()
          tokenId:@"a_token"
       completion:^(NSError *error) {
...
- (void)testRetryCustomConfirmation {
  XCTestExpectation *expectation =
      [self expectationWithDescription:
                @"should try retry confirmation email and fail"];

  [self.app.emailPasswordAuth
      retryCustomConfirmation:@"some-email@email.com"
                   completion:^(NSError *error) {
                     XCTAssertTrue([error.userInfo[@"NSLocalizedDescription"]
                         isEqualToString:@"cannot run confirmation for "
                                         @"some-email@email.com: automatic "
...
- (void)testResendConfirmationEmail {
  XCTestExpectation *expectation =
      [self expectationWithDescription:
                @"should try resend confirmation email and fail"];

  [self.app.emailPasswordAuth
      resendConfirmationEmail:randomEmail()
                   completion:^(NSError *error) {
...
- (void)testResetPassword {
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"should try reset password and fail"];
  [self.app.emailPasswordAuth
      resetPasswordTo:@"APassword123"
                token:@"a_token"
              tokenId:@"a_token_id"
           completion:^(NSError *error) {
             XCTAssertEqual(error.code, RLMAppErrorBadRequest);
...
- (void)testCallResetPasswordFunction {
  XCTestExpectation *expectation =
      [self expectationWithDescription:
                @"should try call reset password function and fail"];
  [self.app.emailPasswordAuth
      callResetPasswordFunction:@"test@mongodb.com"
...
- (void)testUserAPIKeyProviderClientFlow {
  XCTestExpectation *registerExpectation =
      [self expectationWithDescription:@"should try register"];
  XCTestExpectation *loginExpectation =
      [self expectationWithDescription:@"should try login"];
  XCTestExpectation *createAPIKeyExpectationA =
      [self expectationWithDescription:@"should try create an api key"];
  XCTestExpectation *createAPIKeyExpectationB =
      [self expectationWithDescription:@"should try create an api key"];
  XCTestExpectation *fetchAPIKeysExpectation =
      [self expectationWithDescription:@"should try call fetch api keys"];
  XCTestExpectation *disableAPIKeyExpectation =
      [self expectationWithDescription:@"should try disable api key"];
  XCTestExpectation *enableAPIKeyExpectation =
      [self expectationWithDescription:@"should try enable api key"];
  XCTestExpectation *deleteAPIKeyExpectation =
      [self expectationWithDescription:@"should try delete api key"];
  ...
  [self.app.emailPasswordAuth registerUserWithEmail:email
                                           password:randomPassword
                                         completion:^(NSError *error) {
                                           XCTAssert(!error);
                                           [registerExpectation fulfill];
  ...
  [self.app
      loginWithCredential:[RLMCredentials credentialsWithEmail:email
                                                      password:randomPassword]
  ...
  [[syncUser apiKeysAuth]
      createAPIKeyWithName:@"apiKeyName1"
                completion:^(RLMUserAPIKey *userAPIKey, NSError *error) {
                  XCTAssert(!error);
                  XCTAssert([userAPIKey.name isEqualToString:@"apiKeyName1"]);
                  XCTAssert(![userAPIKey.key isEqualToString:@"apiKeyName1"] &&
                            userAPIKey.key.length > 0);
                  userAPIKeyA = userAPIKey;
                  [createAPIKeyExpectationA fulfill];
                }];

  [[syncUser apiKeysAuth]
      createAPIKeyWithName:@"apiKeyName2"
                completion:^(RLMUserAPIKey *userAPIKey, NSError *error) {
                  XCTAssert(!error);
  ...
  [self waitForExpectations:@[
    createAPIKeyExpectationA, createAPIKeyExpectationB
  ]
                    timeout:60.0];

  // sleep for 2 seconds as there seems to be an issue fetching the keys
  // straight after they are created.
  [NSThread sleepForTimeInterval:2];

  [[syncUser apiKeysAuth]
      fetchAPIKeysWithCompletion:^(NSArray<RLMUserAPIKey *> *_Nonnull apiKeys,
                                   NSError *error) {
        XCTAssert(!error);
  ...
  [[syncUser apiKeysAuth] disableAPIKey:userAPIKeyA.objectId
                             completion:^(NSError *error) {
                               XCTAssert(!error);
  ...
  [[syncUser apiKeysAuth] enableAPIKey:userAPIKeyA.objectId
                            completion:^(NSError *error) {
                              XCTAssert(!error);
  ...
  [[syncUser apiKeysAuth] deleteAPIKey:userAPIKeyA.objectId
                            completion:^(NSError *error) {
                              XCTAssert(!error);
...
- (void)testLinkUser {
  ...
  XCTestExpectation *loginExpectation =
      [self expectationWithDescription:@"should try login"];
  XCTestExpectation *linkExpectation =
      [self expectationWithDescription:@"should try link and fail"];
  ...
  [syncUser
      linkUserWithCredentials:[RLMCredentials
                                  credentialsWithFacebookToken:@"a_token"]
                   completion:^(RLMUser *user, NSError *error) {
                     XCTAssert(!user);
...
- (void)testEmailPasswordCredential {
  RLMCredentials *emailPasswordCredential =
      [RLMCredentials credentialsWithEmail:@"test@mongodb.com"
                                  password:@"apassword"];
  XCTAssertEqualObjects(emailPasswordCredential.provider, @"local-userpass");
}

- (void)testJWTCredential {
  RLMCredentials *jwtCredential =
      [RLMCredentials credentialsWithJWT:@"sometoken"];
...
- (void)testUserAPIKeyCredential {
  RLMCredentials *userAPICredential =
      [RLMCredentials credentialsWithUserAPIKey:@"apikey"];
  XCTAssertEqualObjects(userAPICredential.provider, @"api-key");
}

- (void)testServerAPIKeyCredential {
  RLMCredentials *serverAPICredential =
      [RLMCredentials credentialsWithServerAPIKey:@"apikey"];
  XCTAssertEqualObjects(serverAPICredential.provider, @"api-key");
}

- (void)testFacebookCredential {
  RLMCredentials *facebookCredential =
      [RLMCredentials credentialsWithFacebookToken:@"facebook token"];
  XCTAssertEqualObjects(facebookCredential.provider, @"oauth2-facebook");
}

- (void)testGoogleCredential {
  RLMCredentials *googleCredential =
      [RLMCredentials credentialsWithGoogleAuthCode:@"google token"];
  XCTAssertEqualObjects(googleCredential.provider, @"oauth2-google");
}

- (void)testGoogleIdCredential {
  RLMCredentials *googleCredential =
      [RLMCredentials credentialsWithGoogleIdToken:@"id token"];
  XCTAssertEqualObjects(googleCredential.provider, @"oauth2-google");
}

- (void)testAppleCredential {
  RLMCredentials *appleCredential =
      [RLMCredentials credentialsWithAppleToken:@"apple token"];
  XCTAssertEqualObjects(appleCredential.provider, @"oauth2-apple");
}

- (void)testFunctionCredential {
  NSError *error;
  RLMCredentials *functionCredential = [RLMCredentials
      credentialsWithFunctionPayload:@{@"dog" : @{@"name" : @"fido"}}];
...
/// Valid email/password credentials should be able to log in a user. Using the
/// same credentials should return the same user object.
  ...
  RLMUser *secondUser = [self
      logInUserForCredentials:[self
                                  basicCredentialsWithName:NSStringFromSelector(
                                                               _cmd)
                                                  register:NO]];
  // Two users created with the same credential should resolve to the same
  // actual user.
  XCTAssertTrue([firstUser.identifier isEqualToString:secondUser.identifier]);
}

/// An invalid email/password credential should not be able to log in a user and
/// a corresponding error should be generated.
- (void)testInvalidPasswordAuthentication {
  (void)[self userForTest:_cmd];

  RLMCredentials *credentials =
      [RLMCredentials credentialsWithEmail:NSStringFromSelector(_cmd)
                                  password:@"INVALID_PASSWORD"];

  XCTestExpectation *expectation =
      [self expectationWithDescription:@"login should fail"];

  [self.app loginWithCredential:credentials
                     completion:^(RLMUser *user, NSError *error) {
                       XCTAssertNil(user);
                       RLMValidateError(error, RLMAppErrorDomain,
...
/// A non-existsing user should not be able to log in and a corresponding error
/// should be generated.
- (void)testNonExistingEmailAuthentication {
  RLMCredentials *credentials =
      [RLMCredentials credentialsWithEmail:@"INVALID_USERNAME"
                                  password:@"INVALID_PASSWORD"];
...
- (void)testExistingEmailRegistration {
  XCTestExpectation *expectationA =
      [self expectationWithDescription:@"registration should succeed"];
  ...
  XCTestExpectation *expectationB =
      [self expectationWithDescription:@"registration should fail"];
  [[self.app emailPasswordAuth]
      registerUserWithEmail:NSStringFromSelector(_cmd)
                   password:@"password"
                 completion:^(NSError *error) {
                   RLMValidateError(error, RLMAppErrorDomain,
...
- (void)testSyncErrorHandlerErrorDomain {
  ...
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"should fail after setting bad token"];
  self.app.syncManager.errorHandler =
      ^(NSError *error, __unused RLMSyncSession *session) {
...
- (void)testUserProfileInitialization {
  ...
  XCTAssertEqualObjects(profile.metadata[@"other_data"],
                        @{@"some_key" : @"some_value"});
}

#pragma mark - Basic Sync

/// It should be possible to successfully open a WabiRealm configured for sync
/// with a normal user.
...
/// If client B adds objects to a synced WabiRealm, client A should see those
/// objects.
- (void)testAddObjects {
  RLMRealm *realm = [self realmForTest:_cmd];
  ...
  [self writeToPartition:_cmd
                   block:^(RLMRealm *realm) {
                     [realm addObjects:@[
                       [Person john], [Person paul], [Person george]
                     ]];
                     AllTypesSyncObject *obj =
                         [[AllTypesSyncObject alloc] initWithValue:values];
  ...
  AllTypesSyncObject *obj =
      [[AllTypesSyncObject allObjectsInRealm:realm] firstObject];
...
- (void)testAddObjectsWithNilPartitionValue {
  ...
  // This test needs the database to be empty of any documents with a nil
  // partition
  ...
  CHECK_COUNT(0, Person, realm);
  [self writeToPartition:nil
                userName:NSStringFromSelector(_cmd)
                   block:^(RLMRealm *realm) {
                     [realm addObjects:@[
                       [Person john], [Person paul], [Person george],
                       [Person ringo]
...
- (void)testRountripForDistinctPrimaryKey {
  ...
  [self
      writeToPartition:_cmd
                 block:^(RLMRealm *realm) {
                   Person *person = [[Person alloc]
                       initWithPrimaryKey:
                           [[RLMObjectId alloc]
                               initWithString:@"1234567890ab1234567890ab"
                                        error:nil]
                                      age:5
                                firstName:@"Ringo"
                                 lastName:@"Starr"];
                   UUIDPrimaryKeyObject *uuidPrimaryKeyObject =
                       [[UUIDPrimaryKeyObject alloc]
                           initWithPrimaryKey:
                               [[NSUUID alloc]
                                   initWithUUIDString:
                                       @"85d4fbee-6ec6-47df-bfa1-615931903d7e"]
                                       strCol:@"Steve"
                                       intCol:10];
                   StringPrimaryKeyObject *stringPrimaryKeyObject =
                       [[StringPrimaryKeyObject alloc]
                           initWithPrimaryKey:@"1234567890ab1234567890aa"
                                       strCol:@"Paul"
                                       intCol:20];
                   IntPrimaryKeyObject *intPrimaryKeyObject =
                       [[IntPrimaryKeyObject alloc]
                           initWithPrimaryKey:1234567890
                                       strCol:@"Jackson"
                                       intCol:30];
  ...
  Person *person =
      [Person objectInRealm:realm
              forPrimaryKey:[[RLMObjectId alloc]
                                initWithString:@"1234567890ab1234567890ab"
                                         error:nil]];
  XCTAssertEqualObjects(person.firstName, @"Ringo");
  XCTAssertEqualObjects(person.lastName, @"Starr");

  UUIDPrimaryKeyObject *uuidPrimaryKeyObject = [UUIDPrimaryKeyObject
      objectInRealm:realm
      forPrimaryKey:
          [[NSUUID alloc]
  ...
  StringPrimaryKeyObject *stringPrimaryKeyObject =
      [StringPrimaryKeyObject objectInRealm:realm
                              forPrimaryKey:@"1234567890ab1234567890aa"];
  XCTAssertEqualObjects(stringPrimaryKeyObject.strCol, @"Paul");
  XCTAssertEqual(stringPrimaryKeyObject.intCol, 20);

  IntPrimaryKeyObject *intPrimaryKeyObject =
      [IntPrimaryKeyObject objectInRealm:realm forPrimaryKey:@1234567890];
...
- (void)testAddObjectsMultipleApps {
  ...
  if (self.isParent) {
    ...
    XCTAssertEqual(
        [Person objectsInRealm:realm1 where:@"firstName = 'John'"].count, 1UL);
    XCTAssertEqual(
        [Person objectsInRealm:realm1 where:@"firstName = 'Paul'"].count, 1UL);
    XCTAssertEqual(
        [Person objectsInRealm:realm1 where:@"firstName = 'Ringo'"].count, 0UL);
    XCTAssertEqual(
        [Person objectsInRealm:realm1 where:@"firstName = 'George'"].count,
        0UL);

    XCTAssertEqual(
        [Person objectsInRealm:realm2 where:@"firstName = 'John'"].count, 0UL);
    XCTAssertEqual(
        [Person objectsInRealm:realm2 where:@"firstName = 'Paul'"].count, 0UL);
    XCTAssertEqual(
        [Person objectsInRealm:realm2 where:@"firstName = 'Ringo'"].count, 1UL);
    XCTAssertEqual(
...
- (void)testSessionRefresh {
  ...
  RLMUser *user2 = [self
      logInUserForCredentials:[self basicCredentialsWithName:@"lmao@10gen.com"
                                                    register:self.isParent]];
  ...
  if (self.isParent) {
    ...
    [self addPersonsToRealm:realm
                    persons:@[
                      [Person john], [Person paul], [Person ringo],
                      [Person george]
                    ]];
    [self waitForUploadsForRealm:realm];
  }
...
/// If client B deletes objects from a synced WabiRealm, client A should see the
/// effects of that deletion.
...
- (void)testIncomingSyncWritesTriggerNotifications {
  ...
  auto user = [&] {
    NSString *name =
        [baseName stringByAppendingString:[NSUUID UUID].UUIDString];
    return [self logInUserForCredentials:[self basicCredentialsWithName:name
                                                               register:YES]];
  };
  RLMRealm *syncRealm = [self
      openRealmWithConfiguration:[user() configurationWithTestSelector:_cmd]];
  RLMRealm *asyncRealm = [self
      asyncOpenRealmWithConfiguration:[user()
                                          configurationWithTestSelector:_cmd]];
  RLMRealm *writeRealm = [self
      asyncOpenRealmWithConfiguration:[user()
                                          configurationWithTestSelector:_cmd]];

  __block XCTestExpectation *ex =
      [self expectationWithDescription:@"got initial notification"];
  ex.expectedFulfillmentCount = 2;
  RLMNotificationToken *token1 = [[Person allObjectsInRealm:syncRealm]
      addNotificationBlock:^(RLMResults *, RLMCollectionChange *, NSError *) {
        [ex fulfill];
      }];
  RLMNotificationToken *token2 = [[Person allObjectsInRealm:asyncRealm]
      addNotificationBlock:^(RLMResults *, RLMCollectionChange *, NSError *) {
        [ex fulfill];
      }];
...
- (void)testMissingSchema {
  @autoreleasepool {
    auto c = [self.anonymousUser
        configurationWithPartitionValue:NSStringFromSelector(_cmd)];
    c.objectClasses =
        @[ Person.self, AllTypesSyncObject.self, RLMSetSyncObject.self ];
    RLMRealm *realm = [RLMRealm realmWithConfiguration:c error:nil];
    [self waitForDownloadsForRealm:realm];
    AllTypesSyncObject *obj = [[AllTypesSyncObject alloc]
        initWithValue:[AllTypesSyncObject values:0]];
  ...
  RLMRealm *realm = [RLMRealm realmWithConfiguration:c error:nil];
  [self waitForDownloadsForRealm:realm];
  RLMResults<AllTypesSyncObject *> *res =
      [AllTypesSyncObject allObjectsInRealm:realm];
  AllTypesSyncObject *o = res.firstObject;
  Person *p = o.objectCol;
  RLMSet<RLMValue> *anySet = ((RLMObject *)o.anyCol)[@"anySet"];
  XCTAssertTrue(
      [anySet.allObjects[0][@"firstName"] isEqualToString:p.firstName]);
  [realm beginWriteTransaction];
...
/// If client B encrypts its synced WabiRealm, client A should be able to access
/// that WabiRealm with a different encryption key.
- (void)testEncryptedSyncedRealm {
  RLMUser *user = [self userForTest:_cmd];

  NSData *key = RLMGenerateKey();
  RLMRealm *realm =
      [self openRealmForPartitionValue:NSStringFromSelector(_cmd)
                                  user:user
                         encryptionKey:key
...
/// If an encrypted synced WabiRealm is re-opened with the wrong key, throw an
/// exception.
- (void)testEncryptedSyncedRealmWrongKey {
  ...
  RLMAssertRealmExceptionContains(
      [RLMRealm realmWithConfiguration:c error:nil], RLMErrorInvalidDatabase,
      @"Failed to open WabiRealm file at path '%@': header has invalid "
      @"mnemonic. "
      @"The file is either not a WabiRealm file, is an encrypted WabiRealm "
      @"file but no "
      @"encryption key was supplied, or is corrupted.",
      c.fileURL.path);
  c.encryptionKey = RLMGenerateKey();
  RLMAssertRealmExceptionContains(
      [RLMRealm realmWithConfiguration:c error:nil], RLMErrorInvalidDatabase,
      @"Failed to open WabiRealm file at path '%@': WabiRealm file decryption "
      @"failed "
      @"(Decryption failed: 'unable to decrypt after 0 seconds",
      c.fileURL.path);
}

#pragma mark - Multiple WabiRealm Sync

/// If a client opens multiple Realms, there should be one session object for
/// each WabiRealm that was opened.
- (void)testMultipleRealmsSessions {
  ...
  __attribute__((objc_precise_lifetime)) RLMRealm *realmealmA =
      [self openRealmForPartitionValue:partitionValueA user:user];
  __attribute__((objc_precise_lifetime)) RLMRealm *realmealmB =
      [self openRealmForPartitionValue:partitionValueB user:user];
  __attribute__((objc_precise_lifetime)) RLMRealm *realmealmC =
      [self openRealmForPartitionValue:partitionValueC user:user];
  // Make sure there are three active sessions for the user.
  XCTAssert(user.allSessions.count == 3,
            @"Expected 3 sessions, but didn't get 3 sessions");
  ...
  XCTAssertTrue([user sessionForPartitionValue:partitionValueA].state ==
                    RLMSyncSessionStateActive,
                @"Expected active session for URL A");
  XCTAssertTrue([user sessionForPartitionValue:partitionValueB].state ==
                    RLMSyncSessionStateActive,
                @"Expected active session for URL B");
  XCTAssertTrue([user sessionForPartitionValue:partitionValueC].state ==
                    RLMSyncSessionStateActive,
                @"Expected active session for URL C");
}

/// A client should be able to open multiple Realms and add objects to each of
/// them.
- (void)testMultipleRealmsAddObjects {
  ...
  RLMRealm *realmA = [self openRealmForPartitionValue:partitionValueA
                                                 user:user];
  RLMRealm *realmB = [self openRealmForPartitionValue:partitionValueB
                                                 user:user];
  RLMRealm *realmC = [self openRealmForPartitionValue:partitionValueC
                                                 user:user];
  ...
  if (self.isParent) {
    ...
    [self waitForDownloadsForUser:user
                  ...
                  partitionValues:@[
                    partitionValueA, partitionValueB, partitionValueC
                  ]
                   expectedCounts:@[ @3, @2, @5 ]];

    RLMResults *resultsA = [Person objectsInRealm:realmA
                                            where:@"firstName == %@", @"Ringo"];
    RLMResults *resultsB = [Person objectsInRealm:realmB
                                            where:@"firstName == %@", @"Ringo"];
    ...
    [self addPersonsToRealm:realmB persons:@[ [Person john], [Person paul] ]];
...
/// A client should be able to open multiple Realms and delete objects from each
/// of them.
- (void)testMultipleRealmsDeleteObjects {
  ...
  if (self.isParent) {
    ...
    [self addPersonsToRealm:realmC persons:@[ [Person john], [Person paul] ]];
...
/// When a session opened by a WabiRealm goes out of scope, it should stay alive
/// long enough to finish any waiting uploads.
- (void)testUploadChangesWhenRealmOutOfScope {
  const NSInteger OBJECT_COUNT = 3;
  RLMUser *user = [self userForTest:_cmd];

  if (self.isParent) {
    // Open the WabiRealm in an autorelease pool so that it is destroyed as soon
    // as possible.
    @autoreleasepool {
      RLMRealm *realm =
          [self openRealmForPartitionValue:NSStringFromSelector(_cmd)
...
/// A WabiRealm that was opened before a user logged out should be able to
/// resume uploading if the user logs back in.
- (void)testLogBackInSameRealmUpload {
  RLMCredentials *credentials =
      [self basicCredentialsWithName:NSStringFromSelector(_cmd)
                            register:self.isParent];
...
/// A WabiRealm that was opened before a user logged out should be able to
/// resume downloading if the user logs back in.
...
/// A WabiRealm that was opened while a user was logged out should be able to
/// start uploading if the user logs back in.
- (void)testLogBackInDeferredRealmUpload {
  ...
  if (self.isParent) {
    ...
    // Open a WabiRealm after the user's been logged out.
    ...
    RLMRealm *realm = [self openRealmForPartitionValue:partitionValue
                                                  user:user];
    CHECK_COUNT(4, Person, realm);
  }
}
...
/// A WabiRealm that was opened while a user was logged out should be able to
/// start downloading if the user logs back in.
...
/// After logging back in, a WabiRealm whose path has been opened for the first
/// time should properly upload changes.
- (void)testLogBackInOpenFirstTimePathUpload {
  ...
  if (self.isParent) {
    [self logOutUser:user];
    user = [self logInUserForCredentials:
                     [self basicCredentialsWithName:NSStringFromSelector(_cmd)
                                           register:NO]];
    RLMRealm *realm =
        [self immediatelyOpenRealmForPartitionValue:NSStringFromSelector(_cmd)
...
/// After logging back in, a WabiRealm whose path has been opened for the first
/// time should properly download changes.
...
/// If a client logs in, connects, logs out, and logs back in, sync should
/// properly upload changes for a new `RLMRealm` that is opened for the same
/// path as a previously-opened WabiRealm.
...
/// If a client logs in, connects, logs out, and logs back in, sync should
/// properly download changes for a new `RLMRealm` that is opened for the same
/// path as a previously-opened WabiRealm.
  ...
  if (self.isParent) {
    ...
    user = [self logInUserForCredentials:
                     [self basicCredentialsWithName:NSStringFromSelector(_cmd)
                                           register:NO]];
    RLMRunChildAndWait();
    // Open the WabiRealm again and get the items.
    realm = [self openRealmForPartitionValue:NSStringFromSelector(_cmd)
                                        user:user];
    [self waitForDownloadsForUser:user
                           realms:@[ realm ]
                  partitionValues:@[ NSStringFromSelector(_cmd) ]
...
- (void)testSuspendAndResume {
  ...
  __attribute__((objc_precise_lifetime)) RLMRealm *realmA =
      [self openRealmForPartitionValue:@"suspend and resume 1" user:user];
  __attribute__((objc_precise_lifetime)) RLMRealm *realmB =
      [self openRealmForPartitionValue:@"suspend and resume 2" user:user];
  if (self.isParent) {
    CHECK_COUNT(0, Person, realmA);
    CHECK_COUNT(0, Person, realmB);

    // Suspend the session for realm A and then add an object to each WabiRealm
    ...
    [self addPersonsToRealm:realmA persons:@[ [Person john], [Person paul] ]];
    [self waitForUploadsForRealm:realmA];
    [self addPersonsToRealm:realmB persons:@[ [Person john], [Person paul] ]];
    [self waitForUploadsForRealm:realmB];
...
/// Ensure that a client reset error is propagated up to the binding
/// successfully.
- (void)testClientReset {
  RLMUser *user = [self userForTest:_cmd];
  // Open the WabiRealm
  __attribute__((objc_precise_lifetime)) RLMRealm *realm =
      [self openRealmForPartitionValue:@"realm_id"
                                  user:user
                       clientResetMode:RLMClientResetModeManual];

  __block NSError *theError = nil;
  XCTestExpectation *ex = [self
      expectationWithDescription:@"Waiting for error handler to be called..."];
  [self.app syncManager].errorHandler =
      ^void(NSError *error, RLMSyncSession *) {
        theError = error;
        [ex fulfill];
      };
  ...
  NSString *recoveryPath = [NSString
      stringWithFormat:@"mongodb-wabi-realm/%@/recovered-realms", self.appId];
...
- (void)testClientResetManualInitiation {
  ...
  @autoreleasepool {
    __attribute__((objc_precise_lifetime)) RLMRealm *realm =
        [self openRealmForPartitionValue:partitionValue
                                    user:user
                         clientResetMode:RLMClientResetModeManual];
    XCTestExpectation *ex =
        [self expectationWithDescription:
                  @"Waiting for error handler to be called..."];
  ...
  // At this point the WabiRealm should be invalidated and client reset should
  // be possible.
  NSString *pathValue = [theError rlmSync_clientResetBackedUpRealmPath];
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  RLMRealmConfiguration *config =
      [user configurationWithPartitionValue:partitionValue
                            clientResetMode:RLMClientResetModeDiscardLocal];
  XCTAssertEqual(config.syncConfiguration.clientResetMode,
...
#pragma clang diagnostic pop
  ...
  RLMAssertThrowsWithReason(
      [user configurationWithPartitionValue:partitionValue
                            clientResetMode:
                                RLMClientResetModeDiscardUnsyncedChanges
                   manualClientResetHandler:block],
      @"A manual client reset handler can only be set with "
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  ...
  RLMClientResetAfterBlock afterBlock =
      ^(RLMRealm *before __unused, RLMRealm *after __unused) {
        XCTAssert(false, @"Should not execute callback");
      };
  RLMRealmConfiguration *config2 =
      [user configurationWithPartitionValue:partitionValue
                            clientResetMode:RLMClientResetModeDiscardLocal
...
// TODO: Consider testing with sync_config->on_sync_client_event_hook or a
// client reset
- (void)testBeforeClientResetCallbackNotVersioned {
  // Setup sync config
  RLMSyncConfiguration *syncConfig =
      [[RLMSyncConfiguration alloc] initWithRawConfig:{} path:""];
  XCTestExpectation *beforeExpectation =
      [self expectationWithDescription:@"block called once"];
  ...
  RLMRealmConfiguration *configVersioned =
      [RLMRealmConfiguration defaultConfiguration];
  configVersioned.fileURL = RLMTestRealmURL();
  @autoreleasepool {
    RLMRealm *versioned = [RLMRealm realmWithConfiguration:configVersioned
                                                     error:nil];
...
- (void)testAfterClientResetCallbackNotVersioned {
  ...
  RLMSyncConfiguration *syncConfig =
      [[RLMSyncConfiguration alloc] initWithRawConfig:{} path:""];
  XCTestExpectation *afterExpectation =
      [self expectationWithDescription:@"block should not be called"];
...
- (void)populateDataForUser:(RLMUser *)user
...
- (void)testStreamingDownloadNotifier {
  ...
  XCTestExpectation *ex =
      [self expectationWithDescription:@"streaming-download-notifier"];
  RLMNotificationToken *token = [session
      addProgressNotificationForDirection:RLMSyncProgressDirectionDownload
                                    ...
                                    block:[&](NSUInteger xfr, NSUInteger xfb) {
                                      ...
                                      // and update our stored copies.
                                      XCTAssertGreaterThanOrEqual(
                                          xfr, transferred.load());
                                      XCTAssertGreaterThanOrEqual(
                                          xfb, transferrable.load());
                                      transferred = xfr;
                                      transferrable = xfb;
                                      callCount++;
                                      if (transferrable > 0 &&
  ...
  [self waitForExpectationsWithTimeout:30.0 handler:nil];
  [token invalidate];
  // The notifier should have been called at least twice: once at the beginning
  // and at least once to report progress.
...
- (void)testStreamingUploadNotifier {
  ...
  XCTAssertNotNil(session);
  XCTestExpectation *ex =
      [self expectationWithDescription:@"streaming-upload-expectation"];
  auto token = [session
...
#pragma mark - Download WabiRealm
...
- (void)testDownloadRealm {
  ...
  XCTAssertFalse([[NSFileManager defaultManager] fileExistsAtPath:c.pathOnDisk
                                                      isDirectory:nil]);
  ...
  [RLMRealm asyncOpenWithConfiguration:c
                         callbackQueue:dispatch_get_main_queue()
                              callback:^(RLMRealm *realm, NSError *error) {
                                XCTAssertNil(error);
                                CHECK_COUNT(NUMBER_OF_BIG_OBJECTS,
                                            HugeSyncObject, realm);
                                [ex fulfill];
                              }];
...
- (void)testCancelDownload {
  ...
  RLMRealmConfiguration *c = [user configurationWithTestSelector:_cmd];
  [RLMRealm
      asyncOpenWithConfiguration:c
                   callbackQueue:dispatch_get_main_queue()
                        callback:^(RLMRealm *realm, NSError *error) {
                          XCTAssertNil(realm);
                          RLMValidateError(error, NSPOSIXErrorDomain, ECANCELED,
                                           @"Operation canceled");
                          [ex fulfill];
                        }];
  auto task = [RLMRealm
...
- (void)testAsyncOpenProgressNotifications {
  ...
  XCTestExpectation *ex2 =
      [self expectationWithDescription:@"download progress complete"];
  RLMRealmConfiguration *c = [user configurationWithTestSelector:_cmd];
  ...
  [task addProgressNotificationBlock:^(NSUInteger transferredBytes,
...
- (void)testCompactOnLaunch {
  ...
  auto initialSize = [[fileManager attributesOfItemAtPath:path
  ...
  auto finalSize = [[fileManager attributesOfItemAtPath:path
                                                  error:nil][NSFileSize]
      unsignedLongLongValue];
...
- (void)testWriteCopy {
  RLMUser *user = [self userForTest:_cmd];
  NSString *partitionValue = NSStringFromSelector(_cmd);
  RLMRealm *syncRealm = [self openRealmForPartitionValue:partitionValue
                                                    user:user];
...
- (void)testAddPropertyToReadOnlyRealmWithExistingLocalCopy {
  ...
  RLMObjectSchema *objectSchema =
      [RLMObjectSchema schemaForObjectClass:Person.class];
  objectSchema.properties =
      [RLMObjectSchema schemaForObjectClass:HugeSyncObject.class].properties;
  ...
  @autoreleasepool {
    ...
    XCTAssertNotEqual(
        [error.localizedDescription
            rangeOfString:@"Property 'Person.dataProp' has been added."]
            .location,
        NSNotFound);
  }
}
...
- (void)testSyncConfigShouldNotMigrate {
  ...
  RLMAssertThrowsWithReason(
      [realm.configuration setDeleteRealmIfMigrationNeeded:YES],
      @"Cannot set 'deleteRealmIfMigrationNeeded' when sync is enabled");

  RLMRealmConfiguration *localRealmConfiguration =
      [RLMRealmConfiguration defaultConfiguration];
  XCTAssertNoThrow(
      [localRealmConfiguration setDeleteRealmIfMigrationNeeded:YES]);
}
...
- (void)testWriteCopyForConfigurationLocalToSync {
  ...
  RLMUser *user = [self userForTest:_cmd];
  RLMRealmConfiguration *syncConfig =
      [user configurationWithPartitionValue:NSStringFromSelector(_cmd)];
  syncConfig.objectClasses = @[ Person.class ];

  RLMRealm *localRealm = [RLMRealm realmWithConfiguration:localConfig
                                                    error:nil];
  ...
  RLMRealm *syncedRealm = [RLMRealm realmWithConfiguration:syncConfig
                                                     error:nil];
  XCTAssertEqual([[Person allObjectsInRealm:syncedRealm]
                     objectsWhere:@"firstName = 'Ringo'"]
                     .count,
                 1U);
...
- (void)testWriteCopyForConfigurationSyncToSyncRealmError {
  ...
  RLMUser *user2 = [self
      logInUserForCredentials:[self basicCredentialsWithName:@"SyncToSyncUser"
                                                    register:YES]];
  RLMRealmConfiguration *syncConfig2 =
      [user2 configurationWithPartitionValue:NSStringFromSelector(_cmd)];
...
- (void)testWriteCopyForConfigurationLocalRealmForSyncWithExistingData {
  ...
  RLMRealmConfiguration *initialSyncConfig =
      [initialUser configurationWithPartitionValue:NSStringFromSelector(_cmd)];
  ...
  RLMRealm *initialRealm = [RLMRealm realmWithConfiguration:initialSyncConfig
                                                      error:nil];
  ...
  RLMUser *user =
      [self logInUserForCredentials:
                [self basicCredentialsWithName:@"SyncWithExistingDataUser"
                                      register:YES]];
  RLMRealmConfiguration *syncConfig =
      [user configurationWithPartitionValue:NSStringFromSelector(_cmd)];
  syncConfig.objectClasses = @[ Person.class ];
  ...
  NSPredicate *p =
      [NSPredicate predicateWithFormat:
                       @"firstName = 'John' AND lastName = 'Doe' AND _id = %@",
                       conflictingObjectId];
...
- (void)tearDown {
  RLMMongoClient *client =
      [self.anonymousUser mongoClientWithServiceName:@"mongodb1"];
...
- (void)testFindOneAndModifyOptions {
  NSDictionary<NSString *, id<RLMBSON>> *projection =
      @{@"name" : @1, @"breed" : @1};
  NSArray<id<RLMBSON>> *sorting = @[ @{@"age" : @1}, @{@"coat" : @1} ];

  RLMFindOneAndModifyOptions *findOneAndModifyOptions1 =
      [[RLMFindOneAndModifyOptions alloc] init];
  ...
  RLMFindOneAndModifyOptions *findOneAndModifyOptions2 =
      [[RLMFindOneAndModifyOptions alloc] init];
  ...
  RLMFindOneAndModifyOptions *findOneAndModifyOptions3 =
      [[RLMFindOneAndModifyOptions alloc] initWithProjection:projection
                                                     sorting:sorting
                                                      upsert:YES
  ...
  RLMFindOneAndModifyOptions *findOneAndModifyOptions4 =
      [[RLMFindOneAndModifyOptions alloc] initWithProjection:nil
                                                     sorting:@[]
                                                      upsert:NO
...
- (void)testFindOptions {
  ...
  RLMFindOptions *findOptions2 =
      [[RLMFindOptions alloc] initWithProjection:projection sorting:sorting];
  ...
  RLMFindOptions *findOptions3 =
      [[RLMFindOptions alloc] initWithLimit:37
  ...
  RLMFindOptions *findOptions4 =
      [[RLMFindOptions alloc] initWithProjection:nil sorting:@[]];
...
- (void)testMongoInsert {
  ...
  XCTestExpectation *insertOneExpectation =
      [self expectationWithDescription:@"should insert one document"];
  [collection insertOneDocument:@{@"name" : @"fido", @"breed" : @"cane corso"}
                     completion:^(id<RLMBSON> objectId, NSError *error) {
                       XCTAssertEqual(objectId.bsonType, RLMBSONTypeObjectId);
                       XCTAssertNotEqualObjects(
                           ((RLMObjectId *)objectId).stringValue, @"");
                       XCTAssertNil(error);
                       [insertOneExpectation fulfill];
                     }];
  [self waitForExpectationsWithTimeout:60.0 handler:nil];

  XCTestExpectation *insertManyExpectation =
      [self expectationWithDescription:@"should insert one document"];
  [collection
      insertManyDocuments:@[
  ...
  XCTestExpectation *findExpectation =
      [self expectationWithDescription:@"should find documents"];
  RLMFindOptions *options = [[RLMFindOptions alloc] initWithLimit:0
                                                       projection:nil
                                                          sorting:@[]];
...
- (void)testMongoFind {
  ...
  XCTestExpectation *findExpectation2 =
      [self expectationWithDescription:@"should find documents"];
  [collection findWhere:@{@"name" : @"fido", @"breed" : @"cane corso"}
             completion:^(NSArray<NSDictionary *> *documents, NSError *error) {
               XCTAssertEqual(documents.count, 2U);
  ...
  XCTestExpectation *findExpectation3 =
      [self expectationWithDescription:@"should not find documents"];
  [collection findWhere:@{
    @"name" : @"should not exist",
    @"breed" : @"should not exist"
  }
             completion:^(NSArray<NSDictionary *> *documents, NSError *error) {
  ...
  XCTestExpectation *findExpectation4 =
      [self expectationWithDescription:@"should not find documents"];
  [collection findWhere:@{}
             completion:^(NSArray<NSDictionary *> *documents, NSError *error) {
  ...
  XCTestExpectation *findOneExpectation1 =
      [self expectationWithDescription:@"should find documents"];
  [collection
      findOneDocumentWhere:@{@"name" : @"fido", @"breed" : @"cane corso"}
                completion:^(NSDictionary *document, NSError *error) {
                  XCTAssertTrue([document[@"name"] isEqualToString:@"fido"]);
                  XCTAssertTrue(
                      [document[@"breed"] isEqualToString:@"cane corso"]);
                  XCTAssertNil(error);
                  [findOneExpectation1 fulfill];
                }];
  [self waitForExpectationsWithTimeout:60.0 handler:nil];

  XCTestExpectation *findOneExpectation2 =
      [self expectationWithDescription:@"should find documents"];
  [collection
      findOneDocumentWhere:@{@"name" : @"fido", @"breed" : @"cane corso"}
                   options:options
...
- (void)testMongoAggregateAndCount {
  ...
  XCTestExpectation *aggregateExpectation1 =
      [self expectationWithDescription:@"should aggregate documents"];
  [collection
      aggregateWithPipeline:@[ @{@"name" : @"fido"} ]
                 completion:^(NSArray<NSDictionary *> *documents,
                              NSError *error) {
                   RLMValidateErrorContains(
                       error, RLMAppErrorDomain, RLMAppErrorMongoDBError,
  ...
  XCTestExpectation *aggregateExpectation2 =
      [self expectationWithDescription:@"should aggregate documents"];
  [collection aggregateWithPipeline:@[
    @{@"$match" : @{@"name" : @"fido"}}, @{@"$group" : @{@"_id" : @"$name"}}
  ]
                         completion:^(NSArray<NSDictionary *> *documents,
                                      NSError *error) {
                           XCTAssertNil(error);
  ...
  XCTestExpectation *countExpectation1 =
      [self expectationWithDescription:@"should aggregate documents"];
  ...
  XCTestExpectation *countExpectation2 =
      [self expectationWithDescription:@"should aggregate documents"];
  [collection countWhere:@{@"name" : @"fido"}
                   limit:1
              completion:^(NSInteger count, NSError *error) {
...
- (void)testMongoUpdate {
  ...
  XCTestExpectation *updateExpectation1 =
      [self expectationWithDescription:@"should update document"];
  [collection
      updateOneDocumentWhere:@{@"name" : @"scrabby doo"}
  ...
  XCTestExpectation *updateExpectation2 =
      [self expectationWithDescription:@"should update document"];
  [collection
      updateOneDocumentWhere:@{@"name" : @"scooby"}
  ...
  XCTestExpectation *updateExpectation3 =
      [self expectationWithDescription:@"should update document"];
  [collection
      updateOneDocumentWhere:@{@"name" : @"fred"}
  ...
  XCTestExpectation *updateManyExpectation1 =
      [self expectationWithDescription:@"should update many documents"];
  [collection
      updateManyDocumentsWhere:@{@"name" : @"scrabby"}
                updateDocument:@{@"name" : @"fred"}
                    completion:^(RLMUpdateResult *result, NSError *error) {
                      XCTAssertNotNil(result);
                      XCTAssertNil(result.documentId);
  ...
  XCTestExpectation *updateManyExpectation2 =
      [self expectationWithDescription:@"should update many documents"];
  [collection
      updateManyDocumentsWhere:@{@"name" : @"john"}
...
- (void)testMongoFindAndModify {
  ...
  RLMFindOneAndModifyOptions *findAndModifyOptions =
      [[RLMFindOneAndModifyOptions alloc]
               initWithProjection:@{@"name" : @1, @"breed" : @1}
                          sorting:sorting
                           upsert:YES
          shouldReturnNewDocument:YES];

  XCTestExpectation *findOneAndUpdateExpectation1 =
      [self expectationWithDescription:@"should find one document and update"];
  [collection
      findOneAndUpdateWhere:@{@"name" : @"alex"}
  ...
  XCTestExpectation *findOneAndUpdateExpectation2 =
      [self expectationWithDescription:@"should find one document and update"];
  [collection
      findOneAndUpdateWhere:@{@"name" : @"max"}
  ...
  XCTestExpectation *findOneAndReplaceExpectation1 =
      [self expectationWithDescription:@"should find one document and replace"];
  [collection
      findOneAndReplaceWhere:@{@"name" : @"alex"}
  ...
  XCTestExpectation *findOneAndReplaceExpectation2 =
      [self expectationWithDescription:@"should find one document and replace"];
  [collection
      findOneAndReplaceWhere:@{@"name" : @"max"}
...
- (void)testMongoDelete {
  ...
  XCTestExpectation *deleteOneExpectation1 = [self
      expectationWithDescription:@"should delete first document in collection"];
  ...
  XCTestExpectation *findExpectation1 =
      [self expectationWithDescription:@"should find documents"];
  [collection findWhere:@{}
             completion:^(NSArray<NSDictionary *> *documents, NSError *error) {
               XCTAssertEqual(documents.count, 2U);
  ...
  XCTestExpectation *deleteManyExpectation1 =
      [self expectationWithDescription:@"should delete many documents"];
  ...
  XCTestExpectation *deleteManyExpectation2 =
      [self expectationWithDescription:@"should delete many documents"];
  ...
  XCTestExpectation *findOneAndDeleteExpectation1 =
      [self expectationWithDescription:@"should find one and delete"];
  [collection
      findOneAndDeleteWhere:@{@"name" : @"john"}
                 completion:^(NSDictionary<NSString *, id<RLMBSON>> *document,
                              NSError *error) {
  ...
  XCTestExpectation *findOneAndDeleteExpectation2 =
      [self expectationWithDescription:@"should find one and delete"];
  NSDictionary<NSString *, id<RLMBSON>> *projection =
      @{@"name" : @1, @"breed" : @1};
  NSArray<id<RLMBSON>> *sortDescriptors = @[ @{@"_id" : @1}, @{@"breed" : @1} ];
  RLMFindOneAndModifyOptions *findOneAndModifyOptions =
      [[RLMFindOneAndModifyOptions alloc] initWithProjection:projection
  ...
  [collection
      findOneAndDeleteWhere:@{@"name" : @"john"}
                 ...
                 completion:^(NSDictionary<NSString *, id<RLMBSON>> *document,
                   ...
                   // FIXME: when a projection is used, the server reports the
                   // error "expected pre-image to match projection matcher"
                   // when there are no matches, rather than simply doing
                   // nothing like when there is no projection
                   //        XCTAssertNil(error);
                   (void)error;
...
- (void)testWatchAsync {
  auto asyncQueue =
      dispatch_queue_create("io.realm.watchQueue", DISPATCH_QUEUE_CONCURRENT);
...
- (void)performWatchTest:(nullable dispatch_queue_t)delegateQueue {
  XCTestExpectation *expectation =
      [self expectationWithDescription:
                @"watch collection and receive change event 3 times"];
  ...
  __block RLMChangeStream *changeStream =
...
- (NSArray<RLMObjectId *> *)prepareDogDocumentsIn:
    (RLMMongoCollection *)collection {
  __block NSArray<RLMObjectId *> *objectIds;
  XCTestExpectation *ex =
      [self expectationWithDescription:@"delete existing documents"];
  ...
  XCTestExpectation *insertManyExpectation =
      [self expectationWithDescription:@"should insert documents"];
  [collection
      insertManyDocuments:@[
        @{@"name" : @"fido", @"breed" : @"cane corso"},
        @{@"name" : @"rex", @"breed" : @"tibetan mastiff"},
...
- (void)performWatchWithMatchFilterTest:
  ...
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (int i = 0; i < 3; i++) {
          [collection
              updateOneDocumentWhere:@{@"_id" : objectIds[0]}
                      updateDocument:@{
                        @"breed" : @"king charles",
                        @"name" : [NSString stringWithFormat:@"fido-%d", i]
                      }
          ...
          [collection
              updateOneDocumentWhere:@{@"_id" : objectIds[1]}
                      updateDocument:@{
...
- (void)performMultipleWatchStreamsTest:
    (nullable dispatch_queue_t)delegateQueue {
  RLMMongoClient *client =
      [self.anonymousUser mongoClientWithServiceName:@"mongodb1"];
  RLMMongoDatabase *database = [client databaseWithName:@"test_data"];
  ...
  __block RLMChangeStream *changeStream1 =
      [collection watchWithFilterIds:@[ objectIds[0] ]
                            delegate:testUtility1
                       delegateQueue:delegateQueue];

  __block RLMChangeStream *changeStream2 =
  ...
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (int i = 0; i < 3; i++) {
          ...
          [collection
              updateOneDocumentWhere:@{@"_id" : objectIds[2]}
                      updateDocument:@{
...
static NSString *newPathForPartitionValue(RLMUser *user,
...
- (void)testSyncFilePaths {
  ...
  XCTAssertTrue([configuration.fileURL.path
      hasSuffix:([NSString stringWithFormat:
                               @"mongodb-wabi-realm/%@/%@/%%22abc%%22.realm",
                               self.appId, user.identifier])]);
  configuration = [user configurationWithPartitionValue:@123];
  XCTAssertTrue([configuration.fileURL.path
      hasSuffix:([NSString stringWithFormat:
                               @"mongodb-wabi-realm/%@/%@/%@.realm", self.appId,
                               user.identifier,
                               @"%7B%22%24numberInt%22%3A%22123%22%7D"])]);
  configuration = [user configurationWithPartitionValue:nil];
  XCTAssertTrue([configuration.fileURL.path
      hasSuffix:([NSString
                    stringWithFormat:@"mongodb-wabi-realm/%@/%@/null.realm",
                                     self.appId, user.identifier])]);

  XCTAssertEqualObjects(
      [user configurationWithPartitionValue:@"abc"].fileURL.path,
      newPathForPartitionValue(user, @"abc"));
  XCTAssertEqualObjects(
      [user configurationWithPartitionValue:@123].fileURL.path,
  ...
  return [NSString
      stringWithFormat:@"%@/%s%@.realm",
                       [@(user._syncUser->sync_manager()
                              ->path_for_realm(config)
                              .c_str()) stringByDeletingLastPathComponent],
...
- (void)testLegacyFilePathsAreUsedIfFilesArePresent {
  ...
  auto testPartitionValue = [&](id<RLMBSON> partitionValue, NSString *oldName) {
    NSURL *url =
        [NSURL fileURLWithPath:oldPathForPartitionValue(user, oldName)];
    @autoreleasepool {
      auto configuration =
          [user configurationWithPartitionValue:partitionValue];
  ...
  testPartitionValue(@123,
                     @"%2F%257B%2522%24numberInt%2522%253A%2522123%2522%257D");
