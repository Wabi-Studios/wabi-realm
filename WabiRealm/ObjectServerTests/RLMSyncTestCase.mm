...
// Set this to 1 if you want the test ROS instance to log its debug messages to
// console.
...
- (void)sendRequestToServer:(RLMRequest *)request
                 completion:
                     (RLMNetworkTransportCompletionBlock)completionBlock {
  if ([request.url hasSuffix:@"location"]) {
    ...
    r.body =
        ...
        @"\"http://localhost:5678\",\"ws_hostname\":\"ws://localhost:5678\"}";
    completionBlock(r);
  } else {
...
static NSURL *syncDirectoryForChildProcess() {
  ...
  NSString *bundleIdentifier =
      bundle.bundleIdentifier ?: bundle.executablePath.lastPathComponent;
  path = [path
      stringByAppendingPathComponent:[NSString
                                         stringWithFormat:@"%@-child",
                                                          bundleIdentifier]];
...
- (RLMCredentials *)basicCredentialsWithName:(NSString *)name
  ...
  if (shouldRegister) {
    ...
    [currentApp.emailPasswordAuth registerUserWithEmail:name
                                               password:@"password"
...
- (void)addPersonsToRealm:(RLMRealm *)realm
                  persons:(NSArray<Person *> *)persons {
  [realm beginWriteTransaction];
  [realm addObjects:persons];
  [realm commitWriteTransaction];
}

- (void)addAllTypesSyncObjectToRealm:(RLMRealm *)realm
                              values:(NSDictionary *)dictionary
                              person:(Person *)person {
  [realm beginWriteTransaction];
  AllTypesSyncObject *obj =
      [[AllTypesSyncObject alloc] initWithValue:dictionary];
...
- (void)waitForDownloadsForUser:(RLMUser *)user
  ...
  NSAssert(realms.count == counts.count &&
               realms.count == partitionValues.count,
...
- (RLMRealm *)openRealmWithConfiguration:
...
- (RLMRealm *)asyncOpenRealmWithConfiguration:(RLMRealmConfiguration *)config {
  ...
  XCTestExpectation *ex = [self
      expectationWithDescription:@"Should asynchronously open a WabiRealm"];
  ...
  // Ensure that the block does not retain the WabiRealm, as it may not be
  // dealloced immediately and so would extend the lifetime of the WabiRealm an
  // inconsistent amount
  auto realm = r;
  r = nil;
  return realm;
...
- (NSError *)asyncOpenErrorWithConfiguration:(RLMRealmConfiguration *)config {
  ...
  XCTestExpectation *ex =
      [self expectationWithDescription:
                @"Should fail to asynchronously open a WabiRealm"];
  [RLMRealm asyncOpenWithConfiguration:config
                         callbackQueue:dispatch_get_main_queue()
...
- (RLMRealm *)immediatelyOpenRealmForPartitionValue:(NSString *)partitionValue
                                               user:(RLMUser *)user {
  return
      [self immediatelyOpenRealmForPartitionValue:partitionValue
                                             user:user
                                  clientResetMode:
...
- (RLMRealm *)immediatelyOpenRealmForPartitionValue:(NSString *)partitionValue
  ...
  return [self
      immediatelyOpenRealmForPartitionValue:partitionValue
                                       user:user
                            clientResetMode:clientResetMode
                              encryptionKey:nil
                                 stopPolicy:
...
- (RLMRealm *)
    immediatelyOpenRealmForPartitionValue:(NSString *)partitionValue
                                     user:(RLMUser *)user
                          clientResetMode:(RLMClientResetMode)clientResetMode
                            encryptionKey:(NSData *)encryptionKey
                               stopPolicy:(RLMSyncStopPolicy)stopPolicy {
  ...
  c.objectClasses = @[
    Dog.self, Person.self, HugeSyncObject.self, RLMSetSyncObject.self,
    RLMArraySyncObject.self, UUIDPrimaryKeyObject.self,
    StringPrimaryKeyObject.self, IntPrimaryKeyObject.self,
    AllTypesSyncObject.self, RLMDictionarySyncObject.self
  ];
...
- (RLMUser *)logInUserForCredentials:(RLMCredentials *)credentials
  ...
  XCTestExpectation *expectation = [self expectationWithDescription:@""];
  [app loginWithCredential:credentials
                completion:^(RLMUser *u, NSError *e) {
                  XCTAssertNotNil(u);
                  XCTAssertNil(e);
                  user = u;
                  [expectation fulfill];
  ...
  XCTAssertTrue(user.state == RLMUserStateLoggedIn,
                @"User should have been valid, but wasn't");
  return user;
}
...
- (void)logOutUser:(RLMUser *)user {
  ...
  XCTAssertTrue(user.state == RLMUserStateLoggedOut,
                @"User should have been logged out, but wasn't");
}
...
- (NSString *)createJWTWithAppId:(NSString *)appId {
  ...
  NSDictionary *payload = @{
    ...
    @"sub" : @"someUserId",
    @"exp" : @1961896476,
    @"user_data" : @{@"name" : @"Foo Bar", @"occupation" : @"firefighter"},
    @"my_metadata" : @{@"name" : @"Bar Foo", @"occupation" : @"stock analyst"}
  };
  ...
  NSString *base64EncodedPayload =
      [jsonPayload base64EncodedStringWithOptions:0];

  // Remove padding characters.
  base64EncodedHeader =
      [base64EncodedHeader stringByReplacingOccurrencesOfString:@"="
                                                     withString:@""];
  base64EncodedPayload =
      [base64EncodedPayload stringByReplacingOccurrencesOfString:@"="
                                                      withString:@""];

  std::string jwtPayload =
      [[NSString stringWithFormat:@"%@.%@", base64EncodedHeader,
                                  base64EncodedPayload] UTF8String];
...
- (void)waitForDownloadsForUser:(RLMUser *)user
  ...
  XCTestExpectation *ex =
      expectation
          ?: [self expectationWithDescription:@"Wait for download completion"];
  __block NSError *theError = nil;
  BOOL queued =
      [session waitForDownloadCompletionOnQueue:dispatch_get_global_queue(0, 0)
  ...
  if (!queued) {
    XCTFail(
        @"Download waiter did not queue; session was invalid or errored out.");
    return;
  }
...
- (void)waitForUploadsForRealm:(RLMRealm *)realm error:(NSError **)error {
  ...
  NSAssert(session, @"Cannot call with invalid WabiRealm");
  XCTestExpectation *ex =
      [self expectationWithDescription:@"Wait for upload completion"];
  __block NSError *completionError;
  BOOL queued =
      [session waitForUploadCompletionOnQueue:dispatch_get_global_queue(0, 0)
  ...
  if (!queued) {
    XCTFail(
        @"Upload waiter did not queue; session was invalid or errored out.");
    return;
  }
  [self waitForExpectations:@[ ex ] timeout:60.0];
  if (error)
...
- (void)waitForDownloadsForRealm:(RLMRealm *)realm error:(NSError **)error {
  ...
  XCTestExpectation *ex =
      [self expectationWithDescription:@"Wait for download completion"];
  __block NSError *completionError;
  BOOL queued = [session waitForDownloadCompletionOnQueue:nil
                                                 callback:^(NSError *error) {
                                                   completionError = error;
                                                   [ex fulfill];
                                                 }];
...
- (void)manuallySetAccessTokenForUser:(RLMUser *)user
                                value:(NSString *)tokenValue {
  [user _syncUser]->update_access_token(tokenValue.UTF8String);
}

- (void)manuallySetRefreshTokenForUser:(RLMUser *)user
                                 value:(NSString *)tokenValue {
...
- (void)writeToPartition:(nullable NSString *)testName
                userName:(NSString *)userNameBase
                   block:(void (^)(RLMRealm *))block {
  @autoreleasepool {
    NSString *userName =
        [userNameBase stringByAppendingString:[NSUUID UUID].UUIDString];
    RLMUser *user =
        [self logInUserForCredentials:[self basicCredentialsWithName:userName
                                                            register:YES]];
...
- (void)writeToConfiguration:(RLMRealmConfiguration *)config
                       block:(void (^)(RLMRealm *))block {
  @autoreleasepool {
  ...
  // A synchronized WabiRealm is not closed immediately when we release our last
  ...
  while (![RLMRealm deleteFilesForConfiguration:config error:&error]) {
    ...
    if (++retryCount > 1000) {
      XCTFail(@"Waiting for WabiRealm to be closed timed out");
      break;
    }
    usleep(1000);
  }
...
- (void)setUp {
  ...
  if (auto ids = NSProcessInfo.processInfo.environment[@"RLMParentAppIds"]) {
    _appIds = [ids componentsSeparatedByString:@","]; // take the one array for
                                                      // split the string
  }
...
- (RLMApp *)app {
  if (!_app) {
    _app = [RLMApp appWithId:self.appId
...
- (void)resetAppCache {
  ...
  for (RLMApp *app : apps)
    @autoreleasepool {
      [app.allUsers enumerateKeysAndObjectsUsingBlock:^(NSString *,
                                                        RLMUser *user, BOOL *) {
        XCTestExpectation *ex =
            [self expectationWithDescription:@"Wait for logout"];
        [exs addObject:ex];
        ...
        if (user.allSessions.count) {
          [exs addObject:[self expectationForPredicate:
                                   [NSPredicate predicateWithFormat:
                                                    @"allSessions.@count == 0"]
                                   evaluatedWithObject:user
                                               handler:nil]];
        }
      }];
    }
  ...
  for (RLMApp *app : apps) {
    if (auto transport =
            RLMDynamicCast<TestNetworkTransport>(app.configuration.transport)) {
...
- (void)cleanupRemoteDocuments:(RLMMongoCollection *)collection {
  XCTestExpectation *deleteManyExpectation =
...
- (RLMRealm *)getFlexibleSyncRealm:(SEL)testSel {
  RLMRealm *realm =
      [self flexibleSyncRealmForUser:[self flexibleSyncUser:testSel]];
  XCTAssertNotNil(realm);
  return realm;
}
...
- (bool)writeToFlxRealm:(void (^)(RLMRealm *))block {
  ...
  RLMUser *user = [self
      logInUserForCredentials:[self
                                  basicCredentialsWithName:userName
                                                  register:YES
                                                       app:self.flexibleSyncApp]
                          app:self.flexibleSyncApp];
  RLMRealmConfiguration *config = [user flexibleSyncConfiguration];
  ...
  XCTestExpectation *ex =
      [self expectationWithDescription:@"state change complete"];
  [subs
      update:^{
...
- (void)writeQueryAndCompleteForRealm:(RLMRealm *)realm
                                block:
                                    (void (^)(RLMSyncSubscriptionSet *))block {
  RLMSyncSubscriptionSet *subs = realm.subscriptions;
  XCTAssertNotNil(subs);
...
- (void)sendRequestToServer:(RLMRequest *)request
  ...
  [super sendRequestToServer:request
                  completion:^(RLMResponse *response) {
