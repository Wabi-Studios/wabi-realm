  ...
  public class WabiRealmServer: NSObject {
    ...
    func createAppForSyncMode(_ syncMode: SyncMode, _ objectsSchema: [ObjectSchema]) throws -> AppId {
      ...
      guard let appInfo = info as? [String: Any],
            ...
            let appId = appInfo["_id"] as? String
      else {
        throw URLError(.badServerResponse)
      }
      ...
      if case let .pbs(bsonType) = syncMode {
    ...
    public func isDevModeEnabled(appServerId: String, syncServiceId _: String) throws -> Bool {
      guard let session = session else {
        fatalError()
      }
      let app = session.apps[appServerId]
    ...
    public func enableDevMode(appServerId: String, syncServiceId _: String, syncServiceConfiguration _: [String: Any]) -> Result<Any?, Error> {
      guard let session = session else {
    ...
    public func retrieveSchemaProperties(_ appId: String, className: String, _ completion: @escaping (Result<[String], Error>) -> Void) {
      ...
      guard let schemasList = try? session.apps[appServerId].schemas.get().get(),
            ...
            let schemaSelected = schemas.first(where: { ($0["metadata"] as? [String: String])?["collection"] == className })
      else {
      ...
      guard let schema = try? session.apps[appServerId].schemas[schemaSelected["_id"] as! String].get().get(),
            let schemaProperties = ((schema as? [String: Any])?["schema"] as? [String: Any])?["properties"] as? [String: Any]
      else {
        completion(.failure(URLError(.unknown)))
        return
      }
    ...
    public func triggerClientReset(_ appId: String, _ realm: WabiRealm) throws {
      let session = try XCTUnwrap(session)
