...
////////////////////////////////////////////////////////////////////////////
  ...
  func assertSyncError(_ error: Error, _ code: SyncError.Code, _: String,
                       line: UInt = #line, file: StaticString = #file)
  {
  ...
  class SwiftObjectServerTests: SwiftSyncTestCase {
    ...
    func testBasicSwiftSync() throws {
      ...
      XCTAssert(realm.isEmpty, "Freshly synced WabiRealm was not empty...")
    }
    ...
    /// If client B adds objects to a WabiRealm, client A should see those new objects.
    ...
    func testSwiftRountripForDistinctPrimaryKey() throws {
      ...
      if isParent {
        ...
        try realm.write {
          ...
          realm.add(SwiftIntPrimaryKeyObject(id: 1_234_567_890, strCol: "Jackson", intCol: 30))
        }
        waitForUploads(for: realm)
        checkCount(expected: 1, realm, SwiftPerson.self)
        checkCount(expected: 1, realm, SwiftUUIDPrimaryKeyObject.self)
    ...
    /// If client B removes objects from a WabiRealm, client A should see those changes.
    ...
    func testClientReset() throws {
      ...
      XCTAssertTrue(resetInfo.0.contains("mongodb-wabi-realm/\(appId)/recovered-realms/recovered_realm"))
    ...
    // as confirmation.
    func waitForServerHistoryAfterRestart(realm: WabiRealm) {
      let start = Date()
    ...
    func prepareClientReset(_ partition: String, _ user: User) throws {
      ...
      // Write a different object in a different WabiRealm which should appear in
    ...
    func prepareFlexibleClientReset(disableRecoveryMode: Bool = false) throws -> (User, String) {
      ...
      try autoreleasepool {
        ...
        let newRealm = try WabiRealm(configuration: newConfig)
    ...
    func assertManualClientReset(_ user: User, app: App) -> ErrorReportingBlock {
      let ex = expectation(description: "get client reset error")
      return { error, session in
        ...
        XCTAssertTrue(resetInfo.0.contains("mongodb-wabi-realm/\(app.appId)/recovered-realms/recovered_realm"))
        SyncSession.immediatelyHandleError(resetInfo.1, syncManager: app.syncManager)
        ex.fulfill()
      }
    }

    func assertDiscardLocal() -> (@Sendable (WabiRealm) -> Void, @Sendable (WabiRealm, WabiRealm) -> Void) {
      let beforeCallbackEx = expectation(description: "before reset callback")
      @Sendable func beforeClientReset(_ before: WabiRealm) {
      ...
      @Sendable func afterClientReset(_ before: WabiRealm, _ after: WabiRealm) {
        let results = before.objects(SwiftPerson.self)
        XCTAssertEqual(results.count, 1)
        XCTAssertEqual(results.filter("firstName == 'John'").count, 1)
    ...
    func assertRecover() -> (@Sendable (WabiRealm) -> Void, @Sendable (WabiRealm, WabiRealm) -> Void) {
      let beforeCallbackEx = expectation(description: "before reset callback")
      @Sendable func beforeClientReset(_ before: WabiRealm) {
        let results = before.objects(SwiftPerson.self)
        XCTAssertEqual(results.count, 1)
    ...
    func testClientResetManual() throws {
      ...
      try autoreleasepool {
        let realm = try WabiRealm(configuration: configuration)
        waitForDownloads(for: realm)
        // After reopening, the old WabiRealm file should have been moved aside
    ...
    func testClientResetManualEnumCallbackNotManager() throws {
      ...
      try autoreleasepool {
        ...
        syncManager.errorHandler = { error, _ in
          guard error as? SyncError != nil else {
            XCTFail("Bad error type: \(error)")
            return
          }
    ...
    func testClientResetManualWithoutLiveRealmInstance() throws {
      ...
      try autoreleasepool {
        _ = try WabiRealm(configuration: configuration)
        ...
        // need to ensure the WabiRealm is deallocated before the error handler
    ...
    func testClientResetDiscardLocal() throws {
      ...
      case let .discardUnsyncedChanges(before, after):
    ...
    func testClientResetDiscardLocalAsyncOpen() throws {
      ...
      WabiRealm.asyncOpen(configuration: configuration) { result in
    ...
    func testClientResetRecover() throws {
      ...
      let syncConfig = try XCTUnwrap(configuration.syncConfiguration)
      switch syncConfig.clientResetMode {
      case let .recoverUnsyncedChanges(before, after):
        XCTAssertNotNil(before)
        XCTAssertNotNil(after)
      default:
    ...
    func testClientResetRecoverOrDiscardLocalFailedRecovery() throws {
      ...
      let syncConfig = try XCTUnwrap(configuration.syncConfiguration)
      switch syncConfig.clientResetMode {
      case let .recoverOrDiscardUnsyncedChanges(before, after):
        XCTAssertNotNil(before)
        XCTAssertNotNil(after)
      default:
    ...
    func testStreamingDownloadNotifier() throws {
      ...
      var minimumDownloadSize = 1_000_000
      var callCount = 0
      ...
      minimumDownloadSize = p1.transferredBytes + 1_000_000
    ...
    func testDownloadRealm() throws {
      ...
      WabiRealm.asyncOpen(configuration: config) { result in
        switch result {
    ...
    func testCancelDownloadRealm() throws {
      ...
      let config = user.configuration(testName: #function)
      let completion = { (result: Result<WabiRealm, Error>) in
      ...
      WabiRealm.asyncOpen(configuration: config, callback: completion)
      let task = WabiRealm.asyncOpen(configuration: config, callback: completion)
    ...
    func testAsyncOpenProgress() throws {
      ...
      let config = user.configuration(testName: #function)
      let task = WabiRealm.asyncOpen(configuration: config) { result in
    ...
    func testInvalidCredentials() throws {
      ...
      app.login(credentials: credentials2) { result in
        guard case .failure = result else {
    ...
    func testUserExpirationCallback() throws {
      ...
      // Try to open a WabiRealm with the user; this will cause our errorHandler block defined above to be fired.
    ...
    func testSeedFilePathOpenLocalToSync() throws {
      var config = WabiRealm.Configuration()
      config.fileURL = RLMTestRealmURL()
      ...
      let destinationRealm = try WabiRealm(configuration: syncConfig)
    ...
    func testSeedFilePathOpenSyncToLocal() throws {
      ...
      let syncRealm = try WabiRealm(configuration: syncConfig)
    ...
    func testWriteCopySyncedRealm() throws {
      ...
      config.objectTypes = [SwiftHugeSyncObject.self]
      let syncedRealm = try WabiRealm(configuration: config)
    ...
    func testWriteCopyLocalRealmToSync() throws {
      ...
      let localRealm = try WabiRealm(configuration: localConfig)
      ...
      let syncedRealm = try WabiRealm(configuration: syncConfig)
    ...
    func testWriteCopyLocalRealmForSyncWithExistingData() throws {
      ...
      let initialRealm = try WabiRealm(configuration: initialSyncConfig)
    ...
    func testWriteCopyFailBeforeSynced() throws {
      ...
      let user1Realm = try WabiRealm(configuration: user1Config)
      ...
      let realm = try WabiRealm(configuration: user1Config)
      realm.syncSession?.suspend()
      try realm.write {
        realm.add(SwiftPerson())
      }
  ...
  class AnyRealmValueSyncTests: SwiftSyncTestCase {
    ...
    /// version of an app does not have that WabiRealm Object / Schema we can still access that object via
    ...
    func testMissingSchema() throws {
      ...
      // but v2 managed to sync `SwiftMissingObject` to this WabiRealm.
      var config = user.configuration(partitionValue: #function)
  ...
  class CombineObjectServerTests: SwiftSyncTestCase {
    ...
    func testAsyncOpenCombine() {
      ...
      app.emailPasswordAuth.registerUser(email: email, password: password)
        ...
        .flatMap { @Sendable user in
          WabiRealm.asyncOpen(configuration: user.configuration(testName: #function))
        }
      ...
      app.login(credentials: .anonymous)
        .flatMap { @Sendable in
          WabiRealm.asyncOpen(configuration: $0.configuration(testName: #function)).onProgressNotification {
            if $0.isTransferComplete {
              progressEx.fulfill()
            }
          }
    ...
    func testAsyncOpenStandaloneCombine() throws {
      ...
      WabiRealm.asyncOpen().await(self) { realm in
  ...
  class AsyncAwaitObjectServerTests: SwiftSyncTestCase {
    ...
    @MainActor func testAsyncOpenStandalone() async throws {
      try autoreleasepool {
        let configuration = WabiRealm.Configuration(objectTypes: [SwiftPerson.self])
        let realm = try WabiRealm(configuration: configuration)
        try realm.write {
      ...
      let configuration = WabiRealm.Configuration(objectTypes: [SwiftPerson.self])
      let realm = try await WabiRealm(configuration: configuration)
      XCTAssertEqual(realm.objects(SwiftPerson.self).count, 10)
    }

    @MainActor func testAsyncOpenSync() async throws {
      let user = try await app.login(credentials: basicCredentials())
      let realm = try await WabiRealm(configuration: user.configuration(testName: #function))
      try realm.write {
        realm.add(SwiftHugeSyncObject.create())
        realm.add(SwiftHugeSyncObject.create())
      }
      ...
      let realm2 = try await WabiRealm(configuration: user2.configuration(testName: #function),
    ...
    @MainActor func testAsyncOpenDownloadBehaviorNever() async throws {
      // Populate the WabiRealm on the server
      let user1 = try await app.login(credentials: basicCredentials())
      let realm1 = try await WabiRealm(configuration: user1.configuration(testName: #function))
      try realm1.write {
    ...
    @MainActor func testAsyncOpenDownloadBehaviorOnce() async throws {
      ...
      let realm3 = try await WabiRealm(configuration: user2.configuration(testName: #function),
                                       downloadBeforeOpen: .once)
    ...
    @MainActor func testAsyncOpenDownloadBehaviorAlwaysWithFreshRealm() async throws {
      ...
      let user2 = try await app.login(credentials: .anonymous)
      // Open in a Task so that the WabiRealm is closed and re-opened later
      _ = try await Task {
        let realm2 = try await WabiRealm(configuration: user2.configuration(testName: #function),
                                         downloadBeforeOpen: .always)
        XCTAssertEqual(realm2.objects(SwiftHugeSyncObject.self).count, 2)
    ...
    @MainActor func testDownloadPBSRealmCustomColumnNames() async throws {
      ...
      @MainActor func testAsyncOpenTaskCancellation() async throws {
        ...
        func isolatedOpen(_ actor: isolated CustomExecutorActor) async throws {
          _ = try await WabiRealm(configuration: configuration, actor: actor, downloadBeforeOpen: .always)
        }

        // Try opening the WabiRealm with the Task being cancelled at every possible
        ...
        for i in 0 ..< .max {
          ...
          _ = try WabiRealm.deleteFiles(for: configuration)
        ...
        // Repeat the above, but with a cached WabiRealm so that we hit that code path instead
        let cachedRealm = try await WabiRealm(configuration: configuration, downloadBeforeOpen: .always)
        for i in 0 ..< .max {
          let executor = CancellingExecutor(cancelAfter: i)
          executor.task = Task {
            try await isolatedOpen(.init(executor.asUnownedSerialExecutor()))
    ...
    func testCallResetPasswordAsyncAwait() async throws {
      ...
      try await assertThrowsError(await auth.callResetPasswordFunction(email: email,
                                                                       password: randomString(10),
                                                                       args: [[:]]))
      {
        assertAppError($0, .unknown, "failed to reset password for user \"\(email)\"")
    ...
    func testEmailPasswordProviderClientAsyncAwait() async throws {
      ...
      try await assertThrowsError(await auth.confirmUser("atoken", tokenId: "atokenid")) {
        assertAppError($0, .badRequest, "invalid token data")
      }
      try await assertThrowsError(await auth.resendConfirmationEmail(email)) {
        assertAppError($0, .userAlreadyConfirmed, "already confirmed")
      }
      try await assertThrowsError(await auth.retryCustomConfirmation(email)) {
        assertAppError($0, .unknown,
                       "cannot run confirmation for \(email): automatic confirmation is enabled")
      }
      try await assertThrowsError(await auth.sendResetPasswordEmail("atoken")) {
        assertAppError($0, .userNotFound, "user not found")
      }
    }
    ...
    func testUserAPIKeyProviderClientAsyncAwait() async throws {
      ...
      let credentials = Credentials.emailPassword(email: email, password: password)
      let syncUser = try await app.login(credentials: credentials)
