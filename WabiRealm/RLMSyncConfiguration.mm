...
struct CallbackSchema {
  ...
  RLMSchema *getSchema(WabiRealm &realm) {
    if (dynamic) {
...
struct BeforeClientResetWrapper : CallbackSchema {
  ...
  void operator()(std::shared_ptr<WabiRealm> local) {
    @autoreleasepool {
      if (local->schema_version() != RLMNotVersioned) {
        block([RLMRealm realmWithSharedRealm:local
                                      schema:getSchema(*local)
                                     dynamic:false]);
      }
    }
  }
...
struct AfterClientResetWrapper : CallbackSchema {
  ...
  void operator()(std::shared_ptr<WabiRealm> local, ThreadSafeReference remote,
                  bool) {
    @autoreleasepool {
      if (local->schema_version() != RLMNotVersioned) {
        ...
        RLMRealm *remoteRealm = [RLMRealm
            realmWithSharedRealm:WabiRealm::get_shared_realm(std::move(remote))
                          schema:schema
                         dynamic:false];
...
- (RLMUser *)user {
  RLMApp *app = [RLMApp appWithId:@(_config->user->sync_manager()
                                        ->app()
                                        .lock()
                                        ->config()
                                        .app_id.data())];
...
- (RLMClientResetBeforeBlock)beforeClientReset {
  if (_config->notify_before_client_reset) {
    auto wrapper =
        _config->notify_before_client_reset.target<BeforeClientResetWrapper>();
...
- (void)setBeforeClientReset:(RLMClientResetBeforeBlock)beforeClientReset {
  if (!beforeClientReset) {
    ...
    @throw RLMException(
        @"RLMClientResetBeforeBlock reset notifications are not supported in "
        @"Manual mode. Use RLMSyncConfiguration.manualClientResetHandler or "
        @"RLMSyncManager.ErrorHandler");
  } else {
    _config->notify_before_client_reset =
        BeforeClientResetWrapper{.block = beforeClientReset};
  }
}
...
- (RLMClientResetAfterBlock)afterClientReset {
  if (_config->notify_after_client_reset) {
    auto wrapper =
        _config->notify_after_client_reset.target<AfterClientResetWrapper>();
    return wrapper->block;
  } else {
    return nil;
  }
...
- (void)setAfterClientReset:(RLMClientResetAfterBlock)afterClientReset {
  if (!afterClientReset) {
    ...
    @throw RLMException(
        @"RLMClientResetAfterBlock reset notifications are not supported in "
        @"Manual mode. Use RLMSyncConfiguration.manualClientResetHandler or "
        @"RLMSyncManager.ErrorHandler");
  } else {
    _config->notify_after_client_reset =
        AfterClientResetWrapper{.block = afterClientReset};
  }
}
...
- (void)setManualClientResetHandler:
    (RLMSyncErrorReportingBlock)manualClientReset {
  if (!manualClientReset) {
    _manualClientResetHandler = nil;
  } else if (self.clientResetMode != RLMClientResetModeManual) {
    @throw RLMException(@"A manual client reset handler can only be set with "
...
void RLMSetConfigInfoForClientResetCallbacks(realm::SyncConfig &syncConfig,
  ...
  if (syncConfig.notify_before_client_reset) {
    auto before = syncConfig.notify_before_client_reset
                      .target<BeforeClientResetWrapper>();
  ...
  if (syncConfig.notify_after_client_reset) {
    auto after =
        syncConfig.notify_after_client_reset.target<AfterClientResetWrapper>();
...
- (void)setCancelAsyncOpenOnNonFatalErrors:
    (bool)cancelAsyncOpenOnNonFatalErrors {
...
- (void)assignConfigErrorHandler:(RLMUser *)user {
  ...
  _config->error_handler =
      [weakManager, resetHandler](std::shared_ptr<SyncSession> errored_session,
                                  SyncError error) {
        ...
        RLMSyncSession *session =
            [[RLMSyncSession alloc] initWithSyncSession:errored_session];
