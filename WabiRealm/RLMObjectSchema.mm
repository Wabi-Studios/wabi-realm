...
@property(nonatomic, readwrite)
    NSDictionary<id, RLMProperty *> *allPropertiesByName;
...
- (RLMProperty *)objectForKeyedSubscript:
...
- (void)_propertiesDidChange {
  ...
  NSMutableDictionary *map = [NSMutableDictionary
      dictionaryWithCapacity:_properties.count + _computedProperties.count];
  NSUInteger index = 0;
  for (RLMProperty *prop in _properties) {
    ...
    if (prop.isPrimary) {
      if (_primaryKeyProperty) {
        @throw RLMException(@"Properties '%@' and '%@' are both marked as the "
                            @"primary key of '%@'",
...
+ (instancetype)schemaForObjectClass:(Class)objectClass {
  ...
  // determine classname from objectclass as className method has not yet been
  // updated
  ...
  schema.hasCustomEventSerialization =
      [objectClass conformsToProtocol:@protocol(RLMCustomEventRepresentable)];
  ...
  NSArray *computedProperties = [allProperties
      filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(
                                                   RLMProperty *property,
                                                   NSDictionary *) {
        return RLMPropertyTypeIsComputed(property.type);
  ...
  if (allProperties.count !=
      [NSSet setWithArray:[allProperties valueForKey:@"name"]].count) {
    NSCountedSet *countedPropertyNames =
        [NSCountedSet setWithArray:[allProperties valueForKey:@"name"]];
    NSArray *duplicatePropertyNames =
    ...
    if (duplicatePropertyNames.count == 1) {
      @throw RLMException(@"Property '%@' is declared multiple times in the "
                          @"class hierarchy of '%@'",
                          duplicatePropertyNames.firstObject, className);
    } else {
      @throw RLMException(
          @"Object '%@' has properties that are declared multiple times in its "
          @"class hierarchy: '%@'",
          className, [duplicatePropertyNames componentsJoinedByString:@"', '"]);
    }
  }
  ...
  if (NSString *primaryKey = [objectClass primaryKey]) {
    ...
    if (schema.primaryKeyProperty.type != RLMPropertyTypeInt &&
      ...
      @throw RLMException(
          @"Property '%@' cannot be made the primary key of '%@' because it is "
          @"not a 'string', 'int', 'objectId', or 'uuid' property.",
          primaryKey, className);
    }
  }

  for (RLMProperty *prop in schema.properties) {
    if (prop.optional && prop.collection && !prop.dictionary &&
        (prop.type == RLMPropertyTypeObject ||
         prop.type == RLMPropertyTypeLinkingObjects)) {
      // FIXME: message is awkward
      @throw RLMException(@"Property '%@.%@' cannot be made optional because "
                          @"optional '%@' properties are not supported.",
                          className, prop.name, RLMTypeToString(prop.type));
    }
  }

  if ([objectClass shouldIncludeInDefaultSchema] && schema.isSwiftClass &&
      schema.properties.count == 0) {
    @throw RLMException(@"No properties are defined for '%@'. Did you remember "
                        @"to mark them with '@objc' in your model?",
                        schema.className);
  }
  return schema;
}
...
+ (NSArray *)propertiesForClass:(Class)objectClass isSwift:(bool)isSwiftClass {
  ...
  // For Swift subclasses of RLMObject we need an instance of the object when
  // parsing properties
  ...
  NSDictionary *linkingObjectsProperties =
      [objectClass linkingObjectsProperties];
  NSDictionary *columnNameMap = [objectClass _realmColumnNames];

  unsigned int count;
  std::unique_ptr<objc_property_t[], decltype(&free)> props(
      class_copyPropertyList(objectClass, &count), &free);
  NSMutableArray<RLMProperty *> *propArray =
      [NSMutableArray arrayWithCapacity:count];
  NSSet *indexed =
      [[NSSet alloc] initWithArray:[objectClass indexedProperties]];
  for (unsigned int i = 0; i < count; i++) {
    ...
    if (isSwiftClass) {
      prop = [[RLMProperty alloc]
          initSwiftPropertyWithName:propertyName
  ...
  if (auto requiredProperties = [objectClass requiredProperties]) {
    for (RLMProperty *property in propArray) {
      ...
      if (required && property.type == RLMPropertyTypeObject &&
          (!property.collection || property.dictionary)) {
        @throw RLMException(@"Object properties cannot be made required, "
                             "but '+[%@ requiredProperties]' included '%@'",
                            objectClass, property.name);
      }
...
- (id)copyWithZone:(NSZone *)zone {
  ...
  schema->_properties = [[NSArray allocWithZone:zone] initWithArray:_properties
                                                          copyItems:YES];
  schema->_computedProperties =
      [[NSArray allocWithZone:zone] initWithArray:_computedProperties
                                        copyItems:YES];
...
- (wabi_realm::ObjectSchema)objectStoreCopy:(RLMSchema *)schema {
  ...
  objectSchema.primary_key =
      _primaryKeyProperty ? _primaryKeyProperty.columnName.UTF8String : "";
  objectSchema.table_type = _isAsymmetric ? Type::TopLevelAsymmetric
                            : _isEmbedded ? Type::Embedded
                                          : Type::TopLevel;
  for (RLMProperty *prop in _properties) {
...
+ (instancetype)objectSchemaForObjectStoreSchema:
  ...
  schema.isEmbedded =
      objectSchema.table_type == ObjectSchema::ObjectType::Embedded;
  schema.isAsymmetric =
      objectSchema.table_type == ObjectSchema::ObjectType::TopLevelAsymmetric;

  // create array of RLMProperties
  NSMutableArray *properties = [NSMutableArray
      arrayWithCapacity:objectSchema.persisted_properties.size()];
  ...
  NSMutableArray *computedProperties = [NSMutableArray
      arrayWithCapacity:objectSchema.computed_properties.size()];
  for (const Property &prop : objectSchema.computed_properties) {
    [computedProperties
        addObject:[RLMProperty propertyForObjectStoreProperty:prop]];
  }
  schema.computedProperties = computedProperties;
  ...
  if (objectSchema.primary_key.length()) {
    NSString *primaryKeyString =
        [NSString stringWithUTF8String:objectSchema.primary_key.c_str()];
    schema.primaryKeyProperty = schema[primaryKeyString];
    if (!schema.primaryKeyProperty) {
      @throw RLMException(@"No property matching primary key '%@'",
                          primaryKeyString);
    }
  }
