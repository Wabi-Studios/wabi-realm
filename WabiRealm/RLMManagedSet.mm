...
void RLMEnsureSetObservationInfo(std::unique_ptr<RLMObservationInfo> &info,
  ...
  if (!info && set.class == [RLMManagedSet class]) {
    ...
    info = std::make_unique<RLMObservationInfo>(
        *lv->_ownerInfo, lv->_backingSet.get_parent_object_key(), observed);
  }
}
...
- (BOOL)isEqual:(id)object {
  return [object respondsToSelector:@selector(isBackedBySet:)] &&
         [object isBackedBySet:_backingSet];
}
...
static void ensureInWriteTransaction(NSString *message, RLMManagedSet *set,
                                     RLMManagedSet *otherSet) {
  if (!set.realm.inWriteTransaction && !otherSet.realm.inWriteTransaction) {
    @throw RLMException(
        @"Can only perform %@ in a WabiRealm in a write transaction - call "
...
- (RLMManagedSet *)managedObjectFrom:(RLMSet *)set {
  ...
  if (_type != managedSet->_type) {
    @throw RLMException(@"Cannot intersect sets of type '%@' and '%@'.",
                        RLMTypeToString(_type),
                        RLMTypeToString(managedSet->_type));
  }
    ...
    return translateErrors([&] {
      auto results =
          [RLMResults resultsWithObjectInfo:*_objectInfo
                                    results:_backingSet.as_results()];
...
- (id)valueForKey:(NSString *)key {
  ...
  return translateErrors([&]() -> id {
    ...
    return [NSSet
        setWithArray:RLMCollectionValueForKey(_backingSet, key, *_objectInfo)];
  });
  return nil;
}
...
- (id)minOfProperty:(NSString *)property {
  auto column = columnForProperty(property, _backingSet, _objectInfo, _type,
...
- (id)sumOfProperty:(NSString *)property {
  auto column = columnForProperty(property, _backingSet, _objectInfo, _type,
                                  RLMCollectionTypeSet);
  return RLMMixedToObjc(
      translateErrors([&] { return _backingSet.sum(column); }));
}
...
- (RLMResults *)sortedResultsUsingDescriptors:
  ...
  return translateErrors([&] {
    return [RLMResults
        resultsWithObjectInfo:*_objectInfo
                      results:_backingSet.sort(RLMSortDescriptorsToKeypathArray(
                                  properties))];
  });
}
...
- (RLMResults *)objectsWithPredicate:(NSPredicate *)predicate {
  if (_type != RLMPropertyTypeObject) {
    @throw RLMException(@"Querying is currently only implemented for sets of "
                        @"WabiRealm Objects");
  }
  auto query = RLMPredicateToQuery(predicate, _objectInfo->rlmObjectSchema,
                                   _realm.schema, _realm.group);
  auto results =
      translateErrors([&] { return _backingSet.filter(std::move(query)); });
...
- (instancetype)resolveInRealm:(RLMRealm *)realm {
  ...
  return translateErrors([&] {
    return [[self.class alloc]
        initWithBackingCollection:_backingSet.freeze(realm->_realm)
...
- (RLMManagedSetHandoverMetadata *)objectiveCMetadata {
  RLMManagedSetHandoverMetadata *metadata =
      [[RLMManagedSetHandoverMetadata alloc] init];
...
+ (instancetype)
  ...
  return [[RLMManagedSet alloc]
      initWithBackingCollection:std::move(set)
                     parentInfo:parentInfo
