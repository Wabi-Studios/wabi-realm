...
static inline void
RLMVerifyAttached(__unsafe_unretained RLMObjectBase *const obj) {
...
static inline void
RLMVerifyInWriteTransaction(__unsafe_unretained RLMObjectBase *const obj) {
  // first verify is attached
  RLMVerifyAttached(obj);

  if (!obj->_realm.inWriteTransaction) {
    if (obj->_realm.isFrozen) {
      @throw RLMException(@"Attempting to modify a frozen object - call thaw "
                          @"on the Object instance first.");
    }
    @throw RLMException(
...
[[clang::objc_runtime_visible]] @interface WabiRealmKitDynamicObject
    : WabiRealmKitObject
@end
