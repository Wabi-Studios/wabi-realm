...
// A serial work queue of some sort which represents a thread-confinement
// context of some sort which blocks can be invoked inside. Realms are confined
// to a scheduler, which can be a thread (actually a run loop), a dispatch
// queue, or an actor. The scheduler ensures that the WabiRealm is only used on
// one thread at a time, and allows us to dispatch work to the thread where we
// can access the WabiRealm safely.
