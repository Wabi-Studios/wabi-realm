...
template <typename Iterator> Iterator begin(IteratorPair<Iterator> const &p) {
  return p.first;
}
template <typename Iterator> Iterator end(IteratorPair<Iterator> const &p) {
  return p.second;
}

template <typename Container> auto reverse(Container const &c) {
  return IteratorPair<typename Container::const_reverse_iterator>{c.rbegin(),
                                                                  c.rend()};
}
} // namespace
...
RLMObservationInfo::RLMObservationInfo(id object) : object(object) {}
...
void RLMObservationInfo::willChange(NSString *key, NSKeyValueChange kind,
                                    NSIndexSet *indexes) const {
  if (indexes) {
    forEach([=](__unsafe_unretained auto o) {
      [o willChange:kind valuesAtIndexes:indexes forKey:key];
    });
  } else {
    forEach([=](__unsafe_unretained auto o) { [o willChangeValueForKey:key]; });
  }
}

void RLMObservationInfo::didChange(NSString *key, NSKeyValueChange kind,
                                   NSIndexSet *indexes) const {
  if (indexes) {
    forEach([=](__unsafe_unretained auto o) {
    ...
    forEach([=](__unsafe_unretained auto o) { [o didChangeValueForKey:key]; });
  }
}
...
void RLMObservationInfo::recordObserver(
  ...
  // was never observed), but for Swift classes (both WabiRealmKit and
...
void RLMObservationInfo::removeObserver() { --observerCount; }
...
id RLMObservationInfo::valueForKey(NSString *key) {
  ...
  static auto superValueForKey = reinterpret_cast<id (*)(id, SEL, NSString *)>(
      [NSObject methodForSelector:@selector(valueForKey:)]);
  if (!lastProp) {
    // Not a managed property, so use NSObject's implementation of valueForKey:
    return RLMCoerceToNil(
        superValueForKey(object, @selector(valueForKey:), key));
  }

  auto getSuper = [&] {
    return row ? RLMDynamicGet(object, lastProp)
               : RLMCoerceToNil(
                     superValueForKey(object, @selector(valueForKey:), key));
  };
...
RLMObservationTracker::RLMObservationTracker(
    __unsafe_unretained RLMRealm *const realm, bool trackDeletions)
    : _realm(realm), _group(realm.group) {
  if (trackDeletions) {
    this->trackDeletions();
  }
}

RLMObservationTracker::~RLMObservationTracker() { didChange(); }
...
void RLMObservationTracker::trackDeletions() {
  if (_group.has_cascade_notification_handler()) {
    // We're nested inside another call which will handle any cascaded changes
    // for us
    return;
  }
...
void RLMObservationTracker::cascadeNotification(CascadeNotification const &cs) {
  ...
  std::sort(begin(_observedTables), end(_observedTables), [=](auto a, auto b) {
    return tableKey(a->front()) < tableKey(b->front());
  });
  for (auto const &link : cs.links) {
    auto table = std::find_if(
        _observedTables.begin(), _observedTables.end(), [&](auto table) {
    ...
    for (auto observer : **table) {
      ...
      if (observer->getRow().get_table()->get_column_type(
              link.origin_col_key) != type_LinkList) {
      ...
      linkview.find_all(link.old_target_key,
                        [&](size_t index) { [c->indexes addIndex:index]; });
    }
  }
  if (!cs.rows.empty()) {
    ...
    for (auto table : _observedTables) {
      ...
      for (auto info : *table) {
        if (std::binary_search(
                begin, end, Row{currentTableKey, info->getRow().get_key()})) {
  ...
  for (size_t i = changeCount; i < _changes.size(); ++i) {
    ...
    change.info->willChange(change.property, NSKeyValueChangeRemoval,
                            change.indexes);
  }
  for (size_t i = invalidatedCount; i < _invalidated.size(); ++i) {
...
void RLMObservationTracker::didChange() {
  ...
  for (auto const &change : reverse(_changes)) {
    change.info->didChange(change.property, NSKeyValueChangeRemoval,
                           change.indexes);
  }
...
RLMGetObservedRows(RLMSchemaInfo const &schema) {
  ...
  for (auto &table : schema) {
    for (auto info : table.second.observedObjects) {
      ...
      if (!row.is_valid())
        continue;
      observers.push_back({row.get_table()->get_key(), row.get_key(), info});
    }
  }
