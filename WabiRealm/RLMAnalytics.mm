...
// Asynchronously submits build information to WabiRealm if running in an iOS
...
// debated adding this for a long long time. Since WabiRealm is a free product
...
// feel compelled to not send this data back to WabiRealm, then you can set an
// env variable named REALM_DISABLE_ANALYTICS. Since WabiRealm is free we
// believe letting these analytics run is a small price to pay for the product &
// support we give you.
//
// Currently the following information is reported:
// - What version of WabiRealm and core is being used, and from which language
// (obj-c or Swift).
// - Which platform and version of OS X it's running on (in case Xcode
// aggressively drops
...
// - An anonymous MAC address and bundle ID to aggregate the other information
// on.
// - The host platform OSX and version.
// - The XCode version.
// - Some info about the features been used when opening the realm for the first
// time.
...
#import <Foundation/Foundation.h>
    ...
    (TARGET_OS_WATCH && TARGET_OS_SIMULATOR) ||                                \
    (TARGET_OS_TV && TARGET_OS_SIMULATOR)
...
static std::optional<std::array<unsigned char, 6>> getMacAddress(int id) {
  ...
  std::array<int, 6> mib = {
      {CTL_NET, PF_ROUTE, 0, AF_LINK, NET_RT_IFLIST, index}};
  size_t bufferSize;
  ...
  auto sockaddr = reinterpret_cast<sockaddr_dl *>(
      static_cast<if_msghdr *>(buffer.get()) + 1);
  std::array<unsigned char, 6> mac;
  std::memcpy(&mac[0], sockaddr->sdl_data + sockaddr->sdl_nlen, 6);

  // Touch bar internal network interface, which is identical on all touch bar
  // macs
...
static NSString *RLMBuilderId() {
  ...
  NSData *hashedData = [NSData dataWithBytes:buffer
...
static NSDictionary *RLMBaseMetrics() {
  ...
  Class swiftDecimal128 = NSClassFromString(@"WabiRealmKitDecimal128");
  ...
  // in case we cannot obtain it (Pre-built binaries) we use the hashed mac
  // address.
  ...
  return @{
        ...
        // SDK Info
        @"Binding": @"cocoa",
        // Which version of WabiRealm is being used
        @"WabiRealm Version": REALM_COCOA_VERSION,
...
// This will only be executed once but depending on the number of objects, could
// take sometime
...
static NSDictionary *
RLMConfigurationMetrics(RLMRealmConfiguration *configuration) {
  ...
  return @{
    ...
    @"CR_Recover_Discard" :
            (isSync &&
             resetMode == RLMClientResetModeRecoverOrDiscardUnsyncedChanges)
        ? @1
        : @0,
    @"CR_Recover" :
            (isSync && resetMode == RLMClientResetModeRecoverUnsyncedChanges)
        ? @1
        : @0,
    @"CR_Discard" :
            (isSync && resetMode == RLMClientResetModeDiscardUnsyncedChanges)
        ? @1
        : @0,
...
void RLMSendAnalytics(RLMRealmConfiguration *configuration, RLMSchema *schema) {
  ...
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSData *payload = [NSJSONSerialization dataWithJSONObject:payloadN
                                                          options:0
                                                            error:nil];

        NSString *url =
            @"https://data.mongodb-api.com/app/realmsdkmetrics-zmhtm/endpoint/"
            @"metric_webhook/metric?data=%@";
        NSString *formatted = [NSString
            stringWithFormat:url, [payload base64EncodedStringWithOptions:0]];
        // No error handling or anything because logging errors annoyed people
        // for no real benefit, and it's not clear what else we could do
        [[NSURLSession.sharedSession
            dataTaskWithURL:[NSURL URLWithString:formatted]] resume];
      });
}
