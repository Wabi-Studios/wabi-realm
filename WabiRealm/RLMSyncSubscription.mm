...
- (void)updateSubscriptionWhere:(NSString *)predicateFormat, ... {
  ...
  [self updateSubscriptionWhere:predicateFormat args:args];
  va_end(args);
}

- (void)updateSubscriptionWhere:(NSString *)predicateFormat args:(va_list)args {
  [self updateSubscriptionWithPredicate:[NSPredicate
...
- (void)updateSubscriptionWithPredicate:(NSPredicate *)predicate {
  if (self.name != nil) {
    ...
    RLMSyncSubscription *foundSubscription =
        [_subscriptionSet subscriptionWithClassName:self.objectClassName
                                              where:self.queryString];
...
- (instancetype)initWithSubscriptionSet:
    (RLMSyncSubscriptionSet *)subscriptionSet {
  if (self = [super init]) {
    _subscriptionSet = subscriptionSet;
    return self;
  }
...
- (nullable NSError *)error {
  ...
  NSString *errorMessage =
      RLMStringDataToNSString(_subscriptionSet->error_str());
  if (errorMessage.length == 0) {
    return nil;
  }
  return [[NSError alloc]
      initWithDomain:RLMSyncErrorDomain
...
- (void)update:(__attribute__((noescape))void (^)(void))block
  ...
  if (_mutableSubscriptionSet) {
    @throw RLMException(@"Cannot initiate a write transaction on subscription "
...
- (nullable RLMSyncSubscription *)
    subscriptionWithClassName:(NSString *)objectClassName
                        where:(NSString *)predicateFormat
                         args:(va_list)args {
  return [self
      subscriptionWithClassName:objectClassName
...
- (nullable RLMSyncSubscription *)
  ...
  auto query = RLMPredicateToQuery(predicate, info.rlmObjectSchema,
...
- (void)addSubscriptionWithClassName:(NSString *)objectClassName
  ...
  if (name) {
    if (updateExisting || !_mutableSubscriptionSet->find(name.UTF8String)) {
      ...
      @throw RLMException(
          @"A subscription named '%@' already exists. If you meant to update "
          @"the existing subscription please use the `update` method.",
          name);
    }
  } else {
...
- (void)removeSubscription:(RLMSyncSubscription *)subscription {
  ...
  for (auto it = _mutableSubscriptionSet->begin();
       it != _mutableSubscriptionSet->end();) {
...
- (RLMSyncSubscription *)objectAtIndex:(NSUInteger)index {
  ...
  return [[RLMSyncSubscription alloc]
      initWithSubscription:_subscriptionSet->at(size_t(index))
           subscriptionSet:self];
}
...
- (RLMSyncSubscription *)firstObject {
  ...
  return [[RLMSyncSubscription alloc]
      initWithSubscription:_subscriptionSet->at(size_t(0))
           subscriptionSet:self];
}
...
- (RLMSyncSubscription *)lastObject {
  ...
  return [[RLMSyncSubscription alloc]
      initWithSubscription:_subscriptionSet->at(_subscriptionSet->size() - 1)
           subscriptionSet:self];
}
...
- (void)verifyInWriteTransaction {
  if (_mutableSubscriptionSet == nil) {
    @throw RLMException(@"Can only add, remove, or update subscriptions within "
                        @"a write subscription block.");
  }
}
