...
// Translate an in-flight exception resulting from an operation on a SharedGroup
// to an NSError or NSException (if error is nil)
void RLMRealmTranslateException(NSError **error);

// Block until the WabiRealm at the given path is closed.
...
FOUNDATION_EXTERN void RLMAddBeforeNotifyBlock(RLMRealm *realm,
                                               dispatch_block_t block);

// Test hook to run the async notifiers for a WabiRealm which has the background
// thread disabled
FOUNDATION_EXTERN void RLMRunAsyncNotifiers(NSString *path);

// Get the cached WabiRealm for the given configuration and scheduler, if any
FOUNDATION_EXTERN
    RLMRealm *_Nullable RLMGetCachedRealm(RLMRealmConfiguration *,
                                          RLMScheduler *) NS_RETURNS_RETAINED;
// Get a cached WabiRealm for the given configuration and any scheduler. The
// returned WabiRealm is not confined to the current thread, so very few
// operations are safe to perform on it
FOUNDATION_EXTERN RLMRealm *_Nullable RLMGetAnyCachedRealm(
    RLMRealmConfiguration *) NS_RETURNS_RETAINED;

// Scheduler an async refresh for the given WabiRealm
FOUNDATION_EXTERN RLMAsyncRefreshTask *_Nullable RLMRealmRefreshAsync(
    RLMRealm *rlmRealm) NS_RETURNS_RETAINED;
