...
@class RLMUser, RLMSyncSession, RLMRealm, RLMUserIdentity, RLMAPIKeyAuth,
    RLMMongoClient, RLMMongoDatabase, RLMMongoCollection, RLMUserProfile;
@protocol RLMBSON;
...
/// A block which returns a dictionary should there be any custom data set for a
/// user
RLM_SWIFT_SENDABLE
...
/**
 A `RLMUser` instance represents a single WabiRealm App user account.
...
/**
 ...
 Note this is different from an identitiy: A user may have multiple identities
 but has a single indentifier. See RLMUserIdentity.
 */
...
/**
 The user's refresh token used to access the WabiRealm Applcation.

 This is required to make HTTP requests to the WabiRealm App's REST API
...
/**
 The user's refresh token used to access the WabiRealm Application.
...
/**
 Indicates if the user is logged in or not. Returns true if the access token and
 refresh token are not empty.
 */
...
/**
 Create a partition-based sync configuration instance for the given partition
 value.

 @param partitionValue The `RLMBSON` value the WabiRealm is partitioned on.
 @return A default configuration object with the sync configuration set to use
 the given partition value.
 */
- (RLMRealmConfiguration *)configurationWithPartitionValue:
    (nullable id<RLMBSON>)partitionValue NS_REFINED_FOR_SWIFT;
...
/**
 ...
 @param partitionValue The `RLMBSON` value the WabiRealm is partitioned on.
 @param clientResetMode Determines file recovery behavior in the event of a
 client reset. See:
 https://docs.mongodb.com/wabi-realm/sync/error-handling/client-resets/

 @return A configuration object with the sync configuration set to use the given
 partition value.
 */
...
/**
 ...
 client reset. See:
 https://docs.mongodb.com/wabi-realm/sync/error-handling/client-resets/
 @param beforeResetBlock A callback which notifies prior to a client reset
 occurring. See: `RLMClientResetBeforeBlock`
 @param afterResetBlock A callback which notifies after a client reset has
 occurred. See: `RLMClientResetAfterBlock`
...
/**
 ...
 client reset. See:
 https://docs.mongodb.com/wabi-realm/sync/error-handling/client-resets/
 @param manualClientResetHandler An error reporting block that is invoked during
 a client reset.
                                @See ``RLMSyncErrorReportingBlock`` and
 ``RLMClientResetInfo``
...
/**
 Create a flexible sync configuration instance, which can be used to open a
 WabiRealm that supports flexible sync.

 @note A single server-side Device Sync App can sync data with either
 partition-based realms or flexible sync based realms. In order for an
 application to contain both partition-based and flexible sync realms, more than
 one server-side Device Sync App must be used.

 @return A ``RLMRealmConfiguration`` instance with a flexible sync
 configuration.
 */
                               ...
                               notifyBeforeReset:
                                ...
                                notifyAfterReset:
                                    (nullable RLMClientResetAfterBlock)
                                        afterResetBlock NS_REFINED_FOR_SWIFT;
/**
 Create a flexible sync configuration instance, which can be used to open a
 WabiRealm that supports flexible sync.
        ...
        (RLMClientResetMode)clientResetMode
                        manualClientResetHandler:
                            (nullable RLMSyncErrorReportingBlock)
                                manualClientResetHandler NS_REFINED_FOR_SWIFT;
...
/**
 ...
 @param initialSubscriptions A block which receives a subscription set instance,
 that can be used to add an initial set of subscriptions which will be executed
                             when the WabiRealm is first opened.
 @param rerunOnOpen If true, allows to run the initial set of subscriptions
 specified, on every app startup. This can be used to re-run dynamic time ranges
 and other queries that require a re-computation of a static variable.

 @return A `RLMRealmConfiguration` instance with a flexible sync configuration.
 */
- (RLMRealmConfiguration *)
    flexibleSyncConfigurationWithInitialSubscriptions:
        (RLMFlexibleSyncInitialSubscriptionsBlock)initialSubscriptions
...
/**
 Refresh a user's custom data. This will, in effect, refresh the user's auth
 session.
 */
- (void)refreshCustomDataWithCompletion:(RLMUserCustomDataBlock)completion;

/**
 Links the currently authenticated user with a new identity, where the identity
 is defined by the credential specified as a parameter. This will only be
 successful if this `RLMUser` is the currently authenticated with the client
 from which it was created. On success a new user will be returned with the new
 linked credentials.

 @param credentials The `RLMCredentials` used to link the user to a new
 identity.
 @param completion The completion handler to call when the linking is complete.
                   If the operation is  successful, the result will contain a
 new `RLMUser` object representing the currently logged in user.
*/
...
/**
 ...
 This logs out and destroys the session related to this user. The completion
 block will return an error if the user is not found or is already removed.
...
/**
 ...
 The users state will be set to `Removed` is they are an anonymous user or
 `LoggedOut` if they are authenticated by an email / password or third party
 auth clients If the logout request fails, this method will still clear local
...
/**
 ...
 @param completion The completion handler to call when the function call is
 complete. This handler is executed on a non-main global `DispatchQueue`.
*/
- (void)callFunctionNamed:(NSString *)name
                arguments:(NSArray<id<RLMBSON>> *)arguments
          completionBlock:(RLMCallFunctionCompletionBlock)completion
    NS_REFINED_FOR_SWIFT;

/// :nodoc:
- (instancetype)init
    __attribute__((unavailable("RLMUser cannot be created directly")));
/// :nodoc:
+ (instancetype)new
    __attribute__((unavailable("RLMUser cannot be created directly")));
...
/**
 An identity of a user. A user can have multiple identities, usually associated
 with multiple providers. Note this is different from a user's unique identifier
 string.
