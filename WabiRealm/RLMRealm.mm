...
@property(nonatomic, strong)
    NSHashTable<RLMRealmNotificationToken *> *notificationHandlers;
...
void RLMWaitForRealmToClose(NSString *path) {
  ...
  lockfile.set_fifo_path(
      [path stringByAppendingString:@".management"].UTF8String, "lock.fifo");
...
BOOL RLMIsRealmCachedAtPath(NSString *path) {
  return RLMGetAnyCachedRealmForPath(
             [path cStringUsingEncoding:NSUTF8StringEncoding]) != nil;
}
...
- (void)dealloc {
  if (_realm || _block) {
    NSLog(@"RLMNotificationToken released without unregistering a "
          @"notification. You must hold "
          @"on to the RLMNotificationToken returned from addNotificationBlock "
          @"and call "
          @"-[RLMNotificationToken invalidate] when you no longer wish to "
          @"receive RLMRealm notifications.");
  }
}
@end
...
namespace {
// ARC tries to eliminate calls to autorelease when the value is then
// immediately returned, but this results in significantly different semantics
// between debug and release builds for RLMRealm, so force it to always
// autorelease. NEXT-MAJOR: we should switch to NS_RETURNS_RETAINED, which did
// not exist yet when we wrote this but is the correct thing.
id autorelease(__unsafe_unretained id value) {
  // +1 __bridge_retained, -1 CFAutorelease
  return value ? (__bridge id)CFAutorelease((__bridge_retained CFTypeRef)value)
               : nil;
}

RLMRealm *getCachedRealm(RLMRealmConfiguration *configuration,
                         RLMScheduler *options) NS_RETURNS_RETAINED {
  ...
  if ((oldConfig.read_only() || oldConfig.immutable()) !=
      configuration.readOnly) {
    @throw RLMException(@"WabiRealm at path '%@' already opened with different "
                        @"read permissions",
  ...
  if (oldConfig.encryption_key != config.encryption_key) {
    @throw RLMException(
        @"WabiRealm at path '%@' already opened with different encryption key",
        configuration.fileURL.path);
  }
...
bool copySeedFile(RLMRealmConfiguration *configuration, NSError **error) {
  ...
  @autoreleasepool {
    ...
    DB::call_with_lock(configuration.path, [&](auto const &) {
      didCopySeed = [[NSFileManager defaultManager]
          copyItemAtURL:configuration.seedFilePath
                  toURL:configuration.fileURL
                  error:&copyError];
    });
    if (!didCopySeed && copyError != nil &&
        copyError.code != NSFileWriteFileExistsError) {
...
+ (void)initialize {
  // In cases where we are not using a synced WabiRealm, we initialise the
  // default logger before opening any realm.
  [RLMLogger class];
}

+ (void)runFirstCheckForConfiguration:(RLMRealmConfiguration *)configuration
  ...
  // Run Analytics on the very first any WabiRealm open.
...
+ (instancetype)defaultRealm {
  return [RLMRealm
      realmWithConfiguration:[RLMRealmConfiguration rawDefaultConfiguration]
                       error:nil];
}
...
+ (instancetype)realmWithConfiguration:(RLMRealmConfiguration *)configuration
  ...
  return autorelease([self
      realmWithConfiguration:configuration
...
+ (instancetype)realmWithConfiguration:(RLMRealmConfiguration *)configuration
                            confinedTo:(RLMScheduler *)scheduler
                                 error:(NSError **)error {
  // First check if we already have a cached WabiRealm for this config
  ...
  WabiRealm::Config config = configuration.config;
  ...
  try {
    ...
    if (config.scheduler && !config.scheduler->is_on_thread()) {
      throw RLMException(@"WabiRealm opened from incorrect dispatch queue.");
    }
    realm->_realm = WabiRealm::get_shared_realm(config);
  } catch (...) {
  ...
  @autoreleasepool {
    ...
    if (auto cachedRealm = RLMGetAnyCachedRealmForPath(config.path)) {
      ...
      realm->_info =
          cachedRealm->_info.clone(cachedRealm->_realm->schema(), realm);
      realmIsCached = true;
    }
  }
  ...
  if (realm->_schema) {
    ...
    realm->_schema =
        [RLMSchema dynamicSchemaFromObjectStoreSchema:realm->_realm->schema()];
    realm->_info = RLMSchemaInfo(realm);
  } else {
    ...
    if (migrationBlock && configuration.schemaVersion > 0) {
      migrationFunction = [=](SharedRealm old_realm, SharedRealm realm,
                              Schema &mutableSchema) {
        RLMSchema *oldSchema =
            [RLMSchema dynamicSchemaFromObjectStoreSchema:old_realm->schema()];
        ...
        [[[RLMMigration alloc] initWithRealm:newRealm
                                    oldRealm:oldRealm
                                      schema:mutableSchema]
                execute:migrationBlock
            objectClass:configuration.migrationObjectClass];
    ...
    try {
      realm->_realm->update_schema(
          schema.objectStoreCopy, config.schema_version,
  ...
  // Run Analytics and Update checker, this will be run only the first any realm
  // open
...
- (void)verifyNotificationsAreSupported:(bool)isCollection {
  ...
  if (_realm->config().immutable()) {
    @throw RLMException(@"Read-only Realms do not change and do not have "
  ...
  if (_realm->config().automatic_change_notifications &&
      !_realm->can_deliver_notifications()) {
    @throw RLMException(
...
- (void)sendNotifications:(RLMNotification)notification {
  NSAssert(!_realm->config().immutable(),
           @"Read-only realms do not have notifications");
...
- (BOOL)commitWriteTransactionWithoutNotifying:
  ...
  for (RLMNotificationToken *token in tokens) {
    if (token.realm != self) {
      @throw RLMException(
          @"Incorrect WabiRealm: only notifications for the WabiRealm "
          @"being modified can be skipped.");
    }
...
- (RLMAsyncWriteTask *)beginAsyncWrite {
  try {
    ...
    write.transactionId = _realm->async_begin_transaction(
        ^{
...
- (RLMAsyncTransactionId)
    asyncTransactionWithBlock:(void (^)())block
...
/**
 Replaces all string columns in this WabiRealm with a string enumeration column
 and compacts the database file.

 Cannot be called from a write transaction.

 Compaction will not occur if other `RLMRealm` instances exist.

 While compaction is in progress, attempts by other threads or processes to open
 the database will wait.

 Be warned that resource requirements for compaction is proportional to the
 amount of live data in the database.

 Compaction works by writing the database contents to a temporary database file
 and then replacing the database with the temporary one. The name of the
 temporary file is formed by appending
...
- (void)dealloc {
  if (_realm) {
    if (_realm->is_in_transaction()) {
      ...
      NSLog(@"WARNING: An RLMRealm instance was deallocated during a write "
            @"transaction and all "
             "pending changes have been rolled back. Make sure to retain a "
             "reference to the "
...
- (BOOL)refresh {
  if (_realm->config().immutable()) {
    @throw RLMException(
        @"Read-only Realms do not change and cannot be refreshed.");
  }
  try {
...
- (void)addObjects:(id<NSFastEnumeration>)objects {
  for (RLMObject *obj in objects) {
    if (![obj isKindOfClass:RLMObjectBase.class]) {
      @throw RLMException(@"Cannot insert objects of type %@ with addObjects:. "
                          @"Only RLMObjects are supported.",
...
- (void)addOrUpdateObjects:(id<NSFastEnumeration>)objects {
  for (RLMObject *obj in objects) {
    if (![obj isKindOfClass:RLMObjectBase.class]) {
      @throw RLMException(@"Cannot add or update objects of type %@ with "
                          @"addOrUpdateObjects:. Only RLMObjects are"
                           " supported.",
                          NSStringFromClass(obj.class));
    }
...
- (void)deleteObjects:(id<NSFastEnumeration>)objects {
  ...
  if ([idObjects respondsToSelector:@selector(realm)] &&
    ...
    if (self != (RLMRealm *)[idObjects realm]) {
      @throw RLMException(
          @"Can only delete objects from the WabiRealm they belong to.");
    }
  ...
  if (auto array = RLMDynamicCast<RLMArray>(objects)) {
    if (array.type != RLMPropertyTypeObject) {
      @throw RLMException(@"Cannot delete objects from RLMArray<%@>: only "
    ...
    if (set.type != RLMPropertyTypeObject) {
      @throw RLMException(@"Cannot delete objects from RLMSet<%@>: only "
                          @"RLMObjects can be deleted.",
    ...
    if (dictionary.type != RLMPropertyTypeObject) {
      @throw RLMException(@"Cannot delete objects from RLMDictionary of type "
  ...
  for (RLMObject *obj in objects) {
    if (![obj isKindOfClass:RLMObjectBase.class]) {
      @throw RLMException(@"Cannot delete objects of type %@ with "
                          @"deleteObjects:. Only RLMObjects can be deleted.",
                          NSStringFromClass(obj.class));
    }
    RLMDeleteObjectFromRealm(obj, self);
  }
...
- (RLMResults *)objects:(NSString *)objectClassName
                  where:(NSString *)predicateFormat, ... {
  va_list args;
  va_start(args, predicateFormat);
  RLMResults *results = [self objects:objectClassName
...
- (RLMResults *)objects:(NSString *)objectClassName
                  where:(NSString *)predicateFormat
                   args:(va_list)args {
  return [self objects:objectClassName
         withPredicate:[NSPredicate predicateWithFormat:predicateFormat
                                              arguments:args]];
}
...
- (RLMObject *)objectWithClassName:(NSString *)className
                     forPrimaryKey:(id)primaryKey {
...
+ (uint64_t)schemaVersionAtURL:(NSURL *)fileURL
  ...
  try {
    version = WabiRealm::get_schema_version(config.configRef);
  } catch (...) {
    RLMRealmTranslateException(error);
    return version;
  }
...
+ (BOOL)performMigrationForConfiguration:(RLMRealmConfiguration *)configuration
  ...
  BOOL success;
  @autoreleasepool {
    success = [RLMRealm realmWithConfiguration:configuration
...
- (RLMObject *)createObject:(NSString *)className withValue:(id)value {
  return (RLMObject *)RLMCreateObjectInRealmWithValue(self, className, value,
...
- (RLMRealm *)thaw {
  ...
  return self.isFrozen
             ? [RLMRealm realmWithConfiguration:self.configuration error:nil]
             : self;
}
...
- (bool)isFlexibleSync {
#if REALM_ENABLE_SYNC
  return _realm->config().sync_config &&
         _realm->config().sync_config->flx_sync_requested;
#else
  return false;
#endif
}
...
- (RLMSyncSubscriptionSet *)subscriptions {
  ...
  if (!self.isFlexibleSync) {
    @throw RLMException(
        @"This WabiRealm was not configured with flexible sync");
  }
  return [[RLMSyncSubscriptionSet alloc]
      initWithSubscriptionSet:_realm->get_latest_subscription_set()
                        realm:self];
#else
  @throw RLMException(@"WabiRealm was not compiled with sync enabled");
#endif
}
@end
