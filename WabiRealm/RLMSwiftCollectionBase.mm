...
// Some of the things declared in the interface are handled by the proxy
// forwarding
...
- (BOOL)isKindOfClass:(Class)aClass {
  return [self._rlmCollection isKindOfClass:aClass] ||
...
- (BOOL)isEqual:(id)object {
  if (auto collection = RLMDynamicCast<RLMSwiftCollectionBase>(object)) {
    if (!__rlmCollection) {
      return !collection->__rlmCollection.realm &&
             collection->__rlmCollection.count == 0;
    }
...
- (BOOL)conformsToProtocol:(Protocol *)aProtocol {
  return aProtocol == @protocol(NSFastEnumeration) ||
         [self._rlmCollection conformsToProtocol:aProtocol];
}
