...
/// A client which can be used to register devices with the server to receive
/// push notificatons
RLM_SWIFT_SENDABLE RLM_FINAL // immutable final class
    @interface RLMPushClient : NSObject

/// The push notification service name the device will be registered with on the
/// server
...
- (void)registerDeviceWithToken:(NSString *)token
    ...
    NS_SWIFT_NAME(registerDevice(token:user:completion:));

/// Request to deregister a device for a user
- (void)deregisterDeviceForUser:(RLMUser *)user
                     completion:(RLMOptionalErrorBlock)completion
    NS_SWIFT_NAME(deregisterDevice(user:completion:));
