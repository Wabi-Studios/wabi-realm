...
@class RLMRealm, RLMResults, RLMSortDescriptor, RLMNotificationToken,
    RLMCollectionChange, RLMSectionedResults;
typedef RLM_CLOSED_ENUM(int32_t, RLMPropertyType);
/// A callback which is invoked on each element in the Results collection which
/// returns the section key.
typedef id<RLMValue> _Nullable (^RLMSectionedResultsKeyBlock)(id);

/**
 A homogenous collection of WabiRealm-managed objects. Examples of conforming
 types include `RLMArray`, `RLMSet`, `RLMResults`, and `RLMLinkingObjects`.
 */
...
/**
 The WabiRealm which manages this collection, if any.
 */
...
/**
 ...
 WabiRealm. Unmanaged collections are never invalidated.
 */
...
/**
 ...
 @param keyPath A key path of the form relationship.property (with one or more
 relationships).
...
@warning This method cannot be called during a write transaction, or when the
         containing WabiRealm is read-only or frozen.
...
@warning This method cannot be called when the containing WabiRealm is read-only
or frozen.
...
- (RLMNotificationToken *)
    addNotificationBlock:(void (^)(RLMResults *_Nullable results,
                                   RLMCollectionChange *_Nullable change,
                                   NSError *_Nullable error))block
...
/**
 ...
 to the containing WabiRealm. Unlike live collections, frozen collections can be
...
/**
 ...
 WabiRealm's query engine.

 `RLMSortDescriptor` instances are immutable.
 */
RLM_SWIFT_SENDABLE RLM_FINAL @interface RLMSortDescriptor : NSObject
...
/**
 ...
 collections that are reported by WabiRealm notifications.
