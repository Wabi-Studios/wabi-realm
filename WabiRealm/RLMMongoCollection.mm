...
- (instancetype)
    initWithChangeEventSubscriber:(id<RLMChangeEventDelegate>)subscriber
  ...
  if (_watchStream.state() == wabi_realm::app::WatchStream::State::NEED_DATA) {
    [event enumerateByteRangesUsingBlock:^(const void *bytes, NSRange byteRange,
      ...
      _watchStream.feed_buffer(
          std::string_view(static_cast<const char *>(bytes), byteRange.length));
    }];
  }
...
- (void)findWhere:(NSDictionary<NSString *, id<RLMBSON>> *)document
  ...
  self.collection.find(
      ...
      [completion](std::optional<wabi_realm::bson::BsonArray> documents,
        ...
        completion((NSArray<NSDictionary<NSString *, id<RLMBSON>> *> *)
                       RLMConvertBsonToRLMBSON(*documents),
                   nil);
      });
}
...
- (void)findWhere:(NSDictionary<NSString *, id<RLMBSON>> *)document
       completion:(RLMMongoFindBlock)completion {
  [self findWhere:document
          options:[[RLMFindOptions alloc] init]
...
- (void)findOneDocumentWhere:(NSDictionary<NSString *, id<RLMBSON>> *)document
  ...
  self.collection.find_one(
      ...
      [completion](std::optional<wabi_realm::bson::BsonDocument> document,
        ...
        if (document) {
          completion((NSDictionary<NSString *, id<RLMBSON>> *)
                         RLMConvertBsonToRLMBSON(*document),
                     nil);
        } else {
          completion(nil, nil);
        }
...
- (void)findOneDocumentWhere:(NSDictionary<NSString *, id<RLMBSON>> *)document
                  completion:(RLMMongoFindOneBlock)completion {
  [self findOneDocumentWhere:document
                     options:[[RLMFindOptions alloc] init]
                  completion:completion];
}
...
- (void)insertManyDocuments:
  ...
  self.collection.insert_many(
      ...
      [completion](std::vector<wabi_realm::bson::Bson> insertedIds,
        ...
        NSMutableArray *insertedArr =
            [[NSMutableArray alloc] initWithCapacity:insertedIds.size()];
...
- (void)findOneAndUpdateWhere:
  ...
  self.collection.find_one_and_update(
      ...
      [completion](std::optional<wabi_realm::bson::BsonDocument> document,
        ...
        return completion(
            (NSDictionary *)RLMConvertBsonDocumentToRLMBSON(document), nil);
      });
}
...
- (RLMChangeStream *)
    watchWithMatchFilter:(NSDictionary<NSString *, id<RLMBSON>> *)matchFilter
                delegate:(id<RLMChangeEventDelegate>)delegate
...
- (RLMChangeStream *)watchWithMatchFilter:(nullable id<RLMBSON>)matchFilter
  ...
  return [self watchWithMatchFilter:matchFilter
                           idFilter:idFilter
                           delegate:delegate
                          scheduler:^(dispatch_block_t block) {
...
- (RLMChangeStream *)watchWithMatchFilter:(nullable id<RLMBSON>)matchFilter
  ...
  auto changeStream =
      [[RLMChangeStream alloc] initWithChangeEventSubscriber:delegate
                                                   scheduler:scheduler];
  RLMNetworkTransport *transport = self.user.app.configuration.transport;
  RLMRequest *rlmRequest = RLMRequestFromRequest(request);
  changeStream->_session = [transport doStreamRequest:rlmRequest
