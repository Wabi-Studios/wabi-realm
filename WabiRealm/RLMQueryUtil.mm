...
NSString *const RLMUnsupportedTypesFoundInPropertyComparisonException =
    @"RLMUnsupportedTypesFoundInPropertyComparisonException";

NSString *const RLMPropertiesComparisonTypeMismatchReason =
    @"Property type mismatch between %@ and %@";
NSString *const RLMUnsupportedTypesFoundInPropertyComparisonReason =
    @"Comparison between %@ and %@";

namespace {

// small helper to create the many exceptions thrown when parsing predicates
[[gnu::cold]] [[noreturn]] void throwException(NSString *name, NSString *format,
                                               ...) {
  va_list args;
...
// this should be used iff the condition being false indicates a bug in the
// caller of the function checking its preconditions
...
bool isObjectValidForProperty(id value, RLMProperty *prop) {
  if (prop.collection) {
    if (propertyTypeIsLink(prop.type)) {
      ...
      return [RLMObjectBaseObjectSchema(obj).className
          isEqualToString:prop.objectClassName];
    }
...
// Equal and ContainsSubstring are used by QueryBuilder::add_string_constraint
// as the comparator for performing diacritic-insensitive comparisons.
...
bool equal(CFStringCompareFlags options, StringData v1, StringData v2) {
  ...
  auto s1 = util::adoptCF(CFStringCreateWithBytesNoCopy(
      kCFAllocatorSystemDefault, (const UInt8 *)v1.data(), v1.size(),
      kCFStringEncodingUTF8, false, kCFAllocatorNull));
  auto s2 = util::adoptCF(CFStringCreateWithBytesNoCopy(
      kCFAllocatorSystemDefault, (const UInt8 *)v2.data(), v2.size(),
      kCFStringEncodingUTF8, false, kCFAllocatorNull));

  return CFStringCompare(s1.get(), s2.get(), options) == kCFCompareEqualTo;
}

template <CFStringCompareFlags options> struct Equal {
  ...
  static const char *description() {
    return options & kCFCompareCaseInsensitive ? "==[cd]" : "==[d]";
  }
};

template <CFStringCompareFlags options> struct NotEqual {
...
bool contains_substring(CFStringCompareFlags options, StringData v1,
                        StringData v2) {
  if (v2.is_null()) {
...
template <CFStringCompareFlags options> struct ContainsSubstring {
  using CaseSensitive = ContainsSubstring<options & ~kCFCompareCaseInsensitive>;
  using CaseInsensitive =
      ContainsSubstring<options | kCFCompareCaseInsensitive>;
  ...
  static const char *description() {
    return options & kCFCompareCaseInsensitive ? "CONTAINS[cd]" : "CONTAINS[d]";
  }
};
...
NSString *operatorName(NSPredicateOperatorType operatorType) {
  ...
  return [NSString
      stringWithFormat:@"unknown operator %lu", (unsigned long)operatorType];
}

[[gnu::cold]] [[noreturn]] void
unsupportedOperator(RLMPropertyType datatype,
...
bool isNSNull(id value) { return !value || value == NSNull.null; }

template <typename T> bool isNSNull(T) { return false; }

Table &get_table(Group &group, RLMObjectSchema *objectSchema) {
  return *ObjectStore::table_for_object_type(group,
                                             objectSchema.objectStoreName);
}

// A reference to a column within a query. Can be resolved to a Columns<T> for
// use in query expressions.
class ColumnReference {
public:
  ColumnReference(Query &query, Group &group, RLMSchema *schema,
                  RLMProperty *property,
                  std::vector<RLMProperty *> &&links = {})
      : m_links(std::move(links)), m_property(property), m_schema(schema),
        m_group(&group), m_query(&query), m_link_chain(query.get_table()) {
    ...
    m_col = m_link_chain.get_current_table()->get_column_key(
        m_property.columnName.UTF8String);
  }
  ...
  auto resolve(SubQuery &&...subquery) const {
    static_assert(sizeof...(SubQuery) < 2,
                  "resolve() takes at most one subquery");
  ...
  bool is_link() const noexcept { return propertyTypeIsLink(type()); }
  ...
  ColumnReference append(RLMProperty *prop) const {
    ...
    return ColumnReference(*m_query, *m_group, m_schema, prop,
                           std::move(links));
  }

  void validate_comparison(id value) const {
    RLMPrecondition(
        isObjectValidForProperty(value, m_property), @"Invalid value",
        @"Cannot compare value '%@' of type '%@' to property '%@' of type '%@'",
        value, [value class], m_property.name,
        m_property.objectClassName ?: RLMTypeToString(m_property.type));
    if (RLMObjectBase *obj = RLMDynamicCast<RLMObjectBase>(value)) {
      RLMPrecondition(!obj->_row.is_valid() || m_group == &obj->_realm.group,
                      ...
                      @"Object must be from the WabiRealm being queried");
    }
  }
...
private:
  std::pair<Table &, ColKey> link_origin(RLMProperty *prop) const {
    ...
    NSString *column_name =
        link_origin_schema[prop.linkOriginPropertyName].columnName;
    auto link_origin_column =
        link_origin_table.get_column_key(column_name.UTF8String);
...
public:
  ...
  CollectionOperation(Type type, ColumnReference &&link_column,
                      ColumnReference &&column)
      : m_type(type), m_link_column(std::move(link_column)),
  ...
  void validate_comparison(id value) const {
    ...
    switch (m_type) {
    case Count:
      RLMPrecondition([value isKindOfClass:[NSNumber class]],
                      @"Invalid operand",
                      @"%@ can only be compared with a numeric value.",
                      name_for_type(m_type));
      return;
    case Average:
    case Minimum:
    case Maximum:
      // Null on min/max/average matches arrays with no non-null values,
      // including on non-nullable types
      valid = isNSNull(value) ||
              isObjectValidForProperty(value, m_column.property());
      break;
    case Sum:
      // Sums are never null
      valid = !isNSNull(value) &&
      ...
      break;
    case AllKeys:
      RLMPrecondition(isNSNull(value) || [value isKindOfClass:[NSString class]],
                      @"Invalid operand",
    ...
    RLMPrecondition(valid, @"Invalid operand",
                    ...
                    name_for_type(m_type), RLMTypeToString(m_column.type()),
                    value);
  }
  ...
  void validate_comparison(const ColumnReference &column) const {
    ...
    case Sum:
      RLMPrecondition(propertyTypeIsNumeric(column.type()), @"Invalid operand",
                      ...
                      @"property of type '%@'",
                      name_for_type(m_type), RLMTypeToString(m_column.type()),
                      RLMTypeToString(column.type()));
      break;
    case AllKeys:
      RLMPrecondition(column.type() == RLMPropertyTypeString,
                      @"Invalid operand",
                      @"@allKeys can only be compared with a string value.");
      break;
    case None:
...
public:
  ...
  void apply_collection_operator_expression(KeyPath &&kp, id value,
                                            NSComparisonPredicate *pred);
  void apply_value_expression(KeyPath &&kp, id value,
                              NSComparisonPredicate *pred);
  void apply_column_expression(KeyPath &&left, KeyPath &&right,
                               NSComparisonPredicate *predicate);
  void apply_function_expression(RLMObjectSchema *objectSchema,
                                 NSExpression *functionExpression,
                                 NSPredicateOperatorType operatorType,
                                 NSExpression *right);
  void apply_map_expression(RLMObjectSchema *objectSchema,
                            NSExpression *functionExpression,
  ...
  void add_numeric_constraint(RLMPropertyType datatype,
                              NSPredicateOperatorType operatorType, A &&lhs,
                              B &&rhs);

  template <typename A, typename B>
  void add_bool_constraint(RLMPropertyType,
                           NSPredicateOperatorType operatorType, A &&lhs,
                           B &&rhs);
  ...
  template <typename C, typename T>
  void add_diacritic_sensitive_string_constraint(
      NSPredicateOperatorType operatorType,
      NSComparisonPredicateOptions predicateOptions, C &&column, T &&value);
  template <typename C, typename T>
  void do_add_diacritic_sensitive_string_constraint(
      NSPredicateOperatorType operatorType,
      NSComparisonPredicateOptions predicateOptions, C &&column, T &&value);
  ...
  void do_add_constraint(RLMPropertyType type,
                         NSPredicateOperatorType operatorType,
                         NSComparisonPredicateOptions predicateOptions,
                         ColumnReference const &column, T &&value);

  void add_between_constraint(const ColumnReference &column, id value);

  void add_memberwise_equality_constraint(const ColumnReference &column,
                                          RLMObjectBase *obj);
  ...
  template <CollectionOperation::Type Operation, bool IsLinkCollection,
            bool IsDictionary, typename R>
  void add_collection_operation_constraint(
      NSPredicateOperatorType operatorType,
  ...
  ColumnReference column_reference_from_key_path(KeyPath &&kp,
                                                 bool isAggregate);
...
void QueryBuilder::add_substring_constraint(const T &value, Query condition) {
  // Foundation always returns false for substring operations with a RHS of null
  // or "".
...
template <>
void QueryBuilder::add_substring_constraint(const Mixed &value,
                                            Query condition) {
  // Foundation always returns false for substring operations with a RHS of null
  // or "".
...
template <typename T>
void QueryBuilder::add_substring_constraint(const Columns<T> &value,
                                            Query condition) {
  // Foundation always returns false for substring operations with a RHS of null
  // or "". We don't need to concern ourselves with the possibility of value
  // traversing a link list and producing multiple values per row as such
  // expressions will have been rejected.
  m_query.and_query(const_cast<Columns<T> &>(value).size() != 0 &&
                    std::move(condition));
}

template <typename Comparator>
Query make_diacritic_insensitive_constraint(bool caseSensitive,
...
Query make_diacritic_insensitive_constraint(
    NSPredicateOperatorType operatorType, bool caseSensitive,
  ...
  case NSBeginsWithPredicateOperatorType: {
    ...
    return make_diacritic_insensitive_constraint<ContainsSubstring<flags>>(
        caseSensitive, std::move(left), std::move(right));
  }
  case NSEndsWithPredicateOperatorType: {
    constexpr auto flags = kCFCompareDiacriticInsensitive | kCFCompareAnchored |
                           kCFCompareBackwards;
...
template <typename C, typename T>
void QueryBuilder::do_add_diacritic_sensitive_string_constraint(
    NSPredicateOperatorType operatorType,
    NSComparisonPredicateOptions predicateOptions, C &&column, T &&value) {
  ...
  case NSLikePredicateOperatorType:
    ...
    break;
  default: {
    if constexpr (is_any_v<C, Columns<String>, Columns<Lst<String>>,
                           Columns<Set<String>>>) {
      unsupportedOperator(RLMPropertyTypeString, operatorType);
    } else if constexpr (is_any_v<C, Columns<Binary>, Columns<Lst<Binary>>,
                                  Columns<Set<Binary>>>) {
      unsupportedOperator(RLMPropertyTypeData, operatorType);
    } else if constexpr (is_any_v<C, Columns<Mixed>, Columns<Lst<Mixed>>,
                                  Columns<Set<Mixed>>>) {
      unsupportedOperator(RLMPropertyTypeAny, operatorType);
    } else if constexpr (is_any_v<C, Columns<Dictionary>>) {
      // The underlying storage type Dictionary is always Mixed. This creates an
      // issue where we cannot be descriptive about the exception as we do not
      // know the actual value type.
      throwException(
...
template <typename C, typename T>
void QueryBuilder::add_diacritic_sensitive_string_constraint(
    NSPredicateOperatorType operatorType,
    NSComparisonPredicateOptions predicateOptions, C &&column, T &&value) {
  if constexpr (is_any_v<C, Columns<Dictionary>>) {
    // This nesting isnt pretty but without it the compiler will complain about
    // `T` having no known conversion from Columns<StringData> to Mixed. This is
    // due to the fact that all values on a dictionary column are boxed in
    // Mixed.
    if constexpr (is_any_v<T, Mixed, BinaryData, StringData>) {
      do_add_diacritic_sensitive_string_constraint(
          operatorType, predicateOptions, std::forward<C>(column), value);
    }
  } else {
    do_add_diacritic_sensitive_string_constraint(
        operatorType, predicateOptions, std::forward<C>(column), value);
...
template <typename C, typename T>
void QueryBuilder::add_string_constraint(
    NSPredicateOperatorType operatorType,
    NSComparisonPredicateOptions predicateOptions, C &&column, T &&value) {
  if (!(predicateOptions & NSDiacriticInsensitivePredicateOption)) {
    add_diacritic_sensitive_string_constraint(operatorType, predicateOptions,
                                              std::forward<C>(column),
                                              std::move(value));
    return;
  }
  ...
  auto as_subexpr = util::overload{
      ...
      [](BinaryData value) {
        return make_subexpr<ConstantStringValue>(
            StringData(value.data(), value.size()));
      },
      [](Mixed value) {
        ...
        case DataType::Type::Binary:
          return make_subexpr<ConstantStringValue>(
              StringData(value.get_binary().data(), value.get_binary().size()));
        default:
          REALM_UNREACHABLE();
        }
      },
  ...
  case NSEndsWithPredicateOperatorType:
  case NSContainsPredicateOperatorType:
    add_substring_constraint(value, make_diacritic_insensitive_constraint(
                                        operatorType, caseSensitive,
                                        std::move(left), std::move(right)));
    break;
  case NSNotEqualToPredicateOperatorType:
    m_query.and_query(make_diacritic_insensitive_constraint<NotEqual<flags>>(
        caseSensitive, std::move(left), std::move(right)));
    break;
  case NSEqualToPredicateOperatorType:
    m_query.and_query(make_diacritic_insensitive_constraint<Equal<flags>>(
        caseSensitive, std::move(left), std::move(right)));
    break;
  case NSLikePredicateOperatorType:
    throwException(
...
id value_from_constant_expression_or_value(id value) {
  if (NSExpression *exp = RLMDynamicCast<NSExpression>(value)) {
    RLMPrecondition(
        exp.expressionType == NSConstantValueExpressionType, @"Invalid value",
...
void validate_and_extract_between_range(id value, RLMProperty *prop, id *from,
                                        id *to) {
  NSArray *array = RLMDynamicCast<NSArray>(value);
  RLMPrecondition(array, @"Invalid value",
                  @"object must be of type NSArray for BETWEEN operations");
  RLMPrecondition(array.count == 2, @"Invalid value",
                  @"NSArray object must contain exactly two objects for "
                  @"BETWEEN operations");
  ...
  *to = value_from_constant_expression_or_value(array.lastObject);
  RLMPrecondition(isObjectValidForProperty(*from, prop) &&
                      isObjectValidForProperty(*to, prop),
                  @"Invalid value",
...
void QueryBuilder::add_between_constraint(const ColumnReference &column,
                                          id value) {
  if (column.has_any_to_many_links()) {
    auto link_column = column.last_link_column();
    Query subquery =
        get_table(m_group, link_column.link_target_object_schema()).where();
    QueryBuilder(subquery, m_group, m_schema)
        .add_between_constraint(column.column_ignoring_links(subquery), value);

    m_query.and_query(link_column.resolve<Link>(std::move(subquery)).count() >
                      0);
    return;
  }
...
void QueryBuilder::add_memberwise_equality_constraint(
    const ColumnReference &column, RLMObjectBase *obj) {
  for (RLMProperty *property in obj->_objectSchema.properties) {
    // Both of these probably are implementable, but are significantly more
    // complicated.
    RLMPrecondition(!property.collection, @"Invalid predicate",
                    @"Unsupported property '%@.%@' for memberwise equality "
                    @"query: equality on collections is not implemented.",
                    obj->_objectSchema.className, property.name);
    RLMPrecondition(!propertyTypeIsLink(property.type), @"Invalid predicate",
                    @"Unsupported property '%@.%@' for memberwise equality "
                    @"query: object links are not implemented.",
                    obj->_objectSchema.className, property.name);
    add_constraint(NSEqualToPredicateOperatorType, 0, column.append(property),
                   RLMDynamicGet(obj, property));
  }
}
...
void QueryBuilder::add_link_constraint(NSPredicateOperatorType operatorType,
  ...
  // we want to perform memberwise equality rather than equality on the link
  // itself.
  if (![obj isKindOfClass:[RLMObjectBase class]]) {
    ...
    // group. Unary links could also use a subquery but that has worse
    // performance.
    if (column.property().collection) {
      Query subquery =
          get_table(m_group, column.link_target_object_schema()).where();
      QueryBuilder(subquery, m_group, m_schema)
          .add_memberwise_equality_constraint(
              ColumnReference(subquery, m_group, m_schema), obj);
      if (operatorType == NSEqualToPredicateOperatorType) {
        m_query.and_query(column.resolve<Link>(std::move(subquery)).count() >
                          0);
      } else {
        ...
        m_query.and_query(column.resolve<Link>().count() > 0 &&
                          column.resolve<Link>(std::move(subquery)).count() ==
                              0);
      }
    } else {
  ...
  if (!obj->_row.is_valid()) {
    ...
    if (column.property().dictionary) {
      add_bool_constraint(RLMPropertyTypeObject, operatorType,
                          column.resolve<Dictionary>(), obj->_row);
    } else {
      add_bool_constraint(RLMPropertyTypeObject, operatorType,
                          column.resolve<Link>(), obj->_row);
    }
  }
}
...
void QueryBuilder::add_link_constraint(NSPredicateOperatorType operatorType,
  ...
  if (column.property().dictionary) {
    ...
    add_bool_constraint(RLMPropertyTypeObject, operatorType,
                        column.resolve<Link>(), null());
  }
}

void QueryBuilder::add_link_constraint(NSPredicateOperatorType operatorType,
                                       const ColumnReference &a,
                                       const ColumnReference &b) {
  if (a.property().dictionary) {
    add_bool_constraint(RLMPropertyTypeObject, operatorType,
                        a.resolve<Dictionary>(), b.resolve<Dictionary>());
  } else {
    add_bool_constraint(RLMPropertyTypeObject, operatorType, a.resolve<Link>(),
                        b.resolve<Link>());
  }
}

// iterate over an array of subpredicates, using @func to build a query from
// each one and ORing them together
...
template <> auto value_of_type<Mixed>(id value) {
...
template <typename T, typename Fn> void convert_null(T &&value, Fn &&fn) {
  if (isNSNull(value)) {
    fn(null());
  } else {
    fn(value);
...
template <template <typename> typename W, typename T>
void QueryBuilder::do_add_constraint(
    RLMPropertyType type, NSPredicateOperatorType operatorType,
    NSComparisonPredicateOptions predicateOptions,
    ColumnReference const &column, T &&value) {
  switch (type) {
  ...
  case RLMPropertyTypeAny:
    convert_null(value, [&](auto &&value) {
      add_mixed_constraint(operatorType, predicateOptions,
                           column.resolve<W<Mixed>>(), value);
    });
  }
}
...
template <typename C, typename T>
void QueryBuilder::add_mixed_constraint(
    NSPredicateOperatorType operatorType,
    NSComparisonPredicateOptions predicateOptions, Columns<C> &&column,
    T value) {
  // Handle cases where a string might be '1' or '0'. Without this the string
  // will be boxed as a bool and thus string query operations will crash in
  // core.
  ...
  do_add_mixed_constraint(operatorType, predicateOptions, std::move(column),
                          value_of_type<Mixed>(value));
}

template <typename C>
void QueryBuilder::do_add_mixed_constraint(
    NSPredicateOperatorType operatorType,
    NSComparisonPredicateOptions predicateOptions, Columns<C> &&column,
    Mixed &&value) {
  switch (operatorType) {
...
void QueryBuilder::add_mixed_constraint(NSPredicateOperatorType operatorType,
  ...
  add_bool_constraint(RLMPropertyTypeObject, operatorType, column,
                      value.resolve<Mixed>());
}

template <typename T> using Identity = T;
template <typename> using AlwaysDictionary = Dictionary;
...
void QueryBuilder::add_constraint(NSPredicateOperatorType operatorType,
  ...
  if (column.property().array) {
    ...
    do_add_constraint<AlwaysDictionary>(type, operatorType, predicateOptions,
                                        column, rhs);
  } else {
    do_add_constraint<Identity>(type, operatorType, predicateOptions, column,
                                rhs);
  }
}
...
KeyPath key_path_from_string(RLMSchema *schema, RLMObjectSchema *objectSchema,
                             NSString *keyPath) {
  RLMProperty *property;
  std::vector<RLMProperty *> links;
  ...
  for (; end != NSNotFound; start = end + 1) {
    end = [keyPath rangeOfString:@"." options:0 range:{start, length - start}]
              .location;
    RLMPrecondition(end == NSNotFound || end + 1 < length, @"Invalid predicate",
                    @"Invalid keypath '%@': no key name after last '.'",
                    keyPath);
    RLMPrecondition(end > start, @"Invalid predicate",
                    @"Invalid keypath '%@': no key name before '.'", keyPath);

    NSString *propertyName =
        [keyPath substringWithRange:{start, end == NSNotFound ? length - start
                                                              : end - start}];
    ...
    if ([propertyName characterAtIndex:0] == '@') {
      if ([propertyName isEqualToString:@"@allValues"]) {
        RLMPrecondition(property.dictionary, @"Invalid predicate",
                        @"Invalid keypath '%@': @allValues must follow a "
                        @"dictionary property.",
                        keyPath);
        continue;
      }
      RLMPrecondition(collectionOperation == CollectionOperation::None,
                      @"Invalid predicate",
                      @"Invalid keypath '%@': at most one collection operation "
                      @"per keypath is supported.",
                      keyPath);
      collectionOperation = CollectionOperation::type_for_name(propertyName);
      RLMPrecondition(
          collectionOperation != CollectionOperation::None,
          @"Invalid predicate",
          @"Invalid keypath '%@': Unsupported collection operation '%@'",
          keyPath, propertyName);

      RLMPrecondition(property.collection, @"Invalid predicate",
                      @"Invalid keypath '%@': collection operation '%@' must "
      ...
      case CollectionOperation::Count:
        RLMPrecondition(end == NSNotFound, @"Invalid predicate",
                        @"Invalid keypath '%@': @count must appear at the end "
                        @"of a keypath.",
                        keyPath);
        break;
      case CollectionOperation::AllKeys:
        RLMPrecondition(end == NSNotFound, @"Invalid predicate",
                        @"Invalid keypath '%@': @allKeys must appear at the "
                        @"end of a keypath.",
                        keyPath);
        RLMPrecondition(property.dictionary, @"Invalid predicate",
                        @"Invalid keypath '%@': @allKeys must follow a "
                        @"dictionary property.",
                        keyPath);
        break;
      default:
        if (propertyTypeIsLink(property.type)) {
          RLMPrecondition(end != NSNotFound, @"Invalid predicate",
                          @"Invalid keypath '%@': %@ on a collection of "
                          @"objects cannot appear at the end of a keypath.",
                          keyPath, propertyName);
        } else {
          RLMPrecondition(end == NSNotFound, @"Invalid predicate",
                          @"Invalid keypath '%@': %@ on a collection of values "
                          @"must appear at the end of a keypath.",
                          keyPath, propertyName);
          RLMPrecondition(propertyTypeIsNumeric(property.type),
                          @"Invalid predicate",
                          @"Invalid keypath '%@': %@ can only be applied to a "
    ...
    RLMPrecondition(objectSchema, @"Invalid predicate",
                    @"Invalid keypath '%@': %@ property %@ can only be "
                    @"followed by a collection operation.",
                    keyPath, property.typeName, property.name);

    property = objectSchema[propertyName];
    RLMPrecondition(
        property, @"Invalid predicate",
        @"Invalid keypath '%@': Property '%@' not found in object of type '%@'",
        keyPath, propertyName, objectSchema.className);
    RLMPrecondition(
        collectionOperation == CollectionOperation::None ||
            (propertyTypeIsNumeric(property.type) && !property.collection),
        @"Invalid predicate",
        @"Invalid keypath '%@': %@ must be followed by a numeric property.",
        keyPath, collectionOperationName);
    ...
    if (end != NSNotFound) {
      RLMPrecondition(
          property.type == RLMPropertyTypeObject ||
              property.type == RLMPropertyTypeLinkingObjects ||
              property.collection,
          @"Invalid predicate",
          @"Invalid keypath '%@': Property '%@.%@' is not a link or collection "
          @"and can only appear at the end of a keypath.",
          keyPath, objectSchema.className, propertyName);
      objectSchema =
          property.objectClassName ? schema[property.objectClassName] : nil;
    }
  };

  links.pop_back();
  return {std::move(links), property, collectionOperation,
          keyPathContainsToManyRelationship};
}

ColumnReference QueryBuilder::column_reference_from_key_path(KeyPath &&kp,
                                                             bool isAggregate) {
  if (isAggregate && !kp.containsToManyRelationship) {
    throwException(@"Invalid predicate",
                   @"Aggregate operations can only be used on key paths that "
                   @"include an collection property");
  } else if (!isAggregate && kp.containsToManyRelationship) {
    throwException(@"Invalid predicate",
                   @"Key paths that include a collection property must use "
                   @"aggregate operations");
  }

  return ColumnReference(m_query, m_group, m_schema, kp.property,
                         std::move(kp.links));
}
...
template <typename Requested, CollectionOperation::Type OperationType,
          bool IsLinkCollection, bool IsDictionary>
auto collection_operation_expr(CollectionOperation operation) {
  ...
  if constexpr (IsLinkCollection) {
    ...
    return collection_operation_expr_2<OperationType>(
        resolved.template column<Requested>(col));
  } else if constexpr (IsDictionary) {
    return collection_operation_expr_2<OperationType>(
        operation.link_column().resolve<Dictionary>());
  } else {
    return collection_operation_expr_2<OperationType>(
        operation.link_column().resolve<Lst<Requested>>());
  }
}

template <CollectionOperation::Type Operation, bool IsLinkCollection,
          bool IsDictionary, typename R>
void QueryBuilder::add_collection_operation_constraint(
    NSPredicateOperatorType operatorType,
    CollectionOperation const &collectionOperation, R &&rhs,
    NSComparisonPredicateOptions) {
  auto type = IsLinkCollection ? collectionOperation.column().type()
                               : collectionOperation.link_column().type();

  switch (type) {
  case RLMPropertyTypeInt:
    add_numeric_constraint(
        type, operatorType,
        collection_operation_expr<Int, Operation, IsLinkCollection,
                                  IsDictionary>(collectionOperation),
        value_of_type<Int>(rhs));
    break;
  case RLMPropertyTypeFloat:
    add_numeric_constraint(
        type, operatorType,
        collection_operation_expr<Float, Operation, IsLinkCollection,
                                  IsDictionary>(collectionOperation),
  ...
  case RLMPropertyTypeDouble:
    add_numeric_constraint(
        type, operatorType,
        collection_operation_expr<Double, Operation, IsLinkCollection,
                                  IsDictionary>(collectionOperation),
  ...
  case RLMPropertyTypeDecimal128:
    add_numeric_constraint(
        type, operatorType,
        collection_operation_expr<Decimal128, Operation, IsLinkCollection,
                                  IsDictionary>(collectionOperation),
        value_of_type<Decimal128>(rhs));
    break;
  case RLMPropertyTypeDate:
    if constexpr (Operation == CollectionOperation::Sum ||
                  Operation == CollectionOperation::Average) {
      throwException(@"Unsupported predicate value type",
                     @"Cannot sum or average date properties");
    } else {
      add_numeric_constraint(
          type, operatorType,
          collection_operation_expr<Timestamp, Operation, IsLinkCollection,
                                    IsDictionary>(collectionOperation),
  ...
  case RLMPropertyTypeAny:
    add_numeric_constraint(
        type, operatorType,
        collection_operation_expr<Mixed, Operation, IsLinkCollection,
                                  IsDictionary>(collectionOperation),
...
void QueryBuilder::add_collection_operation_constraint(
  ...
  convert_null(rhs, [&](auto &&rhs) {
    if (collectionOperation.link_column().is_link()) {
      add_collection_operation_constraint<Operation, true, false>(
          operatorType, collectionOperation, std::move(rhs), options);
    } else if (collectionOperation.column().property().dictionary) {
      add_collection_operation_constraint<Operation, false, true>(
          operatorType, collectionOperation, std::move(rhs), options);
    } else {
      add_collection_operation_constraint<Operation, false, false>(
          operatorType, collectionOperation, std::move(rhs), options);
    }
  });
}
...
void QueryBuilder::add_collection_operation_constraint(
  ...
  case CollectionOperation::Count: {
    ...
    auto continuation = [&](auto t) {
      add_numeric_constraint(
          type, operatorType,
          column.resolve<std::decay_t<decltype(*t)>>().size(), rhsValue);
    };

    switch (type) {
    case RLMPropertyTypeBool:
      return get_collection_type<Bool>(column.property(),
                                       std::move(continuation));
    case RLMPropertyTypeObjectId:
      return get_collection_type<ObjectId>(column.property(),
                                           std::move(continuation));
    case RLMPropertyTypeDate:
      return get_collection_type<Timestamp>(column.property(),
                                            std::move(continuation));
    case RLMPropertyTypeDouble:
      return get_collection_type<Double>(column.property(),
                                         std::move(continuation));
    case RLMPropertyTypeFloat:
      return get_collection_type<Float>(column.property(),
                                        std::move(continuation));
    case RLMPropertyTypeInt:
      return get_collection_type<Int>(column.property(),
                                      std::move(continuation));
    case RLMPropertyTypeDecimal128:
      return get_collection_type<Decimal128>(column.property(),
                                             std::move(continuation));
    case RLMPropertyTypeString:
      return get_collection_type<String>(column.property(),
                                         std::move(continuation));
    case RLMPropertyTypeData:
      return get_collection_type<Binary>(column.property(),
                                         std::move(continuation));
    case RLMPropertyTypeUUID:
      return get_collection_type<UUID>(column.property(),
                                       std::move(continuation));
    case RLMPropertyTypeAny:
      return get_collection_type<Mixed>(column.property(),
                                        std::move(continuation));
    case RLMPropertyTypeObject:
    case RLMPropertyTypeLinkingObjects:
      return add_numeric_constraint(type, operatorType,
                                    column.resolve<Link>().count(), rhsValue);
    }
  }
  case CollectionOperation::Minimum:
    add_collection_operation_constraint<CollectionOperation::Minimum>(
        operatorType, collectionOperation, std::move(rhs), comparisonOptions);
    break;
  case CollectionOperation::Maximum:
    add_collection_operation_constraint<CollectionOperation::Maximum>(
        operatorType, collectionOperation, std::move(rhs), comparisonOptions);
    break;
  case CollectionOperation::Sum:
    add_collection_operation_constraint<CollectionOperation::Sum>(
        operatorType, collectionOperation, std::move(rhs), comparisonOptions);
    break;
  case CollectionOperation::Average:
    add_collection_operation_constraint<CollectionOperation::Average>(
        operatorType, collectionOperation, std::move(rhs), comparisonOptions);
    break;
  case CollectionOperation::AllKeys: {
    // BETWEEN and IN are not supported by @allKeys as the parsing for
    // collection operators happens before and disection of a rhs array of
    // values.
    add_string_constraint(
        operatorType, comparisonOptions,
        Columns<Dictionary>(collectionOperation.link_column().column(),
                            m_query.get_table())
            .keys(),
...
CollectionOperation
QueryBuilder::collection_operation_from_key_path(KeyPath &&kp) {
  ...
  if (kp.collectionOperation != CollectionOperation::Count &&
      kp.collectionOperation != CollectionOperation::AllKeys &&
      !kp.property.collection) {
    REALM_ASSERT(!kp.links.empty());
    linkPrefix.property = linkPrefix.links.back();
    linkPrefix.links.pop_back();
  }
  return CollectionOperation(
      kp.collectionOperation,
      column_reference_from_key_path(std::move(linkPrefix), true),
      column_reference_from_key_path(std::move(kp), true));
}

NSPredicateOperatorType
invert_comparison_operator(NSPredicateOperatorType type) {
  switch (type) {
  case NSLessThanPredicateOperatorType:
  ...
  case NSLikePredicateOperatorType:
    throwException(@"Unsupported predicate",
                   @"Operator '%@' requires a keypath on the left side.",
                   operatorName(type));
  default:
    return type;
  }
...
void QueryBuilder::apply_collection_operator_expression(
    KeyPath &&kp, id value, NSComparisonPredicate *pred) {
  CollectionOperation operation =
      collection_operation_from_key_path(std::move(kp));
...
void QueryBuilder::apply_value_expression(KeyPath &&kp, id value,
                                          NSComparisonPredicate *pred) {
  ...
  // turn "key.path IN collection" into ored together ==. "collection IN
  // key.path" is handled elsewhere.
  if (pred.predicateOperatorType == NSInPredicateOperatorType) {
    process_or_group(m_query, value, [&](id item) {
      ...
      add_constraint(NSEqualToPredicateOperatorType, pred.options, column,
                     normalized);
    });
    return;
  }
  ...
  if (pred.leftExpression.expressionType == NSKeyPathExpressionType) {
    add_constraint(pred.predicateOperatorType, pred.options, std::move(column),
                   value);
  } else {
    add_constraint(invert_comparison_operator(pred.predicateOperatorType),
                   pred.options, std::move(column), value);
  }
}

void QueryBuilder::apply_column_expression(KeyPath &&leftKeyPath,
                                           KeyPath &&rightKeyPath,
                                           NSComparisonPredicate *predicate) {
  bool left_key_path_contains_collection_operator =
      key_path_contains_collection_operator(leftKeyPath);
  bool right_key_path_contains_collection_operator =
      key_path_contains_collection_operator(rightKeyPath);
  if (left_key_path_contains_collection_operator &&
      right_key_path_contains_collection_operator) {
    throwException(@"Unsupported predicate",
                   @"Key paths including aggregate operations cannot be "
                   @"compared with other aggregate operations.");
  }

  if (left_key_path_contains_collection_operator) {
    CollectionOperation left =
        collection_operation_from_key_path(std::move(leftKeyPath));
    ColumnReference right =
        column_reference_from_key_path(std::move(rightKeyPath), false);
    left.validate_comparison(right);
    add_collection_operation_constraint(predicate.predicateOperatorType,
                                        std::move(left), std::move(right),
                                        predicate.options);
    return;
  }
  if (right_key_path_contains_collection_operator) {
    ColumnReference left =
        column_reference_from_key_path(std::move(leftKeyPath), false);
    CollectionOperation right =
        collection_operation_from_key_path(std::move(rightKeyPath));
    right.validate_comparison(left);
    add_collection_operation_constraint(
        invert_comparison_operator(predicate.predicateOperatorType),
  ...
  bool isAny = false;
  ColumnReference left =
      column_reference_from_key_path(std::move(leftKeyPath), isAny);
  ColumnReference right =
      column_reference_from_key_path(std::move(rightKeyPath), isAny);

  // NOTE: It's assumed that column type must match and no automatic type
  // conversion is supported.
  RLMPrecondition(left.type() == right.type(),
                  RLMPropertiesComparisonTypeMismatchException,
                  RLMPropertiesComparisonTypeMismatchReason,
                  RLMTypeToString(left.type()), RLMTypeToString(right.type()));

  // TODO: Should we handle special case where left row is the same as right row
  // (tautology)
...
// -[NSPredicate predicateWithSubtitutionVariables:] results in function
// expressions of the form [SELF valueForKeyPath:] that apply_predicate cannot
// handle. Replace such expressions with equivalent NSKeyPathExpressionType
// expressions.
NSExpression *
simplify_self_value_for_key_path_function_expression(NSExpression *expression) {
...
void QueryBuilder::apply_map_expression(RLMObjectSchema *objectSchema,
                                        NSExpression *functionExpression,
                                        NSComparisonPredicateOptions options,
                                        NSPredicateOperatorType operatorType,
                                        NSExpression *right) {
  ...
  if (functionExpression.operand.expressionType == NSKeyPathExpressionType) {
    ...
    NSExpression *linkCol =
        [[functionExpression.operand arguments] firstObject];
    NSExpression *mapCol = [mapItems.arguments firstObject];
    mapKey = [mapItems.arguments[1] constantValue];
    keyPath =
        [NSString stringWithFormat:@"%@.%@", linkCol.keyPath, mapCol.keyPath];
  } else {
    keyPath = [functionExpression.arguments.firstObject keyPath];
    mapKey = [functionExpression.arguments[1] constantValue];
  }

  ColumnReference collectionColumn = column_reference_from_key_path(
      key_path_from_string(m_schema, objectSchema, keyPath), true);
  RLMPrecondition(
      collectionColumn.property().dictionary, @"Invalid predicate",
      @"Invalid keypath '%@': only dictionaries support subscript predicates.",
      functionExpression);
  add_mixed_constraint(
      operatorType, options,
      collectionColumn.resolve<Dictionary>().key(mapKey.UTF8String),
      right.constantValue);
}

void QueryBuilder::apply_function_expression(
    RLMObjectSchema *objectSchema, NSExpression *functionExpression,
    NSPredicateOperatorType operatorType, NSExpression *right) {
  RLMPrecondition(functionExpression.operand.expressionType ==
                      NSSubqueryExpressionType,
                  @"Invalid predicate", @"The '%@' function is not supported.",
                  functionExpression.function);
  RLMPrecondition(
      [functionExpression.function isEqualToString:@"valueForKeyPath:"] &&
          functionExpression.arguments.count == 1,
      @"Invalid predicate",
  ...
  RLMPrecondition([keyPathExpression.keyPath isEqualToString:@"@count"],
                  @"Invalid predicate",
                  @"SUBQUERY is only supported when immediately followed by "
                  @".@count that is compared with a constant number.");
  RLMPrecondition(right.expressionType == NSConstantValueExpressionType &&
                      [right.constantValue isKindOfClass:[NSNumber class]],
                  @"Invalid predicate expression",
                  @"SUBQUERY(…).@count is only supported when compared with a "
                  @"constant number.");
  ...
  ColumnReference collectionColumn = column_reference_from_key_path(
      key_path_from_string(m_schema, objectSchema,
                           [subqueryExpression.collection keyPath]),
      true);
  RLMObjectSchema *collectionMemberObjectSchema =
      m_schema[collectionColumn.property().objectClassName];

  // Eliminate references to the iteration variable in the subquery.
  NSPredicate *subqueryPredicate =
      [subqueryExpression.predicate predicateWithSubstitutionVariables:@{
        subqueryExpression.
        variable : [NSExpression expressionForEvaluatedObject]
      }];
  subqueryPredicate = transformPredicate(
      subqueryPredicate, simplify_self_value_for_key_path_function_expression);

  Query subquery = RLMPredicateToQuery(
      subqueryPredicate, collectionMemberObjectSchema, m_schema, m_group);
  add_numeric_constraint(
      RLMPropertyTypeInt, operatorType,
      collectionColumn.resolve<Link>(std::move(subquery)).count(), value);
}

void QueryBuilder::apply_predicate(NSPredicate *predicate,
  ...
  if ([predicate isMemberOfClass:[NSCompoundPredicate class]]) {
    ...
    case NSAndPredicateType:
      if (comp.subpredicates.count) {
        ...
        // NSCompoundPredicate's documentation states that an AND predicate with
        // no subpredicates evaluates to TRUE.
        m_query.and_query(std::unique_ptr<Expression>(new TrueExpression));
      }
      break;
    ...
    if (compp.predicateOperatorType == NSBetweenPredicateOperatorType ||
        compp.predicateOperatorType == NSInPredicateOperatorType) {
      ...
      if (exp1Type == NSKeyPathExpressionType &&
          (exp2Type == NSAggregateExpressionType ||
           exp2Type == NSConstantValueExpressionType)) {
        // "key.path IN %@", "key.path IN {…}", "key.path BETWEEN %@", or
        // "key.path BETWEEN {…}".
        exp2Type = NSConstantValueExpressionType;
      } else if (compp.predicateOperatorType == NSInPredicateOperatorType &&
                 exp1Type == NSConstantValueExpressionType &&
                 exp2Type == NSKeyPathExpressionType) {
        // "%@ IN key.path" is equivalent to "ANY key.path IN %@". Rewrite the
        // former into the latter.
        compp = [NSComparisonPredicate
            predicateWithLeftExpression:compp.rightExpression
                        rightExpression:compp.leftExpression
        ...
        if (compp.predicateOperatorType == NSBetweenPredicateOperatorType) {
          throwException(@"Invalid predicate",
                         @"Predicate with BETWEEN operator must compare a "
                         @"KeyPath with an aggregate with two values");
        } else if (compp.predicateOperatorType == NSInPredicateOperatorType) {
          throwException(@"Invalid predicate",
                         @"Predicate with IN operator must compare a KeyPath "
                         @"with an aggregate");
        }
      }
    }
    ...
    if (exp1Type == NSKeyPathExpressionType &&
      ...
      apply_column_expression(
          ...
          compp);
    } else if (exp1Type == NSKeyPathExpressionType &&
               exp2Type == NSConstantValueExpressionType) {
      // comparing keypath to value
      apply_value_expression(key_path_from_string(m_schema, objectSchema,
                                                  compp.leftExpression.keyPath),
                             compp.rightExpression.constantValue, compp);
    } else if (exp1Type == NSConstantValueExpressionType &&
               exp2Type == NSKeyPathExpressionType) {
      ...
      if (compp.leftExpression.operand.expressionType ==
          NSSubqueryExpressionType) {
        apply_function_expression(objectSchema, compp.leftExpression,
                                  compp.predicateOperatorType,
                                  compp.rightExpression);
      } else {
        apply_map_expression(objectSchema, compp.leftExpression, compp.options,
                             compp.predicateOperatorType,
                             compp.rightExpression);
      }
    } else if (exp1Type == NSSubqueryExpressionType) {
      // The subquery expressions that we support are handled by the
      // NSFunctionExpressionType case above.
      throwException(
          @"Invalid predicate expression",
...
realm::Query RLMPredicateToQuery(NSPredicate *predicate,
  ...
  try {
    @autoreleasepool {
      QueryBuilder(query, group, schema)
          .apply_predicate(predicate, objectSchema);
    }
...
RLMProperty *RLMValidatedProperty(RLMObjectSchema *desc, NSString *columnName) {
  ...
  RLMPrecondition(prop, @"Invalid property name",
                  @"Property '%@' not found in object of type '%@'", columnName,
                  desc.className);
  return prop;
}
