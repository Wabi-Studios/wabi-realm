...
/// that communicate with a particular WabiRealm application.
...
/**
Create a new WabiRealm App configuration.
...
/**
 The `RLMApp` has the fundamental set of methods for communicating with a
 WabiRealm application backend.
...
/// The configuration for this WabiRealm app.
@property(nonatomic, readonly) RLMAppConfiguration *configuration;

/// The `RLMSyncManager` for this WabiRealm app.
...
/// Get the current user logged into the WabiRealm app.
@property(nonatomic, readonly, nullable) RLMUser *currentUser;

/// The app ID for this WabiRealm app.
...
/**
 ...
 @param appId The unique identifier of your WabiRealm app.
 */
...
/**
 Login to a user for the WabiRealm app.
...
/**
 ...
 This sets which user is used by all RLMApp operations which require a user.
 This is a local operation which does not access the network. An exception will
 be throw if the user is not valid. The current user will remain logged in.
...
/**
 A client which can be used to register devices with the server to receive push
 notificatons
 */
...
/**
 RLMApp instances are cached internally by WabiRealm and cannot be created
 directly.
 ...
 */
- (instancetype)init
    __attribute__((unavailable("Use +appWithId or appWithId:configuration:.")));
...
/// Use this delegate to be provided a callback once authentication has succeed
/// or failed
...
- (void)authenticationDidFailWithError:(NSError *)error
    NS_SWIFT_NAME(authenticationDidComplete(error:));
...
- (void)authenticationDidCompleteWithUser:(RLMUser *)user
    NS_SWIFT_NAME(authenticationDidComplete(user:));
...
/// Use this delegate to be provided a callback once authentication has succeed
/// or failed.
@property(nonatomic, weak, nullable) id<RLMASLoginDelegate>
    authorizationDelegate;

/// Sets the ASAuthorizationControllerDelegate to be handled by `RLMApp`
/// @param controller The ASAuthorizationController in which you want `RLMApp`
/// to consume its delegate.
- (void)setASAuthorizationControllerDelegateForController:
    (ASAuthorizationController *)controller NS_REFINED_FOR_SWIFT;
