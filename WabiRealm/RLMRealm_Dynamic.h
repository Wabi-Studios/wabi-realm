...
#pragma mark - Getting Objects from a WabiRealm

/**
 Returns all objects of a given type from the WabiRealm.

 @warning This method is useful only in specialized circumstances, for example,
 when building components that integrate with WabiRealm. The preferred way to
 get objects of a single class is to use the class methods on `RLMObject`.

 @param className   The name of the `RLMObject` subclass to retrieve on (e.g.
 `MyClass.className`).

 @return    An `RLMResults` containing all objects in the WabiRealm of the given
 type.
...
/**
 Returns all objects matching the given predicate from the WabiRealm.

 @warning This method is useful only in specialized circumstances, for example,
 when building components that integrate with WabiRealm. The preferred way to
 get objects of a single class is to use the class methods on `RLMObject`.

 @param className       The type of objects you are looking for (name of the
 class).
 ...
 */
- (RLMResults<RLMObject *> *)objects:(NSString *)className
...
/**
 Returns the object of the given type with the given primary key from the
 WabiRealm.

 @warning This method is useful only in specialized circumstances, for example,
 when building components that integrate with WabiRealm. The preferred way to
 get an object of a single class is to use the class methods on `RLMObject`.
 ...
 @return    An object, or `nil` if an object with the given primary key does not
 exist.

 @see       `+[RLMObject objectForPrimaryKey:]`
 */
- (nullable RLMObject *)objectWithClassName:(NSString *)className
                              forPrimaryKey:(id)primaryKey;

/**
 Creates an `RLMObject` instance of type `className` in the WabiRealm, and
 populates it using a given object.
 ...
 When passing in an array as the `value` argument, all properties must be
 present, valid and in the same order as the properties defined in the model.

 @warning This method is useful only in specialized circumstances, for example,
 when building components that integrate with WabiRealm. If you are simply
 building an app on WabiRealm, it is recommended to use `[RLMObject
