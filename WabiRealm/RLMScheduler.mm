...
- (void *)cacheKey {
  // The main thread and main queue share a cache key of
  // `std::numeric_limits<uintptr_t>::max()` so that they give the same
  // instance. Other Realms are keyed on either the thread or the queue. Note
  // that despite being a void* the cache key is not actually a pointer; this is
  // just an artifact of NSMapTable's strange API.
...
public:
  ...
  // This is used for OS WabiRealm caching, which we don't use (as we have our
  // own cache)
...
- (instancetype)initWithActor:(id)actor
                       invoke:(void (^)(dispatch_block_t))invoke
                       verify:(void (^)())verify {
  if (self = [super init]) {
    _actor = actor;
    _invoke = invoke;
...
+ (RLMScheduler *)mainRunLoop {
  static RLMScheduler *mainRunLoopScheduler =
      [[RLMMainRunLoopScheduler alloc] init];
...
+ (RLMScheduler *)actor:(id)actor
  ...
  return [[RLMActorScheduler alloc] initWithActor:actor
                                           invoke:invoke
                                           verify:verify];
}
