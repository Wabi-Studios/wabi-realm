...
+ (dispatch_queue_t)notificationsQueue {
  static auto queue = dispatch_queue_create(
      "io.realm.sync.sessionsNotificationQueue", DISPATCH_QUEUE_SERIAL);
  return queue;
}

static RLMSyncConnectionState
convertConnectionState(SyncSession::ConnectionState state) {
  switch (state) {
...
- (instancetype)initWithSyncSession:
    (std::shared_ptr<SyncSession> const &)session {
  if (self = [super init]) {
    _session = session;
...
static util::UniqueFunction<void(Status)>
wrapCompletion(dispatch_queue_t queue, void (^callback)(NSError *)) {
...
- (RLMProgressNotificationToken *)
  ...
  if (auto session = _session.lock()) {
    ...
    uint64_t token = session->register_progress_notifier(
    ...
    return [[RLMProgressNotificationToken alloc] initWithTokenValue:token
                                                            session:session];
  }
  return nil;
}
...
+ (void)immediatelyHandleError:(RLMSyncErrorActionToken *)token
  ...
  [syncManager syncManager]->immediately_run_file_actions(
      std::move(token->_originalPath));
}
...
+ (nullable RLMSyncSession *)sessionForRealm:(RLMRealm *)realm {
  ...
  if (auto session = config->user->session_for_on_disk_path(
          realm->_realm->config().path)) {
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<RLMSyncSession: %p> {\n"
                                     "\tstate = %d;\n"
                                     "\tconnectionState = %d;\n"
                                     "\trealmURL = %@;\n"
                                     "\tuser = %@;\n"
                                    ...
                                    self.realmURL, self.parentUser.identifier];
}
