...
static inline void
RLMResultsValidateInWriteTransaction(__unsafe_unretained RLMResults *const ar) {
...
- (BOOL)isOptional {
  return translateErrors([&] { return is_nullable(_results.get_type()); });
}
...
- (NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)state
  ...
  if (state->state == 0) {
    translateErrors([&] { _results.evaluate_query_if_needed(); });
  }
...
- (NSUInteger)indexOfObjectWithPredicate:(NSPredicate *)predicate {
  ...
  return translateErrors([&] {
    ...
    return RLMConvertNotFound(_results.index_of(RLMPredicateToQuery(
        predicate, _info->rlmObjectSchema, _realm.schema, _realm.group)));
  });
}
...
- (id)valueForKeyPath:(NSString *)keyPath {
  ...
  NSUInteger separatorIndex = operatorRange.location != NSNotFound
                                  ? operatorRange.location
                                  : keyPathLength;
  NSString *operatorName =
      [keyPath substringWithRange:NSMakeRange(1, separatorIndex - 1)];
  SEL opSelector = NSSelectorFromString(
      [NSString stringWithFormat:@"_%@ForKeyPath:", operatorName]);
  if (![self respondsToSelector:opSelector]) {
    @throw RLMException(
        @"Unsupported KVC collection operator found in key path '%@'", keyPath);
  }
...
- (NSNumber *)
    _aggregateForKeyPath:(NSString *)keyPath
  ...
  return [self aggregate:keyPath
                  method:method
       returnNilForEmpty:returnNilForEmpty];
}

- (NSNumber *)_minForKeyPath:(NSString *)keyPath {
  return [self _aggregateForKeyPath:keyPath
                             method:&Results::min
                         methodName:@"@min"
                  returnNilForEmpty:YES];
}
...
- (NSArray *)_distinctUnionOfObjectsForKeyPath:(NSString *)keyPath {
  return
      [NSSet setWithArray:[self _unionOfObjectsForKeyPath:keyPath]].allObjects;
}

- (NSArray *)_unionOfArraysForKeyPath:(NSString *)keyPath {
  assertKeyPathIsNotNested(keyPath);
  if ([keyPath isEqualToString:@"self"]) {
    @throw RLMException(@"self is not a valid key-path for a KVC array "
                        @"collection operator as 'unionOfArrays'.");
  }

  return translateErrors([&] {
    NSMutableArray *flatArray = [NSMutableArray new];
    for (id<NSFastEnumeration> array in RLMCollectionValueForKey(
             _results, keyPath, *_info)) {
...
- (NSArray *)_distinctUnionOfArraysForKeyPath:(__unused NSString *)keyPath {
  return
      [NSSet setWithArray:[self _unionOfArraysForKeyPath:keyPath]].allObjects;
}
...
- (RLMResults *)objectsWithPredicate:(NSPredicate *)predicate {
  return translateErrors([&] {
    ...
    auto query = RLMPredicateToQuery(predicate, _info->rlmObjectSchema,
...
- (RLMResults *)sortedResultsUsingDescriptors:
  ...
  return translateErrors([&] {
    ...
    return [self
        subresultsWithResults:_results.sort(RLMSortDescriptorsToKeypathArray(
                                  properties))];
  });
}
...
- (RLMResults *)distinctResultsUsingKeyPaths:(NSArray<NSString *> *)keyPaths {
  for (NSString *keyPath in keyPaths) {
    if ([keyPath rangeOfString:@"@"].location != NSNotFound) {
      @throw RLMException(@"Cannot distinct on keypath '%@': KVC collection "
...
- (id)averageOfProperty:(NSString *)property {
  return [self aggregate:property
                  method:&Results::average
       returnNilForEmpty:YES];
}
...
- (RLMSectionedResults *)
  ...
  return [[RLMSectionedResults alloc]
      initWithResults:[self sortedResultsUsingKeyPath:keyPath
                                            ascending:ascending]
             keyBlock:keyBlock];
}
...
- (RLMSectionedResults *)
    sectionedResultsUsingSortDescriptors:
        (NSArray<RLMSortDescriptor *> *)sortDescriptors
                                keyBlock:(RLMSectionedResultsKeyBlock)keyBlock {
  return [[RLMSectionedResults alloc]
      initWithResults:[self sortedResultsUsingDescriptors:sortDescriptors]
             keyBlock:keyBlock];
}

- (void)deleteObjectsFromRealm {
  if (self.type != RLMPropertyTypeObject) {
    @throw RLMException(@"Cannot delete objects from RLMResults<%@>: only "
...
- (NSString *)description {
  return RLMDescriptionWithMaxDepth(@"RLMResults", self,
...
- (RLMNotificationToken *)addNotificationBlock:
    (void (^)(RLMResults *, RLMCollectionChange *, NSError *))block {
  return RLMAddNotificationBlock(self, block, nil, nil);
}
- (RLMNotificationToken *)addNotificationBlock:(void (^)(RLMResults *,
                                                         RLMCollectionChange *,
                                                         NSError *))block
...
+ (instancetype)objectWithThreadSafeReference:
  ...
  return [RLMResults resultsWithObjectInfo:realm->_info[RLMStringDataToNSString(
                                               results.get_object_type())]
...
- (NSString *)description {
  return RLMDescriptionWithMaxDepth(@"RLMLinkingObjects", self,
                                    RLMDescriptionMaxDepth);
}
@end
