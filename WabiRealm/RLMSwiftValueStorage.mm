...
public:
  ...
  void attach(__unsafe_unretained RLMObjectBase *const obj,
              NSString *property) {
    if (!_property) {
      _property = property;
      _parent = obj;
  ...
  NSString *propertyName() override { return _property; }
...
public:
  ManagedSwiftValueStorage(RLMObjectBase *obj, RLMProperty *prop)
      : _realm(obj->_realm),
        ...
        _columnName(prop.columnName.UTF8String), _ctx(*obj->_info) {}
...
- (BOOL)isKindOfClass:(Class)aClass {
  return [RLMGetSwiftValueStorage(self) isKindOfClass:aClass] ||
         RLMIsKindOfClass(object_getClass(self), aClass);
}
...
id RLMGetSwiftValueStorage(
    __unsafe_unretained RLMSwiftValueStorage *const self) {
  try {
...
void RLMSetSwiftValueStorage(
    __unsafe_unretained RLMSwiftValueStorage *const self,
    __unsafe_unretained const id value) {
  try {
    if (!self->_impl && value) {
...
void RLMInitializeManagedSwiftValueStorage(
    __unsafe_unretained RLMSwiftValueStorage *const self,
...
void RLMInitializeUnmanagedSwiftValueStorage(
    __unsafe_unretained RLMSwiftValueStorage *const self,
    __unsafe_unretained RLMObjectBase *const parent,
    __unsafe_unretained RLMProperty *const prop) {
  if (parent->_realm) {
  ...
  static_cast<UnmanagedSwiftValueStorage &>(*self->_impl)
      .attach(parent, prop.name);
}

NSString *
RLMSwiftValueStorageGetPropertyName(RLMSwiftValueStorage *const self) {
