...
static void createAccessors(RLMObjectSchema *objectSchema) {
  ...
  snprintf(start, bufferSize - strlen(className), "%llu %s", count++,
           objectSchema.className.UTF8String);
  objectSchema.accessorClass = RLMManagedAccessorClassForObjectClass(
      objectSchema.objectClass, objectSchema, className);
...
static RLMObjectSchema *registerClass(Class cls) {
  ...
  {
    util::ScopeExit cleanup(
        [&]() noexcept { s_sharedSchemaState = prevState; });
  ...
  if ([cls shouldIncludeInDefaultSchema] &&
      prevState != SharedSchemaState::Initialized) {
...
static void RLMRegisterClassLocalNames(Class *classes, NSUInteger count) {
  for (NSUInteger i = 0; i < count; i++) {
    ...
    if ([className hasPrefix:@"RLM:"] ||
        [className hasPrefix:@"NSKVONotifying"]) {
      continue;
    }
    ...
    // indicates it's a type, and C indicates it's a class.
      ...
      @throw RLMException(@"Object subclass '%@' must explicitly set the "
                          @"class's objective-c name with @objc(ClassName) "
                          @"because it is not a top-level public class.",
                          className);
    }

    if (Class existingClass = s_localNameToClass[className]) {
      if (existingClass != cls) {
        @throw RLMException(@"RLMObject subclasses with the same name cannot "
                            @"be included twice in the same target. "
                            @"Please make sure '%@' is only linked once to "
                            @"your current target.",
                            className);
      }
      continue;
...
- (void)setObjectSchema:(NSArray *)objectSchema {
  ...
  _objectSchemaByName =
      [NSMutableDictionary dictionaryWithCapacity:objectSchema.count];
...
- (RLMObjectSchema *)objectForKeyedSubscript:
  ...
  if (!schema) {
    @throw RLMException(@"Object type '%@' not managed by the WabiRealm",
                        className);
  }
  return schema;
}
...
+ (instancetype)schemaWithObjectClasses:(NSArray *)classes {
  ...
  @synchronized(s_localNameToClass) {
    ...
    schema->_objectSchemaByName =
        [NSMutableDictionary dictionaryWithCapacity:count];
    for (Class cls in classes) {
      if (!RLMIsObjectSubclass(cls)) {
        @throw RLMException(@"Can't add non-Object type '%@' to a schema.",
                            cls);
      }
  ...
  [schema->_objectSchemaByName enumerateKeysAndObjectsUsingBlock:^(
                                   id, RLMObjectSchema *objectSchema, BOOL *) {
    for (RLMProperty *prop in objectSchema.properties) {
      ...
      if (!schema->_objectSchemaByName[prop.objectClassName]) {
        [errors
            addObject:[NSString
                          stringWithFormat:@"- '%@.%@' links to class '%@', "
                                           @"which is missing from the list of "
                                           @"classes managed by the WabiRealm",
...
+ (RLMObjectSchema *)sharedSchemaForClass:(Class)cls {
  @synchronized(s_localNameToClass) {
    ...
    if (cls == [RLMObjectBase class] ||
        class_getSuperclass(cls) == [RLMObjectBase class]) {
      return nil;
    }
...
+ (instancetype)sharedSchema {
  @synchronized(s_localNameToClass) {
    ...
    if (s_sharedSchemaState == SharedSchemaState::Initializing) {
      @throw RLMException(
          @"Illegal recursive call of +[%@ %@]. Note: "
          @"Properties of Swift `Object` classes must not be "
          @"prepopulated with queried results from a WabiRealm.",
    ...
    try {
      // Make sure we've discovered all classes
      {
        ...
        using malloc_ptr =
            std::unique_ptr<__unsafe_unretained Class[], decltype(&free)>;
        malloc_ptr classes(objc_copyClassList(&numClasses), &free);
        RLMRegisterClassLocalNames(classes.get(), numClasses);
      }

      [s_localNameToClass
          enumerateKeysAndObjectsUsingBlock:^(NSString *, Class cls, BOOL *) {
            registerClass(cls);
          }];
    } catch (...) {
      s_sharedSchemaState = SharedSchemaState::Uninitialized;
...
+ (instancetype)dynamicSchemaFromObjectStoreSchema:
    (Schema const &)objectStoreSchema {
  // cache descriptors for all subclasses of RLMObject
  NSMutableArray *schemaArray =
      [NSMutableArray arrayWithCapacity:objectStoreSchema.size()];
  for (auto &objectSchema : objectStoreSchema) {
    RLMObjectSchema *schema =
        [RLMObjectSchema objectSchemaForObjectStoreSchema:objectSchema];
...
+ (Class)classForString:(NSString *)className {
  ...
  {
    unsigned int numClasses;
    std::unique_ptr<__unsafe_unretained Class[], decltype(&free)> classes(
        objc_copyClassList(&numClasses), &free);
    RLMRegisterClassLocalNames(classes.get(), numClasses);
  }
...
- (id)copyWithZone:(NSZone *)zone {
  ...
  schema->_objectSchemaByName = [[NSMutableDictionary allocWithZone:zone]
      initWithDictionary:_objectSchemaByName
               copyItems:YES];
  return schema;
}
...
- (BOOL)isEqualToSchema:(RLMSchema *)schema {
  ...
  [_objectSchemaByName
      enumerateKeysAndObjectsUsingBlock:^(
          NSString *name, RLMObjectSchema *objectSchema, BOOL *stop) {
        if (![schema->_objectSchemaByName[name]
                isEqualToObjectSchema:objectSchema]) {
          *stop = YES;
          matches = NO;
        }
      }];
...
- (NSString *)description {
  ...
  NSArray *sort = @[ [NSSortDescriptor sortDescriptorWithKey:@"className"
                                                   ascending:YES] ];
  for (RLMObjectSchema *objectSchema in
       [self.objectSchema sortedArrayUsingDescriptors:sort]) {
    [objectSchemaString
        appendFormat:@"\t%@\n",
...
- (Schema)objectStoreCopy {
  if (_objectStoreSchema.size() == 0) {
    ...
    std::sort(begin(schema), end(schema),
              [](auto &&a, auto &&b) { return a.name < b.name; });
    schema.erase(std::unique(begin(schema), end(schema),
                             [](auto &&a, auto &&b) {
                               if (a.name == b.name) {
                                 // If we make _realmObjectName public this
                                 // needs to be turned into an exception
                                 REALM_ASSERT_DEBUG(a.persisted_properties ==
                                                    b.persisted_properties);
                                 return true;
                               }
                               return false;
                             }),
