...
+ (instancetype)credentialsWithFacebookToken:(RLMCredentialsToken)token {
  return [[self alloc]
      initWithAppCredentials:AppCredentials::facebook(token.UTF8String)];
}

+ (instancetype)credentialsWithGoogleAuthCode:(RLMCredentialsToken)token {
  return [[self alloc] initWithAppCredentials:AppCredentials::google(
                                                  AuthCode(token.UTF8String))];
}

+ (instancetype)credentialsWithGoogleIdToken:(RLMCredentialsToken)token {
  return [[self alloc]
      initWithAppCredentials:AppCredentials::google(IdToken(token.UTF8String))];
}

+ (instancetype)credentialsWithAppleToken:(RLMCredentialsToken)token {
  return [[self alloc]
      initWithAppCredentials:AppCredentials::apple(token.UTF8String)];
}

+ (instancetype)credentialsWithEmail:(NSString *)username
                            password:(NSString *)password {
  return [[self alloc]
      initWithAppCredentials:AppCredentials::username_password(
                                 username.UTF8String, password.UTF8String)];
}

+ (instancetype)credentialsWithJWT:(NSString *)token {
  return [[self alloc]
      initWithAppCredentials:AppCredentials::custom(token.UTF8String)];
}
...
+ (instancetype)credentialsWithUserAPIKey:(NSString *)apiKey {
  return [[self alloc]
      initWithAppCredentials:AppCredentials::user_api_key(apiKey.UTF8String)];
}

+ (instancetype)credentialsWithServerAPIKey:(NSString *)apiKey {
  return [[self alloc]
      initWithAppCredentials:AppCredentials::server_api_key(apiKey.UTF8String)];
}
...
- (BOOL)isEqual:(id)object {
  if (auto that = RLMDynamicCast<RLMCredentials>(object)) {
    return [self.provider isEqualToString:that.provider] &&
           self.appCredentials.serialize_as_json() ==
               that.appCredentials.serialize_as_json();
  }
  return NO;
}
@end
