...
@end
/// :nodoc:
@protocol RLMDecimal128
@end
/// :nodoc:
@protocol RLMObjectId
@end
/// :nodoc:
@protocol RLMUUID
@end
...
/**
 `RLMProperty` instances represent properties managed by a WabiRealm in the
 context of an object schema. Such properties may be persisted to a WabiRealm
 file or computed from other data from the WabiRealm.

 When using WabiRealm, `RLMProperty` instances allow performing migrations and
 introspecting the database's schema.

 These property instances map to columns in the core database.
 */
RLM_SWIFT_SENDABLE
    RLM_FINAL // not actually immutable, but the public API kinda is
...
/**
 For `RLMObject` and `RLMCollection` properties, the name of the class of object
...
/**
 ...
 class.
 */
RLM_SWIFT_SENDABLE RLM_FINAL @interface RLMPropertyDescriptor : NSObject
