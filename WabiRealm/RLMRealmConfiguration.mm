...
NSString *RLMRealmPathForFileAndBundleIdentifier(NSString *fileName,
...
- (instancetype)init {
  self = [super init];
  if (self) {
    static NSURL *defaultRealmURL =
        [NSURL fileURLWithPath:RLMRealmPathForFile(c_defaultRealmFileName)];
...
- (void)setFileURL:(NSURL *)fileURL {
  ...
  if (path.length == 0) {
    @throw RLMException(@"WabiRealm path must not be empty");
  }
...
- (void)setInMemoryIdentifier:(NSString *)inMemoryIdentifier {
  ...
  RLMNSStringToStdString(
      _config.path, [NSTemporaryDirectory()
                        stringByAppendingPathComponent:inMemoryIdentifier]);
...
- (NSData *)encryptionKey {
  return _config.encryption_key.empty()
             ? nil
             : [NSData dataWithBytes:_config.encryption_key.data()
                              length:_config.encryption_key.size()];
}
...
- (void)setSchemaVersion:(uint64_t)schemaVersion {
  if (schemaVersion == RLMNotVersioned) {
    @throw RLMException(@"Cannot set schema version to %llu (RLMNotVersioned)",
                        RLMNotVersioned);
  }
...
- (void)setObjectClasses:(NSArray *)objectClasses {
  _customSchema =
      objectClasses ? [RLMSchema schemaWithObjectClasses:objectClasses] : nil;
  [self updateSchemaMode];
}

- (NSUInteger)maximumNumberOfActiveVersions {
  if (_config.max_number_of_active_versions >
...
- (void)setMaximumNumberOfActiveVersions:
    (NSUInteger)maximumNumberOfActiveVersions {
  if (maximumNumberOfActiveVersions == 0) {
    _config.max_number_of_active_versions =
...
- (void)setDisableAutomaticChangeNotifications:
    (bool)disableAutomaticChangeNotifications {
...
- (RLMSyncConfiguration *)syncConfiguration {
  ...
  RLMSyncConfiguration *syncConfig =
      [[RLMSyncConfiguration alloc] initWithRawConfig:*_config.sync_config
                                                 path:_config.path];
...
- (wabi_realm::WabiRealm::Config)config {
  ...
  if (_eventConfiguration) {
    config.audit_config =
        [_eventConfiguration auditConfigWithRealmConfiguration:self];
  }
#endif
  return config;
}
